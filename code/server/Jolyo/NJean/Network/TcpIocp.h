...
namespace Network
{
	namespace TcpIOCP
	{
		class CTcpIocp : public INetwork, IO::IOCP::IIocpHandler
		{
			//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			// 생성/소멸자
		public:
			CTcpIocp();
//				CTcpIocp(ITcpNetworkHandler* pNetworkHandler);
			~CTcpIocp(void);
		...
		private:
			struct TagListener
			{
				COverlappedListenSocket		OverlappedListenSocket;
				Common::DataStructure::CMemoryPoolSafty<COverlappedCommSocket>
											OverlappedCommSocketPool;
			};

			IO::IOCP::CAsyncIocp		m_AsyncIocp;					// Iocp Object
			ITcpNetworkHandler*			m_pNetworkHandler;				// Network handler

			Common::DataStructure::CMemoryPoolSafty<COverlappedCommSocket>
										m_OverlappedConnectSocketPool;	// Overlapped Communication Socket Pool : IOCP에 Recv/Send 이벤트 적용
		...
		protected:
			...
			INT			m_nNumOfConnectSockets;		// TcpIocp가 접속할 수 있는 소켓의 수
			INT			m_nNumOfListenSockets;		// TcpIocp에 접속할 수 있는 Port의 수
		...
		public:
			...
			virtual void	ResetRttTime				( ULONG_PTR lpCompletionKey );
			virtual BOOL	CanStartRTT					( ULONG_PTR lpCompletionKey );
			virtual void	SetRttStartTime				( ULONG_PTR lpCompletionKey );
			virtual void	SetRttEndTime				( ULONG_PTR lpCompletionKey );
			virtual void	GetRttTime					( ULONG_PTR lpCompletionKey, DWORD& out_dwRttTime );
			...
			virtual INT			GetServicerIndex( ULONG_PTR lpNetworkKey )
			{
				...
				if( pCommSocket->m_pParentOvSocket )
				{
					return ((COverlappedListenSocket*)pCommSocket->m_pParentOvSocket)->GetServiceIndex();
				}
				else
				{
		...
		public:
			...
			{
				COverlappedSocket* pSocket = (COverlappedSocket*)lpNetworkKey;
			...
			{
				COverlappedCommSocket*						pCommSocket;
				Common::DataStructure::CMemoryPoolSafty<COverlappedCommSocket>*	pCommSocketPool;
			...
			{
				COverlappedCommSocket* pSocket = (COverlappedCommSocket*)lpNetworkKey;
