...
BOOL CCommSocket::Send( const char* pData, int nDataLen )
{
	...
	/*
	short nCommand;
	memcpy( &nCommand, m_SendBuffer.buf+sizeof(short), sizeof(short) );
	if( nCommand == 1018 )
	{
		char szTemp[128];
		sprintf( szTemp, "2[%10d] %d", m_sSocket, nCommand );
...
//	m_dwSentBytes = nDataLen - m_dwSentBytes;
	if( nDataLen == m_dwSentBytes )
		return TRUE;
