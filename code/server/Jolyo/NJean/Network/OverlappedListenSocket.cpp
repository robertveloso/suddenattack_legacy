...
////////////////////////////////////////////////////////////////////////////////////////
//
//	COverlappedListenSocket
//

// 생성/소멸자
COverlappedListenSocket::COverlappedListenSocket(void)
{
...
// 리소스들을 생성한다.
BOOL COverlappedListenSocket::Create( CMemoryPoolSafty<COverlappedCommSocket>* pCommSocketPool, UINT nRecvBuffSize/* = 0*/, UINT nAcceptPoolSize/* = 100*/ )
{
...
// 리소스들을 해제한다.
void COverlappedListenSocket::Destroy()
{
	::closesocket( m_sSocket );
...
// 바인드를 한다.
BOOL COverlappedListenSocket::Bind( const sockaddr* addr, int namelen )
{
...
// 바인드를 한다.
BOOL COverlappedListenSocket::Bind( int nPort, u_long hostAddr/* = INADDR_ANY*/, short family/* = AF_INET*/ )
{
...
// 리슨을 한다.
BOOL COverlappedListenSocket::Listen( int nBacklog/* = 5*/ )
{
...
// Accept Pool을 등록한다.
BOOL COverlappedListenSocket::RunAcceptPool()
{
	// Recv 이벤트를 받을 경우
	if( m_nBuffSize > 16 )
	{
...
BOOL COverlappedListenSocket::AcceptEx( LPOVERLAPPED ovlappedSocket )
{
	DWORD dwError;
	...
	DWORD       dwRecvNumBytes = 0;
	::ZeroMemory( pOvSocket, sizeof(WSAOVERLAPPED) );
	BOOL bRet = ::AcceptEx(
		m_sSocket,
	...
	if( !bRet ) 
	{
		...
		{
			m_dwErrCode = ErrorCode::AcceptExFail;
//			int nClose = ::closesocket( pOvSocket->sAcceptSocket );

			::printf( "Fail AcceptEx : Can't Reg IOCP : %d\n", ErrCode );
			return FALSE;
		}
	}
...
// iocp용 소켓을 생성한다.
inline SOCKET COverlappedListenSocket::CreateSocket( DWORD& nOutErrorcode )
{
	SOCKET socket;
...
// IO Type을 반환한다.
COverlappedSocket::IOType COverlappedListenSocket::GetIOType( LPOVERLAPPED lpOv )
{
	// Recv 이벤트를 받을 경우
	if( m_nBuffSize > 16 )
	{
