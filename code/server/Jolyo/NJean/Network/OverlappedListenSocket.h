/*****************************************************************************************************************
	COverlappedListenSocket : COverlappedSocket
...
namespace Network
{
	namespace TcpIOCP
	{
		class COverlappedListenSocket :
			public COverlappedSocket
		{
			// 생성/소멸자
		public:
			COverlappedListenSocket(void);
			~COverlappedListenSocket(void);
		...
		public:
			// 리소스들을 생성한다.
			BOOL	Create( Common::DataStructure::CMemoryPoolSafty<COverlappedCommSocket>* pCommSocketPool, UINT nRecvBuffSize = 0, UINT nAcceptPoolSize = 100 );
			...
			sockaddr *	GetPeerAddr( LPOVERLAPPED lpOv )
			{
				TagOverlappedSocket* pOvSocket = (TagOverlappedSocket*)lpOv;
				...
				sockaddr * pLSA		= NULL;
				sockaddr * pRSA		= NULL;
				ULONG ulSALength	= sizeof( sockaddr_in ) + 16;

				GetAcceptExSockaddrs( pOvSocket->Buff, 0, ulSALength, ulSALength, & pLSA, & nLSALength, & pRSA, & nRSALength );
				return pRSA;
			}
