...
void CModelEditDlg::OnRotationButton() 
{
	...
	if (dlg.DoModal() == IDOK)
	{
		vRot.x = DegreesToRadians(( float )atof( dlg.m_sRotX ));
		vRot.y = DegreesToRadians(( float )atof( dlg.m_sRotY ));
		vRot.z = DegreesToRadians(( float )atof( dlg.m_sRotZ ));
...
void CModelEditDlg::OnCalcInternalRadius()
{
	...
	for( uint32 nAnimLoop = 0; nAnimLoop < m_pModel->NumAnims(); ++nAnimLoop )
	{
		...
		for( uint32 nKeyframeLoop = 0; nKeyframeLoop < pAnim->NumKeyFrames(); ++nKeyframeLoop )
		{
			...
			SetCurrentPosition( (uint32)0, nKeyframeLoop, nKeyframeLoop, 0.0f );
