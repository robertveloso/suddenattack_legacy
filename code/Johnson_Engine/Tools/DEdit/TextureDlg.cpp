...
void CTextureDlg::OnKeySelTexture( LV_KEYDOWN FAR * pnkd, LRESULT * pResult )
{
...
BOOL CTextureDlg::DoTextureProperties( CMoArray<DFileIdent*>& selections )
{
	...
	BeginWaitCursor();
		...
		// Look for the LIGHTDEFS string.
		pSection = dtx_FindSection(pTexture, "LIGHTDEFS");

		dlgProp.m_LightDefsString = pSection ? pSection->m_Data : "";
	...
	if(dlgProp.DoModal() == IDOK)
	{
		for(i=0; i < selections; i++)
		{
			...
			// Replace the LIGHTDEFS section.
			if(dlgProp.m_ChangeFlags & TPROP_LIGHTDEFS)
			{
				if(pSection = dtx_FindSection(pTexture, "LIGHTDEFS"))
				{
					dResult = dtx_RemoveSection(pTexture, pSection);
					ASSERT(dResult == DE_OK);
				}

				const char *pNewString = (LPCTSTR)dlgProp.m_LightDefsString;
				if(strlen(pNewString) > 0)
				{
					pSection = (DtxSection*)dalloc(sizeof(DtxSection) + strlen(pNewString));
					strcpy(pSection->m_Header.m_Type, "LIGHTDEFS");
					strcpy(pSection->m_Header.m_Name, "");
					pSection->m_Header.m_DataLen = strlen(pNewString) + 1;
					strcpy(pSection->m_Data, pNewString);
					pSection->m_pNext = pTexture->m_pSections;
					pTexture->m_pSections = pSection;
				}	
			}
