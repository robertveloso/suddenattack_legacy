...
BOOL CRVTrackerTextureMove::OnStart()
{
	...
	else if(m_pView->GetEditMode() == PATCH_EDITMODE)
	{
		//run through all the selections and add the patches to the list
		for(uint32 nCurrSel = 0; nCurrSel < m_pView->GetRegion()->m_Selections.GetSize(); nCurrSel++)
		{
			if(m_pView->GetRegion()->m_Selections[nCurrSel]->GetType() == Node_Patch)
			{
				CEditPatch* pPatch = (CEditPatch*)m_pView->GetRegion()->m_Selections[nCurrSel];

				m_Textures.Add((CTexturedPlane*)pPatch);
				m_UndoNodes.Add((CWorldNode*)pPatch);
				m_TextureNormals.Add(pPatch->GetTextureNormal());
			}
		}
	}
