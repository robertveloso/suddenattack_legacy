...
BOOL CRVTrackerTag::OnStart()
{
	...
	bModeOK |= (iEditMode == PATCH_EDITMODE) && m_bPatch;
	if (!bModeOK)
		return FALSE;
...
BOOL CRVTrackerTag::OnEnd()
{
	...
	else if (iEditMode == OBJECT_EDITMODE)
	{
		...
		m_pView->GetObjectsInRect(m_cRect, objects);

		if (!objects.GetSize())
		{
			CWorldNode *obj = m_pView->GetClosestObject( m_cCurPt, FALSE );
			if (obj)
				objects.Add(obj);
		}
	...
	//tagging in patch mode
	else if(iEditMode == PATCH_EDITMODE)
	{
		//create an inflated version of the rectangle to ensure that if any part of the point
		//is in the rectangle, it will be included
		CRect cRect(m_cRect);
		uint32 nHandleSize = GetApp()->GetOptions().GetDisplayOptions()->GetHandleSize();
		cRect.InflateRect(nHandleSize, nHandleSize, nHandleSize, nHandleSize);

		//run through all the patch control points and classify if they are in the box
		for(uint32 nCurrHandle = 0; nCurrHandle < m_pView->GetRegionDoc()->m_PatchHandles.GetSize(); nCurrHandle++)
		{
			//get the handle
			CPatchHandle* pHandle = (CPatchHandle*)&m_pView->GetRegionDoc()->m_PatchHandles[nCurrHandle];

			//need to classify if it is in the area
			LTVector vTransformed;
			m_pView->m_Transform.Apply( pHandle->GetPos(), vTransformed );

			bool bInRect = false;

			//make sure it is infront of the camera
			if(vTransformed.z > 0)
			{
				CPoint ptTrans;
				m_pView->m_pViewDef->ProjectPt(vTransformed, ptTrans);


				//see if it is in the rect
				if( (ptTrans.x >= cRect.left) && (ptTrans.x <= cRect.right) && 
					(ptTrans.y >= cRect.top) && (ptTrans.y <= cRect.bottom))
				{
					bInRect = true;			
				}
			}			

			//determine how this point should be handled
			if(m_bAdd)
			{
				if(bInRect)
			...
			{
				if(bInRect)
					pHandle->SetSelected(!pHandle->IsSelected());
			}
			else
			{
				pHandle->SetSelected(bInRect);
			}
		}
