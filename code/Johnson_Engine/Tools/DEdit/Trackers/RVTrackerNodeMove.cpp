...
BOOL CRVTrackerNodeMove::OnStart()
{
	...
	if ((m_pView->GetEditMode() != BRUSH_EDITMODE) && 
		...
		(m_pView->GetEditMode() != PATCH_EDITMODE))
		return FALSE;

	m_bPatchMode = (m_pView->GetEditMode() == PATCH_EDITMODE) ? TRUE : FALSE;

	if(m_bPatchMode)
	{
		//make sure that we are over a patch handle
		m_nPatchHandle = m_pView->GetCurrentMouseOverPatchHandle();
		...
		//setup the undo
		m_pView->GetRegionDoc()->SetupUndoForSelections(TRUE, Node_Patch);
	}
	else
	{
	...
	if( m_pView->GetVertexFromPoint(m_cCurPt, newVert))
	{
		...
		if(m_bPatchMode)
		{
			for(uint32 nCurrHandle = 0; nCurrHandle < m_pView->GetRegionDoc()->m_PatchHandles.GetSize(); nCurrHandle++)
			{
				...
				if(pHandle->IsSelected() || (nCurrHandle == m_nPatchHandle))
				{
					pHandle->SetPos(pHandle->GetPos() + moveOffset);
				}
			}
		}
		else
