...
#define __REGIONVIEW_H__
	...
	#define END_MEMORYEXCEPTION()		} catch( CMemoryException x ) { GiveMemoryWarning(); }

	// Edit modes.
	#define BRUSH_EDITMODE						0
	#define GEOMETRY_EDITMODE					1
	#define OBJECT_EDITMODE						2
	#define PATCH_EDITMODE						3
	...
	public:
		...
		CWorldNode*					GetClosestObject(const CPoint& point, BOOL bSelectedOnly, CReal *pDist=NULL );
		...
		// Tags all points/edges/polies in the tag rect.
		void						GetObjectsInRect( CRect &rect, CMoArray< CWorldNode * > &objects );
	...
	public:
		...
		//are under the mouse
		int32						GetCurrentMouseOverPatchHandle();
	...
	private:
		...
		void RecurseAndGetObjectsInRect(CRect &rect, CMoArray< CWorldNode * > &objects, CWorldNode *pRoot);
		// Helper function for GetClosestObject
		CWorldNode *RecurseAndGetClosestObject(const CPoint &point, BOOL bSelectedOnly, CWorldNode *pRoot, SDWORD &nDistMin, float &fViewMin);
	...
	public:
		...
		afx_msg void OnUpdateAddPatch(CCmdUI* pCmdUI);
		afx_msg void OnSetPatchResolution();
		afx_msg void OnUpdateSetPatchResolution(CCmdUI* pCmdUI);
