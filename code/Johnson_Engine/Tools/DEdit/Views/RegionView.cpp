...
BEGIN_MESSAGE_MAP(CRegionView, CView)
	...
	ON_COMMAND(ID_STICKTEXTURE, OnStickTexture)
	...
	ON_COMMAND(ID_ADD_PATCH, OnAddPatch)
	ON_UPDATE_COMMAND_UI( ID_ADD_PATCH, OnUpdateAddPatch)
	ON_COMMAND(ID_SET_PATCH_RESOLUTION, OnSetPatchResolution)
	ON_UPDATE_COMMAND_UI( ID_SET_PATCH_RESOLUTION, OnUpdateSetPatchResolution)
...
void CRegionView::SetPatchEditMode()
{
	((CMainFrame *)AfxGetMainWnd())->OnPatchEditMode();
}
...
void CRegionView::OnStickTexture( )
{
	if( GetEditMode() == GEOMETRY_EDITMODE )
	{
		// Setup an undo..
		...
		IPoly()()->GetTexture(GetCurrTexture()).m_bStickToPoly = !( IPoly()()->GetTexture(GetCurrTexture()).m_bStickToPoly );
	}
}
...
void CRegionView::OnEditUndo() 
{
	BEGIN_MEMORYEXCEPTION()
	{
		...
		//clear out patch handles
		GetRegionDoc()->m_PatchHandles.RemoveAll();
		...
		//this needs to be done so that patch handles don't point to deleted patches
		GetRegionDoc()->UpdatePatchHandles();
...
void CRegionView::OnUpdate(CView* pSender, LPARAM lHint, CObject* pHint) 
{
	...
	else if( lHint == REGIONVIEWUPDATE_EDITMODE )
	{
		if (GetInFocus())
		{
			...
			//also see if we are entering patch mode. If we are, we need to create the list
			//of handles
			GetRegionDoc()->UpdatePatchHandles();
		}
...
BOOL CRegionView::ImportLTAFile(CString sFilename, BOOL bPositionAtCrosshair)
{
	...
	uint32 nFileVersion;
	RegionLoadStatus status=region.LoadLTA(sFilename, GetProject(), nFileVersion);
	EndWaitCursor();
...
void CRegionView::OnContextMenu(CWnd* pWnd, CPoint point)
{
	...
	if( m_bDoMenu )
	{
		...
		pDirect3dOptions = pDirect3dOptions->GetSubMenu(8);
		ASSERT( pDirect3dOptions );		// someone changed the menu layout, fix the index
	...
	if(polies.GetSize() > 0)
	{
		...
		if(pPoly)
		{
			...
			if(pTexture && pTexture->m_pDib)
			{
				...
				dlg.m_bStickToPoly = PolyTex.m_bStickToPoly;
	...
	if( (GetRegion()->m_Selections == 1) && (GetRegion()->m_Selections[0]->GetType() == Node_Brush) )
	{
		...
		if( bOk )
		{
			...
			for( i=0; i < pBrush->m_Polies; i++ )
			{
				...
				for(uint32 nCurrTex = 0; nCurrTex < CEditPoly::NUM_TEXTURES; nCurrTex++)
				{
					pNewPoly->GetTexture(nCurrTex).m_pTextureName = pPoly->GetTexture(nCurrTex).m_pTextureName;
					pNewPoly->GetTexture(nCurrTex).m_pTextureFile = pPoly->GetTexture(nCurrTex).m_pTextureFile;
				}
	
				// Extrude.
				polyRef.Init( pNewBrush, 0 );
				if( ExtrudePoly( polyRef, newVerts, FALSE, FALSE /*flip all*/) )
				{
...
void CRegionView::OnApplyTexture()
{
		...
		(GetEditMode() == PATCH_EDITMODE))
	{
		...
		if( GetCurrentTextureName(pCurTexName, TRUE) )
		{
			...
			for(pos=actionList; pos; )
			{
				...
				else if(pNode->GetType() == Node_Patch)
				{
					pPatch = (CEditPatch*)pNode;
					pPatch->m_pTextureName = pCurTexName;
					pPatch->UpdateTextureID();

					//get the dimensions of the texture
					pPatch->SetPatchTextureWidth(pPatch->GetBaseTextureWidth());
					pPatch->SetPatchTextureHeight(pPatch->GetBaseTextureHeight());
				}
			}
		}
	}
	else if(GetEditMode() == GEOMETRY_EDITMODE)
	{
		...
		if( IPoly().IsValid() && GetCurrentTextureName(pCurTexName, TRUE) )
		{
			...
			IPoly()()->GetTexture(GetCurrTexture()).m_bStickToPoly = TRUE;			//SCHLEGZ 1/20/98: All textures applied are now "stuck"
...
void CRegionView::OnRemoveTexture()
{
		...
		(GetEditMode() == PATCH_EDITMODE))
	{
		...
		for(i=0; i < GetRegion()->m_Selections; i++)
		{
			...
			//now do the same with the patches
			else if(pNode->GetType() == Node_Patch)
			{
				CTexturedPlane* pTex = (CTexturedPlane*)pNode;
		...
		for(pos=actionList; pos; )
		{
			...
			{
				CEditPatch* pPatch = (CEditPatch*)pNode;
				pPatch->m_pTextureName = "Default";
				pPatch->UpdateTextureID();

				//get the dimensions of the texture
				pPatch->SetPatchTextureWidth(0);
				pPatch->SetPatchTextureHeight(0);
			}
		}
	}
	else if(GetEditMode() == GEOMETRY_EDITMODE)
			...
			switch( nViewMode )
			{
				...
				case VM_PERSPECTIVE:
				{
					...
					*/				
					...
					rRot = LTRotation(Nav().Forward(), Nav().Up());
								
					const float PI       =  3.14159265358979323846f;

					double sqrR1 = rRot.m_Quat[0] * rRot.m_Quat[0];
					double sqrR2 = rRot.m_Quat[1] * rRot.m_Quat[1];
					double sqrR3 = rRot.m_Quat[2] * rRot.m_Quat[2];
					double sqrR4 = rRot.m_Quat[3] * rRot.m_Quat[3];

					nAngle[0] = (uint32) (asin(-2 * (rRot.m_Quat[1] * rRot.m_Quat[2] - rRot.m_Quat[0] * rRot.m_Quat[3])) * 180.0f / PI);
					nAngle[1] = (uint32) ((-2 * (rRot.m_Quat[1] * rRot.m_Quat[3] + rRot.m_Quat[0] * rRot.m_Quat[2]) / (sqrR1 + sqrR2 - sqrR3 - sqrR4)) * 180.0f / PI);
					nAngle[2] = (uint32) ((-2 * (rRot.m_Quat[0] * rRot.m_Quat[1] + rRot.m_Quat[2] * rRot.m_Quat[3]) / (sqrR1 - sqrR2 + sqrR3 - sqrR4)) * 180.0f / PI);

					sCamera.Format( "(%3d,%3d,%3d)@(%1.0f,%1.0f,%1.0f)", 
						nAngle[1], nAngle[0], nAngle[2],
						vCamPos.x, vCamPos.y, vCamPos.z );
					// }
...
{		
	...
	for(i=0; i < pRegion->m_Selections; i++)
	{
		if(pRegion->m_Selections[i]->GetType() == Node_Brush)
		{
			pToSplit = pRegion->m_Selections[i]->AsBrush();
			...
			if(pToSplit->m_Polies)
			{
				...
				for (uint32 j=0; j < pToSplit->m_Polies.GetSize(); j++)
				{
					pPoly = pToSplit->m_Polies[j];

					for(uint32 k=0; k < pPoly->m_Indices.GetSize(); k++ )
					{
						dot = edgePlane.m_Normal.Dot(pPoly->Pt(k)) - edgePlane.m_Dist;
				...
				if (onBack && onFront)
				{
					...
					if(sides[0]->m_Polies >= 1 && sides[1]->m_Polies >= 1)
					{
						...
						for(j=0; j < 2; j++)
						{
							sides[j]->RemoveUnusedVerts();
							sides[j]->UpdateBoundingInfo();
						}
	...
	{
		GetNodeView( )->DeleteNode(toDelete[i]);
		no_DestroyNode(pRegion, toDelete[i], FALSE);
	}
...
{	
	...
	SetMenuHotKeyText(UIE_CARVE_BRUSH, pCmdUI);
}
...
void CRegionView::OnCenterMarkerOnSelection() 
{
	...
	// This should only be called if there are selections
	ASSERT(pRegion->GetNumSelections() > 0);
...
void CRegionView::OnSelectionSavePrefab() 
{
	...
	CString sFileName;
	sFileName = GetProject()->m_BaseProjectDir + "\\Prefab\\*.lta;*.ltc";
	// Let the user choose the file to save to
	CFileDialog fileDlg(FALSE, "lta", (LPCTSTR)sFileName, OFN_OVERWRITEPROMPT, "DEdit World (*.lta;*.ltc)|*.lta;*.ltc|All Files (*.*)|*.*||");
...
// Show the scale selected dialog
	...
	for( i=0; i < pRegion->m_Selections; i++ )
	{
		...
		else if(pNode->GetType() == Node_Patch)
		{
			...
			for(uint32 nX = 0; nX < pPatch->GetPatch().GetWidth(); nX++)
			{
				for(uint32 nY = 0; nY < pPatch->GetPatch().GetHeight(); nY++)
				{
					LTVector vPt = pPatch->GetPatch().GetControlPoint(nX, nY);
					...
					pPatch->GetPatch().SetControlPoint(nX, nY, vPt);
				}
			}
		}
	...
	if((GetEditMode() == BRUSH_EDITMODE) || (GetEditMode() == GEOMETRY_EDITMODE))
	{
		...
		if (GetEditMode() == BRUSH_EDITMODE)
		{
			...
			for(i=0; i < polies; i++)
			{
				...
				polies[i]->GetTexture(GetCurrTexture()).GetO().Init(0.0f, 0.0f, 0.0f);
			}
		}
		else if (GetEditMode() == GEOMETRY_EDITMODE)
		{
			...
			if (polies.GetSize() < 2)
			{
				...
				PolyTex.GetO() = pPoly->Pt(0);
				PolyTex.GetP() = pPoly->Pt(0) - pPoly->Pt(1);
				PolyTex.GetP().Norm();
				PolyTex.GetQ() = PolyTex.GetP().Cross(pPoly->Normal());
				PolyTex.GetQ().Norm();
			}
			else
			{
				...
				{
					pPoly = polies[i];
					pPoly->GetTexture(GetCurrTexture()).GetO() = cVectorO;
					pPoly->GetTexture(GetCurrTexture()).GetP() = cVectorP;
					pPoly->GetTexture(GetCurrTexture()).GetQ() = cVectorQ;
				}
			}
		}
	}

	//see if we need to reset the patch texture coordinates
	if(GetEditMode() != GEOMETRY_EDITMODE)
	{
		//setup the undo
		uint32 nCurrSel;

		for(nCurrSel = 0; nCurrSel < GetRegion()->m_Selections.GetSize(); nCurrSel++)
		{
			if(GetRegion()->m_Selections[nCurrSel]->GetType() == Node_Patch)
			{
				actionList.AddTail(new CPreAction(ACTION_MODIFYNODE, GetRegion()->m_Selections[nCurrSel]));
			}
		}
		...
		for(nCurrSel = 0; nCurrSel < GetRegion()->m_Selections.GetSize(); nCurrSel++)
		{
			if(GetRegion()->m_Selections[nCurrSel]->GetType() == Node_Patch)
			{
				CEditPatch* pPatch = (CEditPatch*)GetRegion()->m_Selections[nCurrSel];
				pPatch->SetupBaseTextureSpace();
			}
		}
	}
...
BOOL CRegionView::OnSelectNone(CUITracker *pTracker)
{
	...
	switch (GetEditMode())
	{
		...
		case PATCH_EDITMODE :
		{
			for(uint32 nCurrHandle = 0; nCurrHandle < GetRegionDoc()->m_PatchHandles.GetSize(); nCurrHandle++)
			{
				GetRegionDoc()->m_PatchHandles[nCurrHandle].SetSelected(false);
			}
			GetRegionDoc()->RedrawAllViews();
			break;
		}
...
void CRegionView::OnAddPatch()
{
	//prompt them with the dialog
	CAddPatchDlg PatchDlg;

	//load all the defaults from the registry
	PatchDlg.m_nWidth			= ::GetApp()->GetOptions().GetDWordValue("DefaultPatchWidth", 256);
	PatchDlg.m_nHeight			= ::GetApp()->GetOptions().GetDWordValue("DefaultPatchHeight", 256);
	PatchDlg.m_nXPoints			= ::GetApp()->GetOptions().GetDWordValue("DefaultPatchXPoints", 4);
	PatchDlg.m_nYPoints			= ::GetApp()->GetOptions().GetDWordValue("DefaultPatchYPoints", 4);
	PatchDlg.m_nOrientation		= ::GetApp()->GetOptions().GetDWordValue("DefaultPatchOrientation", 0);
	PatchDlg.m_nXSubdiv			= ::GetApp()->GetOptions().GetDWordValue("DefaultPatchXSubdiv", DEFAULT_PATCH_RESOLUTION);
	PatchDlg.m_nYSubdiv			= ::GetApp()->GetOptions().GetDWordValue("DefaultPatchYSubdiv", DEFAULT_PATCH_RESOLUTION);
	
	if(PatchDlg.DoModal() == IDOK)
	{
		//clear out any old selections
		GetRegion()->ClearSelections();

		CEditPatch *pPatch = new CEditPatch;
		ASSERT(pPatch);

		SetupWorldNode("Brush", pPatch->AsNode(), GetRegion());

		//clear out the orientation
		pPatch->SetOr(LTVector(0, 0, 0));

		pPatch->SetName(PatchDlg.m_sName);

		no_InitializeNewNode(GetRegion(), pPatch, GetRegion()->GetActiveParentNode());

		//select this new node
		GetRegion()->SelectNode(pPatch);

		GetRegionDoc()->Modify(new CPreAction(ACTION_ADDEDNODE, pPatch), TRUE);

		//now set up the control points for the patch
		pPatch->GetPatch().Init(PatchDlg.m_nXPoints, PatchDlg.m_nYPoints);

		pPatch->SetXResolution(PatchDlg.m_nXSubdiv);
		pPatch->SetYResolution(PatchDlg.m_nYSubdiv);

		//figure out the right and down vectors
		LTVector vRight, vDown;
		LTVector vRot;

		switch(PatchDlg.m_nOrientation)
		{
		case 0: //+XY plane
		case 3:
			vRight.Init(1, 0, 0);
		...
		//scale the vectors appropriately
		vRight = vRight * (PatchDlg.m_nWidth  / (float)PatchDlg.m_nXPoints);
		vDown  = vDown  * (PatchDlg.m_nHeight / (float)PatchDlg.m_nYPoints);
		...
		for(uint32 nX = 0; nX < pPatch->GetPatch().GetWidth(); nX++)
		{
			for(uint32 nY = 0; nY < pPatch->GetPatch().GetHeight(); nY++)
			{
				...
				//positive plane
				vPt = vRight * (float)nX + vDown * (float)nY;
				pPatch->GetPatch().SetControlPoint(nX, nY, vPt);
			}
		}

		//see if we need to flip the direction
		if(PatchDlg.m_nOrientation > 2)
		{
			pPatch->SetOr(vRot * PI);
		}		

		char* pCurTexName;
		//see if we want to apply the currently selected texture
		if( GetCurrentTextureName(pCurTexName, TRUE) )
		{
			pPatch->m_pTextureName = pCurTexName;
			pPatch->UpdateTextureID();
		...
		pPatch->SetPos(GetRegion()->GetMarker());

		//save the settings 
		::GetApp()->GetOptions().SetDWordValue("DefaultPatchWidth", PatchDlg.m_nWidth);
		::GetApp()->GetOptions().SetDWordValue("DefaultPatchHeight", PatchDlg.m_nHeight);
		::GetApp()->GetOptions().SetDWordValue("DefaultPatchXPoints", PatchDlg.m_nXPoints);
		::GetApp()->GetOptions().SetDWordValue("DefaultPatchYPoints", PatchDlg.m_nYPoints);
		::GetApp()->GetOptions().SetDWordValue("DefaultPatchOrientation", PatchDlg.m_nOrientation);		
		::GetApp()->GetOptions().SetDWordValue("DefaultPatchXSubdiv", PatchDlg.m_nXSubdiv);
		::GetApp()->GetOptions().SetDWordValue("DefaultPatchYSubdiv", PatchDlg.m_nYSubdiv);

		//have everything update in the region
		GetRegionDoc()->UpdateSelectionBox();
		GetRegionDoc()->RedrawAllViews();		
	}	
}

void CRegionView::OnUpdateAddPatch(CCmdUI* pCmdUI)
{
	SetMenuHotKeyText(UIE_ADD_PATCH, pCmdUI);
}


void CRegionView::OnSetPatchResolution()
{
	//see if we have any patches selected and aren't in geometry mode
	if(GetEditMode() == GEOMETRY_EDITMODE)
	{
		return;
	}
	
	//get the dimensions of the first selected patch
	CSetPatchResDlg Dlg;
	...
	for(uint32 nCurrSel = 0; nCurrSel < GetRegion()->m_Selections.GetSize(); nCurrSel++)
	{
		...
		{
			CEditPatch* pPatch = (CEditPatch*)GetRegion()->m_Selections[nCurrSel];
			Dlg.m_nXRes = pPatch->GetXResolution();
			Dlg.m_nYRes = pPatch->GetYResolution();
			break;
		}
	}

	//let the user do the dialog
	if(Dlg.DoModal() == IDOK)
	{
		//they hit ok, so we need to go through and set all patches to the resolution
		//specified
		for(uint32 nCurrSel = 0; nCurrSel < GetRegion()->m_Selections.GetSize(); nCurrSel++)
		{
			if(GetRegion()->m_Selections[nCurrSel]->GetType() == Node_Patch)
			{
				CEditPatch* pPatch = (CEditPatch*)GetRegion()->m_Selections[nCurrSel];
				pPatch->SetXResolution(Dlg.m_nXRes);
				pPatch->SetYResolution(Dlg.m_nYRes);
			}
		}
...
void CRegionView::OnUpdateSetPatchResolution(CCmdUI* pCmdUI)
{
	BOOL bActive = FALSE;
	...
	SetMenuHotKeyText(UIE_SET_PATCH_RESOLUTION, pCmdUI);
}
