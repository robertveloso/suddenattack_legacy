...
//control how patches are rendered
#define RENDERFLAG_PATCHNOFILL			0x0040
#define RENDERFLAG_PATCHNOWIREFRAME		0x0080
#define RENDERFLAG_PATCHONLYSELECTED	0x0100
#define RENDERFLAG_PATCHNOTSELECTED		0x0200
...
void DrawBase::Draw()
{
	...
	if(m_pView->IsPerspectiveViewType())
	{
		...
		else
		{
			...
			//solids...
			m_nRenderFlags = RENDERFLAG_BRUSHNOWIREFRAME | RENDERFLAG_OBJECTNOEXTRA | RENDERFLAG_PATCHNOWIREFRAME;
			DrawNode(m_pView->GetRegion()->GetRootNode(), NULL);
			...
			m_nRenderFlags = RENDERFLAG_BRUSHNOFILL | RENDERFLAG_OBJECTNOBASE | RENDERFLAG_PATCHNOFILL;
			DrawNode(m_pView->GetRegion()->GetRootNode(), NULL);
		}
	}
	else
	{
		...
		m_nRenderFlags = RENDERFLAG_BRUSHNOFILL | RENDERFLAG_BRUSHNOTSELECTED | 
						 RENDERFLAG_PATCHNOTSELECTED | RENDERFLAG_OBJECTNOEXTRA;
		DrawNode(m_pView->GetRegion()->GetRootNode(), NULL);
		...
		m_nRenderFlags = RENDERFLAG_BRUSHNOFILL | RENDERFLAG_BRUSHONLYSELECTED | 
						 RENDERFLAG_PATCHONLYSELECTED | RENDERFLAG_OBJECTNOBASE;
		DrawNode(m_pView->GetRegion()->GetRootNode(), NULL);
	}
	...
	//draw the patch handles if needed
	if(m_pView->GetEditMode() == PATCH_EDITMODE)
	{
		DrawPatchHandles();
	}
}
...
void DrawBase::DrawPatch(CWorldNode *pNode, CNodeStackInfo *pInfo)
{
	if(IsRenderFlag(RENDERFLAG_NOPATCH))
		return;

	ASSERT(pNode);

	//bail if we are hidden, or if this is not the correct shade mode
	if(	pNode->IsFlagSet(NODEFLAG_HIDDEN) ||
		(m_pView->GetShadeMode() == SM_LIGHTMAPSONLY) ||
		(pNode->IsFlagSet(NODEFLAG_FROZEN) && m_pDisplayOptions->IsHideFrozenNodes()))
	{
		return;
	}

	//now we can draw the patch if it is visible
	CEditPatch* pPatch = (CEditPatch*)pNode;

	//first off, see if we are out of range
	LTVector vMin, vMax;
	pPatch->GetBoundingBox(vMin, vMax);

	//get the center and radius
	LTVector vCenter = (vMax + vMin) / 2;
	float fRadius = ((vMax - vMin) / 2).Mag();

	//transform the center
	LTMatrix mTransform = m_pView->m_Transform;
	
	if(pInfo)
	{
		mTransform = mTransform * pInfo->m_Transform;
	}

	LTVector vTransCenter;
	m_pView->m_Transform.Apply(vCenter, vTransCenter);

	if(!m_pView->SphereInsideFrustum(vTransCenter, fRadius))
	{
		//out of the frustum
		return;
	}
	...
	BOOL bSelected	= (pInfo) ? pInfo->m_bSelected : pNode->IsFlagSet(NODEFLAG_SELECTED);
	BOOL bFrozen	= (pInfo) ? pInfo->m_bSelected : pNode->IsFlagSet(NODEFLAG_FROZEN);

	if( (IsRenderFlag(RENDERFLAG_PATCHONLYSELECTED) && !bSelected) ||
		(IsRenderFlag(RENDERFLAG_PATCHNOTSELECTED) && bSelected))
		return;

	//the original patch vertices
	static LTVector3f vOPatchPos[CBezierPatch::MAX_DIMENSION][CBezierPatch::MAX_DIMENSION];

	uint32 nX, nY;

	//run through and transform the vertices, saving the originals
	for(nX = 0; nX < pPatch->GetPatch().GetWidth(); nX++)
	{
		for(nY = 0; nY < pPatch->GetPatch().GetHeight(); nY++)
		{
			//save
			vOPatchPos[nX][nY] = pPatch->GetPatch().GetControlPoint(nX, nY);
			//transform
			pPatch->GetPatch().SetControlPoint(nX, nY, mTransform * vOPatchPos[nX][nY]);
		}
	}

	//now render the patch
	static const uint32 MAX_RENDER_RESOLUTION = 16;
	static LTVector3f vPatchPos[MAX_RENDER_RESOLUTION][MAX_RENDER_RESOLUTION];

	uint32 nXRes = LTMIN(pPatch->GetXResolution(), MAX_RENDER_RESOLUTION);
	uint32 nYRes = LTMIN(pPatch->GetYResolution(), MAX_RENDER_RESOLUTION);

	ASSERT(nXRes >= 1);
	ASSERT(nYRes >= 1);

	//evaluate all points on the surface
	for(nX = 0; nX < nXRes; nX++)
	{
		for(nY = 0; nY < nYRes; nY++)
		{
			vPatchPos[nX][nY] = pPatch->GetPatch().Evaluate(nX / (float)(nXRes - 1), nY / (float)(nYRes - 1));
		}
	}

	COptionsDisplay* pDisplayOptions = m_pDisplayOptions;
	BOOL bTint =	(bFrozen && pDisplayOptions->IsTintFrozen()) || 
					(bSelected && pDisplayOptions->IsTintSelected());

	//figure out what color this patch should be
	DWORD nColor = GetDisplayColorD3D(COptionsDisplay::kColorPatch);

	//check for states
	if(bSelected)
	{
		nColor = GetDisplayColorD3D(COptionsDisplay::kColorSelectedBrush);
	}
	if(bFrozen)
	{
		nColor = GetDisplayColorD3D(COptionsDisplay::kColorFrozenBrush);
	}

	//see if we are in wireframe rendering, or solid rendering
	bool bWireframe = ((m_pView->IsParallelViewType()) || (m_pView->GetShadeMode() == SM_WIREFRAME));

	//bail if the render flags conflict with our rendering choice
	if(IsRenderFlag(RENDERFLAG_PATCHNOWIREFRAME) && bWireframe)
		return;
	...
	if(bWireframe)
	{
		...
		for(nX = 0; nX < nXRes; nX++)
		{
			for(nY = 0; nY < nYRes - 1; nY++)
			{
				vPts[0].m_Vec = vPatchPos[nX][nY];
				vPts[1].m_Vec = vPatchPos[nX][nY + 1];
				Draw3dLine(vPts);
			}
		}
		for(nY = 0; nY < nYRes; nY++)
		{
			for(nX = 0; nX < nXRes - 1; nX++)
			{
				vPts[0].m_Vec = vPatchPos[nX][nY];
				vPts[1].m_Vec = vPatchPos[nX + 1][nY];
				Draw3dLine(vPts);
			}
		}
	}
	...
	{
		bool bDrawFlat = (m_pView->GetShadeMode() == SM_FLAT) ? true : false;
		bool bDrawFrontFacing = !IsRenderFlag(RENDERFLAG_PATCHNOFILL);
		bool bDrawBackFacing  = !IsRenderFlag(RENDERFLAG_PATCHNOWIREFRAME);
		...
		//for if we need to do wireframe on backfacing polies
		TLVertex LinePts[2];
		LinePts[0].color = nColor;
		LinePts[1].color = nColor;

		//break the color into components for faster color composition
		uint8 nR = (nColor >> 16) & 0xFF;
		uint8 nG = (nColor >> 8) & 0xFF;
		uint8 nB = nColor & 0xFF;

		//texture increments
		float fTX = 0.0f;
		float fTY = 0.0f;
		float fTXInc = PATCH_TEXTURE_SCALE / (float)(nXRes - 1);
		float fTYInc = PATCH_TEXTURE_SCALE / (float)(nYRes - 1);

		LTVector O, P, Q;
		pPatch->GetTextureSpace(O, P, Q);

		float fPDotO = P.Dot(O);
		float fQDotO = Q.Dot(O);

		//the color for the vertex
		DWORD nVertColor;


		//init the vertex colors if we aren't going to shade them
		if(!m_pDisplayOptions->IsShadePolygons())
		{
			for(uint32 nCurrPoly = 0; nCurrPoly < 2; nCurrPoly++)
			{
				for(uint32 nCurrVert = 0; nCurrVert < 3; nCurrVert++)
					Points[nCurrPoly][nCurrVert].color = 0xFFFFFF;
			}
		}



		for(nX = 0; nX < nXRes - 1; nX++, fTX += fTXInc)
		{
			//reset this
			fTY = 0.0f;

			for(nY = 0; nY < nYRes - 1; nY++, fTY += fTYInc)
			{
				//calculate the vertex positions for the two triangles
				Points[0][0].m_Vec = vPatchPos[nX][nY + 1];
				Points[0][1].m_Vec = vPatchPos[nX][nY];
				Points[0][2].m_Vec = vPatchPos[nX + 1][nY];

				Points[1][0].m_Vec = vPatchPos[nX][nY + 1];
				Points[1][1].m_Vec = vPatchPos[nX + 1][nY];
				Points[1][2].m_Vec = vPatchPos[nX + 1][nY + 1];

				
				//get the normal
				vNormal[0] = (Points[0][2].m_Vec - Points[0][0].m_Vec).Cross(Points[0][1].m_Vec - Points[0][0].m_Vec);
				vNormal[0].Norm();
				vNormal[1] = (Points[1][2].m_Vec - Points[1][0].m_Vec).Cross(Points[1][1].m_Vec - Points[1][0].m_Vec);
				vNormal[1].Norm();

				//see if this poly is backfacing. If it is, we need to draw it wireframe
				if(	(vNormal[0].Dot(Points[0][0].m_Vec) > 0.0f) &&
					(vNormal[1].Dot(Points[1][0].m_Vec) > 0.0f) &&
					bDrawBackFacing)
				{
					//we are in wireframe
					LinePts[0].m_Vec = vPatchPos[nX + 1][nY];
					LinePts[1].m_Vec = vPatchPos[nX + 1][nY + 1];
					Draw3dLine(LinePts);

					LinePts[0].m_Vec = vPatchPos[nX][nY + 1];
					LinePts[1].m_Vec = vPatchPos[nX + 1][nY + 1];
					Draw3dLine(LinePts);

					//check for the borders (we have to draw extra lines there)
					if(nX == 0)
					{
						LinePts[0].m_Vec = vPatchPos[nX][nY];
						LinePts[1].m_Vec = vPatchPos[nX][nY + 1];
						Draw3dLine(LinePts);
					}
					if(nY == 0)
					{
						LinePts[0].m_Vec = vPatchPos[nX][nY];
						LinePts[1].m_Vec = vPatchPos[nX + 1][nY];
						Draw3dLine(LinePts);
					}
					continue;
				}

				//we are front facing...see if we want to render this
				if(!bDrawFrontFacing)
					continue;

				//setup the texture coordinates
				Points[0][0].tu = -(P.Dot(LTVector(fTX, 0, fTY + fTYInc)) - fPDotO);	
				Points[0][0].tv = -(Q.Dot(LTVector(fTX, 0, fTY + fTYInc)) - fQDotO);
				Points[0][1].tu = -(P.Dot(LTVector(fTX, 0, fTY)) - fPDotO);
				Points[0][1].tv = -(Q.Dot(LTVector(fTX, 0, fTY)) - fQDotO);
				Points[0][2].tu = -(P.Dot(LTVector(fTX + fTXInc, 0, fTY)) - fPDotO);	
				Points[0][2].tv = -(Q.Dot(LTVector(fTX + fTXInc, 0, fTY)) - fQDotO);

				Points[1][0].tu = Points[0][0].tu;
				Points[1][0].tv = Points[0][0].tv;
				Points[1][1].tu = Points[0][2].tu;
				Points[1][1].tv = Points[0][2].tv;
				Points[1][2].tu = -(P.Dot(LTVector(fTX + fTXInc, 0, fTY + fTYInc)) - fPDotO);
				Points[1][2].tv = -(Q.Dot(LTVector(fTX + fTXInc, 0, fTY + fTYInc)) - fQDotO);

				//setup the color of the points
				if(m_pDisplayOptions->IsShadePolygons())
				{
					for(uint32 nCurrPoly = 0; nCurrPoly < 2; nCurrPoly++)
					{
						nVertColor = 128 +	(uint8)(fabs(vNormal[nCurrPoly].x) * 96.0f) + 
											(uint8)(fabs(vNormal[nCurrPoly].z) * 32.0f);
						nVertColor = D3DRGB_255(nVertColor, nVertColor, nVertColor);

						for(uint32 nCurrVert = 0; nCurrVert < 3; nCurrVert++)
							Points[nCurrPoly][nCurrVert].color = nVertColor;
					}
				}
				...
				if(bDrawFlat)
				{
					DrawFlatPoly2(Points[0], 3, &vNormal[0], nColor);
					DrawFlatPoly2(Points[1], 3, &vNormal[1], nColor);
				}
				else
				{
					DrawTexturedPoly(Points[0], 3, bTint, nColor, pPatch->m_pTextureFile, pPatch->m_pDetailTextureFile, nColor);
					DrawTexturedPoly(Points[1], 3, bTint, nColor, pPatch->m_pTextureFile, pPatch->m_pDetailTextureFile, nColor);
				}
			}
		}
	}

	//now restore the verts
	for(nX = 0; nX < pPatch->GetPatch().GetWidth(); nX++)
	{
		for(nY = 0; nY < pPatch->GetPatch().GetHeight(); nY++)
		{
			pPatch->GetPatch().SetControlPoint(nX, nY, vOPatchPos[nX][nY]);
		}
	}
}
...
void DrawBase::DrawPatchHandles()
{
	//just run through the list of patch handles and draw them in the appropriate
	//color
	DWORD lineColor = GetDisplayColorD3D(COptionsDisplay::kColorHandleOutline);

	for(uint32 nCurrHandle = 0; nCurrHandle < m_pView->GetRegionDoc()->m_PatchHandles.GetSize(); nCurrHandle++)
	{
		CPatchHandle* pHandle = &(m_pView->GetRegionDoc()->m_PatchHandles[nCurrHandle]);

		// Draw the move handles.
		DWORD fillColor;
		uint32 nExpandAmount;
			
		if(pHandle->IsSelected())
		{
			fillColor = GetDisplayColorD3D(COptionsDisplay::kColorSelPatchHandleFill);
			nExpandAmount = LTMAX(GetHandleSize() / 3, 3);
		}
		else
		{
			fillColor = GetDisplayColorD3D(COptionsDisplay::kColorPatchHandleFill);
			nExpandAmount = 0;
		}


		TransformAndDrawVert(pHandle->GetPos(), lineColor, fillColor, GetHandleSize() + nExpandAmount);
	}
}
