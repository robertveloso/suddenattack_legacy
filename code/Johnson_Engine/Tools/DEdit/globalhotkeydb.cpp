...
void CGlobalHotKeyDB::SetDefaults(CHotKeyDB& DB)
{
	...
	*/
	...
	MAKE_HOTKEY(UIE_CARVE_BRUSH, IDS_UIE_CARVE_BRUSH);
	...
	//hotkey for setting patch resolutions
	MAKE_HOTKEY(UIE_SET_PATCH_RESOLUTION, IDS_UIE_SET_PATCH_RESOLUTION);
	...
	MAKE_HOTKEY(UIE_ADD_PATCH, IDS_UIE_ADD_PATCH);
	...
	MAKE_HOTKEY(UIE_PATCH_EDIT_MODE, IDS_UIE_PATCH_EDIT_MODE);
...
bool CGlobalHotKeyDB::SetKeyDefault(CHotKey& HotKey)
{
	...
	else if(stricmp(pszName, UIE_SET_PATCH_RESOLUTION) == 0)
	{
	}
	...
	else if(stricmp(pszName, UIE_CARVE_BRUSH) == 0)
	{
	...
	else if(stricmp(pszName, UIE_ADD_PATCH) == 0)
	{
		HotKey.AddEvent(CUIKeyEvent(UIEVENT_KEYDOWN, VK_CONTROL));
		HotKey.AddEvent(CUIKeyEvent(UIEVENT_KEYDOWN, 'L'));
	}
	...
	else if(stricmp(pszName, UIE_PATCH_EDIT_MODE) == 0)
	{
		HotKey.AddEvent(CUIKeyEvent(UIEVENT_KEYDOWN, VK_CONTROL));
		HotKey.AddEvent(CUIKeyEvent(UIEVENT_KEYDOWN, 'F'));
	}
