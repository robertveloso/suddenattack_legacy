...
void CMapTextureCoordsDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CH_STICK, m_bStickToPoly);
...
BEGIN_MESSAGE_MAP(CMapTextureCoordsDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_CH_XZONLY, GenericChangeHandler)
	ON_BN_CLICKED(IDC_CH_STICK, GenericChangeHandler)
...
void CMapTextureCoordsDlg::ApplyChanges()
{
	...
	for(i=0; i < polies; i++)
	{
		...
		pPoly->GetTexture(GetCurrTexture()).m_bStickToPoly = m_bStickToPoly;
	}
