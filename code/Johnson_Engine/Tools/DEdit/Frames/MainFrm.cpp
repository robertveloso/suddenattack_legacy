...
BEGIN_MESSAGE_MAP(CMainFrame, CMRCMDIFrameWndSizeDock)
	...
	ON_COMMAND(ID_PATCH_EDITMODE, OnPatchEditMode)
	ON_UPDATE_COMMAND_UI(ID_PATCH_EDITMODE, OnUpdatePatchEditMode)
...
void CMainFrame::OnPatchEditMode() 
{
	m_WorldEditMode = PATCH_EDITMODE;
	UpdateAllRegions( REGIONVIEWUPDATE_EDITMODE );
...
void CMainFrame::OnUpdatePatchEditMode(CCmdUI* pCmdUI) 
{
	CRegionView::SetMenuHotKeyText(UIE_PATCH_EDIT_MODE, pCmdUI);

	pCmdUI->Enable( GetNumRegionDocs() > 0 );
	pCmdUI->SetRadio( GetWorldEditMode() == PATCH_EDITMODE );
}
