...
void CEditProjectMgr::LoadBinaries()
{
	...
	// Cant find it.  Look in the registry keys under:
	// HKEY_LOCAL_MACHINE\Software\LithTech Inc.\Jupiter\Demo\default_app_dlls.
	if(status == CB_CANTFINDMODULE)
	{
		hKey = HKEY_LOCAL_MACHINE;
		if(RegOpenKeyEx(hKey, "Software", 0, KEY_READ, &hKey) == ERROR_SUCCESS)
		{
			if(RegOpenKeyEx(hKey, "LithTech Inc.", 0, KEY_READ, &hKey) == ERROR_SUCCESS)
			{
				if(RegOpenKeyEx(hKey, "Jupiter", 0, KEY_READ, &hKey) == ERROR_SUCCESS)
				{
					if(RegOpenKeyEx(hKey, "Demo", 0, KEY_READ, &hKey) == ERROR_SUCCESS)
					{
						dataBufSize = 260;
						if(RegQueryValueEx(hKey, "default_app_dlls", NULL, (unsigned long *)&valType, dataBuf, (unsigned long *)&dataBufSize) == ERROR_SUCCESS)
						{
							dllFilename = dfm_BuildName((LPCTSTR)dataBuf, "object.lto");
							status = cb_LoadModule((char*)(LPCTSTR)dllFilename, NULL, &m_hModule, &version);
						}
					}
				}
	...
	if(status != CB_NOERROR)
	{
		if(status == CB_CANTFINDMODULE)
		{
			str.Format(IDS_CANTFINDCLASSMODULE, (LPCTSTR)dllFilename);
		}
		else if(status == CB_NOTCLASSMODULE)
		{
			str.Format(IDS_INVALIDCLASSMODULE, (LPCTSTR)dllFilename);
		}
		else if(status == CB_VERSIONMISMATCH)
		{
			str.Format(IDS_INVALIDCLASSMODULEVERSION, SERVEROBJ_VERSION, version);
		}
