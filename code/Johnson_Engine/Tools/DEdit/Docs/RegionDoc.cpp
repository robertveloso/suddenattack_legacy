...
bool CRegionDoc::SaveLTA(bool bFullSave)
{
	...
	//update the filename to make sure that the extension is LTA if we are doing
	//a full save
	if(bFullSave)
	{
		...
		if( (stricmp(ext, ".lta") != 0) && (stricmp(ext, ".ltc") != 0) )
		{
			m_FileName = dir;
			m_FileName += "\\";
			m_FileName += fname;
	...
	if(!bBackupWorked)
	{
		...
		if(bFullSave)
		{
			OutFile.Open(	LPCTSTR(m_FileName), false, 
							CLTAUtil::IsFileCompressed(LPCTSTR(m_FileName)));
...
bool CRegionDoc::SaveOBJ(bool bFullSave)
{
	...
	if( exportObjFileDlg.m_isOK )
	{
		...
		{
			uint32 nFileVersion;
			if( REGIONLOAD_OK == region.LoadLTA(m_FileName, NULL, nFileVersion) )
			{
...
void CRegionDoc::StartPreProcessor()
{
	...
	try
	{
		...
		if(SaveModifiedLTA()) // Ask user if he/she wishes to save level, if changed
		{
			...
			sCommand.Format("\"%s\\Processor\" \"%s\"", GetApp()->m_ExeDirectory, GetPathName());
...
bool CRegionDoc::LoadLTAIntoRegion( const char* filename, bool bForceUpdateObjects, bool bAllowPropSync)
{
	...
	uint32 nFileVersion;
	if( m_Region.LoadLTA((char*)filename, GetProject(), nFileVersion) != REGIONLOAD_OK )
		return false;
...
void CRegionDoc::UpdateSelectionBox( LPARAM extraHint )
{
	...
	int nNumSelObjects	= 0;
	int nNumSelBrushes	= 0;
	int nNumSelOther	= 0;
	int nNumSelPrefabs	= 0;
	int nNumSelPatches	= 0;
	...
	if(nNumSelObjects || nNumSelBrushes || nNumSelPrefabs || nNumSelOther)
	{
		...
		if(nNumSelBrushes)
		{
			sFormatText.Format("Brsh: %d ", nNumSelBrushes);
			sSelectionText += sFormatText;
		}
		...
		if(nNumSelPatches)
		{
			sFormatText.Format("Ptc: %d ", nNumSelPatches);
			sSelectionText += sFormatText;
		}
