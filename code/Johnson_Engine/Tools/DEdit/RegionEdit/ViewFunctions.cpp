...
void CRegionView::RecurseAndGetObjectsInRect(CRect &rect, CMoArray< CWorldNode * > &objects, CWorldNode *pRoot)
{
	if (!pRoot)
		return;

	if ((pRoot->GetType() == Node_Object) &&
		(!pRoot->IsFlagSet(NODEFLAG_HIDDEN)) && (!pRoot->IsFlagSet(NODEFLAG_FROZEN)))
	{
		CPoint testPt;
	...
	while (iCurChild)
	{
		...
		RecurseAndGetObjectsInRect(rect, objects, pChild);
	}
}

void CRegionView::GetObjectsInRect( CRect &rect, CMoArray< CWorldNode * > &objects )
{
	...
	RecurseAndGetObjectsInRect(norm, objects, m_pRegion->GetRootNode());
}
...
CWorldNode *CRegionView::RecurseAndGetClosestObject(const CPoint &point, BOOL bSelectedOnly, CWorldNode *pRoot, SDWORD &nDistMin, float &fViewMin)
{
	CWorldNode *pResult = LTNULL;

	// Is this one the closest?
	if (((pRoot->GetType() == Node_Object) || (pRoot->GetType() == Node_PrefabRef) || (pRoot->GetType() == Node_Patch)) &&
		(!bSelectedOnly || pRoot->IsFlagSet(NODEFLAG_SELECTED)) &&
		(!pRoot->IsFlagSet(NODEFLAG_HIDDEN)) && (!pRoot->IsFlagSet(NODEFLAG_FROZEN)))
	{
		...
		//have to special case for prefabs since a lot of the time, the position won't
		//be in the center
		if(pRoot->GetType() == Node_Patch)
		{
			LTVector vMin, vMax;
			((CEditPatch*)pRoot)->GetBoundingBox(vMin, vMax);
			pt = (vMin + vMax) / 2;
		}
	...
	{
		CWorldNode *pChild = pRoot->m_Children.GetNext(iCurChild);
		CWorldNode *pChildResult = RecurseAndGetClosestObject(point, bSelectedOnly, pChild, nDistMin, fViewMin);
		if (pChildResult)
...
CWorldNode *CRegionView::GetClosestObject( const CPoint &point, BOOL bSelectedOnly, CReal *pDist )
{
	float fMinView = (float)MAX_CREAL;
	int32 nMinDist = MAX_SDWORD;
	CWorldNode *pResult = RecurseAndGetClosestObject(point, bSelectedOnly, m_pRegion->GetRootNode(), nMinDist, fMinView);
	if (pResult && pDist)
	{
...
void CRegionView::UpdateCursor()
{
	...
	else if( IsParallelViewType() && (GetEditMode() == PATCH_EDITMODE))
	{
		int32 nHandle = GetCurrentMouseOverPatchHandle();

		if(nHandle != m_CurMouseOverHandle)
		{
			m_CurMouseOverHandle = nHandle;

			if(nHandle != -1)
			{
				::SetClassLong( m_hWnd, GCL_HCURSOR, (LONG)m_CursorSizeAll );
			}
			else
			{
...
//finds the patch handle that is underneath the mouse. It will return -1 if none 
//are under the mouse
int32 CRegionView::GetCurrentMouseOverPatchHandle()
{
	//get the mouse position
	CPoint ptMouse = GetCurMousePos();

	CReal fMinDist = (CReal)MAX_CREAL;


	int32 nMinIndex = -1;

	uint32 nHandleSize = GetApp()->GetOptions().GetDisplayOptions()->GetHandleSize();

	// Search the rest of the handles
	for( uint32 nCurrHandle = 0; nCurrHandle < GetRegionDoc()->m_PatchHandles.GetSize(); nCurrHandle++)
	{
		CPatchHandle* pHandle = (CPatchHandle*)(&GetRegionDoc()->m_PatchHandles[nCurrHandle]);
		
		CPoint ptHandle;
		if(TransformAndProjectInFrustum(pHandle->GetPos(), ptHandle))
		{
			//we need to find the greatest distance on a square basis (since the handles are
			//square)
			uint32 nDist = LTMAX(abs(ptMouse.x - ptHandle.x), abs(ptMouse.y - ptHandle.y));

			uint32 nExpandAmount = (pHandle->IsSelected()) ? LTMAX(nHandleSize / 3, 3) : 0;
			
			if( nDist <= nHandleSize + nExpandAmount )
			{
				CReal fTestDist = (pHandle->GetPos() - Nav().Pos()).Mag();
				if( fTestDist < fMinDist )
				{
					fMinDist = fTestDist;
					nMinIndex = nCurrHandle;
				}
			}
		}
	}
...
void CRegionView::DoCarveOperation()
{
	CMoArray<CEditBrush*>	intersects;
	uint32					i, k;

	CEditBrush				*pBsp;
	CEditBrush				*pBrush;
	CWorldNode				*pNull, *pNode;

	CMoArray<CEditBrush*>	newBrushes;
	LPOS pos, lFound;
	PreActionList actionList;
	CEditRegion *pRegion;
	CMoArray<CEditBrush*> toRemove;


	pRegion = GetRegion();


	if( (pRegion->m_Selections == 1) && (pRegion->m_Selections[0]->GetType() == Node_Brush) )
	{
		pBrush = pRegion->m_Selections[0]->AsBrush();

		// If anything potentially intersects, chop it up.
		m_pRegion->GetBoxIntersectBrushes( pBrush, intersects );
		
		// Get rid of hidden ones.
		for(i=0; i < intersects; i++)
		{
			if(	intersects[i]->IsFlagSet(NODEFLAG_HIDDEN) ||
				intersects[i]->IsFlagSet(NODEFLAG_FROZEN))
			{
				intersects.Remove(i);
				--i;
			}
		...
		if( intersects > 0 )
		{
			...
			for( i=0; i < intersects; i++ )
			{
				if( pBrush->Subtract(pRegion, intersects[i], pBsp, newBrushes) )
				{										
					// Add a null node where the old brush was.
					pNull = pRegion->AddNullNode(intersects[i]->AsNode()->GetParent( ));					
					actionList.AddTail(new CPreAction(ACTION_ADDEDNODE, pNull));

					// Setup nodes for all the new brushes.
					for( k=0; k < newBrushes; k++ )
					{
//						SetupBrushProperties(newBrushes[k], pRegion);
						pRegion->AttachNode(newBrushes[k]->AsNode(), pNull);
						GetNodeView()->AddNode(newBrushes[k]->AsNode());
						actionList.AddTail(new CPreAction(ACTION_ADDEDNODE, newBrushes[k]));

						//also update their bounding spheres
						newBrushes[k]->UpdateBoundingInfo();
					}					
					
					// Remove the old brush.
					actionList.AddTail(new CPreAction(ACTION_MODIFYNODE, intersects[i]));
					toRemove.Append(intersects[i]);
			...
			// Remove the brushes we need to remove.
			for(i=0; i < toRemove; i++)
			{
				no_DestroyNode(pRegion, toRemove[i], FALSE);
			}
	...
	{
		AppMessageBox( IDS_CANTCARVEWITHOUTSELECTEDBRUSH, MB_OK );
	}															  				
}
