...
BOOL CAdvancedSelect::IsPassClassCriteria(CWorldNode *pNode)
{
	...
	{		
		...
		for (i=0; i < m_classCriteria.GetSize(); i++)
		{
			...
			//This is not the best approach, but since the prefab references and patches
			//don't actually have corresponding classes, we need to look for the automatically
			//added in classes PrefabRef and Patch for selection
			...
			if(	(pNode->GetType() == Node_Patch) &&
				(stricmp(m_classCriteria[i].GetClass(), "Patch") == 0))
			{
				return TRUE;
			}
		}
