...
typedef void (*LightCallbackFn)( CReal percent );
...
class LMMWorld : public WorldTreeObj
{
public:
					LMMWorld();

	virtual void	GetBBox(PVector *pMin, PVector *pMax);
...
class CLightMapMaker : public WorldTreeHelper
{
public:
	...
	PVector LightDirectionalPoint(	const PVector& vPos, const PVector& vNormal,
									bool bApplySunLight = true, bool bClipLight = true) const;

	//this will determine if a light hits the sample point specified
	bool	DoesLightHit(const CLightDef* pLight, const PVector& vPos, bool bClipLight = true) const;

	// Calculate the lighting color for a given point/light/normal combo (no intersections performed)
	LTVector CalcLightColor(const CLightDef *pLight, const LTVector &vPos, const LTVector &vNormal) const;

	// Get the ambient lighting contribution for a given point in space
	LTVector GetAmbientLight(const LTVector &vPos) const;

	// Get the sunlight contribution for a given point in space (also calculates a ray intersection)
	LTVector GetSunlight(const LTVector &vPos, const LTVector &vNormal) const;

	// Set the progress bar to a position
	void SetProgressBar(float fPercent) const { m_CallbackFn(fPercent); }
...
protected:
	...
	PVector LightGridPoint( const PVector& vPos, bool bClipLight, bool bAmbient);
	...
	bool	DoesSunlightHit( const PVector& vPos) const;

	PVector	CalcSunLightSample(const PVector& dir, bool bLambert) const;

protected:
  
	bool				BuildLightGroups(const LightTable &Dims);
	bool				CalculateLightFrame();
	bool				CalculateLightGroupData(const LightTable &Dims);
	...
	bool				LightPolygon(CPrePoly *pPoly, LTVector *pColorArray = 0);
	...
	// Load and free the class module.
	bool				LoadClassModule();
	void				TermClassModule();
...
protected:
	...
	bool				ThreadLightPoly(uint32 iPoly);
	...
	bool				ApplyLightsToPoly(CPrePoly *pPoly, uint32 nPolyAnimIndex, uint32 nNumLights, CLightDef **pLights);
...
protected:
 ...
 protected:
	...
	// Stores the handle to object.lto
	HCLASSMODULE			m_hClassModule;

	CPreMainWorld			*m_pWorld;

	// The intersection trees used for the lighting (one for sky polys, one for the rest)
	CLMPolyTree				m_cPolyTree_Sky;
	CLMPolyTree				m_cPolyTree_World;

	// Used for point inside/outside testing
