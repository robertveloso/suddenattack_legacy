...
NODEREF IndexToNode(int index, CMoArray<CNode*> &allocatedNodes)
{
	if(index == -1)
		return NODE_IN;
	else if(index == -2)
...
bool CPreMainWorld::ValidateLightData()
{
	uint32 iAnim, iPoly, iFrame, iMap;
	CPreLightAnim *pAnim;
	CPrePolyRef *pRef;
	CPreLightFrame *pFrame;
	CPreLightMap *pMap;
	...
	{
		pAnim = m_LightAnims[iAnim];

		// Make sure indices are valid.
		for(iPoly=0; iPoly < pAnim->m_Polies; iPoly++)
		{
			pRef = &pAnim->m_Polies[iPoly];

			if(pRef->m_iWorld >= m_WorldModels.GetSize())
				return FALSE;

			if(pRef->m_iPoly >= m_WorldModels[pRef->m_iWorld]->m_Polies.GetSize())
				return FALSE;
		}
		...
		for(iFrame=0; iFrame < pAnim->m_Frames; iFrame++)
		{
			...
			if(pFrame->m_PolyMaps.GetSize() != pAnim->m_Polies.GetSize())
				return FALSE;

			for(iMap=0; iMap < pFrame->m_PolyMaps; iMap++)
			{
				pMap = pFrame->m_PolyMaps[iMap];
				pRef = &pAnim->m_Polies[iMap];
				
				pPoly = GetLMPoly(pRef);
				if(!pPoly)
					return false;

				if(pMap->m_Width != pPoly->m_LMWidth || 
					pMap->m_Height != pPoly->m_LMHeight)
				{
					return false;
				}

				if(pMap->m_Width > LIGHTMAP_MAX_PIXELS_I || 
					pMap->m_Height > LIGHTMAP_MAX_PIXELS_I)
				{
					return false;
				}
			}
...
float CPreMainWorld::CalcLMCompressionRatio()
{
	uint32 iAnim, iFrame, iPoly;
	CPreLightAnim *pAnim;
	CPreLightFrame *pFrame;
	CPreLightMap *pLightMap;
	CPrePolyRef *pRef;
	uint32 originalSize, compressedSize;
	CPrePoly *pPoly;
	...
	for(iAnim=0; iAnim < m_LightAnims; iAnim++)
	{
		...
		for(iFrame=0; iFrame < pAnim->m_Frames; iFrame++)
		{
			...
			if(pFrame->m_PolyMaps.GetSize() == pAnim->m_Polies.GetSize())
			{
				for(iPoly=0; iPoly < pAnim->m_Polies; iPoly++)
				{
					pLightMap = pFrame->m_PolyMaps[iPoly];
					pRef = &pAnim->m_Polies[iPoly];
					
					if(pPoly = GetLMPoly(pRef))
					{
						originalSize += pPoly->m_LMWidth * pPoly->m_LMHeight * 3;
						compressedSize += pLightMap->m_Data.GetSize();
					}
				}
			}
		}
	...
	//if there are no lightmaps in the level, it is possible for the compressed size
	//to be zero, we need to accomodate for that
	if(compressedSize == 0)
	{
		//not positive if any returns will give valid feedback, but at least
		//0 will be unique in the context to indicate a specific situation
		return 0;
	}
...
CPreLightAnim* CPreMainWorld::FindLightAnim(const char *pName)
{
	CPreLightAnim *pAnim;
	uint32 i;

	for(i=0; i < m_LightAnims; i++)
	{
...
CPreWorld* CPreMainWorld::GetVisBSP()
{
	GenListPos pos;
	CPreWorld *pTest;
	...
	for(pos=m_WorldModels.GenBegin(); m_WorldModels.GenIsValid(pos); )
	{
		...
		if(pTest->m_WorldInfoFlags & WIF_VISBSP)
			return pTest;
	}
...
const CPreWorld* CPreMainWorld::GetVisBSP() const
{
	GenListPos pos;
	const CPreWorld *pTest;
...
bool CPreMainWorld::ConvertTempTouchLists()
{
	CMoArray<CPrePolyPtrArray> worldPolies;
	uint32 i, iCurWorld;
	CPreWorld *pWorldModel, *pVisBSP;
	GenListPos pos;
	CPreLeaf *pLeaf;
	CTempTouchInfo *pInfo;

	
	worldPolies.SetSize(m_WorldModels.GenGetSize());

	iCurWorld = 0;
	for(pos=m_WorldModels.GenBegin(); m_WorldModels.GenIsValid(pos); )
	{
		pWorldModel = m_WorldModels.GenGetNext(pos);
		worldPolies[iCurWorld].GenAppendList(pWorldModel->m_Polies);
		++iCurWorld;
	}
	...
	for(pos=pVisBSP->m_Leafs.GenBegin(); pVisBSP->m_Leafs.GenIsValid(pos); )
	{
		pLeaf = pVisBSP->m_Leafs.GenGetNext(pos);

		pLeaf->m_Touching.GenRemoveAll();

		for(i=0; i < pLeaf->m_TempTouchInfo; i++)
		{
			pInfo = &pLeaf->m_TempTouchInfo[i];

			if(pInfo->m_WorldIndex >= worldPolies.GetSize() || 
				pInfo->m_PolyIndex >= worldPolies[pInfo->m_WorldIndex].GetSize())
			{
				return false;
			}
			
			pLeaf->m_Touching.GenAppend(
				worldPolies[pInfo->m_WorldIndex].Get(pInfo->m_PolyIndex));			
		}

		pLeaf->m_TempTouchInfo.GenRemoveAll();
	}
...
uint32 CPreMainWorld::CollectParticleBlockerPolys( void )
{
	...
	for( uint32 nCurWorld = 0; nCurWorld < m_WorldModels.GetSize(); ++nCurWorld )
	{
		...
		while( pos )
		{
			...
			if( pBlocker )
			{
				for( uint32 nCurVert = 0; nCurVert < pPoly->NumVertsAndTVerts(); nCurVert++ )
				{
...
void CPreWorld::TermGeometry()
{
	...
	DeleteAndClearArray( m_Leafs );
	GDeleteAndRemoveElements(m_Nodes);
	GDeleteAndRemoveElements(m_Portals);
...
void CPreWorld::InitPolyVertexIndices()
{
	...
	for(pos=m_Polies; pos; )
	{
		...
		{
			pPoly->m_TVertices[i].m_Index = 0xFFFFFFFF;
			theQueue.push(PointElement(&pPoly->m_TVertices[i].m_Vec, (uint32 *)&pPoly->m_TVertices[i].m_Index));
			++nPointsInQueue;
		}
	}
...
bool CPreWorld::RemoveUnusedPoints()
{
	...
	for(pos=m_Polies; pos; )
	{
		...
		{
			iVert = pPoly->m_TVertices[k].m_Index;
			if(indexRename[iVert] == iVert)
			{
				// Ok, make a new output vert.
				newPoints.Append(m_Points[iVert]);
	...
	for(pos=m_Polies; pos; )
	{
		...
		for( k=0; k < pPoly->m_TVertices; k++ )
		{
			pPoly->m_TVertices[k].m_Index = indexRename[pPoly->m_TVertices[k].m_Index];
		}
	}
...
uint32 CPreWorld::TagUsedSurfaces()
{
	...
	for(pos=m_Portals; pos; )
		m_Portals.GetNext(pos)->m_pSurface->m_bUsed = TRUE;
...
void CPreWorld::SetupIndexMap( CMoByteArray &pointsUsed, CMoDWordArray &indexMap, uint32 &nPointsUsed )
{
	...
	for(pos=m_Polies; pos; )
	{
		...
		for( k=0; k < pPoly->m_TVertices; k++ )
			pointsUsed[pPoly->m_TVertices[k].m_Index] = TRUE;
	}
...
PrePortal* CPreWorld::FindPortal(char *pName)
{
	GPOS pos;
	PrePortal *pPortal;
	...
	for(pos=m_Portals; pos; )
	{
		pPortal = m_Portals.GetNext(pos);
	
		if(strcmp(pPortal->m_pName, pName) == 0)
			return pPortal;
	}
...
void CPreWorld::SetLeafIndices()
{
	assert((m_Leafs.GetSize() < 65536) && "Overflow");
	for(uint32 i=0; i < m_Leafs.GetSize(); i++)
	{
		m_Leafs[i]->m_Index = i;
	}
}
...
void MinimizeTCoordValues(CPreSurface *pSurface, CPrePoly *pPoly)
{
	...
	//update the surface's O to reflect the first texture
...
void CPreWorld::RemoveAllLeafs()
{
	...
	// Make the polies not reference any leaves.
	for(pos=m_Nodes; pos; )
		m_Nodes.GetNext(pos)->m_pLeaf = NULL;
}
...
bool CPreWorld::GenerateVertexNormals(CMoDWordArray &indexMap, CMoArray<PVector> &normals)
{
	CMoArray<PolyAdjacency> adjacent;
	PolyAdjacency *pAdjacent;
	GPOS pos;
	CPrePoly *pPoly;
	uint32 i, j;


	if(!adjacent.SetSize(m_Points.GetSize()) || !normals.SetSize(m_Points.GetSize()))
		return FALSE;

	// Figure out which polies touch which points.
	for(pos=m_Polies; pos; )
	{
		pPoly = m_Polies.GetNext(pos);
		...
		for(i=0; i < pPoly->NumVerts(); i++)
		{
			pAdjacent = &adjacent[indexMap[pPoly->Index(i)]];
			if(pAdjacent->m_nPolies < MAX_POLYADJACENCY_POLIES)
			{
				pAdjacent->m_Polies[pAdjacent->m_nPolies] = pPoly;
				pAdjacent->m_nPolies++;
			}
		}
	}
	...
	{
		pAdjacent = &adjacent[i];

		normals[i].Init();
		for(j=0; j < pAdjacent->m_nPolies; j++)
		{
			normals[i] += pAdjacent->m_Polies[j]->Normal();
		}

		normals[i] /= (float)pAdjacent->m_nPolies;
		normals[i].Norm();

		if(normals[i].MagSqr() < 0.01f)
			normals[i].Init(0.0f, 1.0f, 0.0f);
	}
...
void CPreWorld::JoinPolies(PReal convexThreshold)
{
	JoinPolyList(m_Polies, convexThreshold);
}
...
uint32 CPreWorld::NumDetailNodes()
{
	uint32 num=0;
	GPOS pos;

	for(pos=m_Nodes; pos; )
		if(m_Nodes.GetNext(pos)->m_pPoly->GetSurface()->IsDetailNode())
			++num;
...
uint32 CPreWorld::NumInternalNodes()
{
...
uint32 CPreWorld::TotalVisListSize()
{
	uint32 i, j, num;
	...
	for(i=0; i < m_Leafs; i++)
	{
		for(j=0; j < m_Leafs[i]->m_VisList.m_LeafLists; j++)
		{
			num += m_Leafs[i]->m_VisList.m_LeafLists[j]->m_LeafList.GetSize();
		}
	}
...
uint32 CPreWorld::TotalNumLeafLists()
{
	uint32 i, num;
	...
	for(i=0; i < m_Leafs; i++)
	{
		num += m_Leafs[i]->m_VisList.m_LeafLists.GetSize();
	}
...
uint32 CPreWorld::NumVisibleLeaves()
{
	uint32		i, num=0;

	for( i=0; i < m_Leafs; i++ )
		if( m_Leafs[i]->m_Flags & LEAF_VISIBLE )
			num++;
...
uint32 CPreWorld::TotalNumTouching()
{
	uint32	i, num=0;

	for( i=0; i < m_Leafs; i++ )
		num += m_Leafs[i]->m_Touching.GetSize();

	return num;
}


void CPreWorld::CalcPortalExtents(PrePortal *pPortal, PVector &min, PVector &max)
{
	GPOS pos;
	CPrePoly *pPoly;

	min.Init((PReal)MAX_CREAL, (PReal)MAX_CREAL, (PReal)MAX_CREAL);
	max = -min;
	...
	for(pos=m_Polies; pos; )
	{
		...
		if(pPoly->m_pSurface == pPortal->m_pSurface)
		{
			pPoly->ExtendBounds(min, max);
		}
	}
}

void CPreWorld::CalcLeafExtents()
{
	uint32 i, j;
	CPreLeaf *pLeaf;
	//LPOS pos;
	...
	for(i=0; i < m_Leafs; i++)
	{
		...
		min.Init((PReal)MAX_CREAL, (PReal)MAX_CREAL, (PReal)MAX_CREAL);
		max.Init((PReal)-MAX_CREAL, (PReal)-MAX_CREAL, (PReal)-MAX_CREAL);
		/*
		for(pos=pLeaf->m_Touching; pos; )
		{
			pPoly = pLeaf->m_Touching.GetNext(pos);
		*/
		for(pos = 0; pos < pLeaf->m_HullPolies.GetSize(); pos++)
		{
			pPoly = pLeaf->m_HullPolies[pos];
		
			for(j=0; j < pPoly->NumVerts(); j++)
			{
				VEC_MIN(min, min, pPoly->Pt(j));
				VEC_MAX(max, max, pPoly->Pt(j));
			}
		}

		pLeaf->m_Center = min + (max - min) * 0.5f;
		pLeaf->m_Radius = ((max - min) * 0.5f).Mag();
	}
}
