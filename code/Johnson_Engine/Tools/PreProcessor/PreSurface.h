...
//------------------------------------------------------------------
	...
	class CPreSurface : public CGLLNode
	{
		public:
			...
			BOOL			ShouldLightmap();
			BOOL			IsDetailNode();
			uint8			GetDetailLevel()			{return m_DetailLevel;}
			void			SetupLMVectors();

			inline uint32	GetLMPlaneIndex()			{return m_Flags >> 29;}
			...
			void GetLightmapTextureVectors(LTVector &P, LTVector &Q);
		...
		public:
			...
			// The portal this surface represents (if any).
			PrePortal		*m_pPortal;
			...
			// Detail level on this surface .. set in the preprocessor.
			uint8			m_DetailLevel;
