...
CPreLeaf *FindPointLeaf(CPreWorld *pWorld, PVector &point, NODEREF pRoot)
{
	PReal		dot;
	uint32		side;
	CPreLeaf	*pLastLeaf;
	...
	while(1)
	{
		...
		if(pRoot->m_pLeaf)
			pLastLeaf = pRoot->m_pLeaf;

		dot = pRoot->Normal().Dot(point) - pRoot->Dist();
		side = (dot >= -0.0001f) ? FrontSide : BackSide;
		
		if(!IsValidNode(pRoot->m_Sides[side]))
			return (side == FrontSide) ? pLastLeaf : NULL;
...
CPreLeaf* GetObjectLeaf(CPreWorld *pWorld, char *pClassName, CBaseEditObj **ppObj)
{
	CBaseEditObj	*pObj;
	PVector			pos;
	uint32			i;
	...
	for( i=0; i < pWorld->NumObjects(); i++ )
	{
		pObj = pWorld->GetObject(i);

		pos.x = (PReal)pObj->GetPos().x;
		pos.y = (PReal)pObj->GetPos().y;
		pos.z = (PReal)pObj->GetPos().z;

		if(strcmp(pObj->GetClassName(), pClassName) == 0)
		{
			pLeaf = FindPointLeaf(pWorld, pos, pWorld->m_RootNode);
			if(pLeaf)
			{
				if(ppObj)
				{
...
CPreLeaf* FindInsideLeaf(CPreWorld *pWorld, CBaseEditObj **ppObj)
{
	CPreLeaf *pLeaf;

	pLeaf = GetObjectLeaf(pWorld, "GameStartPoint", ppObj);
	if(!pLeaf)
	{
		pLeaf = GetObjectLeaf(pWorld, "StartPoint", ppObj);
		if(!pLeaf)
		{
			pLeaf = GetObjectLeaf(pWorld, "Light", ppObj);
			if(!pLeaf)
			{
				pLeaf = GetObjectLeaf(pWorld, "DirLight", ppObj);
				if(!pLeaf)
				{
					return NULL;
				}
