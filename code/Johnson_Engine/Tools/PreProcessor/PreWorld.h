...
typedef CMoArray<PolyAnimRef> PolyAnimRefList;
...
public:
	...
	bool						m_bShadowMap;	// Is this a shadow map light animation?
};
...
public:
	...
	uint32				NumObjects();
	CBaseEditObj*		GetObject(uint32 i);
	...
	PrePortal*			FindPortal(char *pName);
	...
	bool				GenerateVertexNormals(CMoDWordArray &indexMap, CMoArray<PVector> &normals);

// Polygon joining stuff.
public:

	void				JoinPolies(PReal convexThreshold=DEFAULT_CONVEX_THRESHOLD);
...
public:
	...
	// Number of detail nodes.
	uint32				NumDetailNodes();

	// These are called while saving the world.
	uint32				NumInternalNodes();
	
	uint32				NumPolyVerts();
	uint32				TotalVisListSize();
	uint32				TotalNumLeafLists();

	uint32				NumVisibleLeaves();
	uint32				TotalNumTouching();

	void				CalcPortalExtents(PrePortal *pPortal, PVector &min, PVector &max);

	void				CalcLeafExtents();
...
public:
	...
	CGLinkedList<PrePortal*>	m_Portals;		// All our portals.
	CGLinkedList<CPrePoly*>		m_Polies;
	CMoArray<CPreLeaf*>			m_Leafs;		
...
public:
	...
	// Makes sure all the lightmap data is formatted correctly.
	bool					ValidateLightData();
	...
	float					CalcLMCompressionRatio();
	CPreLightAnim*			FindLightAnim(const char *pName);
	...
	CPreWorld*				GetVisBSP();
	const CPreWorld*		GetVisBSP() const;

	// In the visibility BSP, convert CPreLeaf::m_TempTouchLists to CPreLeaf::m_Touching.
	bool					ConvertTempTouchLists();
...
public:
	...
	//list of the physics collision data objects
	CMoArray<CPhysicsWorldModel*>	m_PhysicsWorldModels;
...
inline uint32 CPreWorld::NumObjects()
{
	return m_pMainWorld->m_Objects.GetSize();
}

inline CBaseEditObj* CPreWorld::GetObject(uint32 i)
{
	return m_pMainWorld->m_Objects[i];
}
