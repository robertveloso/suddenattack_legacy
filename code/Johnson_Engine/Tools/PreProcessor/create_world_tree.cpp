...
public:
	...
	void		Init(CEditPoly *pPoly, CWorldModelDef *pDef)
				{
					m_pPoly = pPoly;
					m_pWorldModel = pDef;
				}
...
PReal GetWorldNodeSize(char *pInfoString)
{
	ConParse	parse;
	PReal		ret;
...
bool DoesPolyIntersectNode(CPrePoly *pPoly, WorldTreeNode *pNode)
{
	...
	for(i=0; i < NUM_WTPLANES; i++)
	{
		...
		{
			// Clip.
			pNewPoly = CreatePoly(CPrePoly, pTemp->NumVerts()+2, false);
			if(!pNewPoly)
				return false;
...
bool CWTCallback(WorldTree *pWorldTree, 
	WorldTreeNode *pNode, 
	CMoArray<CWTPoly> *pPolies,
	CMoArray<CWTPoly> *pInsidePolies)
{
	CWTPoly		*pPoly;
	uint32		i;
	bool		bMainWorld, bRet;
	CPrePoly	*pTemp;
	...
	for(i=0; i < pPolies->GetSize(); i++)
	{
		pPoly = &pPolies->Get(i);

		pTemp = EditPolyToPrePoly(pPoly->m_pPoly);
		if(!pTemp)
			return false;

		bRet = DoesPolyIntersectNode(pTemp, pNode);
		DeletePoly(pTemp);

		if(bRet)
		{
			pInsidePolies->Append(*pPoly);
...
bool CreateNodes_R(	WorldTree *pWorldTree, WorldTreeNode *pNode,
					PVector *pMin, PVector *pMax, uint32 depth, CMoArray<CWTPoly> *pPolies)
{
	WorldTreeNode		*pChild;
	uint32				i;
	CMoArray<CWTPoly>	insidePolies;
	...
	CMoArray<CWorldModelDef*> &worldModels, char *pInfoString)
{
	...
	for(i=0; i < worldModels.GetSize(); i++)
	{
		...
		for(iBrush=0; iBrush < pWorldModel->m_Brushes; iBrush++)
		{
			...
			{
				rootPolies[iOutPoly].Init(pBrush->m_Polies[iPoly], pWorldModel);
				iOutPoly++;
			}
		}
	}
	...
	if(rootPolies.GetSize() > 0)
	{
		...
		{
			pPoly = rootPolies[iPoly].m_pPoly;
