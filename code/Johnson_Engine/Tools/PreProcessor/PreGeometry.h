...
#define __PREGEOMETRY_H__
	...
	CPreLeaf *FindPointLeaf(CPreWorld *pWorld, PVector &point, NODEREF iRoot);

	// Finds the leaf that the first object of the given class is in.
	CPreLeaf* GetObjectLeaf(CPreWorld *pWorld, char *pClassName, CBaseEditObj **ppObj=NULL);

	// Finds a leaf defining the inside of the world by looking for StartPoints,
	// GameStartPoints, Lights, and DirLights.
	CPreLeaf* FindInsideLeaf(CPreWorld *pWorld, CBaseEditObj **ppObj=NULL);
