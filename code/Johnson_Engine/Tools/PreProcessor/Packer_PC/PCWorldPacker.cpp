...
static bool WriteWorldBspData( CPreWorld* pWorld, CAbstractIO &file )
{
	...
	CMoArray<PolyAnimRefList> polyAnimRefs;
	...
	file << (uint32)0;//pWorld->TotalVisListSize();
	//or leaf lists
	file << (uint32)0;//pWorld->TotalNumLeafLists();
...
static bool SaveFile(CPreMainWorld* pMainWorld, CAbstractIO &outFile, const char* pszFilename, bool bObjectsOnly)
{
	...
	DrawStatusText(eST_Normal, "  ");
	DrawStatusText(eST_Normal, "World .DAT file Size : %d bytes", file.GetCurPos());
	DrawStatusText(eST_Normal, "  World Tree : %d bytes", nWorldModelStart - nWorldTreeStart);
	DrawStatusText(eST_Normal, "  World Models : %d bytes", nObjectStart - nWorldModelStart);
	DrawStatusText(eST_Normal, "  Objects : %d bytes", nBlindDataStart - nObjectStart);
	DrawStatusText(eST_Normal, "  Blind Object Data : %d bytes", nLightGridStart - nBlindDataStart);
	DrawStatusText(eST_Normal, "  Light Grid : %d bytes", nPhysicsStart - nLightGridStart);
	DrawStatusText(eST_Normal, "  Blockers : %d bytes", nParticleStart - nPhysicsStart);
	DrawStatusText(eST_Normal, "  Particle Blockers : %d bytes", nRenderStart - nParticleStart);
	DrawStatusText(eST_Normal, "  Render Data : %d bytes", file.GetCurPos() - nRenderStart);
