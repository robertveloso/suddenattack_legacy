...
bool CPCRenderWorld::Process(const CPreMainWorld *pWorld)
{
	...
	if (pWorld->m_WorldModels.GetSize() > 2)
	{
		DrawStatusText(eST_Normal, "  Packing worldmodels");

		m_aWorldModels.resize(pWorld->m_WorldModels.GetSize() - 2);
			...
			++nCurWorldModelIndex, ++nOutputWorldModelIndex)
		{
			...
			// Skip the physics and visibility BSP's
			if ((pCurWorldModel->m_WorldInfoFlags & (WIF_PHYSICSBSP | WIF_VISBSP)) != 0)
			{
