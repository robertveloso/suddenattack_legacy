...
void CPCRenderTreeNode::AddPoly(const CPrePoly *pPoly, bool bOverrideLightmap)
{
	...
	const CPrePolyFragments *pFragments = pPoly->GetFragments();
	if (!pFragments)
	{
		// Count the Section
	...
	{
		// Count the Section
		uint32 nSection = CountSection(pPoly, eShader, pFragments->NumTris());

		CPrePolyFragments::SVert vPt0, vPt1, vPt2;
		for (uint32 nCurTri = 0; nCurTri < pFragments->NumTris(); ++nCurTri)
		{
			pFragments->GetTri(nCurTri, &vPt0, &vPt1, &vPt2);
			m_aTris.push_back(CTri(
				CPCRenderVert3T(pPoly, vPt0.m_vPos, vPt0.m_vNormal, vPt0.m_vColor, vPt0.m_fAlpha),
				CPCRenderVert3T(pPoly, vPt1.m_vPos, vPt1.m_vNormal, vPt1.m_vColor, vPt1.m_fAlpha),
				CPCRenderVert3T(pPoly, vPt2.m_vPos, vPt2.m_vNormal, vPt2.m_vColor, vPt2.m_fAlpha), 
				pPoly, nSection
				)
			);
		}
...
void CPCRenderTreeNode::ReduceTriCount()
{
	...
	if (g_pGlobs->m_bFixTJunc && !GetParent())
		FixTJunctions();
...
CPCRenderVert2T GetShaderVert(const CPCRenderVert3T &cVert, EPCShaderType eShader)
{
	switch (eShader)
	{
		...
		case ePCShader_Lightmap :
			return CPCRenderVert2T(cVert.m_vPos, cVert.m_fU2, cVert.m_fV2, 0.0f, 0.0f, cVert.m_nColor, cVert.m_vNormal);
		case ePCShader_Lightmap_DualTexture:
		case ePCShader_DualTexture:
			return CPCRenderVert2T(cVert.m_vPos, cVert.m_fU0, cVert.m_fV0, cVert.m_fU1, cVert.m_fV1, cVert.m_nColor, cVert.m_vNormal);
		default :
			return CPCRenderVert2T(cVert.m_vPos, cVert.m_fU0, cVert.m_fV0, 0.0f, 0.0f, cVert.m_nColor, cVert.m_vNormal);
	}
}
