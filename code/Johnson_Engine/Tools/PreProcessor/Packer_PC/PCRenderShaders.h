...
inline EPCShaderType GetPCShaderType(const CPrePoly *pPoly)
{
	...
	if ((nSurfaceFlags & SURF_VISBLOCKER) != 0)
	...
	else if ((nSurfaceFlags & (SURF_INVISIBLE | SURF_HULLMAKER | SURF_PORTAL | SURF_TERRAINOCCLUDER)) != 0)
		...
		(SURF_GOURAUDSHADE|SURF_LIGHTMAP|SURF_DIRECTIONALLIGHT|SURF_FLATSHADE|SURF_BRIGHT)) != 0)
		return ePCShader_Gouraud;
	else if ((nSurfaceFlags & SURF_PANNINGSKY) != 0)
		return ePCShader_Skypan;
	else
