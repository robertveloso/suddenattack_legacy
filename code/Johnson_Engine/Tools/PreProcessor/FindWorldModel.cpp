...
uint32 GetOldBrushType(CEditBrush *pBrush)
{
	uint32 brushType = 0;
	if(GetBoolPropVal(&pBrush->m_PropList, "Solid")) brushType |= SURF_SOLID;
	if(GetBoolPropVal(&pBrush->m_PropList, "Nonexistent")) brushType |= SURF_NONEXISTENT;
	if(GetBoolPropVal(&pBrush->m_PropList, "Invisible")) brushType |= SURF_INVISIBLE;
	if(GetBoolPropVal(&pBrush->m_PropList, "Translucent")) brushType |= SURF_TRANSPARENT;
	if(GetBoolPropVal(&pBrush->m_PropList, "SkyPortal")) brushType |= SURF_SKY;
	if(GetBoolPropVal(&pBrush->m_PropList, "FullyBright")) brushType |= SURF_BRIGHT;
	if(GetBoolPropVal(&pBrush->m_PropList, "FlatShade")) brushType |= SURF_FLATSHADE;
	if(GetBoolPropVal(&pBrush->m_PropList, "GouraudShade")) brushType |= SURF_GOURAUDSHADE;
	if(GetBoolPropVal(&pBrush->m_PropList, "LightMap") && !(brushType & SURF_INVISIBLE)) brushType |= SURF_LIGHTMAP;
	if(!GetBoolPropVal(&pBrush->m_PropList, "Subdivide")) brushType |= SURF_NOSUBDIV;
	if(GetBoolPropVal(&pBrush->m_PropList, "HullMaker")) brushType |= SURF_HULLMAKER;
	if(GetBoolPropVal(&pBrush->m_PropList, "DirectionalLight")) brushType |= SURF_DIRECTIONALLIGHT;
	if(GetBoolPropVal(&pBrush->m_PropList, "Portal")) brushType |= SURF_PORTAL;
	if(GetBoolPropVal(&pBrush->m_PropList, "SkyPan")) brushType |= SURF_PANNINGSKY;

	if(GetBoolPropVal(&pBrush->m_PropList, "TerrainOccluder"))
	{
		brushType |= SURF_TERRAINOCCLUDER;
	}

	if(GetBoolPropVal(&pBrush->m_PropList, "VisBlocker"))
	{
		brushType |= SURF_VISBLOCKER;
	}
	...
	// Ensure valid flags..
	if(brushType & SURF_PORTAL)
	{
		brushType = SURF_PORTAL | SURF_HULLMAKER | SURF_INVISIBLE | SURF_NOSUBDIV;
	}
	else if (brushType & SURF_VISBLOCKER)
	{
		brushType = SURF_VISBLOCKER | SURF_INVISIBLE | SURF_NOSUBDIV | SURF_NONEXISTENT;
	}
	else if(brushType & SURF_HULLMAKER)
	{
		brushType = SURF_HULLMAKER | SURF_INVISIBLE | SURF_NOSUBDIV;
	}
	else if(brushType & SURF_TERRAINOCCLUDER)
	{
		brushType = SURF_TERRAINOCCLUDER | SURF_NOSUBDIV | SURF_INVISIBLE;
	}
...
uint32 GetBrushFlags_Lighting(CEditBrush *pBrush)
{
	...
	else if (stricmp(pStringValue, "SkyPan") == 0)
	{
		brushType |= SURF_PANNINGSKY | SURF_GOURAUDSHADE;
	}
...
uint32 GetBrushType(CEditBrush *pBrush)
{
	...
	// Use the old brush definition if we don't have a type property
	if ((pBrush->m_PropList.GetMatchingProp("Type", PT_STRING) == LTNULL) || 
		(pBrush->m_PropList.GetMatchingProp("Lighting", PT_STRING) == LTNULL))
	{
		return GetOldBrushType(pBrush);
	}

	// Get the base lighting
	brushType = GetBrushFlags_Lighting(pBrush);
	...
	if(GetBoolPropVal(&pBrush->m_PropList, "Mirror", false))
	{
		brushType |= SURF_MIRROR;
	}
...
uint32 GetDetailLevel(CEditBrush *pBrush)
{
	uint32	val = GetBoolPropVal(&pBrush->m_PropList, "Detail", 0);

	if(val > MAX_SURFACE_DETAIL_LEVEL)
		val = MAX_SURFACE_DETAIL_LEVEL;
