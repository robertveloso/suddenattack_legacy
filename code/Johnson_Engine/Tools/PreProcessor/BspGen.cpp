...
#define POLYTYPE_HULLMAKER			2
#define POLYTYPE_DETAIL				3
#define NUM_POLYTYPES				4
#define POLYTYPE_OCCLUDER			4 // Note : These don't ever go in the BSP
#define POLYTYPE_NONEXISTENT		5
#define POLYTYPE_BLOCKER			6
#define POLYTYPE_PARTICLEBLOCKER	7

// Mask to determine if a poly shouldn't be removed, ever..)
#define MASK_NOREMOVE (SURF_HULLMAKER | SURF_VISBLOCKER | SURF_NONEXISTENT | SURF_PHYSICSBLOCKER)
...
inline uint32 GetPolyType(CPrePoly *pPoly)
{
	if (pPoly->GetSurfaceFlags() & SURF_VISBLOCKER)
		...
		return POLYTYPE_SKY;
	else if(pPoly->m_pSurface->IsDetailNode())
		return POLYTYPE_DETAIL;
	else if(pPoly->GetSurfaceFlags() & SURF_HULLMAKER)
		return POLYTYPE_HULLMAKER;
	else
...
CNode* AddNode(CNodeList &nodeList, CPrePlane *pPlane, CPrePoly *pPoly)
{
...
bool CBspGen::GenerateBspTree(CBspGenOptions *pOptions)
{
	...
	for(pos=pPolies->GetHeadPosition(); pos; )
	{
		...
		if (type >= NUM_POLYTYPES)
			continue;
		if ((type == POLYTYPE_HULLMAKER) && !pOptions->m_bAddHullMakers)
			continue;
...
bool CBspGen::MinimizeNodePolySize(NODEREF inNode, CReal maxX, CReal maxY)
{
	...
	//static int32 nMinimizeCalls=0;
	//++nMinimizeCalls;


	pNode = m_pWorld->GetNode(inNode);
	pPoly = pNode->m_pPoly;
	pSurface = pPoly->GetSurface();
...
void CBspGen::RecurseOnList(CPolyList &polies, NODEREF *pRootNode, uint32 nLevel)
{
	...
	// If the backside list only has hull makers, remove them.. these hull makers
	// are outisde the BSP.
	if(!(pSplitPoly->m_pSurface->m_Flags & SURF_HULLMAKER) && (backList->m_PolyLists[POLYTYPE_NORMAL].GetSize() == 0))
	{
		backList->m_PolyLists[POLYTYPE_HULLMAKER].RemoveAll();
	}
...
static inline int GetQuickSide(CPrePlane *pPlane, CPrePoly *pPoly)
{
...
CPrePoly* CBspGen::GetBestSplit(CPolyList &polies, uint32 nLevel)
{
	...
	if (listPos)
	{
		...
		// If we're not going to balance the tree, just use the first one
		if (!g_pGlobs->m_bBalanceBSPs)
...
void CBspGen::SplitPolyArray(CPolyList &polies, CPrePoly *pSplitOn, CPolyList *sideArrays[2])
{
	...
	if( m_StatusFn )
		m_StatusFn( this, (CReal)(m_nSplitPolyArrayCalls+1) / m_pWorld->m_Polies.GetSize() );
...
// Add hullmakers into the BSP. Pass in a blank CLinkedList when calling this
// so it can track the nodes it has added and not add hullmakers infinitely.
static void AddHullMakerBacksides_R(
	CPreWorld *pWorld,
	GenList<CNode*> &addedNodes, 
	CNode *pNode)
{
	CPrePoly *pPoly;
	GenListPos testPos;
	...
	if((pPoly->m_pSurface->m_Flags & SURF_HULLMAKER) && 
		!addedNodes.GenFindElement(pNode, testPos))
	{
		// Add the backside.
		if(pNewNode = AddBacksideNode(pWorld, pNode))
		{
			addedNodes.GenAppend(pNewNode);
		}
	}

	AddHullMakerBacksides_R(pWorld, addedNodes, pNode->m_Sides[BackSide]);
	AddHullMakerBacksides_R(pWorld, addedNodes, pNode->m_Sides[FrontSide]);
}


void AddHullMakerBacksides(CPreWorld *pWorld)
{
	CLinkedList<CNode*> newNodes;
	
	AddHullMakerBacksides_R(pWorld, newNodes, pWorld->m_RootNode);
}
...
static void ReplaceWithOriginalPolies(CPreWorld *pWorld, CNode *pRoot)
{
	...
	if(pTest = pRoot->m_pPoly->m_pOriginalBrushPoly)
	{
		ASSERT(pWorld->m_OriginalBrushPolies.FindElement(pTest) != BAD_INDEX);

		if(pTest->GetPPFlags() & PP_DONTPRESERVEORIGINAL)
		{
			// Keep the split version (this poly overlapped another poly so we
			// have to use the split version to avoid z-buffer issues).
			pTest = NULL;
		}
		else
		{
	...
	{
		ASSERT(pRoot->m_pPoly->m_pSurface->m_Flags & SURF_HULLMAKER);
	}
...
bool CBspGen::DoPostProcessing()
{
	...
	AddHullMakerBacksides(m_pWorld);
