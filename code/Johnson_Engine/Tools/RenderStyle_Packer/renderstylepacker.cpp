...
bool PackedRenderStyle(CD3DRenderStyle* pRenderStyle, FILE* f, uint32 iOverallPass, uint32* pSize)
{
	...
	if (!f) return false;		// Figure out the size on the first pass
	
	for (uint32 iPass = 0; iPass < 2; ++iPass) {
		if (iPass==1)		{ 
			if (!iOverallPass) { *pSize += iSize; return true; } 
			fwrite(&iSize,1,sizeof(iSize),f); 
		}

		// Lighting Material...
		if (iPass)			{ fwrite(&pRenderStyle->m_LightingMaterial,1,sizeof(pRenderStyle->m_LightingMaterial),f); }				else { iSize += sizeof(pRenderStyle->m_LightingMaterial); }
		...
		for (list<CD3DRenderPass>::iterator it = pRenderStyle->m_RenderPasses.begin(); it != pRenderStyle->m_RenderPasses.end(); ++it) 
		{
			...
			if (iHasRSD3DRenderPass) 
			{
				...
				if (iPass)	
				{ 
					fwrite(&it->pD3DRenderPass->bExpandForSkinning,1,sizeof(it->pD3DRenderPass->bExpandForSkinning),f); 
				}	
				else 
				{ 
					iSize += sizeof(it->pD3DRenderPass->bExpandForSkinning); 
				} 

				uint32 iSizeOf = ((uint32)&it->pD3DRenderPass->Declaration_Stream_UVCount[3] - (uint32)&it->pD3DRenderPass->ConstVector_ConstReg1 + sizeof(it->pD3DRenderPass->Declaration_Stream_UVCount[3]));
				if (iPass)	
				{ 
					fwrite(&it->pD3DRenderPass->ConstVector_ConstReg1,1,iSizeOf,f); 
				}
				else 
				{ 
					iSize += iSizeOf; 
				...
				// Vertex Shader - need to compile it if it's a .vsh file, just pack it in if it's a .ash file...
				char* szDot = strstr(it->pD3DRenderPass->VertexShaderFilename.c_str(),"."); 
				if (!szDot) { OutputMsg("Unknown file type in render style compile.\n"); { fclose(f); return false; } }

				// Get the compiled VertShaderCode...
				LPD3DXBUFFER pVertShaderCode[VERTSHADER_TYPES_COUNT]; LPD3DXBUFFER pErrors = NULL; uint32 iType;
				for (iType = 0; iType < VERTSHADER_TYPES_COUNT; ++iType) { pVertShaderCode[iType] = NULL; }
				if (stricmp(szDot,".vsh")==0) 
				{
					for (uint32 iType = 0; iType < VERTSHADER_TYPES_COUNT; ++iType) 
					{
						if (!(it->pD3DRenderPass->bExpandForSkinning) && (iType > 0)) 
							continue;	
						
						string szCode;  // Skip out if we're just doing a rigid shader...
						if (!PreProcessShaderFile(it->pD3DRenderPass->VertexShaderFilename.c_str(),szCode,iType) || D3DXAssembleShader(szCode.c_str(),szCode.size(),NULL,NULL,&pVertShaderCode[iType],&pErrors) != D3D_OK) 
						{
							string szTmp = g_WorkingDir; szTmp += "\\"; szTmp += it->pD3DRenderPass->VertexShaderFilename.c_str();
							if (!PreProcessShaderFile(szTmp.c_str(),szCode,iType) || D3DXAssembleShader(szCode.c_str(),szCode.size(),NULL,NULL,&pVertShaderCode[iType],&pErrors) != D3D_OK) 
							{
								OutputMsg("Error compiling vertex shader: %s\nFile not found or VC not installed (for .vsh files)\n",it->pD3DRenderPass->VertexShaderFilename.c_str()); if (pErrors) pErrors->Release(); 
								{ 
									fclose(f); return false; 
								} 
							} 
				...
				{
					FILE* f1 = fopen(it->pD3DRenderPass->VertexShaderFilename.c_str(),"rb");
					if (!f1) 
					{ 
						string szTmp = g_WorkingDir; szTmp += "\\"; 
						szTmp += it->pD3DRenderPass->VertexShaderFilename.c_str();
						f1 = fopen(szTmp.c_str(),"rb"); 
					}
					if (!f1) 
					{ 
						OutputMsg("Couldn't open assembled vertex shader file\n"); 
						fclose(f); 
						return false; 
					}

					uint32 iVersion = 0; fread(&iVersion,1,sizeof(iVersion),f1); 
					if (iVersion != 1) 
						return false;

					bool bExpandForSkinning = 0; 
					fread(&bExpandForSkinning,1,sizeof(bExpandForSkinning),f1);

					for (uint32 iType = 0; iType < VERTSHADER_TYPES_COUNT; ++iType) 
					{
						if (!bExpandForSkinning && (iType > 0)) 
							continue;

						uint32 iSize; fread(&iSize,1,sizeof(iSize),f1); 
						if (iSize > 1000000) 
						{ 
							fclose(f); 
							fclose(f1); 
						...
						if (D3DXCreateBuffer(iSize,&pVertShaderCode[iType])!=D3D_OK) 
						{ 
							fclose(f); 
							fclose(f1); 
							return false; 
						}
						fread(pVertShaderCode[iType]->GetBufferPointer(),1,iSize,f1); 
					}
					fclose(f1); 
				}
				else 
				{ 
					OutputMsg("Unknown file type in render style compile.\n"); 
					fclose(f); 
					return false; 
				}

				// Write out the sucker...
				for (iType = 0; iType < VERTSHADER_TYPES_COUNT; ++iType) {
					uint32 iSize1 = pVertShaderCode[iType] ? pVertShaderCode[iType]->GetBufferSize() : 0; 
					if (iPass)	{ fwrite(&iSize1,1,sizeof(iSize1),f); }																		else { iSize += sizeof(iSize); } 
					if (iSize1)  {
						if (iPass) { fwrite(pVertShaderCode[iType]->GetBufferPointer(),1,pVertShaderCode[iType]->GetBufferSize(),f); }		else { iSize += pVertShaderCode[iType]->GetBufferSize(); } } } } } }
