...
//{{AFX_MSG_MAP(CAppForm)
	...
	ON_COMMAND(IDM_FILE_COMPILE_PS2, OnCompilePS2)
	ON_COMMAND(IDM_PLATFORM_D3D, OnPlatformD3D)
	ON_COMMAND(IDM_PLATFORM_PS2, OnPlatformPS2)
	ON_COMMAND(IDM_PLATFORM_XBOX, OnPlatformXBox)
	...
	ON_UPDATE_COMMAND_UI(IDM_PLATFORM_D3D, OnUpdatePlatformD3D)
	ON_UPDATE_COMMAND_UI(IDM_PLATFORM_PS2, OnUpdatePlatformPS2)
	ON_UPDATE_COMMAND_UI(IDM_PLATFORM_XBOX, OnUpdatePlatformXBox)
...
void CAppForm::OnTextureList_Browse1()  
{
	...
	if (BrowseBox.DoModal() == IDOK) 
	{
		...
		m_pEB_TextureList_Filename[0]->SetWindowText(buffer); 	
...
void CAppForm::OnTextureList_Browse2()  
{
	...
	if (BrowseBox.DoModal() == IDOK) 
	{
		...
		m_pEB_TextureList_Filename[1]->SetWindowText(buffer); 	
...
void CAppForm::OnTextureList_Browse3()  
{
	...
	if (BrowseBox.DoModal() == IDOK) 
	{
		...
		m_pEB_TextureList_Filename[2]->SetWindowText(buffer); 	
...
void CAppForm::OnTextureList_Browse4()  
{
	...
	if (BrowseBox.DoModal() == IDOK) 
	{
		...
		m_pEB_TextureList_Filename[3]->SetWindowText(buffer); 	
...
{
	// Debug Params...
	m_pCB_DBGParam_Model->AddString("LowPoly Sphere");
	m_pCB_DBGParam_Model->AddString("HighPoly Sphere");
	...
	m_pCB_LightConfig->AddString("Light Config 3"); m_pCB_LightConfig->SetCurSel(0);
...
void CAppForm::SetDefaultRenderStyleData()
{
	...
	// Debug Params...
	m_DebugParams.bUseRefRast							= true;
...
// Load a texture file...
LPDIRECT3DTEXTURE8 CAppForm::LoadTexture(const char* szFilename)
{
	LPDIRECT3DTEXTURE8 pTexture = NULL;
	char* szDot = strstr(szFilename,"."); 
	if (szDot && szDot[0]=='.' && szDot[1]=='d' && szDot[2]=='t' && szDot[3]=='x') { 
		return LoadDTXFile(szFilename); }		// Let the DTX file loader handle it...
	else {										// Not a DTX file - let D3DX handle it...
...
void CAppForm::OnRefRast_Enable()
{
	...
	for (uint32 i = 0; i < m_TextureArray.size(); ++i) {				// Free all the textures we have loaded...
   		if (m_TextureArray[i]) { m_TextureArray[i]->Release(); m_TextureArray[i] = NULL; } }
...
bool CAppForm::CreateTheRenderObject()
{
	...
	case 6 : m_DebugParams.ModelName = "Severed_Ant_Head";
   		RenderObjectID = IDR_ANTHEADNOEYES; break;
...
void CAppForm::OutputMsg(LPSTR fmt, ...)
{
	...
	char buff[512]; va_list va; int32 iByte = sprintf(buff,"%d: ",iIndex);
	va_start(va, fmt); wvsprintf(&buff[iByte], fmt, va); va_end(va);
...
{
	char filename[255], buffer[255], extension[4]; // temporary files for path splitting
...
bool CAppForm::Create3DEnv()
{
	...
	// Make sure we're only doing software vertex processing (untill hardware supports us)...
...
void CAppForm::DebugTextureSettingChanged()
{
	...
	CString WindowText; LPDIRECT3DTEXTURE8 pTexture = NULL;
	for (i = 0; i < 4; ++i) 
	{
...
void CAppForm::SceneLightsChanged()
{
	...
	// Add a couple lights...
	D3DLIGHT8 Light; memset(&Light,0,sizeof(Light));
	...
	for (uint32 i = 0; i < 8; ++i) {					// Enable all the requested lights (make sure all the others are disabled)...
		if (i < m_DebugParams.LightCount) g_RenderStateMgr.LightEnable(i,true);
		else g_RenderStateMgr.LightEnable(i,false); }
...
//	m_pRenderStyle->SetDefaults();						// Reset to defaults...
	...
	for (uint32 i = 0;i < 4; ++i) {
		if (m_MainDlgData.RenderPass_Enable[i]) {
			...
			if (m_pRenderStyle->GetRenderPassCount() > i) { 
				if (!m_pRenderStyle->SetRenderPass(i,RenderPass)) return false; }
			else {
				if (!m_pRenderStyle->AddRenderPass(RenderPass)) return false; }

			// Now set the Direct3D render pass data...
			if (m_RenderPassData[i].VertexShader_Enable) {
				RSD3DRenderPass D3DOptions;
				D3DOptions.bUseVertexShader							= m_RenderPassData[i].VertexShader_Enable;
				D3DOptions.bExpandForSkinning						= m_RenderPassData[i].Direct3DData.bExpandForSkinning;
				D3DOptions.ConstVector_ConstReg1					= m_RenderPassData[i].Direct3DData.ConstVector_ConstReg1;
				D3DOptions.ConstVector_ConstReg2					= m_RenderPassData[i].Direct3DData.ConstVector_ConstReg2;
				D3DOptions.ConstVector_ConstReg3					= m_RenderPassData[i].Direct3DData.ConstVector_ConstReg3;
				D3DOptions.ConstVector_Param1						= m_RenderPassData[i].Direct3DData.ConstVector_Param1;
				D3DOptions.ConstVector_Param2						= m_RenderPassData[i].Direct3DData.ConstVector_Param2;
				D3DOptions.ConstVector_Param3						= m_RenderPassData[i].Direct3DData.ConstVector_Param3;
				D3DOptions.WorldViewTransform_ConstReg				= m_RenderPassData[i].Direct3DData.WorldViewTransform_ConstReg;
				D3DOptions.WorldViewTransform_Count					= m_RenderPassData[i].Direct3DData.WorldViewTransform_Count;
				D3DOptions.ProjTransform_ConstReg					= m_RenderPassData[i].Direct3DData.ProjTransform_ConstReg;
				D3DOptions.WorldViewProjTransform_ConstReg			= m_RenderPassData[i].Direct3DData.WorldViewProjTransform_ConstReg;
				D3DOptions.ViewProjTransform_ConstReg				= m_RenderPassData[i].Direct3DData.ViewProjTransform_ConstReg;
				D3DOptions.CamPos_MSpc_ConstReg						= m_RenderPassData[i].Direct3DData.CamPos_MSpc_ConstReg;
				D3DOptions.Light_Count								= m_RenderPassData[i].Direct3DData.Light_Count;
				D3DOptions.LightPosition_MSpc_ConstReg				= m_RenderPassData[i].Direct3DData.LightPosition_MSpc_ConstReg;
				D3DOptions.LightPosition_CSpc_ConstReg				= m_RenderPassData[i].Direct3DData.LightPosition_CSpc_ConstReg;
				D3DOptions.LightColor_ConstReg						= m_RenderPassData[i].Direct3DData.LightColor_ConstReg;
				D3DOptions.LightAtt_ConstReg						= m_RenderPassData[i].Direct3DData.LightAtt_ConstReg;
				D3DOptions.Material_AmbDifEm_ConstReg				= m_RenderPassData[i].Direct3DData.Material_AmbDifEm_ConstReg;
				D3DOptions.Material_Specular_ConstReg				= m_RenderPassData[i].Direct3DData.Material_Specular_ConstReg;
				D3DOptions.AmbientLight_ConstReg					= m_RenderPassData[i].Direct3DData.AmbientLight_ConstReg;
				D3DOptions.PrevWorldViewTrans_ConstReg				= m_RenderPassData[i].Direct3DData.PrevWorldViewTrans_ConstReg;
				D3DOptions.PrevWorldViewTrans_Count					= m_RenderPassData[i].Direct3DData.PrevWorldViewTrans_Count;
				D3DOptions.Last_ConstReg							= m_RenderPassData[i].Direct3DData.Last_ConstReg;


				// We need to search for where the shader file is on disk
				...
				// Try looking directly in the recorded path first
				if (fp = fopen(m_RenderPassData[i].Direct3DData.VertexShaderFilename.c_str(), "r"))
				{
					fclose(fp);
					D3DOptions.VertexShaderFilename	 = m_RenderPassData[i].Direct3DData.VertexShaderFilename; 
				} 
				else // Next, try loading as file-relative
				{
					sprintf(buffer, "%s%s", filePath, m_RenderPassData[i].Direct3DData.VertexShaderFilename.c_str());

					if (fp = fopen(buffer, "r"))	
					{
						fclose(fp);
						D3DOptions.VertexShaderFilename	 = buffer;
					}
					else // Next, try loading as project-relative
					{ 
						sprintf(buffer, "%s%s", projectPath, m_RenderPassData[i].Direct3DData.VertexShaderFilename.c_str());
						...
						{
							string szTmp = g_StartingDirectory; szTmp += "\\"; szTmp += m_RenderPassData[i].Direct3DData.VertexShaderFilename.c_str();
						
							if (fp = fopen(szTmp.c_str(), "r")) 
							{
								fclose(fp);
								D3DOptions.VertexShaderFilename	 = szTmp.c_str(); 
							}
							else 
							{ 
								D3DOptions.VertexShaderFilename	 = m_RenderPassData[i].Direct3DData.VertexShaderFilename; 
				...
				for (uint32 z=0;z<4;++z) {
					D3DOptions.bDeclaration_Stream_Position[z]		= m_RenderPassData[i].Direct3DData.bDeclaration_Stream_Position[z];
					D3DOptions.bDeclaration_Stream_Normal[z]		= m_RenderPassData[i].Direct3DData.bDeclaration_Stream_Normal[z];
					D3DOptions.bDeclaration_Stream_UVSets[z]		= m_RenderPassData[i].Direct3DData.bDeclaration_Stream_UVSets[z];
					D3DOptions.Declaration_Stream_UVCount[z]		= m_RenderPassData[i].Direct3DData.Declaration_Stream_UVCount[z];
					D3DOptions.bDeclaration_Stream_BasisVectors[z]	= m_RenderPassData[i].Direct3DData.bDeclaration_Stream_BasisVectors[z]; }
				if (!m_pRenderStyle->SetRenderPass_D3DOptions(i,&D3DOptions)) return false; } } }
	...
	if (!bNeedsReCompile && (m_pRenderStyle->m_RenderPasses.size() > 0)) {
		...
		while (it != m_pRenderStyle->m_RenderPasses.end()) {
			...
			if (pD3DRenderPass->pD3DRenderPass && pD3DRenderPass->pD3DRenderPass->bUseVertexShader && !pD3DRenderPass->hD3DVertexShader[0]) {
				bNeedsReCompile = true; } ++it; } }
	if (!m_bNoRecompile && bNeedsReCompile) { 
		m_bRecompiling = true;

		// Assemble the vertex shader(s) as needed - If it's a .vsh file, then it needs to be compiled - .ash's are already compiled...
		for (uint32 i = 0; i < 4; ++i) {
			RSD3DRenderPass D3DRenderPass; LPD3DXBUFFER* ppVertShaderCode = NULL;
			if (!m_pRenderStyle->GetRenderPass_D3DOptions(i,&D3DRenderPass)) continue;
			char* szDot = strstr(D3DRenderPass.VertexShaderFilename.c_str(),"."); 
			if (!szDot) { OutputDebugString("Unknown file type in render style compile.\n"); m_bRecompiling = false; return false; }

			// Get the compiled VertShaderCode...
			LPD3DXBUFFER pErrors = NULL;
			if (stricmp(szDot,".vsh")==0) { 
				for (uint32 iType = 0; iType < VERTSHADER_TYPES_COUNT; ++iType) {
					if (!(ppVertShaderCode = m_pRenderStyle->GetVertexShaderCode(i,iType))) continue;
					if (*ppVertShaderCode) { uint32 iRef = (*ppVertShaderCode)->Release(); assert(iRef == 0); *ppVertShaderCode = NULL; } string szCode; 
					if (!(D3DRenderPass.bExpandForSkinning) && (iType > 0)) continue;	// Skip out if we're just doing a rigid shader...

					m_RenderCS.Enter();
					if (!PreProcessShaderFile(D3DRenderPass.VertexShaderFilename.c_str(),szCode,iType) || D3DXAssembleShader(szCode.c_str(),szCode.size(),NULL,NULL,ppVertShaderCode,&pErrors) != D3D_OK) {
						string szTmp = g_StartingDirectory; szTmp += "\\"; szTmp += D3DRenderPass.VertexShaderFilename.c_str();
						if (!PreProcessShaderFile(szTmp.c_str(),szCode,iType) || D3DXAssembleShader(szCode.c_str(),szCode.size(),NULL,NULL,ppVertShaderCode,&pErrors) != D3D_OK) {
							OutputDebugString("Error compiling vertex shader.\nYou must have VC installed\n"); if (pErrors) pErrors->Release(); m_RenderCS.Leave(); m_bRecompiling = false; return false; } } 
					m_RenderCS.Leave(); } }
			else if (stricmp(szDot,".ash")==0) {
				FILE* f = NULL;
				if (!(f = fopen(D3DRenderPass.VertexShaderFilename.c_str(),"rb"))) {
					string szTmp = g_StartingDirectory; szTmp += "\\"; szTmp += D3DRenderPass.VertexShaderFilename.c_str();
					f = fopen(szTmp.c_str(),"rb"); }	
				if (!f) { OutputDebugString("Couldn't open assembled vertes shader file\n"); m_bRecompiling = false; return false; }
				uint32 iVersion = 0; fread(&iVersion,1,sizeof(iVersion),f); if (iVersion != 1) { m_bRecompiling = false; return false; }
				bool bExpandForSkinning = 0; fread(&bExpandForSkinning,1,sizeof(bExpandForSkinning),f);
				for (uint32 iType = 0; iType < VERTSHADER_TYPES_COUNT; ++iType) {
					if (!bExpandForSkinning && (iType > 0)) continue;
					if (!(ppVertShaderCode = m_pRenderStyle->GetVertexShaderCode(i,iType))) continue;
					if (*ppVertShaderCode) { uint32 iRef = (*ppVertShaderCode)->Release(); assert(iRef == 0); *ppVertShaderCode = NULL; }
					uint32 iSize; fread(&iSize,1,sizeof(iSize),f); if (iSize > 1000000) { m_bRecompiling = false; return false; } assert(*ppVertShaderCode==NULL);
					if (D3DXCreateBuffer(iSize,ppVertShaderCode)!=D3D_OK) { m_bRecompiling = false; return false; }
					fread((*ppVertShaderCode)->GetBufferPointer(),1,iSize,f); }
				fclose(f); }
			else { OutputDebugString("Unknown file type in render style compile.\n"); m_bRecompiling = false; return false; }
			if (pErrors) pErrors->Release(); }

		m_RenderCS.Enter();
		if (!m_pRenderStyle->Compile()) {
			OutputMsg("Error: RenderStyle compile failed..."); m_RenderCS.Leave(); m_bRecompiling = false; return false; }
		else {
			OutputMsg("RenderStyle compiled successfully..."); } 
		m_RenderCS.Leave(); m_bRecompiling = false; }
...
void CAppForm::DrawBackGroundImage()
{
	if (m_BackGndTexture) {														
		...
		BASIC_VERTEX_TRANSFORMED_UV v[4]; vector<LPDIRECT3DTEXTURE8> vTex; vTex.push_back(m_BackGndTexture); RenderPassOp RenderPass;
		...
		g_RenderStateMgr.SetVertexShader(BASIC_VERTEX_TRANSFORMED_UV_FLAGS);
...
void CAppForm::RenderScene()
{
	...
	if(m_pRenderObject) {
		...
		case CRenderObject::eRigidMesh : {											// Rigid...
			if (m_pRenderObject) ((CD3DRigidMesh*)m_pRenderObject)->Render(WorldMatrix[0],m_pRenderStyle,m_TextureArray); 
			if (m_pAntEyeRenderObject) m_pAntEyeRenderObject->Render(WorldMatrix[0],m_pAntEyeRenderStyle,m_TextureArray); } break;
		case CRenderObject::eSkelMesh  : { D3DXMATRIX Tmp;							// Skel..
			...
			if (m_pRenderObject) ((CD3DSkelMesh*)m_pRenderObject)->Render(WorldMatrix,m_pRenderStyle,m_TextureArray); } break; } }
