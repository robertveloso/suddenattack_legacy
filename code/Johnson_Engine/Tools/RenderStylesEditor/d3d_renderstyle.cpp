...
CD3DRenderStyle::~CD3DRenderStyle()
{
	...
	for (list<CD3DRenderPass>::iterator it = m_RenderPasses.begin(); it != m_RenderPasses.end(); ++it) {
		...
		for (uint32 i = 0; i < VERTSHADER_TYPES_COUNT; ++i) {
			if (it->hD3DVertexShader[i])						{ g_RenderStateMgr.SetVertexShader(D3DFVF_XYZ | D3DFVF_NORMAL); PD3DDEVICE->DeleteVertexShader(it->hD3DVertexShader[i]); it->hD3DVertexShader[i] = NULL; }
			if (it->pVertShaderCode[i])							{ it->pVertShaderCode[i]->Release(); it->pVertShaderCode[i] = NULL; } } }
}
...
{ 
	...
	while ((i < iPass) && (it != m_RenderPasses.end()))			{ ++it; ++i; } if (it != m_RenderPasses.end()) { 
		if (pD3DRenderPass == NULL) { 
			...
			for (uint32 i = 0; i < VERTSHADER_TYPES_COUNT; ++i) {
				if (it->hD3DVertexShader[i])					{ g_RenderStateMgr.SetVertexShader(D3DFVF_XYZ | D3DFVF_NORMAL); PD3DDEVICE->DeleteVertexShader(it->hD3DVertexShader[i]); it->hD3DVertexShader[i] = NULL; }
				if (it->pVertShaderCode[i])						{ it->pVertShaderCode[i]->Release(); it->pVertShaderCode[i] = NULL; } return true; } }
...
bool CD3DRenderStyle::Compile()
{
	...
	for (list<CD3DRenderPass>::iterator it = m_RenderPasses.begin(); it != m_RenderPasses.end(); ++it) {
		for (uint32 i = 0; i < VERTSHADER_TYPES_COUNT; ++i) {
			if (it->hD3DVertexShader[i]) {						// Get rid of the old vertex shaders...
				g_RenderStateMgr.SetVertexShader(D3DFVF_XYZ | D3DFVF_NORMAL); PD3DDEVICE->DeleteVertexShader(it->hD3DVertexShader[i]); it->hD3DVertexShader[i] = NULL; }

			if (it->pD3DRenderPass && !(it->pD3DRenderPass->bExpandForSkinning) && (i > 0)) continue;	// Skip out if we're just doing a rigid shader...
			if (it->pD3DRenderPass && it->pD3DRenderPass->bUseVertexShader) {
				...
				for (uint32 iStream = 0; iStream < 4; ++iStream) {
					...
					if ((it->pD3DRenderPass->bDeclaration_Stream_Position[iStream]) || 
						...
						(it->pD3DRenderPass->bDeclaration_Stream_BasisVectors[iStream])) { Declaration.push_back(D3DVSD_STREAM(iStream)); }
					if (it->pD3DRenderPass->bDeclaration_Stream_Position[iStream])	{ Declaration.push_back(D3DVSD_REG(iVertexReg, D3DVSDT_FLOAT3)); ++iVertexReg; }

					if (iStream==0) {							// Assume stream 0 (for now - need a switch for this)...
						switch (i+1) {							// Which type are we doing (this is bone count)...
						case 2 : 								// 2 Bones...
					...
					if (it->pD3DRenderPass->bDeclaration_Stream_Normal[iStream])	{ Declaration.push_back(D3DVSD_REG(iVertexReg, D3DVSDT_FLOAT3)); ++iVertexReg; }
...
iUsage |= D3DUSAGE_SOFTWAREPROCESSING; // TODO: Check and remove...
				...
				if (PD3DDEVICE->CreateVertexShader(&Declaration[0],(unsigned long*)(it->pVertShaderCode[i]->GetBufferPointer()),(&it->hD3DVertexShader[i]),iUsage) != D3D_OK) { return false; } } } }
