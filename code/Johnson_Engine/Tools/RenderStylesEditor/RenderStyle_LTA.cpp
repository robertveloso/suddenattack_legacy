				...
				fnListBuild.Push("renderpass");
					...
					for (uint32 j = 0; j < 4; ++j) {
						fnListBuild.Push("texturestageops");
							...
							case RENDERSTYLE_COLOROP_DECAL				: fnListBuild.AddValue("RENDERSTYLE_COLOROP_DECAL"); break;
					...
					if (m_pRenderStyle->GetRenderPass_D3DOptions(i,&D3DOptions)) {
						fnListBuild.Push("d3doptions");
							...
							fnListBuild.Pop();
							fnListBuild.Push("vertexshaderfilename"); 
							string szTmp = "\""; 
							
							// If the texture path is explicit, strip out the file or project paths, if possible
							_splitpath(D3DOptions.VertexShaderFilename.c_str(), drive, buffer, buffer, buffer);

							if (drive[0] != '\0')
							{
								if (StripPath(D3DOptions.VertexShaderFilename.c_str(), filePath, buffer))
								{
									szTmp += buffer;
								}
								else if (StripPath(D3DOptions.VertexShaderFilename.c_str(), projectPath, buffer))
								{
									szTmp += buffer;
								}
							}
							else
								szTmp += D3DOptions.VertexShaderFilename.c_str(); 
							
							szTmp += "\"";
							fnListBuild.AddValue(szTmp.c_str());
							fnListBuild.Pop();
							fnListBuild.Push("expandskinning");
							fnListBuild.AddValue(D3DOptions.bExpandForSkinning ? "1" : "0");
							fnListBuild.Pop();
							fnListBuild.Push("constvector_constreg1");
							fnListBuild.AddValue(D3DOptions.ConstVector_ConstReg1);
							fnListBuild.Pop();
							fnListBuild.Push("constvector_param1");
							fnListBuild.AddValue(D3DOptions.ConstVector_Param1.x); fnListBuild.AddValue(D3DOptions.ConstVector_Param1.y); fnListBuild.AddValue(D3DOptions.ConstVector_Param1.z); fnListBuild.AddValue(D3DOptions.ConstVector_Param1.w);
							fnListBuild.Pop();
							fnListBuild.Push("constvector_constreg2");
							fnListBuild.AddValue(D3DOptions.ConstVector_ConstReg2);
							fnListBuild.Pop();
							fnListBuild.Push("constvector_param2");
							fnListBuild.AddValue(D3DOptions.ConstVector_Param2.x); fnListBuild.AddValue(D3DOptions.ConstVector_Param2.y); fnListBuild.AddValue(D3DOptions.ConstVector_Param2.z); fnListBuild.AddValue(D3DOptions.ConstVector_Param2.w);
							fnListBuild.Pop();
							fnListBuild.Push("constvector_constreg3");
							fnListBuild.AddValue(D3DOptions.ConstVector_ConstReg3);
							fnListBuild.Pop();
							fnListBuild.Push("constvector_param3");
							fnListBuild.AddValue(D3DOptions.ConstVector_Param3.x); fnListBuild.AddValue(D3DOptions.ConstVector_Param3.y); fnListBuild.AddValue(D3DOptions.ConstVector_Param3.z); fnListBuild.AddValue(D3DOptions.ConstVector_Param3.w);
							fnListBuild.Pop();
							fnListBuild.Push("worldviewtransform_constreg");
							fnListBuild.AddValue(D3DOptions.WorldViewTransform_ConstReg);
							fnListBuild.Pop();
							fnListBuild.Push("worldviewtransform_count");
							fnListBuild.AddValue((int32)D3DOptions.WorldViewTransform_Count);
							fnListBuild.Pop();
							fnListBuild.Push("projtransform_constreg");
							fnListBuild.AddValue(D3DOptions.ProjTransform_ConstReg);
							fnListBuild.Pop();
							fnListBuild.Push("worldviewprojtransform_constreg");
							fnListBuild.AddValue(D3DOptions.WorldViewProjTransform_ConstReg);
							fnListBuild.Pop();
							fnListBuild.Push("viewprojtransform_constreg");
							fnListBuild.AddValue(D3DOptions.ViewProjTransform_ConstReg);
							fnListBuild.Pop();
							fnListBuild.Push("campos_mspc_constreg");
							fnListBuild.AddValue(D3DOptions.CamPos_MSpc_ConstReg);
							fnListBuild.Pop();
							fnListBuild.Push("light_count");
							fnListBuild.AddValue((int32)D3DOptions.Light_Count);
							fnListBuild.Pop();
							fnListBuild.Push("lightposition_mspc_constreg");
							fnListBuild.AddValue(D3DOptions.LightPosition_MSpc_ConstReg);
							fnListBuild.Pop();
							fnListBuild.Push("lightposition_cspc_constreg");
							fnListBuild.AddValue(D3DOptions.LightPosition_CSpc_ConstReg);
							fnListBuild.Pop();
							fnListBuild.Push("lightcolor_constreg");
							fnListBuild.AddValue(D3DOptions.LightColor_ConstReg);
							fnListBuild.Pop();
							fnListBuild.Push("lightatt_constreg");
							fnListBuild.AddValue(D3DOptions.LightAtt_ConstReg);
							fnListBuild.Pop();
							fnListBuild.Push("material_ambdifem_constreg");
							fnListBuild.AddValue(D3DOptions.Material_AmbDifEm_ConstReg);
							fnListBuild.Pop();
							fnListBuild.Push("material_specular_constreg");
							fnListBuild.AddValue(D3DOptions.Material_Specular_ConstReg);
							fnListBuild.Pop();
							fnListBuild.Push("ambientlight_constreg");
							fnListBuild.AddValue(D3DOptions.AmbientLight_ConstReg);
							fnListBuild.Pop();
							fnListBuild.Push("prevworldviewtrans_constreg");
							fnListBuild.AddValue(D3DOptions.PrevWorldViewTrans_ConstReg);
							fnListBuild.Pop();
							fnListBuild.Push("prevworldviewtrans_count");
							fnListBuild.AddValue((int32)D3DOptions.PrevWorldViewTrans_Count);
							fnListBuild.Pop();
//							fnListBuild.Push("last_constreg");
//							fnListBuild.AddValue(D3DOptions.Last_ConstReg);
//							fnListBuild.Pop();
							for (uint32 z = 0; z < 4; ++z) {
								char szTmp[64]; sprintf(szTmp,"%s%d","declaration_stream_position",z);
								fnListBuild.Push(szTmp);
								fnListBuild.AddValue(D3DOptions.bDeclaration_Stream_Position[z] ? "1" : "0");
								fnListBuild.Pop();
								sprintf(szTmp,"%s%d","declaration_stream_normal",z);
								fnListBuild.Push(szTmp);
								fnListBuild.AddValue(D3DOptions.bDeclaration_Stream_Normal[z] ? "1" : "0");
								fnListBuild.Pop();
								sprintf(szTmp,"%s%d","declaration_stream_uvsets",z);
								fnListBuild.Push(szTmp);
								fnListBuild.AddValue(D3DOptions.bDeclaration_Stream_UVSets[z] ? "1" : "0");
								fnListBuild.Pop();
								sprintf(szTmp,"%s%d","declaration_stream_uvcount",z);
								fnListBuild.Push(szTmp);
								fnListBuild.AddValue(D3DOptions.Declaration_Stream_UVCount[z]);
								fnListBuild.Pop();
								sprintf(szTmp,"%s%d","declaration_stream_basis",z);
								fnListBuild.Push(szTmp);
								fnListBuild.AddValue(D3DOptions.bDeclaration_Stream_BasisVectors[z] ? "1" : "0");
								fnListBuild.Pop(); }
						fnListBuild.Pop(); } 
				fnListBuild.Pop(); } 
