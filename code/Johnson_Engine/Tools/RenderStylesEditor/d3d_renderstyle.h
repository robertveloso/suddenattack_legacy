...
public:
	...
	HD3DVERTEXSHADER				hD3DVertexShader[VERTSHADER_TYPES_COUNT];				// The Vertex Shader handle we create...
...
public:
	...
	LPD3DXBUFFER*					GetVertexShaderCode(uint32 iPass, uint32 iType)			{ list<CD3DRenderPass>::iterator it = m_RenderPasses.begin(); uint32 i = 0; while ((i < iPass) && (it != m_RenderPasses.end())) { ++it; ++i; } if (it != m_RenderPasses.end()) { assert(iType < VERTSHADER_TYPES_COUNT); return &(it->pVertShaderCode[iType]); } return NULL; }
