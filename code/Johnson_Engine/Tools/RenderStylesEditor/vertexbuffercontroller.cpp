...
iUsage |= D3DUSAGE_SOFTWAREPROCESSING; // TODO: Check and remove...
	...
	if (FAILED(PD3DDEVICE->CreateVertexBuffer(iVBSizeBytes,iUsage,iVertFlags,D3DPOOL_MANAGED,&m_pVB_VertStream[iStreamNum])))				{ assert(0); return false; }
...
iUsage |= D3DUSAGE_SOFTWAREPROCESSING; // TODO: Check and remove...
	...
	if (FAILED(PD3DDEVICE->CreateIndexBuffer(iVBSizeBytes,iUsage,D3DFMT_INDEX16,D3DPOOL_MANAGED,&m_pVB_Index)))				{ assert(0); return false; }
...
void VertexBufferController::SetStreamSources()
{
	...
	if (m_pVB_Index) { 
		if (FAILED(PD3DDEVICE->SetIndices(m_pVB_Index,0)))									{ assert(0); return; } }
}
...
{
	// Render the sucker...
	if (FAILED(PD3DDEVICE->DrawIndexedPrimitive(D3DPT_TRIANGLELIST,MinVert,VertCount,MinIndex,TriCount)))	{ return; } //assert(0); return; }
}
...
bool VertexBufferController::Lock(VB_TYPE Type,bool bDiscardContents)
{
	...
	case eVERTSTREAM0 :
		...
		if (FAILED(m_pVB_VertStream[0]->Lock(0,0,(uint8**)&m_pVert_Data[0],uFlags)))		{ assert(0); return false; } break;
	case eVERTSTREAM1 :
		if (m_pVert_Data[1]) return true;
		if (FAILED(m_pVB_VertStream[1]->Lock(0,0,(uint8**)&m_pVert_Data[1],uFlags)))		{ assert(0); return false; } break;
	case eVERTSTREAM2 :
		if (m_pVert_Data[2]) return true;
		if (FAILED(m_pVB_VertStream[2]->Lock(0,0,(uint8**)&m_pVert_Data[2],uFlags)))		{ assert(0); return false; } break;
	case eVERTSTREAM3 :
		if (m_pVert_Data[3]) return true;
		if (FAILED(m_pVB_VertStream[3]->Lock(0,0,(uint8**)&m_pVert_Data[3],uFlags)))		{ assert(0); return false; } break;
	case eINDEX :
		if (m_pIndex_Data) return true;
		if (FAILED(m_pVB_Index->Lock(0,0,(uint8**)&m_pIndex_Data,uFlags)))					{ assert(0); return false; } break;
	default : assert(0); return false; }
	return true;
}
