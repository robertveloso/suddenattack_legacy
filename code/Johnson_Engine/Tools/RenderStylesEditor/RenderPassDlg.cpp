...
BEGIN_MESSAGE_MAP(CRenderPassDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_VERTEXSHADER_CONFIG, OnD3DOptions_VertexShader_Config)
...
BOOL CRenderPassDlg::OnInitDialog()
{
	...
	m_pBn_VertexShader_Config	= (CButton*)GetDlgItem(IDC_VERTEXSHADER_CONFIG);	assert(m_pBn_VertexShader_Config);
...
uint32 CRenderPassDlg::GetIDfromRSEnum_ColorOp(uint32 RSEnum) const
{
	...
	switch (RSEnum)
	{
		...
		case RENDERSTYLE_COLOROP_BUMPENVMAPLUM:
			...
			return m_pCB_ColorOp[0]->FindStringExact(-1, str);
			break;
		case RENDERSTYLE_COLOROP_DECAL:
			strcpy(str, "Decal");
			return m_pCB_ColorOp[0]->FindStringExact(-1, str);
			break;
...
ERenStyle_ColorOp CRenderPassDlg::StringToColorOp(const char* pszString)
{
	...
	else if (strcmp(pszString, "Decal") == 0)			return RENDERSTYLE_COLOROP_DECAL;
...
void CRenderPassDlg::LimitDialogByPlatforms(bool platformD3D, bool platformPS2, bool platformXBox)
{
	int index;
	...
	if (platformPS2) 
	{
		index = m_pCB_BlendOp->FindStringExact(-1, "Cs * (1 - Cd) + Cd");
		ASSERT(index != CB_ERR);
		m_pCB_BlendOp->DeleteString(index);

		index = m_pCB_BlendOp->FindStringExact(-1, "Cs * Cs + Cd * (1 - Cs)");
		ASSERT(index != CB_ERR);
		m_pCB_BlendOp->DeleteString(index);

		index = m_pCB_BlendOp->FindStringExact(-1, "Cs * Cd + Cd * (1 - Cd)");
		ASSERT(index != CB_ERR);
		m_pCB_BlendOp->DeleteString(index);

		index = m_pCB_BlendOp->FindStringExact(-1, "Cs * Cs + Cd * Cd");
		ASSERT(index != CB_ERR);
		m_pCB_BlendOp->DeleteString(index);

		index = m_pCB_BlendOp->FindStringExact(-1, "Cs * Cs + Cd");
		ASSERT(index != CB_ERR);
		m_pCB_BlendOp->DeleteString(index);

		index = m_pCB_BlendOp->FindStringExact(-1, "Cs * Cd");
		ASSERT(index != CB_ERR);
		m_pCB_BlendOp->DeleteString(index);
	}

	index = GetIDfromRSEnum_Blend(m_pRenderPassData->Blend);
	if (index == -1)  index = 0;
	m_pCB_BlendOp->SetCurSel(index);
	OnBlend();


	// Color Arg 1 ComboBox

	m_pCB_ColorArg1[0]->ResetContent();

	m_pCB_ColorArg1[0]->AddString("Current");	
	m_pCB_ColorArg1[0]->AddString("Diffuse");			
	m_pCB_ColorArg1[0]->AddString("Texture");
	m_pCB_ColorArg1[0]->AddString("TFactor");
	
	if (platformPS2) 
	{
		index = m_pCB_ColorArg1[0]->FindStringExact(-1, "Current");
		ASSERT(index != CB_ERR);
		m_pCB_ColorArg1[0]->DeleteString(index);

		index = m_pCB_ColorArg1[0]->FindStringExact(-1, "TFactor");
		ASSERT(index != CB_ERR);
		m_pCB_ColorArg1[0]->DeleteString(index);
	}

	index = GetIDfromRSEnum_ColorArg1(m_pRenderPassData->ColorArg1[0]);
	if (index == -1)  index = 0;
	m_pCB_ColorArg1[0]->SetCurSel(index);
	OnColorArg11();


	// Color Op ComboBox

	m_pCB_ColorOp[0]->ResetContent();

	m_pCB_ColorOp[0]->AddString("Disable");	
	m_pCB_ColorOp[0]->AddString("Select Arg1");			
	m_pCB_ColorOp[0]->AddString("Select Arg2");
	m_pCB_ColorOp[0]->AddString("Modulate");
	m_pCB_ColorOp[0]->AddString("Modulate2x");	
	m_pCB_ColorOp[0]->AddString("Mod CurAlpha");			
	m_pCB_ColorOp[0]->AddString("Mod TFactor");
	m_pCB_ColorOp[0]->AddString("Add");
	m_pCB_ColorOp[0]->AddString("DotProduct3");	
	m_pCB_ColorOp[0]->AddString("BumpEnvMap");			
	m_pCB_ColorOp[0]->AddString("BumpEnvMapLum");
	m_pCB_ColorOp[0]->AddString("Decal");
	m_pCB_ColorOp[0]->AddString("Highlight");
	m_pCB_ColorOp[0]->AddString("Highlight2");
	
	if (platformD3D || platformXBox)
	{
		index = m_pCB_ColorOp[0]->FindStringExact(-1, "Decal");
		ASSERT(index != CB_ERR);
		m_pCB_ColorOp[0]->DeleteString(index);

		index = m_pCB_ColorOp[0]->FindStringExact(-1, "Highlight");
		ASSERT(index != CB_ERR);
		m_pCB_ColorOp[0]->DeleteString(index);

		index = m_pCB_ColorOp[0]->FindStringExact(-1, "Highlight2");
		ASSERT(index != CB_ERR);
		m_pCB_ColorOp[0]->DeleteString(index);
	}
	if (platformPS2) 
	{
		index = m_pCB_ColorOp[0]->FindStringExact(-1, "Select Arg1");
		ASSERT(index != CB_ERR);
		m_pCB_ColorOp[0]->DeleteString(index);

		index = m_pCB_ColorOp[0]->FindStringExact(-1, "Select Arg2");
		ASSERT(index != CB_ERR);
		m_pCB_ColorOp[0]->DeleteString(index);

		index = m_pCB_ColorOp[0]->FindStringExact(-1, "Modulate2x");
		ASSERT(index != CB_ERR);
		m_pCB_ColorOp[0]->DeleteString(index);

		index = m_pCB_ColorOp[0]->FindStringExact(-1, "Mod CurAlpha");
		ASSERT(index != CB_ERR);
		m_pCB_ColorOp[0]->DeleteString(index);

		index = m_pCB_ColorOp[0]->FindStringExact(-1, "Mod TFactor");
		ASSERT(index != CB_ERR);
		m_pCB_ColorOp[0]->DeleteString(index);

		index = m_pCB_ColorOp[0]->FindStringExact(-1, "Add");
		ASSERT(index != CB_ERR);
		m_pCB_ColorOp[0]->DeleteString(index);

		index = m_pCB_ColorOp[0]->FindStringExact(-1, "DotProduct3");
		ASSERT(index != CB_ERR);
		m_pCB_ColorOp[0]->DeleteString(index);

		index = m_pCB_ColorOp[0]->FindStringExact(-1, "BumpEnvMap");
		ASSERT(index != CB_ERR);
		m_pCB_ColorOp[0]->DeleteString(index);

		index = m_pCB_ColorOp[0]->FindStringExact(-1, "BumpEnvMapLum");
		ASSERT(index != CB_ERR);
		m_pCB_ColorOp[0]->DeleteString(index);
	}

	index = GetIDfromRSEnum_ColorOp(m_pRenderPassData->ColorOp[0]);
	if (index == -1)  index = 0;
	m_pCB_ColorOp[0]->SetCurSel(index);
	OnColorOp1();


	// TexFilter ComboBox

	m_pCB_TexFilter[0]->ResetContent();

	m_pCB_TexFilter[0]->AddString("Point");	
	m_pCB_TexFilter[0]->AddString("BiLinear");			
	m_pCB_TexFilter[0]->AddString("TriLinear");
	m_pCB_TexFilter[0]->AddString("Anisotropic");
	m_pCB_TexFilter[0]->AddString("Point w/ PtMip");
	
	if (platformPS2) 
	{
		index = m_pCB_TexFilter[0]->FindStringExact(-1, "TriLinear");
		ASSERT(index != CB_ERR);
		m_pCB_TexFilter[0]->DeleteString(index);

		index = m_pCB_TexFilter[0]->FindStringExact(-1, "Anisotropic");
		ASSERT(index != CB_ERR);
		m_pCB_TexFilter[0]->DeleteString(index);

		index = m_pCB_TexFilter[0]->FindStringExact(-1, "Point w/ PtMip");
		ASSERT(index != CB_ERR);
		m_pCB_TexFilter[0]->DeleteString(index);
	}

	index = GetIDfromRSEnum_TexFilter(m_pRenderPassData->TexFilter[0]);
	if (index == -1)  index = 0;
	m_pCB_TexFilter[0]->SetCurSel(index);
	OnTexFilter1();


	// UAddress ComboBox

	m_pCB_UAddress[0]->ResetContent();

	m_pCB_UAddress[0]->AddString("Wrap");	
	m_pCB_UAddress[0]->AddString("Clamp");			
	m_pCB_UAddress[0]->AddString("Mirror");
	m_pCB_UAddress[0]->AddString("MirrorOnce");
	...
	if (platformPS2) 
	{
		index = m_pCB_UAddress[0]->FindStringExact(-1, "Mirror");
		ASSERT(index != CB_ERR);
		m_pCB_UAddress[0]->DeleteString(index);

		index = m_pCB_UAddress[0]->FindStringExact(-1, "MirrorOnce");
		ASSERT(index != CB_ERR);
		m_pCB_UAddress[0]->DeleteString(index);
	}

	index = GetIDfromRSEnum_UAddress(m_pRenderPassData->UAddress[0]);
	if (index == -1)  index = 0;
	m_pCB_UAddress[0]->SetCurSel(index);
	OnUAddress1();


	// VAddress ComboBox

	m_pCB_VAddress[0]->ResetContent();

	m_pCB_VAddress[0]->AddString("Wrap");	
	m_pCB_VAddress[0]->AddString("Clamp");			
	m_pCB_VAddress[0]->AddString("Mirror");
	m_pCB_VAddress[0]->AddString("MirrorOnce");
	...
	if (platformPS2) 
	{
		index = m_pCB_VAddress[0]->FindStringExact(-1, "Mirror");
		ASSERT(index != CB_ERR);
		m_pCB_VAddress[0]->DeleteString(index);

		index = m_pCB_VAddress[0]->FindStringExact(-1, "MirrorOnce");
		ASSERT(index != CB_ERR);
		m_pCB_VAddress[0]->DeleteString(index);
	}

	index = GetIDfromRSEnum_VAddress(m_pRenderPassData->VAddress[0]);
	if (index == -1)  index = 0;
	m_pCB_VAddress[0]->SetCurSel(index);
	OnVAddress1();


	// Render States

	GetDlgItem(IDC_TEXTUREFACTOR)->EnableWindow(true);
	GetDlgItem(IDC_FILLMODE)->EnableWindow(true);

	if (platformPS2) 
	{
	GetDlgItem(IDC_TEXTUREFACTOR)->EnableWindow(false);
	GetDlgItem(IDC_FILLMODE)->EnableWindow(false);
	}


	// Texture Stage 1

	GetDlgItem(IDC_COLORARG11)->EnableWindow(true);
	GetDlgItem(IDC_COLORARG21)->EnableWindow(true);
	GetDlgItem(IDC_ALPHAARG11)->EnableWindow(true);
	GetDlgItem(IDC_ALPHAOP1)->EnableWindow(true);
	GetDlgItem(IDC_ALPHAARG21)->EnableWindow(true);
	GetDlgItem(IDC_UVSOURCE1)->EnableWindow(true);

	if (platformPS2) 
	{
	GetDlgItem(IDC_COLORARG11)->EnableWindow(false);
	m_pCB_ColorArg2[0]->SelectString(-1, "Diffuse");
	GetDlgItem(IDC_COLORARG21)->EnableWindow(false);
	GetDlgItem(IDC_ALPHAARG11)->EnableWindow(false);
	GetDlgItem(IDC_ALPHAOP1)->EnableWindow(false);
	GetDlgItem(IDC_ALPHAARG21)->EnableWindow(false);
	GetDlgItem(IDC_UVSOURCE1)->EnableWindow(false);
	m_pCB_UVSource[0]->SelectString(-1, "UV Set 0");
	OnUVSource1();
	}
	...
	GetDlgItem(IDC_TEXTURE2)->EnableWindow(true);
	GetDlgItem(IDC_COLORARG12)->EnableWindow(true);
	GetDlgItem(IDC_COLOROP2)->EnableWindow(true);
	GetDlgItem(IDC_COLORARG22)->EnableWindow(true);
	GetDlgItem(IDC_ALPHAARG12)->EnableWindow(true);
	GetDlgItem(IDC_ALPHAOP2)->EnableWindow(true);
	GetDlgItem(IDC_ALPHAARG22)->EnableWindow(true);
	GetDlgItem(IDC_UVSOURCE2)->EnableWindow(true);
	GetDlgItem(IDC_UADDRESS2)->EnableWindow(true);
	GetDlgItem(IDC_VADDRESS2)->EnableWindow(true);
	GetDlgItem(IDC_TEXFILTER2)->EnableWindow(true);
	GetDlgItem(IDC_UVTRANSFORM_ENABLE2)->EnableWindow(true);
	GetDlgItem(IDC_UVTRANSFORM_MATRIX2)->EnableWindow(true);

	if (platformPS2) 
	{
	GetDlgItem(IDC_TEXTURE2)->EnableWindow(false);
	GetDlgItem(IDC_COLORARG12)->EnableWindow(false);
	GetDlgItem(IDC_COLOROP2)->EnableWindow(false);
	GetDlgItem(IDC_COLORARG22)->EnableWindow(false);
	GetDlgItem(IDC_ALPHAARG12)->EnableWindow(false);
	GetDlgItem(IDC_ALPHAOP2)->EnableWindow(false);
	GetDlgItem(IDC_ALPHAARG22)->EnableWindow(false);
	GetDlgItem(IDC_UVSOURCE2)->EnableWindow(false);
	GetDlgItem(IDC_UADDRESS2)->EnableWindow(false);
	GetDlgItem(IDC_VADDRESS2)->EnableWindow(false);
	GetDlgItem(IDC_TEXFILTER2)->EnableWindow(false);
	GetDlgItem(IDC_UVTRANSFORM_ENABLE2)->EnableWindow(false);
	GetDlgItem(IDC_UVTRANSFORM_MATRIX2)->EnableWindow(false);
	}


	// Texture Stage 3

	GetDlgItem(IDC_TEXTURE3)->EnableWindow(true);
	GetDlgItem(IDC_COLORARG13)->EnableWindow(true);
	GetDlgItem(IDC_COLOROP3)->EnableWindow(true);
	GetDlgItem(IDC_COLORARG23)->EnableWindow(true);
	GetDlgItem(IDC_ALPHAARG13)->EnableWindow(true);
	GetDlgItem(IDC_ALPHAOP3)->EnableWindow(true);
	GetDlgItem(IDC_ALPHAARG23)->EnableWindow(true);
	GetDlgItem(IDC_UVSOURCE3)->EnableWindow(true);
	GetDlgItem(IDC_UADDRESS3)->EnableWindow(true);
	GetDlgItem(IDC_VADDRESS3)->EnableWindow(true);
	GetDlgItem(IDC_TEXFILTER3)->EnableWindow(true);
	GetDlgItem(IDC_UVTRANSFORM_ENABLE3)->EnableWindow(true);
	GetDlgItem(IDC_UVTRANSFORM_MATRIX3)->EnableWindow(true);

	if (platformPS2) 
	{
	GetDlgItem(IDC_TEXTURE3)->EnableWindow(false);
	GetDlgItem(IDC_COLORARG13)->EnableWindow(false);
	GetDlgItem(IDC_COLOROP3)->EnableWindow(false);
	GetDlgItem(IDC_COLORARG23)->EnableWindow(false);
	GetDlgItem(IDC_ALPHAARG13)->EnableWindow(false);
	GetDlgItem(IDC_ALPHAOP3)->EnableWindow(false);
	GetDlgItem(IDC_ALPHAARG23)->EnableWindow(false);
	GetDlgItem(IDC_UVSOURCE3)->EnableWindow(false);
	GetDlgItem(IDC_UADDRESS3)->EnableWindow(false);
	GetDlgItem(IDC_VADDRESS3)->EnableWindow(false);
	GetDlgItem(IDC_TEXFILTER3)->EnableWindow(false);
	GetDlgItem(IDC_UVTRANSFORM_ENABLE3)->EnableWindow(false);
	GetDlgItem(IDC_UVTRANSFORM_MATRIX3)->EnableWindow(false);
	}


	// Texture Stage 4

	GetDlgItem(IDC_TEXTURE4)->EnableWindow(true);
	GetDlgItem(IDC_COLORARG14)->EnableWindow(true);
	GetDlgItem(IDC_COLOROP4)->EnableWindow(true);
	GetDlgItem(IDC_COLORARG24)->EnableWindow(true);
	GetDlgItem(IDC_ALPHAARG14)->EnableWindow(true);
	GetDlgItem(IDC_ALPHAOP4)->EnableWindow(true);
	GetDlgItem(IDC_ALPHAARG24)->EnableWindow(true);
	GetDlgItem(IDC_UVSOURCE4)->EnableWindow(true);
	GetDlgItem(IDC_UADDRESS4)->EnableWindow(true);
	GetDlgItem(IDC_VADDRESS4)->EnableWindow(true);
	GetDlgItem(IDC_TEXFILTER4)->EnableWindow(true);
	GetDlgItem(IDC_UVTRANSFORM_ENABLE4)->EnableWindow(true);
	GetDlgItem(IDC_UVTRANSFORM_MATRIX4)->EnableWindow(true);

	if (platformPS2) 
	{
	GetDlgItem(IDC_TEXTURE4)->EnableWindow(false);
	GetDlgItem(IDC_COLORARG14)->EnableWindow(false);
	GetDlgItem(IDC_COLOROP4)->EnableWindow(false);
	GetDlgItem(IDC_COLORARG24)->EnableWindow(false);
	GetDlgItem(IDC_ALPHAARG14)->EnableWindow(false);
	GetDlgItem(IDC_ALPHAOP4)->EnableWindow(false);
	GetDlgItem(IDC_ALPHAARG24)->EnableWindow(false);
	GetDlgItem(IDC_UVSOURCE4)->EnableWindow(false);
	GetDlgItem(IDC_UADDRESS4)->EnableWindow(false);
	GetDlgItem(IDC_VADDRESS4)->EnableWindow(false);
	GetDlgItem(IDC_TEXFILTER4)->EnableWindow(false);
	GetDlgItem(IDC_UVTRANSFORM_ENABLE4)->EnableWindow(false);
	GetDlgItem(IDC_UVTRANSFORM_MATRIX4)->EnableWindow(false);
	}


	// Direct3D

	GetDlgItem(IDC_VERTEXSHADER_CONFIG)->EnableWindow(true);
	GetDlgItem(IDC_VERTEXSHADER_ENABLE)->EnableWindow(true);
	GetDlgItem(IDC_BUMPENVMAP)->EnableWindow(true);
	GetDlgItem(IDC_BUMPENVMAP_STAGE)->EnableWindow(true);
	GetDlgItem(IDC_BUMPENVMAP_SCALE)->EnableWindow(true);
	GetDlgItem(IDC_BUMPENVMAP_OFFSET)->EnableWindow(true);

	if (platformPS2) 
	{
	GetDlgItem(IDC_VERTEXSHADER_CONFIG)->EnableWindow(false);
	GetDlgItem(IDC_VERTEXSHADER_ENABLE)->EnableWindow(false);
	GetDlgItem(IDC_BUMPENVMAP)->EnableWindow(false);
	GetDlgItem(IDC_BUMPENVMAP_STAGE)->EnableWindow(false);
	GetDlgItem(IDC_BUMPENVMAP_SCALE)->EnableWindow(false);
	GetDlgItem(IDC_BUMPENVMAP_OFFSET)->EnableWindow(false);
	}
}