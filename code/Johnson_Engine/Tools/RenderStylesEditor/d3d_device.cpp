...
bool CD3D_Device::CreateDevice(D3DAdapterInfo* pAdapter,D3DDeviceInfo* pDevice,D3DModeInfo* pMode)
{
	...
	if ((hResult != D3D_OK) || !m_pD3DDevice) {	// Give it more more try - Presentation params might have been changed by D3D to something acceptable...
		...
		if ((hResult != D3D_OK) || !m_pD3DDevice) { FreeAll(); return false; } }

	if (FAILED(m_pD3DDevice->GetDeviceCaps(&m_DeviceCaps)))	{ FreeAll(); return false; }
...
void CD3D_Device::SetPresentationParams(D3DPRESENT_PARAMETERS& PresentationParam,D3DModeInfo* pMode)
{
	...
	PresentationParam.FullScreen_PresentationInterval	= NULL;
}
...
D3DMULTISAMPLE_TYPE CD3D_Device::GetDefaultMultiSampleType(uint32 Samples)
{
	...
	if (PDIRECT3D->CheckDeviceMultiSampleType(m_pAdapter->iAdapterNum,m_pDevice->DeviceType,m_pMode->Format,g_bRunWindowed,MultiSampleType) != D3D_OK) {
