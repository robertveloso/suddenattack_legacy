...
public:
	...
	inline HRESULT		SetVertexShader(HD3DVERTEXSHADER hVertShader)						{
		if (hVertShader != m_VertexShader) { m_VertexShader = hVertShader; return PD3DDEVICE->SetVertexShader(m_VertexShader); } return D3D_OK; }
	inline HD3DVERTEXSHADER GetVertexShader()												{ return m_VertexShader; }
	...
	inline void			SetMaterial(D3DMATERIAL8& Material)									{ m_Material = Material; PD3DDEVICE->SetMaterial(&Material); }
	inline HRESULT		SetLight(uint32 iLight, D3DLIGHT8* pLight)							{ assert(iLight < MAX_D3DLIGHTS); m_Lights[iLight] = *pLight; return PD3DDEVICE->SetLight(iLight,pLight); }
	inline D3DLIGHT8	GetLight(uint32 iLight)												{ assert(iLight < MAX_D3DLIGHTS); return m_Lights[iLight]; }
	...
	// HELPER FUNCTIONS...
	bool				SetRenderStyleStates(CD3DRenderStyle* pRenderStyle, uint32 iRenderPass,vector<LPDIRECT3DTEXTURE8>& TextureList);		// Set the render states from a particular pass of a render style...
...
private:
	...
	D3DLIGHT8			m_Lights[MAX_D3DLIGHTS];
