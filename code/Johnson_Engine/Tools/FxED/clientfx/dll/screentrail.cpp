					 ...
					 uint32 dwExtraFlags)
{
	...
	while (pNode->m_pNext)
	{
		...
		*pVerts ++ = vStart - vBisector1;
		...
		SetupVert(pTri, 1, g_pVerts[g_nVerts + 1].x, g_pVerts[g_nVerts + 1].y, g_pVerts[g_nVerts + 1].z, r2, g2, b2, a2, u2, 1.0f);
		...
		SetupVert(pTri, 2, g_pVerts[g_nVerts + 3].x, g_pVerts[g_nVerts + 3].y, g_pVerts[g_nVerts + 3].z, r1, g1, b1, a1, u1, 0.0f);
