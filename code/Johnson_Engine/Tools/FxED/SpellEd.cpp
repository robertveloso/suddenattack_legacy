...
//		return FALSE;
	...
	if (sRezFile.IsEmpty())
	{
		...
		if (sRezFile.IsEmpty())
		{
			AfxMessageBox("You must fill out the resource file location!!!");
			return FALSE;
		}
	}
	...
	{
		AfxMessageBox("Could not open resource file !!");
		...
		{
			sRezFile = dlg.m_sRez;
			CString sDllPath = dlg.m_sDll;

			WriteProfileString("", "ResourceFileLocation", sRezFile);
			WriteProfileString("", "DllLocation", sDllPath);
		}
