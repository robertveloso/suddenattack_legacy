...
void CTrackWnd::OnDestroy() 
{
	if (m_pFxPropDlg)
	{
		m_pFxPropDlg->DestroyWindow();
		delete m_pFxPropDlg;
		m_pFxPropDlg = NULL;
	}
...
void CTrackWnd::OnTrackEditKey() 
{
	...
	m_pFxPropDlg = new CFxPropDlg();

//	if (!m_pFxPropDlg->IsCreated())
	CString strTemp;
	GetParent()->GetParent()->GetWindowText(strTemp);

	m_pFxPropDlg->CreateDlg(m_pSelKey, m_pSelKey->GetCollProps());

	m_pFxPropDlg->ShowWindow(SW_SHOW);

	CString strTemp2;
	m_pFxPropDlg->GetWindowText(strTemp2);

	strTemp2 += " - ";
	strTemp2 += strTemp;
	m_pFxPropDlg->SetWindowText(strTemp2);
