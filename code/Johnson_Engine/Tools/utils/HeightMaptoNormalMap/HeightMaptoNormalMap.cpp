...
BOOL CApp::InitInstance()
{
	...
	if (cmdInfo.GetParamVal("scale"))					{ g_fScale		= atof(cmdInfo.GetParamVal("scale")); }
...
void DoConversion(const char* szInputFile, const char* szOutputFile)
{
	...
	LPDIRECT3DTEXTURE8 pSrcSurf	  = NULL;
	...
	for (j = 0; j < SurfDesc.Height; ++j) {
		g_UpdateDlg.SetProgress((float)j/(float)SurfDesc.Height * 50.0f);
		...
		for (i = 0; i < SurfDesc.Width; ++i) {
            ...
            D3DXVECTOR3 thePoint0(u,				   v,					   theHeight0);
            D3DXVECTOR3 thePoint1(u + oneTexelWidth,   v,					   theHeight1);
            D3DXVECTOR3 thePoint2(u,				   v + oneTexelHeight,	   theHeight2);
            D3DXVECTOR3 thePoint3(u + oneTexelWidth,   v + oneTexelHeight,	   theHeight3);
            D3DXVECTOR3 thePoint4(u+oneTexelWidth/2.0, v + oneTexelHeight/2.0, theHeight4);
    ...
    for (j = 0; j < SurfDesc.Height; ++j) {
		g_UpdateDlg.SetProgress(50.0f + ((float)j/(float)SurfDesc.Height * 40.0f));
		if (g_UpdateDlg.WasCancelPressed())				{ goto BAIL_AND_EXIT; }
        for (i = 0; i < SurfDesc.Width; ++i) {
			...
			AlphaAndVectorToRGBA(theHeight0, theNormal, theNewColor, g_fMinimumUp);
