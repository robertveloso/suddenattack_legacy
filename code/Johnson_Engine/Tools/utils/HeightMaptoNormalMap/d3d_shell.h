...
struct D3DAdapterInfo
{
    ...
    D3DADAPTER_IDENTIFIER8	AdapterID;		// Adapter data
...
struct D3DModeInfo
{
	...
	bool operator < (const D3DModeInfo& other) { 
...
public:
	...
	D3DAdapterInfo*			GetAdapterInfo(int iAdapterID)							{ if (iAdapterID < m_AdapterList.size()) return &m_AdapterList[iAdapterID]; return NULL; }
	D3DDeviceInfo*			GetDeviceInfo(int iAdapterID,int iDeviceID)				{ D3DAdapterInfo* pAdapter = GetAdapterInfo(iAdapterID); if (!pAdapter) return NULL; if (iDeviceID < pAdapter->Devices.size()) return &pAdapter->Devices[iDeviceID]; return NULL; }
	D3DModeInfo*			GetModeInfo(int iAdapterID,int iDeviceID,int iModeID)	{ D3DDeviceInfo* pDevice = GetDeviceInfo(iAdapterID,iDeviceID); if (!pDevice) return NULL; if (iModeID < pDevice->Modes.size()) return &pDevice->Modes[iModeID]; return NULL; }
