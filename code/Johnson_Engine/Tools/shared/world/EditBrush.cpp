...
// Splits the brush on pRoot's plane and recurses.
void BspSplitBrush( CEditRegion *pRegion, CEditPoly *pRoot, CEditBrush *pToSplit, CMoArray<CEditBrush*> *pSides[] )
{
	CEditBrush *newBrushes[2];
	uint32 i;

	SimpleSplitBrush(pRegion, pRoot, pToSplit, newBrushes);
	
	// Either recurse or add the polies to the right list.
	for( i=0; i < 2; i++ )
	{
		if( newBrushes[i]->m_Polies < 4 )
		{
			no_DestroyNode(pRegion, newBrushes[i], FALSE);
		}
		else
		{
			if( pRoot->m_Sides[i] )
			{
				BspSplitBrush( pRegion, pRoot->m_Sides[i], newBrushes[i], pSides );
				no_DestroyNode(pRegion, newBrushes[i], FALSE);
			}
			else
				pSides[i]->Append( newBrushes[i] );
		}
	}
}


BOOL CEditBrush::Subtract( CEditRegion *pRegion, CEditBrush *pOther, CEditBrush *pBsp, CMoArray<CEditBrush*> &newBrushes )
{
	CMoArray<CEditBrush*>	*sides[2];
	CMoArray<CEditBrush*>	backSideArray;
	uint32 i, nOnFront, nOnBack;
	

	// Chop up the input brush.
	sides[FrontSide] = &newBrushes;
	sides[BackSide] = &backSideArray;
	
	BspSplitBrush( pRegion, pBsp->m_pBspRoot, pOther, sides );

	// Erase the back side.
	nOnFront = sides[FrontSide]->GetSize();
	nOnBack = sides[BackSide]->GetSize();

	for(i=0; i < sides[BackSide]->GetSize(); i++)
	{
		no_DestroyNode(pRegion, sides[BackSide]->Get(i), FALSE);
	}

	// See if anything actually needed to be split.
	if( (nOnBack == 0) || (nOnFront == 0) )
	{
...
// ----------------------------------------------------------------------- //
//      ROUTINE:        CEditBrush::ChopUpBrush and SplitPolyList
//      PURPOSE:        These do boolean operations, and output each side as
//                      a single brush.
// ----------------------------------------------------------------------- //

CEditBrush* CEditBrush::ChopUpBrush( CEditBrush *pOther, CEditBrush *pBsp, BOOL *keepSides, uint32 *sideIndices )
{
	ASSERT( m_Polies > 0 );


	CEditBrush *pNew = new CEditBrush;
	CMoArray<CEditPoly*> fullSides[2];


	// Create the new one and split it up.
	pNew = new CEditBrush;
	pNew->CopyEditBrush( pOther );

	BspSplitPolyList( pBsp->m_pBspRoot, pNew, pNew->m_Polies, fullSides );

	// Put the necessary stuff in pNew and clean it up.
	pNew->m_Polies.Term();
	for(uint32 i=0; i < 2; i++ )
	{
		if( keepSides[i] )
		{
			sideIndices[i] = pNew->m_Polies.GetSize();
			pNew->m_Polies.AppendArray( fullSides[i] );
		}
		else
			DeleteAndClearArray( fullSides[i] );
	}

	pNew->RemoveUnusedVerts();
	return pNew;	
}


void CEditBrush::BspSplitPolyList(  CEditPoly *pRoot, CEditBrush *pAddStuffTo, CMoArray<CEditPoly*> &polies, 
									CMoArray<CEditPoly*> *fullSides )
{
	PolySide				side;
	CBasePoly				*newPolies[2];
	CMoArray<CEditPoly*>	sides[2];
	...
	for( i=0; i < polies; i++ )
	{
		side = pRoot->m_Plane.GetPolySide( polies[i] );
		if( side == Intersect )
		{
			newPolies[0] = new CEditPoly( pAddStuffTo );
			newPolies[1] = new CEditPoly( pAddStuffTo );

			pRoot->m_Plane.SplitPoly( polies[i], newPolies, pAddStuffTo->m_Points );
			sides[0].Append( (CEditPoly*)newPolies[0] );
			sides[1].Append( (CEditPoly*)newPolies[1] );
			
			delete polies[i];
		}
		else
			sides[side].Append( polies[i] );
	}
	...
	{
		if( pRoot->m_Sides[i] )
			BspSplitPolyList( pRoot->m_Sides[i], pAddStuffTo, sides[i], fullSides );
		else
			fullSides[i].AppendArray( sides[i] );
	}
}


CEditBrush* CEditBrush::GetBsp()
{
	ASSERT( m_Polies > 0 );
					  
	CEditBrush				*pNew;
	CMoArray<CEditPoly*>	finalList;


	// Create the new one and split it up.
	pNew = new CEditBrush;
	pNew->CopyEditBrush( this );

	pNew->m_pBspRoot = SplitPolyList( pNew, pNew->m_Polies, finalList );
	if( pNew->m_pBspRoot )
		finalList.Append( pNew->m_pBspRoot );

	DeleteAndClearArray( pNew->m_Polies );
	pNew->m_Polies.CopyArray( finalList );
...
CEditPoly* CEditBrush::SplitPolyList( CEditBrush *pAddTo, CMoArray<CEditPoly*> &polies, CMoArray<CEditPoly*> &finalList )
{
	CMoArray<CEditPoly*>	sides[2];
	CEditPoly				*pNew;
	...
	{
		pNew = new CEditPoly( polies[0] );
		return pNew;
	}

	pNew = DividePolies( pAddTo, polies, sides );	
	
	pNew->m_Sides[0] = SplitPolyList( pAddTo, sides[0], finalList );
	pNew->m_Sides[1] = SplitPolyList( pAddTo, sides[1], finalList );

	if( pNew->m_Sides[0] )	finalList.Append( pNew->m_Sides[0] );
	if( pNew->m_Sides[1] )	finalList.Append( pNew->m_Sides[1] );

	DeleteAndClearArray( sides[0] );
	DeleteAndClearArray( sides[1] );

	return pNew;
}


CEditPoly* CEditBrush::DividePolies( CEditBrush *pAddTo, CMoArray<CEditPoly*> &polies, CMoArray<CEditPoly*> *sides )
{
	CEditPoly		*pSplitter;
	CBasePoly		*newPolies[2];
	PolySide		side;
	...
	pSplitter = new CEditPoly( polies[0] );
	
	for( i=1; i < polies; i++ )
	{
		side = pSplitter->m_Plane.GetPolySide( polies[i] );
		if( side == Intersect )
		{
			newPolies[0] = new CEditPoly( pAddTo );
			newPolies[1] = new CEditPoly( pAddTo );

			pSplitter->m_Plane.SplitPoly( polies[i], newPolies, pAddTo->m_Points );
		...
		{
			sides[side].Append( new CEditPoly(polies[i]) );
		}
	}
