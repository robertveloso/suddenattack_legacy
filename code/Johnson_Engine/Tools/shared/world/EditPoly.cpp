...
void CEditPoly::SetFirstVert( uint32 index )
{
	if( m_Indices.FindElement(index) == BAD_INDEX )
	{
		ASSERT( FALSE );
		return;
	}

	while( m_Indices[0] != index )
	{
		uint32 tempIndex = m_Indices[0];
		m_Indices.Append( m_Indices[0] );
		m_Indices.Remove( 0 );
	}
}
...
void CEditPoly::SetupBaseTextureSpace(uint32 nTex)
{
	...
	GetTexture(nTex).GetP() = g_RightVectors[closestPlane];
	GetTexture(nTex).GetQ() = GetTexture(nTex).GetP().Cross( g_TexturePlanes[closestPlane] );
}
...
bool CEditPoly::SetUVTextureSpace( uint32 nTex, const float* coords, const int texWidth, const int texHeight )
{
	...
	return ConvertUVToOPQ(vPos, coords, texWidth, texHeight, Texture.GetO(), Texture.GetP(), Texture.GetQ());	
}
