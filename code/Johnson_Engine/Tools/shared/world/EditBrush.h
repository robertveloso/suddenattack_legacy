...
#define __EDITBRUSH_H__
	...
	class CEditBrush : public CWorldNode
	{
		...
		public:
			...
			// These two chop stuff up into a bunch of brushes.
			BOOL					Subtract( CEditRegion *pRegion, CEditBrush *pOther, CEditBrush *pBsp, CMoArray<CEditBrush*> &newBrushes );

			// These two chop everything up into a single brush.
			CEditBrush*				ChopUpBrush( CEditBrush *pOther, CEditBrush *pBsp, BOOL *keepSides, uint32 *sideIndices );
			void					BspSplitPolyList(	CEditPoly *pRoot, CEditBrush *pAddStuffTo, CMoArray<CEditPoly*> &polies, 
														CMoArray<CEditPoly*> *fullSides );

			CEditBrush*				GetBsp();

			CEditPoly*				SplitPolyList( CEditBrush *pAddTo, CMoArray<CEditPoly*> &polies, CMoArray<CEditPoly*> &finalList );
			CEditPoly*				DividePolies( CEditBrush *pAddTo, CMoArray<CEditPoly*> &polies, CMoArray<CEditPoly*> *sides );
		...
		protected:
			...
			// Bsp data (if it even has one).
			CEditPoly				*m_pBspRoot;
