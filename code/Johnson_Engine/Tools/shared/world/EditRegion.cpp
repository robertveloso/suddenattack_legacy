...
void LoadGlobalPropList(CLTANode* pParseNode, CPropListContainer& container )
{
	CLTANode*	propListArrayNode = PairCdrNode(pParseNode);

	if( propListArrayNode )
	{
		uint32 listSize = propListArrayNode->GetNumElements();
		for( uint32 i = 0; i < listSize; i++ )
		{
			LoadPropList(PairCdrNode(propListArrayNode->GetElement(i)), container );
		}
	}
}
...
BOOL CEditRegion::RecurseAndLoadNodeLTA( CLTANode* pParseNode, CWorldNode *pNode, CEditProjectMgr* pProject )
{
	...
	for( uint32 i=0; i < nChildren; i++ )
	{
		...
		else if (strcmp(pNodeType, "patch") == 0)
		{
			...
			((CEditPatch*)pChild)->LoadLTA(pCurrChild, m_pStringHolder);
		}
		else if( strcmp(pNodeType, "prefabref" ) == 0 )
		{
			const char *pFilename = PairCdrNode(pCurrChild->GetElement(2))->GetValue(); // prefabfile
...
void CEditRegion::RecurseAndSaveNodeLTA( CLTAFile* pFile, CWorldNode *pNode, uint32 level )
{
	...
	pFile->WriteStr("( worldnode ");
		...
		else if(pNode->m_Type == Node_Patch)
			pFile->WriteStrF(" patch ");
		else
		...
		{
			((CEditPatch*)pNode)->SaveLTA(pFile, level + 1);
		}
...
//of textures changed
static uint32 RecusivelyReplacePatchTex(CEditRegion* pRegion, CWorldNode* pNode, const char* pszInName, const char* pszOutName)
{
	...
	if(pNode->GetType() == Node_Patch)
	{
		...
		if(CHelpers::UpperStrcmp(pPatch->m_pTextureName, pszInName))
		{
			nNumChanged++;

			pPatch->m_pTextureName = pRegion->m_pStringHolder->AddString(pszOutName);
		}
	}
	...
	for( Pos = pNode->m_Children; Pos; )
	{
		...
		nNumChanged += RecusivelyReplacePatchTex(pRegion, pChild, pszInName, pszOutName);
	}
...
uint32 CEditRegion::RenameTexture(const char *pSrcTextureName, const char *pDestTextureName, bool bScaleOPQs)
{
	...
	//need to recursively update the patches as well 
	nOccurences += RecusivelyReplacePatchTex(this, GetRootNode(), pSrcTextureName, pDestTextureName);
