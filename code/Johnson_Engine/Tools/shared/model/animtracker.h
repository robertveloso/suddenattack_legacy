...
#define __ANIMTRACKER_H__
	...
	INLINE_FN uint32 trk_GetCurAnimIndex(LTAnimTracker *pTracker)
	{
		return pTracker->m_TimeRef.m_Cur.m_iAnim;
	}

	INLINE_FN void trk_Stop(LTAnimTracker *pTracker)
	{
		pTracker->m_Flags &= ~AT_PLAYING;
	}

	INLINE_FN void trk_Start(LTAnimTracker *pTracker)
	{
		pTracker->m_Flags |= AT_PLAYING;
	}
	
	INLINE_FN void trk_Loop(LTAnimTracker *pTracker, bool bLoop)
	{
		if(bLoop)
	...
	INLINE_FN void trk_Play(LTAnimTracker *pTracker, bool bPlaying)
	{
		if(bPlaying)
			pTracker->m_Flags |= AT_PLAYING;
		else
