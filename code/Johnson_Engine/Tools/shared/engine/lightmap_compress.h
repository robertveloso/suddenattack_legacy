...
#define __LIGHTMAP_COMPRESS_H__
	...
	// Compresses the data as a mask.  You should always use DecompressLMDataAsMask
	// to uncompress this type of data.
	// pOutBuffer must be at least LIGHTMAP_MAX_TOTAL_PIXELS large.
	LTBOOL CompressShadowMap(const uint8 *pData, 
		uint32 width, uint32 height, 
		uint8 *pOutBuffer, uint32 &outLen);

	// Decompress a buffer compressed with CompressLMDataAsMask.  pOut is filled with
	// 0's and 255's.  pOut must be at least LIGHTMAP_MAX_TOTAL_PIXELS large.
	LTBOOL DecompressShadowMap(const uint8 *pCompressed, uint32 dataLen, uint8 *pOut);
