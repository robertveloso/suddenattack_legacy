...
#ifdef __PS2
	if (pCnt->TmpStartTime.QuadPart > uTmpEnd.QuadPart) {
		pCnt->FrameCount.QuadPart += (65535 - pCnt->TmpStartTime.QuadPart);
		pCnt->FrameCount.QuadPart += uTmpEnd.QuadPart; }
	else {
...
void CDIPerfCounterMan::FrameMarker()
{
	LARGE_INTEGER uTmpEnd; GetPerfCounter(uTmpEnd); 
#ifdef __PS2
	if (m_FrameTimeTmp.QuadPart > uTmpEnd.QuadPart) {
		m_FrameTimeCount.QuadPart  = (65535 - m_FrameTimeTmp.QuadPart);
		m_FrameTimeCount.QuadPart += uTmpEnd.QuadPart; }
	else {
...
#ifdef __PS2
	uint32 lightColors[] = { 0x80804040, 0x80408040, 0x80404080, 0x80808000, 0x80008080, 0x80800080 }; 
	uint32 darkColors[]  = { 0x80401010, 0x80104010, 0x80101040, 0x80404000, 0x80004040, 0x80400040 }; 
	LTRGBColor LegendColor; LegendColor.dwordVal = 0xFF404040;
	LTRGBColor FPSColor; FPSColor.dwordVal = 0xFF404080;
	LTRGBColor PercentColor; PercentColor.dwordVal = 0xFF404040;
#endif
#ifdef __D3D
