...
*/
	...
	// 광식 : fire 부분에만 쓰려고 하나 추가
	bool (*IntersectSegmentClient)(ClientIntersectQuery *pQuery, ClientIntersectInfo *pInfo);
...
\param pDeclaration  Vertex shader input declaration created from the directx vertex shader declarator macros.
\param DeclarationSize  Size in bytes of the vertex shader input declaration array.
...
*/
	virtual bool				AddVertexShader(const char *pFileName, int VertexShaderID,
												const uint32 *pDeclaration, uint32 DeclarationSize) = 0;
...
Load a pixel shader from file.

Used for: Models.
*/
	virtual bool				AddPixelShader(const char *pFileName, int PixelShaderID) = 0;
...
*/
	...
	// 광식
	virtual LTRESULT GetMapExtentsMinMax(LTVector& vMin, LTVector& vMax)=0;
/*!
...
Used for: Networking.
*/
    LTRESULT (*InitNetworking)(const char *pDriver, uint32 dwFlags,bool bTermShell=true);
...
//[MURSUM] Cone
	...
	LTBOOL	 (*CheckValidObject)( HOBJECT hObject, int32 nType );
	uint8	 (*GetNetState)();
