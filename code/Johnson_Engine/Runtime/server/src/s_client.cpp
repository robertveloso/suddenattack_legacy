...
#ifndef _FINAL
extern uint32	g_CV_TestG;
extern uint32	g_CV_TestUG;
#endif
...
bool Client::Init( CBaseConn *pBaseConn, bool bIsLocal )
{
	...
	*/
	...
	while (hIterator) 
	{
		...
		if( !(pUsedFile->m_Flags & FFLAG_DONTSEND) )
		{
...
void WriteUnguaranteedInfo(LTObject *pObject, CPacket_Write &cPacket, bool bLastClient = false) 
{
 	uint32 flags = 0;
	...
	//////////////////////////////////////////////////////////////////////////
	if ( pObject->DoesNeedPitchUpdate() )
	{
		flags |= UUF_PITCH;
	}
	//////////////////////////////////////////////////////////////////////////
	...
	// ANI 정보는 Packet으로 날리지 않기.
	//flags &= ~UUF_ANIMINFO;
	/*
	// 광식
	//------------------------------------------------------------------
	if( pObject->m_UserFlags & (1<<1) )
	{
		flags &= ~UUF_POS;
		flags &= ~UUF_PITCH;
		flags &= ~UUF_YROTATION;
	...
	*/
	...
	if (flags & UUF_POS)
	{
		...
		if (bWriteVelocity)
		{
			...
			CLTMessage_Write_Server::WriteCompPos(cPacket, pObject->m_Velocity);
		}
	}
	...
	if ( flags & UUF_PITCH )
	{
		CLTMessage_Write_Server::WriteCompPitch( cPacket, pObject->GetPitch() );
	...
	{
		ModelInstance* pModel = ToModel(pObject);		
		LTAnimTracker* pTracker = pModel->GetTracker(1);
		uint8 nAnimIndex;
		if(pTracker)
		{
			nAnimIndex = (uint8)trk_GetCurAnimIndex(pTracker);
			cPacket.Writeint8(nAnimIndex);

			bool bLoop = !!(pTracker->m_Flags&AT_LOOPING);
			//----------------------------------------------------------
...
#ifdef _DEBUG
	if( pObject->m_ObjectType == OT_NORMAL )
	{
		int a=0;
		++a;
	}
...
inline bool ShouldSendToClient(const Client *pClient, LTObject *pObject) 
{
	...
	if( pObject->m_InternalFlags & IFLAG_DONTSEND ) 
	{
		return false;
	}
...
//  float fMaxDistSqr;
	...
	for (pCur = g_pServerMgr->m_SoundTrackList.m_Head.m_pNext; pCur != &g_pServerMgr->m_SoundTrackList.m_Head; pCur = pCur->m_pNext)
	{
		...
		if( pSoundTrack->m_dwFlags & PLAYSOUND_AMBIENT ) 
			continue;
...
static void WriteUnguaranteedDataWithAttachments(LTObject* pObject, CPacket_Write& cUnguaranteed, const uint32 k_nUnguaranteedMask, bool bLastClient = false)
{
	//write out the unguaranteed data for the object itself
	WriteUnguaranteedInfo(pObject, cUnguaranteed, bLastClient);
	...
	for (Attachment *pAttachment = pObject->m_Attachments; pAttachment; pAttachment = pAttachment->m_pNext) 
	{
		...
		WriteUnguaranteedInfo(pAttachedObj, cUnguaranteed, bLastClient);
	}
}
...
void SendAllObjectsUnguaranteed(ObjectMgr *pObjectMgr, UpdateInfo *pInfo, bool bLastClient = false) 
{
	//determine if we are dealing with a local client. 
	bool bLocalClient = !!(pInfo->m_pClient->m_ClientFlags & CFLAG_LOCAL);
	...
	if(bLocalClient)
	{
		...
		for (uint32 i = 0; i < NUM_OBJECTTYPES; i++)
		{
			...
			for (LTLink *pCur = pListHead->m_pNext; pCur != pListHead; pCur = pCur->m_pNext)
			{
				...
				// 광식 : Player 가 죽었으면 Ungaranteed Packet은 무조건 안날리도록 수정
				//------------------------------------------------------------------------
				if( pObject->m_UserFlags & (1<<1) )
					continue;
				//------------------------------------------------------------------------
				...
				WriteUnguaranteedDataWithAttachments(pObject, pInfo->m_cUnguaranteed, k_nUnguaranteedMask, bLastClient);
	...
	else
	{
		...
		for (uint32 i = 0; i < NUM_OBJECTTYPES; i++)
		{
			...
			for (LTLink *pCur = pListHead->m_pNext; pCur != pListHead; pCur = pCur->m_pNext)
			{
				...
				//[MURSUM] 내정보는 내가 받지말자!
				if( pInfo->m_pClient->m_pObject == pObject )
				{
					continue;
				}
...
void sm_UpdateClientInWorld( Client *pClient, bool bLastClient, bool bUpdateUnguaranteed )
{
	...
	// 광식 : 만약 맵을 로딩 중이면 Unguaranteed Packet 은 날리지 않도록 수정
	if( bUpdateUnguaranteed && !(g_pServerMgr->m_ServerFlags & SS_PAUSED) )
	{
		SendAllObjectsUnguaranteed(&g_pServerMgr->m_ObjectMgr, &updateInfo, bLastClient);
	}
	...
	*/
	...
	if( g_CV_TestG > 0 )
	{
		char* pTempData = new char[g_CV_TestG*256];
		CPacket_Write	cTempPacket;
		cTempPacket.Writeuint8(SMSG_MESSAGE);
		cTempPacket.Writeuint8(110);
		cTempPacket.WriteData( pTempData, g_CV_TestG*256 );
		delete pTempData;

		sm_FlushUpdate(&updateInfo, CPacket_Read(cTempPacket), MESSAGE_GUARANTEED);
	}

	if( g_CV_TestUG > 0 )
	{
		char* pTempData = new char[g_CV_TestUG*256];
		CPacket_Write	cTempPacket;
		cTempPacket.Writeuint8(SMSG_MESSAGE);
		cTempPacket.Writeuint8(110);
		cTempPacket.WriteData( pTempData, g_CV_TestUG*256 );
		delete pTempData;

		sm_FlushUpdate(&updateInfo, CPacket_Read(cTempPacket), 0);
	}
...
void sm_TellClientAboutGlobalLight(Client *pClient)
{
	...
	CLTMessage_Write::WriteCompLTVector(cPacket, rRotation.Forward());
	...
	CLTMessage_Write::WriteCompLTVector(cPacket, pSun->m_InnerColor * fBrightness);
