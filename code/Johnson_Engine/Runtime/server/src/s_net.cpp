...
void WriteAnimInfo(ModelInstance *pInst, CPacket_Write &cPacket)
{
	for (LTAnimTracker *pTracker = pInst->m_AnimTrackers; pTracker; pTracker=pTracker->GetNext())
    {
		...
		//by default it shoudl fall into the first category
...
// 광식
void WriteAnimationInfo(ModelInstance *pInst, CPacket_Write &cPacket, uint16 nFlags)
{
	if( nFlags & NETFLAG_ANIM_PLAYEROBJ )
	{
		LTAnimTracker* pTracker = pInst->GetTracker(1);
		uint8 nAnimIndex;
		if(pTracker)
		{
		...
		pTracker = pInst->GetTracker(44); // Recoil Tracker
		if(pTracker)
		{					
			nAnimIndex = (uint8)trk_GetCurAnimIndex(pTracker);
			if( nAnimIndex != 0 )
	...
	else if ( nFlags & NETFLAG_ANIM_BODY )
	{
		cPacket.Writebool(true);
		LTAnimTracker* pTracker = pInst->GetTracker(0xFF);
		uint8 nAnimIndex;
		if( pTracker )
		{
			nAnimIndex = (uint8)trk_GetCurAnimIndex(pTracker);
			...
			cPacket.Writebool((pTracker->m_Flags & AT_LOOPING) != 0);
			cPacket.Writebool(true);
			cPacket.Writeuint16(pTracker->m_TimeRef.m_Cur.m_Time);
		}
		
		pTracker = pInst->GetTracker(1);
		if( pTracker )
		{
			nAnimIndex = (uint8)trk_GetCurAnimIndex(pTracker);
			cPacket.Writeuint8(nAnimIndex);
		}
		else
		{
			cPacket.Writebool(false);
...
bool FillPacketFromInfo(Client *pClient, LTObject *pObj, ObjInfo *pInfo, CPacket_Write &cPacket)
{
    ...
    if (pObj->sd->m_NetFlags & NETFLAG_POSUNGUARANTEED)
    {
		...
		changeFlags &= ~CF_POSITION_PREDICTIONCAP;
    }
	...
	//---------------------------------------------------------------------
	/*
	if( (pObj->sd->m_NetFlags & NETFLAG_ANIM_HHWEAPON) ||
		(pObj->sd->m_NetFlags & NETFLAG_ANIM_PLAYEROBJ) )
	{
		changeFlags &= ~CF_MODELINFO;
		changeFlags &= ~CF_FORCEMODELINFO;			
		...
		//if( changeFlags & CF_TELEPORT )
		//	dsi_ConsolePrint( "!!!!!Teleport");
	}
	*/
	//---------------------------------------------------------------------
	if( pObj->sd->m_NetFlags & NETFLAG_ANIM_HHWEAPON )
	...
	else if( pObj->sd->m_NetFlags & NETFLAG_ANIM_PLAYEROBJ )
	{
		//changeFlags &= ~CF_MODELINFO;
		//changeFlags &= ~CF_FORCEMODELINFO;	
	...
	//---------------------------------------------------------------------
	else if( pObj->sd->m_NetFlags & NETFLAG_ANIM_BODY )
	{
		/*
		if(changeFlags & CF_FORCEMODELINFO)
		{
		...
		else if(changeFlags & CF_NEWOBJECT )
		{
			changeFlags &= ~CF_MODELINFO;
			changeFlags &= ~CF_FORCEMODELINFO;
			changeFlags &= ~CF_ROTATION;
			changeFlags &= ~(CF_ROTATION|CF_SNAPROTATION);
			changeFlags &= ~CF_OTHERFLAGMASK;
		...
		*/
		...
		if( changeFlags != 1825 )	// 중간 난입시 Ani 사라지는거 방지
		{
			if(changeFlags & CF_FORCEMODELINFO)
			{
				changeFlags |= CF_MODELINFO;
	...
	else if( pObj->sd->m_NetFlags & NETFLAG_ANIM_THROWMODEL )
	{
		if(changeFlags & CF_FORCEMODELINFO)
		{
			changeFlags |= CF_MODELINFO;
	...
	if( changeFlags != 0 )
	{
		if( pObj->sd )
		{
			if( pObj->sd->m_pClass )
			{
				if( strcmp("WorldModel", pObj->sd->m_pClass->m_ClassName) == 0 )
				{
					if( changeFlags & CF_NEWOBJECT )
					{
					}
				...
				if( strcmp("RotatingDoor", pObj->sd->m_pClass->m_ClassName ) == 0 )
				{
					if( changeFlags & CF_NEWOBJECT )
					{
					}
	...
	*/
    ...
    if (changeFlags & CF_NEWOBJECT)
    {
		...
		//광식 : 원본은 WriteLTVector 를 사용했는데 압축을 했다가 아에 삭제 함...
		//----------------------------------------------------------------------------------------
		//cPacket.WriteLTVector(pObj->GetGlobalForceOverride());
		//CLTMessage_Write::WriteCompLTVector(cPacket, pObj->GetGlobalForceOverride());
		//----------------------------------------------------------------------------------------
	...
	// 광식
	//-------------------------------------------------------------
	//if( pObj->m_ObjectType == OT_MODEL ) return true;
	//-------------------------------------------------------------
    ...
    if (changeFlags & (CF_MODELINFO|CF_FORCEMODELINFO))
    {
        if (pObj->m_ObjectType == OT_MODEL)
        {
			// 광식 [Ani Packet 수정]			
			//===================================================================	
			if(pObj->sd->m_NetFlags & NETFLAG_ANIM_HHWEAPON)	// m_ClassName -> CHHWeaponModel
			{
				cPacket.Writebool(true);
				ModelInstance* pModel = ToModel(pObj);
				...
				else
				{
					//cPacket.Writeint8(0);
					//cPacket.Writebool(false);
					dsi_ConsolePrint( "ERROR %s %d", __FILE__, __LINE__ );
				}
			}
			else if(pObj->sd->m_NetFlags & NETFLAG_ANIM_PLAYEROBJ)	// m_ClassName -> CPlayerObj
			{
				cPacket.Writebool(false);
				ModelInstance* pModel = ToModel(pObj);
				LTAnimTracker* pTracker = pModel->GetTracker(0xFF);	// Main Tracker
				uint8 nAnimIndex;
				if(pTracker)
				{
					//nAnimIndex = (uint8)trk_GetCurAnimIndex(pTracker);
			...
			else if(pObj->sd->m_NetFlags & NETFLAG_ANIM_BODY)	// m_ClassName -> Body
			{
				cPacket.Writebool(true);
				ModelInstance* pModel = ToModel(pObj);
				LTAnimTracker* pTracker = pModel->GetTracker(0xFF);
 	...
 	if (changeFlags & (CF_POSITION|CF_TELEPORT|CF_POSITION_PREDICTIONCAP))
    {
        ...
        else 
		{
			...
			// 광식 : 새로 만든 모델은 Velocity를 0 으로 하자.
			//-------------------------------------------------------------------
    ...
    if (changeFlags & CF_ATTACHMENTS)
    {
        ...
        while (pCur)
        {
			...
			// 광식 : 게임에서 Offset을 모두 0으로 Setting
			//-------------------------------------------------------------------------------------
			/*
	...
	// 광식 : DIMS 는 게임내에서 모두 동일하게 사용.. X : 24.000, Y : 66.000, Z : 24.000
	/*
	if (changeFlags & CF_DIMS)
	{
		...
		CLTMessage_Write::WriteCompLTVector(cPacket, pObj->GetDims());
	}
	*/
