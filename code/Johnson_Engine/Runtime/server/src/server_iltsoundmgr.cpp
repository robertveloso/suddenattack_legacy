...
public:
    ...
    LTRESULT KillSound(HLTSOUND& hSound,bool bTerm=false);
    LTRESULT KillSoundLoop(HLTSOUND hSound);
	LTRESULT SetFrequency(HLTSOUND& hSound, unsigned long dwFrequency);
};
...
LTRESULT CLTSoundMgrServer::PlaySound(PlaySoundInfo *pPlaySoundInfo, HLTSOUND &hResult) {
	...
	short ftFlag = 0;
	if( pPlaySoundInfo->m_dwFlags & PLAYSOUND_AMBIENT ) ftFlag |= (1<<3); //FFLAG_DONTSEND
	
	if (server_filemgr->AddUsedFile(pszFinalFilename, ftFlag, &pFile) == 0) 
	{
		if (g_DebugLevel >= 2) {
...
LTRESULT CLTSoundMgrServer::KillSound(HLTSOUND& hSound,bool bTerm) {
    CSoundTrack *pSoundTrack;
...
LTRESULT CLTSoundMgrServer::SetFrequency(HLTSOUND& hSound, unsigned long dwFrequency)
{
	return LT_OK;
}
