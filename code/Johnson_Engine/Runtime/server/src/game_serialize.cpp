...
typedef std::vector<uint8> TSerializeBuffer;
TSerializeBuffer s_aSerializeBuffer;

// Objects larger than this size are considered "suspicious" and spit out a warning.
const uint32 k_nSuspiciousObjectSize = 65536 * 8;
...
static void sm_SaveObjectData(LTObject *pObj, ILTStream *pStream, uint32 dwParam) 
{
	...
	uint32 nTempBuffSize = (LTMAX(nSpecialEffectLen, nObjectMsgLen) + 7) / 8;
	s_aSerializeBuffer.resize(nTempBuffSize);
	uint8 *pTempBuff = (uint8*)&s_aSerializeBuffer.front();
    ...
    *pStream << pObj->GetGlobalForceOverride();
    ...
    if (bSpecialEffectMessage) 
	{
		...
		pObj->sd->m_cSpecialEffectMsg.PeekData(pTempBuff, nSpecialEffectLen);
        pStream->Write(pTempBuff, (nSpecialEffectLen + 7) / 8);
    }
    ...
    *pStream << pObj->GetDims();
	...
	cSaveMessage_Read.ReadData(pTempBuff, nObjectMsgLen);
	GS_STREAM_WRITE(nObjectMsgLen);
	pStream->Write(pTempBuff, (nObjectMsgLen + 7) / 8);
									...
									uint32 dwParam)
{
	...
	*pStream >> createStruct.m_GlobalForceOverride;
    ...
    if (bSpecialEffectMessage) 
	{
		...
		s_aSerializeBuffer.resize(nMessageBytes);
		uint8 *pTempBuff = (uint8*)&s_aSerializeBuffer.front();
		pStream->Read(pTempBuff, nMessageBytes);
		CPacket_Write cTempMsg;
		cTempMsg.WriteData(pTempBuff, messageLen);
...
static LTRESULT sm_RestoreNextObject(ILTStream *pStream, LTObject *pObj, uint32 dwParam) 
{
	...
	s_aSerializeBuffer.resize(nObjectMsgBytes);
	uint8 *pTempBuffer = (uint8*)&s_aSerializeBuffer.front();
	pStream->Read(pTempBuffer, nObjectMsgBytes);
	CPacket_Write cTempMsg;
	cTempMsg.WriteData(pTempBuffer, nObjectMsgLen);
...
void sm_SaveObjects(ILTStream *pStream, ObjectList *pList, uint32 dwParam, uint32 flags)
{
	...
	// Clean up the serialization buffer
	s_aSerializeBuffer.swap(TSerializeBuffer());
}
