...
void sm_UpdateClientStates() 
{
	...
	while( pCur != &g_pServerMgr->m_Clients.m_Head ) 
	{
		...
		else if( pClient->m_ClientFlags & CFLAG_SILENT_KICKED )
		{
			g_pServerMgr->m_NetMgr.Disconnect(pClient->m_ConnectionID, DISCONNECTREASON_SILENT_KICKED);
		}
		else
		{
...
void sm_UpdateClientsInWorld( bool bUpdateUnguaranteed ) 
{
	LTLink *pListHead = &g_pServerMgr->m_Clients.m_Head;
	uint32 nCount = g_pServerMgr->m_Clients.m_nElements;
	
	for (LTLink *pCur = pListHead->m_pNext; pCur != pListHead; pCur = pCur->m_pNext) 
	{
		Client *pClient = (Client*)pCur->m_pData;
		sm_UpdateClientInWorld(pClient, !(--nCount), bUpdateUnguaranteed );
	}
...
bool CServerMgr::SetPlayerName(int32 index, const char *pName, int32 nMaxLen)
{
	...
	int32 i = 0;
	for (LTLink *pCur = m_Clients.m_Head.m_pNext; i < index; ++i)
		pCur = pCur->m_pNext;
...
// clears queues, etc.
void sm_FinishUpdateFrame( bool bUpdateUnguaranteed )
{
	...
	sm_UpdateClientsInWorld(bUpdateUnguaranteed);
}
...
bool CServerMgr::Update( int32 updateFlags, uint32 nCurTimeMS, bool bUpdateUnguaranteed )
{
	#ifdef LOCK_SERVER_FPS
	...
	if (m_State == SERV_RUNNINGWORLD)
	{
		...
		// Finish the frame.
		sm_FinishUpdateFrame(bUpdateUnguaranteed);
								 ...
								 //bool bVisibleCheck)
{
	...
	for (LTLink* pCur=pListHead->m_pNext; pCur != pListHead; pCur = pCur->m_pNext)
	{
		...
		//LTRotation rRot = pClient->m_pObject->m_Rotation;
		//LTVector vForward = rRot.Forward();
		if( pClient->m_pObject )
		{
			//LTVector vEffect = vPos - pClient->m_pObject->m_Pos;
			//float fDest = vEffect.Mag();

			//vEffect.Normalize();
			//float fDot = vForward.Dot(vEffect);
			//if( fDot > 0.5 && fDest < 1700 )
			if( /*fDest < 1700 &&*/ hSender != pClient->m_pObject )
			{
				SendToClient(pClient, cPacket, true, messageFlags);
			}
		}
...
LTRESULT CServerMgr::DoStartWorld(const char *pWorldName, uint32 flags, uint32 nCurTimeMS)
{
	...
	dsi_ConsolePrint( "CServerMgr::DoStartWorld : %s", pWorldName ); 
...
bool CServerMgr::StartClientTCP( const char* lpszWorldName )
{
	if( !i_server_shell ) return false;
	return i_server_shell->StartClientTCP( lpszWorldName );
}

int32 CServerMgr::GetWorldFileID()
{
	if( m_pWorldFile ) return m_pWorldFile->m_FileID;
	return -1;
}

bool CServerMgr::SetInternalObjectSetup( void* pVoidSetup, LTObject* pObj )
{
	InternalObjectSetup* pSetup			= (InternalObjectSetup*)pVoidSetup;
	uint8	nObjectType = pObj->m_ObjectType;

	pSetup->m_pSetup->m_GlobalForceOverride.Init();	
	pSetup->m_Filename[0].m_FileType	= FILE_CLIENTFILE;
	pSetup->m_pSetup->m_Pos				= pObj->m_Pos;
	pSetup->m_pSetup->m_Rotation		= pObj->m_Rotation;
	pSetup->m_pSetup->m_Scale			= pObj->m_Scale;
	pSetup->m_pSetup->m_Flags			= (pObj->m_Flags&CLIENT_FLAGMASK);
	pSetup->m_pSetup->m_Flags2			= pObj->m_Flags2;
	pSetup->m_pSetup->m_ObjectType		= nObjectType;

	if( nObjectType == OT_WORLDMODEL ) 
	{
		LTStrCpy( pSetup->m_pSetup->m_Filename,
				  ToWorldModel(pObj)->m_pOriginalBsp->m_WorldName,
				  MAX_CS_FILENAME_LEN );		
    }
    else if( nObjectType == OT_CONTAINER ) 
	{
		LTStrCpy( pSetup->m_pSetup->m_Filename,
				  ToContainer(pObj)->m_pOriginalBsp->m_WorldName,
				  MAX_CS_FILENAME_LEN );
        pSetup->m_pSetup->m_ContainerCode = ToContainer(pObj)->m_ContainerCode;
    }
    else if( nObjectType == OT_MODEL ) 
	{
		return false;	//모델은 미리 생성 안함~!!!
    }
    else if( nObjectType == OT_SPRITE ) 
	{
        pSetup->m_Filename[0].m_pFilename = pObj->sd->m_pFile->GetFilename();
		pSetup->m_pSetup->m_Flags |= FLAG_VISIBLE;
	}
	return true;
}

bool CServerMgr::SetPlaySoundInfo( PlaySoundInfo* pPlaySoundInfo, void* pVoidFileRef, void* pVoidSoundTrack )
{
	CSoundTrack *pSoundTrack	= (CSoundTrack*)pVoidSoundTrack;
	FileRef*	pFile			= (FileRef*)pVoidFileRef;

	pFile->m_FileType	= FILE_CLIENTFILE;
	pFile->m_FileID		= 0;
	pFile->m_pFilename	= pSoundTrack->m_pFile->GetFilename();
	...
	pPlaySoundInfo->m_dwFlags		= (uint16)(0xFFFF & pSoundTrack->m_dwFlags);
	pPlaySoundInfo->m_nPriority		= pSoundTrack->m_nPriority;
	pPlaySoundInfo->m_fOuterRadius	= (float)nOuter;
	pPlaySoundInfo->m_fInnerRadius	= (float)nInner * pPlaySoundInfo->m_fOuterRadius / 255.0f;
	pPlaySoundInfo->m_vPosition		= pSoundTrack->m_vPosition;
	pPlaySoundInfo->m_UserData		= pSoundTrack->m_UserData;
	pPlaySoundInfo->m_dwFlags		|= PLAYSOUND_CLIENT;
	
	if( pPlaySoundInfo->m_dwFlags & PLAYSOUND_CTRL_VOL)
    {
		pPlaySoundInfo->m_nVolume = pSoundTrack->m_nVolume;
    } 
	else
	{
    ...
    {
		pPlaySoundInfo->m_fPitchShift = pSoundTrack->m_fPitchShift;
    }
	else
	{
