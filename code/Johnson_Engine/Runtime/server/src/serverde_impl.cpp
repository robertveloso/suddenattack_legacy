...
#ifndef _FINAL
//[MURSUM]
extern int32 g_CV_OutputS2C;
void si_OutputS2C( ILTMessage_Read *pMsg );
#endif
...
public:
	...
	//////////////////////////////////////////////////////////////////////////
	// [NOVICE] Pitch
	virtual LTRESULT GetObjectPitch(HLOCALOBJ hObj, float* pfPitch );
	//////////////////////////////////////////////////////////////////////////
	...
	// ±¤½Ä
	//virtual LTRESULT SendSFXMessage(ILTMessage_Read *pMsg, const LTVector &pos, uint32 flags, bool bVisibleCheck);
	virtual LTRESULT SendSFXMessage(ILTMessage_Read *pMsg, const LTVector &pos, uint32 flags, HOBJECT hSender);
	
	//[MURSUM]
	virtual LTRESULT SendWeaponSFXMessage(ILTMessage_Read *pMsg, const LTVector &pos, uint32 flags);
	virtual LTRESULT SendWeaponSFXMessage(ILTMessage_Read *pMsg, const LTVector &pos, uint32 flags, HOBJECT hSender);
...
LTRESULT CLTServer::GetClientAddr(HCLIENT hClient, uint8 pAddr[4], uint16 *pPort)
{
	...
	if( !pClient || !pClient->m_ConnectionID->GetIPAddress(pAddr, pPort))
	{
		pAddr[0] = 0;
		pAddr[1] = 0;
		pAddr[2] = 0;
...
LTRESULT CLTServer::GetObjectPitch(HLOCALOBJ hObj, float* pfPitch )
{
	FN_NAME(CLTServer::GetObjectPitch);
	CHECK_PARAMS2(hObj && pfPitch);

	*pfPitch = HandleToServerObj(hObj)->GetPitch();
	return LT_OK;
}
...
// ±¤½Ä
//LTRESULT CLTServer::SendSFXMessage(ILTMessage_Read *pMsg, const LTVector &pos, uint32 flags, bool bVisibleCheck) 
LTRESULT CLTServer::SendSFXMessage(ILTMessage_Read *pMsg, const LTVector &pos, uint32 flags, HOBJECT hSender)
{
	CHECK_PARAMS(pMsg, CLTServer::SendSFXMessage);
	...
	// Send the packet...
	//sm_SendToVisibleClients(cFinalMsg, false, pos, flags, bVisibleCheck);
	sm_SendToVisibleClients(cFinalMsg, false, pos, flags, hSender);

	return LT_OK;
}

//[MURSUM]
LTRESULT CLTServer::SendWeaponSFXMessage(ILTMessage_Read *pMsg, const LTVector &pos, uint32 flags) 
{
	CHECK_PARAMS(pMsg, CLTServer::SendSFXMessage);
	...
	// Add the "Special effect" message ID
	CPacket_Write cWrapMsg;
	cWrapMsg.Writeuint8(SMSG_WEAPON_SFX_MESSAGE);
	cWrapMsg.WritePacket(pServerMsg->GetPacket());
	CPacket_Read cFinalMsg(cWrapMsg);
...
LTRESULT CLTServer::SendWeaponSFXMessage(ILTMessage_Read *pMsg, const LTVector &pos, uint32 flags, HOBJECT hSender)
{
	CHECK_PARAMS(pMsg, CLTServer::SendSFXMessage);
...
LTRESULT si_SetSkyDef(SkyDef *pDef)
{
	...
	*/
	memcpy(&g_pServerMgr->m_SkyDef, pDef, sizeof(SkyDef));
#ifndef DE_SERVER_COMPILE
	memcpy(&g_pClientMgr->m_SkyDef, pDef, sizeof(SkyDef));
#endif // DE_SERVER_COMPILE
	sm_SetSendSkyDef();
	return LT_OK;
}
...
LTRESULT si_AddObjectToSky(HOBJECT hObject, uint32 index)
{
	...
	// ±¤½Ä
#ifndef DE_SERVER_COMPILE
	g_pClientMgr->m_SkyObjects[index] = pObject->m_ObjectID;
#endif // DE_SERVER_COMPILE
	sm_SetSendSkyDef();
...
HCLIENT si_GetClientFromID( uint8 nID )
{
	if (g_pServerMgr->m_Clients.m_nElements == 0) return LTNULL;
	...
	for (LTLink *pCur = pListHead->m_pNext; pCur != pListHead; pCur = pCur->m_pNext)
	{
		...
		if (pClient->m_ClientID == nID )
		{
			return (HCLIENT)pClient;
		}
	}
...
void* si_GetClientUserDataFromID( uint8 nID )
{
	if (g_pServerMgr->m_Clients.m_nElements == 0) return LTNULL;
	...
	for (LTLink *pCur = pListHead->m_pNext; pCur != pListHead; pCur = pCur->m_pNext)
	{
		...
		if (pClient->m_ClientID == nID )
		{
			return pClient->m_pPluginUserData;
		}
	}
...
LTRESULT si_SilentKickClient(HCLIENT hClient)
{
	if (!hClient)
		RETURN_ERROR(1, ILTServer::KickClient, LT_INVALIDPARAMS);
	...
	pClient->m_ClientFlags |= CFLAG_SILENT_KICKED;
...
void si_SetObjectPosNonTeleport(HOBJECT hObj, const LTVector *pos)
{
	LTObject *pObj = HandleToServerObj(hObj);
...
LTRESULT si_SetObjectPitch( HOBJECT hObj, const float fPitch )
{
	LTObject* pObj = HandleToServerObj( hObj );
	
	if ( !pObj )
	{
		RETURN_ERROR( 1, ILTServer::SetObjectPitch, LT_INVALIDPARAMS );
	}
	
	pObj->SetPitch( fPitch, LTTRUE );
...
void CLTServer::SetupFunctionPointers() 
{
	...
	SetObjectPosNonTeleport = si_SetObjectPosNonTeleport;
	SetObjectRotation = si_SetObjectRotation;

	//////////////////////////////////////////////////////////////////////////
	//[NOVICE] Pitch
	SetObjectPitch = si_SetObjectPitch;
	//////////////////////////////////////////////////////////////////////////
	...
	GetClientFromID = si_GetClientFromID;
	GetClientUserDataFromID = si_GetClientUserDataFromID;
	...
	SilentKickClient = si_SilentKickClient;
...
void si_OutputS2C( ILTMessage_Read *pMsg )
{
	...
	switch( nCommand )
	{
	case 101: // MID_PLAYER_SKILLS
		LTSNPrintF(aMsgBuff, nBufferSize, "P_SKILLS" );
		break;
	...
	case 104: // MID_PLAYER_SCORE
		LTSNPrintF(aMsgBuff, nBufferSize, "PRELOAD_WPN" );
		break;		

	case 105: // MID_PLAYER_STATE_CHANGE
	{
		LTSNPrintF(aMsgBuff, nBufferSize, "P_STATE_CHANGE" );
		uint8	nState = pMsg->Readuint8();
		switch( nState )
		{
		case 1:	
			LTStrCat(aMsgBuff, " ALIVE", nBufferSize);
			break;
		case 2:	
			LTStrCat(aMsgBuff, " DEAD", nBufferSize);
			break;
		case 3:	
			LTStrCat(aMsgBuff, " DYING", nBufferSize);
			break;
		case 4:	
			LTStrCat(aMsgBuff, " GHOST", nBufferSize);
			break;
		case 0:	
		default:
			LTStrCat(aMsgBuff, " UNKNOWN", nBufferSize);
	...
	case 109: // MID_PLAYER_INFO
		LTSNPrintF(aMsgBuff, nBufferSize, "P_INFO" );
		break;

	case 111: // MID_PLAYER_SCORED
		LTSNPrintF(aMsgBuff, nBufferSize, "P_SCORED" );
		break;

	case 114: // MID_PLAYER_MULTIPLAYER_INIT
		LTSNPrintF(aMsgBuff, nBufferSize, "MP_INIT" );
		break;

	case 115: //MID_PLAYER_HEALTH
		LTSNPrintF(aMsgBuff, nBufferSize, "P_HEALTH" );
		break;

	case 123: //MID_PLAYER_ARMOR
		LTSNPrintF(aMsgBuff, nBufferSize, "P_ARMOR" );
		break;
	...
	case 128: // MID_PLAYER_CONNECTED
		LTSNPrintF(aMsgBuff, nBufferSize, "P_CONNECTED" );
		break;

	case 130: // MID_WEAPON_CHANGE
		LTSNPrintF(aMsgBuff, nBufferSize, "WEAPON_CHANGE" );
		break;

	case 140: // MID_SWITCHINGWORLDSSTATE
	{
		LTSNPrintF(aMsgBuff, nBufferSize, "SWITCHING_WORLDS_STATE" );
		uint8	nState = pMsg->Readuint8();
		switch( nState )
		{
		case 0:	
			LTStrCat(aMsgBuff, " NONE", nBufferSize);
			break;
		case 1:	
			LTStrCat(aMsgBuff, " PLAYER_HOOKUP", nBufferSize);
			break;
		case 2:	
			LTStrCat(aMsgBuff, " AUTO_SAVE", nBufferSize);
			break;
		case 3:	
			LTStrCat(aMsgBuff, " WAIT_FOR_CLIENT", nBufferSize);
			break;
		case 4:	
			LTStrCat(aMsgBuff, " FINISHED", nBufferSize);
			break;
		default:
			LTStrCat(aMsgBuff, " UNKNOWN", nBufferSize);
			break;
	...
	case 149: // MID_TEAM_INFO
		LTSNPrintF(aMsgBuff, nBufferSize, "TEAM_INFO" );	
		break;

	case 160: // MID_GAME_PAUSE
		LTSNPrintF(aMsgBuff, nBufferSize, "GAME_PAUSE" );	
		break;

	case 212: // MID_CLIENT_PLAYER_UPDATE
		LTSNPrintF(aMsgBuff, nBufferSize, "CLIENT_PLAYER_UPDATE" );	
		break;

	case 231: // MID_SFX_MESSAGE
	{
		LTSNPrintF(aMsgBuff, nBufferSize, "SFX" );
		
		uint8 nID = pMsg->Readuint8();
		HOBJECT	hObj = pMsg->ReadObject();
		uint8 nSubID = pMsg->Readuint8();

		switch (nID)
		{
		case 1:
			LTStrCat(aMsgBuff, " POLYGRID", nBufferSize);
			break;
		case 2:
			LTStrCat(aMsgBuff, " PARTICLETRAIL", nBufferSize);
			break;
		case 3:
			LTStrCat(aMsgBuff, " PARTICLESYSTEM", nBufferSize);
			break;
		case 4:
			LTStrCat(aMsgBuff, " PARTICLESHOWER", nBufferSize);
			break;
		case 5:
			LTStrCat(aMsgBuff, " TRACER", nBufferSize);
			break;
		case 6:
			LTStrCat(aMsgBuff, " WEAPON", nBufferSize);
			break;
		case 7:
			LTStrCat(aMsgBuff, " DYNAMICLIGHT", nBufferSize);
			break;
		case 8:
			LTStrCat(aMsgBuff, " PARTICLETRAILSEG", nBufferSize);
			break;
		case 9:
			LTStrCat(aMsgBuff, " SMOKE", nBufferSize);
			break;
		case 10:
			LTStrCat(aMsgBuff, " BULLETTRAIL", nBufferSize);
			break;
		case 11:
			LTStrCat(aMsgBuff, " VOLUMEBRUSH", nBufferSize);
			break;
		case 12:
			LTStrCat(aMsgBuff, " SHELLCASING", nBufferSize);
			break;
		case 13:
			LTStrCat(aMsgBuff, " CAMERA", nBufferSize);
			break;
		case 14:
			LTStrCat(aMsgBuff, " PARTICLEEXPLOSION", nBufferSize);
			break;
		case 15:
			LTStrCat(aMsgBuff, " SCALE", nBufferSize);
			break;
		case 16:
			LTStrCat(aMsgBuff, " DEBRIS", nBufferSize);
			break;
		case 17:
			LTStrCat(aMsgBuff, " DEATH", nBufferSize);
			break;
		case 18:
			LTStrCat(aMsgBuff, " GIB", nBufferSize);
			break;
		case 19:
			LTStrCat(aMsgBuff, " PROJECTILE", nBufferSize);
			break;
		case 20:
			LTStrCat(aMsgBuff, " MARK", nBufferSize);
			break;
		case 21:
			LTStrCat(aMsgBuff, " LIGHT", nBufferSize);
			break;
		case 22:
			LTStrCat(aMsgBuff, " RANDOMSPARKS", nBufferSize);
			break;
		case 23:
			LTStrCat(aMsgBuff, " PICKUPITEM", nBufferSize);
			break;
		case 24:
			LTStrCat(aMsgBuff, " CHARACTER", nBufferSize);
			break;
		case 25:
			LTStrCat(aMsgBuff, " PLAYERSOUND", nBufferSize);
			break;
		case 26:
			LTStrCat(aMsgBuff, " NODELINES", nBufferSize);
			break;
		case 27:
			LTStrCat(aMsgBuff, " WEATHER", nBufferSize);
			break;
		case 28:
			LTStrCat(aMsgBuff, " LIGHTNING", nBufferSize);
			break;
		case 29:
			LTStrCat(aMsgBuff, " SPRINKLES", nBufferSize);
			break;
		case 30:
			LTStrCat(aMsgBuff, " FIRE", nBufferSize);
			break;
		case 31:
			LTStrCat(aMsgBuff, " LENSFLARE", nBufferSize);
			break;
		case 32:
			LTStrCat(aMsgBuff, " MUZZLEFLASH", nBufferSize);
			break;
		case 33:
			LTStrCat(aMsgBuff, " SEARCHLIGHT", nBufferSize);
			break;
		case 34:
			LTStrCat(aMsgBuff, " POLYDEBRIS", nBufferSize);
			break;
		case 35:
			LTStrCat(aMsgBuff, " STEAM", nBufferSize);
			break;
		case 36:
			LTStrCat(aMsgBuff, " EXPLOSION", nBufferSize);
			break;
		case 37:
			LTStrCat(aMsgBuff, " POLYLINE", nBufferSize);
			break;
		case 38:
			LTStrCat(aMsgBuff, " BODY", nBufferSize);
			break;
		case 39:
			LTStrCat(aMsgBuff, " LASERTRIGGER", nBufferSize);
			break;
		case 40:
			LTStrCat(aMsgBuff, " MINE", nBufferSize);
			break;
		case 41:
			LTStrCat(aMsgBuff, " BEAM", nBufferSize);
			break;
		case 42:
			LTStrCat(aMsgBuff, " PLAYERVEHICLE", nBufferSize);
			break;
		case 43:
			LTStrCat(aMsgBuff, " SOUND", nBufferSize);
			break;
		case 44:
			LTStrCat(aMsgBuff, " OBJSPRITE", nBufferSize);
			break;
		case 45:
			LTStrCat(aMsgBuff, " LIGHTGROUP", nBufferSize);
			break;
		case 46:
			LTStrCat(aMsgBuff, " DEBUGLINE", nBufferSize);
			break;
		case 47:
			LTStrCat(aMsgBuff, " TEXTUREFX", nBufferSize);
			break;
		case 48:
			LTStrCat(aMsgBuff, " SNOW", nBufferSize);
			break;
		case 49:
			LTStrCat(aMsgBuff, " JUMPVOLUME", nBufferSize);
			break;
		case 50:
			LTStrCat(aMsgBuff, " PLAYERLURE", nBufferSize);
			break;
		case 51:
			LTStrCat(aMsgBuff, " GADGETTARGET", nBufferSize);
			break;
		case 52:
			LTStrCat(aMsgBuff, " DISPLAYTIMER", nBufferSize);
			break;
		case 53:
			LTStrCat(aMsgBuff, " DYNAMIC_OCCLUDER", nBufferSize);
			break;
		case 54:
			LTStrCat(aMsgBuff, " SCATTER", nBufferSize);
			break;
		case 55:
			LTStrCat(aMsgBuff, " TRIGGER", nBufferSize);
			break;
		case 56:
			LTStrCat(aMsgBuff, " RADAROBJECT", nBufferSize);
			break;
		case 57:
			LTStrCat(aMsgBuff, " ACTIVATEOBJECT", nBufferSize);
			break;
		case 58:
			LTStrCat(aMsgBuff, " DOOMSDAYPIECE", nBufferSize);
			break;
		default:
			LTStrCat(aMsgBuff, " UNKNOWN", nBufferSize);
			break;
		...
		LTSNPrintF( aTemp, nBufferSize, " 0x%08x %d", hObj, nSubID ); 
	...
	dsi_ConsolePrint( "[S2C ] (%10d) %s |%d|", timeGetTime(), aMsgBuff, pMsg->Size() );
}
#endif