...
void PhysicsUpdateObject(LTObject *pObj)
{
	...
	//[MURSUM] SyncPos
	if( pObj->sd && pObj->sd->m_NetFlags & NETFLAG_ANIM_PLAYEROBJ )
	{
		vVel = pObj->m_Velocity;
	}
	///////////////////
	/*
	//[yaiin]
	//위 Mursum SyncPos는 값을 '빽업'하는 것.
	//문제:원래 주석 상태. 다시 풀면....?
	//답1:둘 다 있어야 클라이언트와 같은 인자값으로 계산해 같은 결과를 냄.
#ifdef _DEBUG
	LTVector vvVel(0.0f, 0.0f, 0.0f), vvAccel(0.0f, 0.0f, 0.0f), vvPos(0.0f, 0.0f, 0.0f);
	vvPos = pObj->m_Pos;
	vvVel = pObj->m_Velocity;
	vvAccel = pObj->m_Acceleration;
	if(vVel.Mag() >= 299.0f)
		dsi_ConsolePrint("Eng:PyUO BF : vel(%.2f), accel(%.2f), pos(%.2f, %.2f, %.2f)",
		vvVel.Mag(), vvAccel.Mag(),
		vvPos.x, vvPos.y, vvPos.z);
#endif*/
...
/*
	...
	vvAccel = pObj->m_Acceleration;
	if(vVel.Mag() > 299.0f)
		dsi_ConsolePrint("Eng:PyUO AF : vel(%.2f), accel(%.2f), pos(%.2f, %.2f, %.2f)",
		vvVel.Mag(), vvAccel.Mag(),
		vvPos.x, vvPos.y, vvPos.z);
#endif*/
	...
	if( pObj->sd && pObj->sd->m_NetFlags & NETFLAG_ANIM_PLAYEROBJ )
	{
		pObj->m_Velocity.x = vVel.x;
		pObj->m_Velocity.z = vVel.z;
	}
	///////////////////
}
...
LTRESULT LoadObjects(ILTStream *pStream, const char *pWorldName, bool bAllObjects, uint32 nObjectDataOffset)
{
	...
	if( LTNULL == pStream || LTNULL == pWorldName )
	{
	...
	for (i=0; i < nObjects; i++)
    {
        ...
        if (pClass && pObject)
        {
			...
			if (nPreCreateResult && ((pClass->m_ClassFlags & CF_CLASSONLY) == 0))
			{
				...
				case OT_SPRITE:
				case OT_CONTAINER:
					if( true == sm_SouldSendPacket(pObj) )
					{
						pObj->m_InternalFlags |= IFLAG_DONTSEND;
					}
					break;
				}
			}
...
bool sm_SouldSendPacket( LTObject* pObject )
{
