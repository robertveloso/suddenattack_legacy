...
enum EDisconnectReason { DISCONNECTREASON_VOLUNTARY_CLIENTSIDE =0,
						 ...
						 DISCONNECTREASON_SILENT_KICKED = 7};
...
class CBaseDriver
{
	...
	public:
		...
		virtual	LTRESULT	HostSession(NetHost* pHost,SOCKET Sock) {return LT_ERROR;}
		...
		virtual LTRESULT StartCheckConeType( SOCKET Sock, const char* lpszIP1, uint16 nPort1,
											 const char* lpszIP2, uint16 nPort2, const char* lpszID, uint8 nIDLen ) { return LT_ERROR; }
		virtual void	 StopCheckConeType() {};
		virtual uint32	 GetConeType() { return 0; }
		...
		virtual LTRESULT	ConnectTCP(const char* sAddress, SOCKET Sock, uint16 nLocalPort ) { return(false); }
