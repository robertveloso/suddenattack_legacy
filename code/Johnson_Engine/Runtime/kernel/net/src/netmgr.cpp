...
void ParsePacket(const CPacket_Read &cPacket, bool bFirst, char *aMsgBuff, uint32 nBufferSize, uint8 nTravelDir = NETMGR_TRAVELDIR_UNKNOWN)
{
	...
	switch(nPacketID)
	{
		...
		case 8 : //SMSG_UPDATE
		{
			LTSNPrintF(aTemp, sizeof(aTemp), "(UPD) |%d| ", cParsePacket.TellEnd() / 8);
			LTStrCat(aMsgBuff, aTemp, nBufferSize);
			
			while (!cParsePacket.EOP()) 
			{
				uint32 nUpdateSize = cParsePacket.Readuint32();
				uint32 nUpdateStart = cParsePacket.Tell();

				CPacket_Read cSubPacket(cParsePacket, nUpdateStart, nUpdateSize );				
				uint8 flags = cSubPacket.Readuint8();
				...
				if (flags )
				{
					...
					{
						LTStrCat(aMsgBuff, "CHG ", nBufferSize);	
					}
				}
				else
				{					
					uint8 subType = cSubPacket.Readuint8();

					if (subType == UPDATESUB_PLAYSOUND) 
					{
						LTStrCat(aMsgBuff, "SND ", nBufferSize);
					}
					else if (subType == UPDATESUB_SOUNDTRACK) 
					{
						LTStrCat(aMsgBuff, "SNDT ", nBufferSize);
					}
					else if (subType == UPDATESUB_OBJECTREMOVES) 
					{
						LTStrCat(aMsgBuff, "OBJM ", nBufferSize);
					}
					else 
					{
				...
				cParsePacket.Seek(nUpdateSize+nUpdateStart);
			}
		...
		case 0xD : //SMSG_MESSAGE
		{
			...
			*/
			...
			switch( nMsgID )
			{
				...
				{
					LTSNPrintF(aTemp, sizeof(aTemp), "[PLU] |%d| ", cParsePacket.TellEnd() / 8 );
					LTStrCat(aMsgBuff, aTemp, nBufferSize);
					...
					*/
					...
					if( nChangeID & 0x0001 ) // CLIENTUPDATE_CAMERAOFFSET
						LTStrCat(aMsgBuff, "COP ", nBufferSize);

					if( nChangeID & 0x0020 ) // CLIENTUPDATE_PLAYERROT
						LTStrCat(aMsgBuff, "ROT ", nBufferSize);
					
					if( nChangeID & 0x0040 ) // CLIENTUPDATE_ACCURATEPLAYERROT
						LTStrCat(aMsgBuff, "ART ", nBufferSize);

					if( nChangeID & 0x0080 ) // CLIENTUPDATE_FULLPLAYERROT
						LTStrCat(aMsgBuff, "FRT ", nBufferSize);
				...
				{
					LTStrCat(aMsgBuff, "[PIC] |%d|", nBufferSize);
				...
				case 108 : // MID_PLAYER_ORIENTATION
					LTSNPrintF(aTemp, sizeof(aTemp), "[PLO] |%d|", cParsePacket.TellEnd()/8);
					LTStrCat(aMsgBuff, aTemp, nBufferSize);
					break;

				case 117 : // MID_PLAYER_DAMAGE
					LTSNPrintF(aTemp, sizeof(aTemp), "[PLD] |%d|", cParsePacket.TellEnd()/8);
					LTStrCat(aMsgBuff, aTemp, nBufferSize);
					break;
				...
				{
					LTSNPrintF(aTemp, sizeof(aTemp), "[PCM] |%d| ", cParsePacket.TellEnd()/8);
					LTStrCat(aMsgBuff, aTemp, nBufferSize);

					uint16 nCP = cParsePacket.Readuint8();

					switch(nCP)
					{
					case 1:
						LTStrCat(aMsgBuff, "MST ", nBufferSize);break; // CP_MOTION_STATUS
					case 2:
						LTStrCat(aMsgBuff, "DMG ", nBufferSize);break; // CP_DAMAGE
					case 3:
						LTStrCat(aMsgBuff, "WST ", nBufferSize);break; // CP_WEAPON_STATUS
					case 4:
						LTStrCat(aMsgBuff, "FLT ", nBufferSize);break; // CP_FLASHLIGHT
					case 5:
						LTStrCat(aMsgBuff, "PLN ", nBufferSize);break; // CP_PLAYER_LEAN
					case 6:
						LTStrCat(aMsgBuff, "PSM ", nBufferSize);break; // CP_PHYSICSMODEL
					case 7:
						LTStrCat(aMsgBuff, "VIM ", nBufferSize);break; // CP_DAMAGE_VEHICLE_IMPACT
					case 8:
						LTStrCat(aMsgBuff, "DEF ", nBufferSize);break; // CP_DEFLECT
					}
				...
				case 112 : // MID_PLAYER_RESPAWN
					LTSNPrintF(aTemp, sizeof(aTemp), "[RESPAWN] |%d| ", cParsePacket.TellEnd()/8);
					LTStrCat(aMsgBuff, aTemp, nBufferSize);					
					break;

				case 130 : // MID_WEAPON_CHANGE
					LTSNPrintF(aTemp, sizeof(aTemp), "[WPN] |%d| ", cParsePacket.TellEnd()/8);
					LTStrCat(aMsgBuff, aTemp, nBufferSize);
					LTStrCat(aMsgBuff, "CHG", nBufferSize);
					break;
				...
				case 132 : // MID_WEAPON_SOUND_LOOP
					LTSNPrintF(aTemp, sizeof(aTemp), "[WPN] |%d| ", cParsePacket.TellEnd()/8);
					LTStrCat(aMsgBuff, aTemp, nBufferSize);
					LTStrCat(aMsgBuff, "SDL", nBufferSize);
					break;
				case 133 : // MID_WEAPON_FIRE
					LTSNPrintF(aTemp, sizeof(aTemp), "[WPN] |%d| ", cParsePacket.TellEnd()/8);
					LTStrCat(aMsgBuff, aTemp, nBufferSize);
					LTStrCat(aMsgBuff, "FIR", nBufferSize);
					break;
				case 134 : // MID_WEAPON_RELOAD
					LTSNPrintF(aTemp, sizeof(aTemp), "[WPN] |%d| ", cParsePacket.TellEnd()/8);
					LTStrCat(aMsgBuff, aTemp, nBufferSize);
					LTStrCat(aMsgBuff, "RLD", nBufferSize);
					break;
				case 138 : // MID_PROJECTILE
					LTSNPrintF(aTemp, sizeof(aTemp), "[WPN] |%d| ", cParsePacket.TellEnd()/8);
					LTStrCat(aMsgBuff, aTemp, nBufferSize);
					LTStrCat(aMsgBuff, "THW", nBufferSize);
					break;
				case 146 : // MID_PLAYER_TAUNT
					LTStrCat(aMsgBuff, "[RADIO]", nBufferSize);
					break;

				//===========================================

				case 190 : //MID_PHYSICS_UPDATE
					LTSNPrintF(aTemp, sizeof(aTemp), "[PHU] |%d| ", cParsePacket.TellEnd()/8);
					LTStrCat(aMsgBuff, "PHU ", nBufferSize);
					break;

				case 214 : //MID_PINGTIMES
					LTSNPrintF(aTemp, sizeof(aTemp), "[PING] |%d| ", cParsePacket.TellEnd()/8);
					LTStrCat(aMsgBuff, aTemp, nBufferSize);
					break;

				case 231 : //MID_SFX_MESSAGE
					LTSNPrintF(aTemp, sizeof(aTemp), "[SFX] |%d| ", cParsePacket.TellEnd()/8);
					LTStrCat(aMsgBuff, aTemp, nBufferSize);
					break;				

				default :
				{
					LTSNPrintF(aTemp, sizeof(aTemp), "%d |%d| ", (uint32)nMsgID, cParsePacket.TellEnd()/8);
					LTStrCat(aMsgBuff, aTemp, nBufferSize);
					break;
				}
			}
...
float CNetMgr::GetHostPing()
{
	if( m_Connections <=1 ) return 0.0f;

	float	fAccumPing = 0;
	
	for (uint32 i = 0; i < m_Connections; ++i)
	{
		CBaseConn *pConn = m_Connections[i];
		fAccumPing += pConn->GetPing();
	}

	return ( fAccumPing/(m_Connections-1) );
}
...
bool CNetMgr::SendPacket(const CPacket_Read &cPacket, CBaseConn *idSendTo, uint32 packetFlags)
{
	...
	if ( g_CV_ParseSend )
	{
		if ( FALSE ==  (idSendTo->m_ConnFlags & CONNFLAG_LOCAL) )
		{
			NFPacketDebuger::GetInstance()->NFParseSend( 0, cPacket );
		}
	}
...
bool CNetMgr::GetPacket(uint8 nTravelDir, CPacket_Read *pPacket, CBaseConn **pSender)
{
	...
	for (uint32 i=0; i < m_Drivers; i++)
	{
		...
		while (pDriver->GetPacket(&cCurPacket, &pCurSender))
		{
			...
			if ( g_CV_ParseRecv )
			{
				NFPacketDebuger::GetInstance()->NFParseRecv( nTravelDir, cCurPacket );
			}
#endif //SA_NETMONITORING
			// [NOVICE] END -----------------------
...
bool CNetMgr::NewConnectionNotify(CBaseConn *id)
{
	...
	if (m_pHandler)
	{
		...
		if (m_pHandler->NewConnectionNotify(id, id->m_ConnFlags & CONNFLAG_LOCAL))
		{
			// [NOVICE] BEGIN ---------------------------------------
#ifdef SA_NETMONITORING
			NFPacketDebuger::GetInstance()->AddPlayer();
#endif //SA_NETMONITORING
			// [NOVICE] END -----------------------------------------
			return true;
		}
...
void CNetMgr::DisconnectNotify(CBaseConn *id, EDisconnectReason eDisconnectReason )
{
	...
	if (index != BAD_INDEX)
	{
		...
		// [NOVICE] BEGIN ---------------------------------------
#ifdef SA_NETMONITORING
		NFPacketDebuger::GetInstance()->RemovePlayer();
#endif //SA_NETMONITORING
		// [NOVICE] END -----------------------------------------
...
bool CNetMgr::ReallySendPacket(const CPacket_Read &cPacket, CBaseConn *idSendTo, uint32 nPacketFlags)
{
	...
	// Send it...
//#################################################################################################
//	자자.. 자세히 봐야한다. m_pDriver 는 CUDPDriver의 포인터이다.
//	요걸로 Find in Files 해보자. CUDPDriver::SendPacket
