...
public:
	...
	virtual bool GetIPAddress(uint8 pAddr[4], uint16 *pPort) { 
		pAddr[0] = (uint8)m_RemoteAddr.sin_addr.S_un.S_un_b.s_b1;
		pAddr[1] = (uint8)m_RemoteAddr.sin_addr.S_un.S_un_b.s_b2;
		pAddr[2] = (uint8)m_RemoteAddr.sin_addr.S_un.S_un_b.s_b3;
		pAddr[3] = (uint8)m_RemoteAddr.sin_addr.S_un.S_un_b.s_b4;
...
private:
	...
	enum {
		...
		k_nPingDelay = 5000, // Wait at least this long between ping requests
		...
		k_nMaxPingNAKTime = 20000, // Don't wait longer than this to send a NAK
...
public:
    ...
    virtual LTRESULT GetHasPassword(bool *bHasPassword) { CWinSync_CSAuto cCS(m_cCS_SessionData); *bHasPassword = m_bPassword; return LT_OK; }
    virtual LTRESULT SetHasPassword(bool bHasPassword)  { CWinSync_CSAuto cCS(m_cCS_SessionData); m_bPassword = bHasPassword; return LT_OK; };

    virtual LTRESULT GetGameType(uint8 *nGameType) { CWinSync_CSAuto cCS(m_cCS_SessionData); *nGameType = m_nGameType; return LT_OK; }
    virtual LTRESULT SetGameType(uint8 nGameType)  { CWinSync_CSAuto cCS(m_cCS_SessionData); m_nGameType = nGameType; return LT_OK; };

    virtual LTRESULT GetMaxConnections(uint32 &nMaxConnections);

    virtual void Update();

	virtual LTRESULT HostSession( NetHost* pHost, SOCKET Sock );
    virtual LTRESULT JoinSession(NetSession *pSession);

	//[MURSUM] Cone
	virtual LTRESULT StartCheckConeType( SOCKET Sock, const char* lpszIP1, uint16 nPort1,
										  const char* lpszIP2, uint16 nPort2, const char* lpszID, uint8 nIDLen );
	virtual void	 StopCheckConeType();
	virtual uint32	 GetConeType() { return m_nConeType; }
	virtual void	 SendDummyPacket( SOCKET Sock, const char* lpszIP, uint16 nPort );

    virtual bool GetLocalIpAddress(char* sBuffer, uint32 dwBufferSize, uint16 &hostPort);

	LTRESULT ReallyJoinSession(SOCKET Sock, sockaddr_in *addr);

	virtual LTRESULT ConnectTCP( const char* sAddress, SOCKET Sock, uint16 nLocalPort );
	...
	enum {
		UNCONNECTED_DATA_TOKEN	= 0x01337F10, // Packet identifier for unconnected communication
		UNCONNECTED_CHECK_CONE	= UNCONNECTED_DATA_TOKEN+1,
		UNCONNECTED_DATA_DUMMY	= UNCONNECTED_DATA_TOKEN+2,
		UNCONNECTED_DATA_TERM	= UNCONNECTED_DATA_TOKEN+3,
		...
		UNCONNECTED_MSG_SUCCESS	= 0,
		UNCONNECTED_MSG_HOST_FULL = 1,
		UNCONNECTED_MSG_WRONG_VERSION = 2,
		UNCONNECTED_MSG_HOST_NOT_READY = 3,
	};
...
protected:
	...
	enum { CONN_SEND_INTERVAL =300, CONN_WAIT_TIME =20000 };

	void HandleUnconnectedData(CPacket_Read &cPacket, sockaddr_in *pSender);

	//[MURSUM] Cone
	void HandleCheckConeType(CPacket_Read &cPacket);
	...
	enum {
		...
		k_nCheckPacket_Delay = 100,
		k_nCheckConeSleep	= 10,
	};
private:
	// Threading support
	...
	//[MURSUM] Cone
	void StartThread_CheckCone();
	void StopThread_CheckCone();
	static unsigned long _stdcall ThreadBootstrap_CheckCone(void *pUserData);
	uint32 Thread_CheckCone();
	
	uint8	m_nLocalIDLen;
	char	m_szGameServerIP1[16];	//MAX_IP_LEN
	uint16	m_nGameServerPort1;
	char	m_szGameServerIP2[16];
	uint16	m_nGameServerPort2;
	char	m_szLocalID[256];
	uint32	m_nConeType;
	
	HANDLE m_hThread;
	CWinSync_Event m_cEvent_Thread_Ready;
	CWinSync_Event m_hEvent_Thread_Shutdown;
...
private:
	...
	CWinSync_CS m_cCS_UnknownMessages;
	...
	CWinSync_CS m_cCS_DisconnectQueue;
	...
	typedef std::list<CConnectRequest*> CON_LIST;
	typedef CON_LIST::iterator			CON_LIST_ITR;
		
	CWinSync_CS m_cCS_ConnectQueue;
	CON_LIST	m_cConnectList;
	...
	CWinSync_CS m_cCS_ActivePings;
	TPingMap m_cActivePings;

	// Misc. data
	CWinSync_CS m_cCS_GUID;
	LTGUID m_cGUID;
...
public:
	...
	bool IsHosting() { CWinSync_CSAuto cCS(m_cCS_SessionData); return m_bHosting; }

	CWinSync_CS m_cCS_Connections;
