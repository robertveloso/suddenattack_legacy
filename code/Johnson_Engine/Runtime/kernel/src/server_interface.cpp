...
public:
        ...
        virtual bool	InitNetworking(char *pDriver, uint32 dwFlags, bool bTermShell=true);
...
bool CLTServerInterface::HostGame(NetHost* pHostInfo)
{
    ...
    if (pDriver->HostSession(pHostInfo,0) != LT_OK)
        return LTFALSE;
...
bool CLTServerInterface::InitNetworking(char *sDriver, uint32 dwFlags,bool bTermShell)
{
	CSEntry entry(&g_ServerInterface);
