...
LTRESULT r_InitRender(RMode *pMode)
{
	...
	{
		if( g_Render.ConsolePrint ) g_Render.ConsolePrint( "[DX_INIT ERROR] cis_RendererIsHere" );
...
#endif
	{
		if( g_RMode.m_Width == init.m_Mode.m_Width &&
			g_RMode.m_Height == init.m_Mode.m_Height &&
			g_RMode.m_BitDepth == init.m_Mode.m_BitDepth &&
			g_RMode.m_bHWTnL == init.m_Mode.m_bHWTnL )
		{
			g_pClientMgr->BindSharedTextures();	//[ÆÃ]
		}
	}
...
void r_ToggleFullScreen()
{
#ifndef _FINAL
	RMode rMode;
	if( LT_OK == dsi_GetRenderMode( &rMode ) )
	{
		char cmd[32];
		LTSNPrintF(cmd, sizeof(cmd), "windowed %d", !g_bRunWindowed );
		c_CommandHandler(cmd);
		dsi_SetRenderMode( &rMode );
	}
#endif
}