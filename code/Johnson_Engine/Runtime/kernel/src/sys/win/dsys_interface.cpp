...
LTRESULT dsi_SetRenderMode(RMode *pMode) {
	...
	{
		dsi_ConsolePrint( "[DX_INIT ERROR] r_InitRender (First Chance)" );
        // Ok, try to restore the old mode.
        if (r_InitRender(&currentMode) != LT_OK)
		{
			dsi_ConsolePrint( "[DX_INIT ERROR] r_InitRender (Second Chance)" );
            //dsi_SetupMessage(message, sizeof(message)-1, LT_UNABLETORESTOREVIDEO, LTNULL);
            //dsi_OnClientShutdown(message);
            RETURN_ERROR(0, SetRenderMode, LT_UNABLETORESTOREVIDEO);
        }
							 ...
							 char* pszOutName, int outNameLen, bool& bFileCopied ) 
{
	...
	*/
	...
	if( szTempFilename[strlen(szTempFilename)-1] != '\\' && 
		szTempFilename[strlen(szTempFilename)-1] != '/' )
	{
		strcat( szTempFilename, "\\" );
	}
	strcat( szTempFilename, pszFilename );
	/////////////////////////////////////////////////////////////
...
void dsi_SendPingToGameServer()
{
	if( i_client_shell ) i_client_shell->SendPingToGameServer();
}

void dsi_FlushErrorLog()
{
	con_FlushErrorLog();
}
...
void dsi_OnMemoryFailure() {
    ...
    MessageBox(GetDesktopWindow(), "Out of memory", "SuddenAttack", MB_OK);
    exit(2222);
}
