...
void CLTMessage_Write::WriteCompPitch(CPacket_Write &cPacket, const float fPitch)
{
	 cPacket.Writeuint8( g_pCompressor->CompressAngleToByte(fPitch) );
}
...
{
	const int nTotalSize	= 8;
	const int nLossSize		= 2;
	
	LTVector forward = cRotation.Forward();
	float fAngle = (float)atan2(forward.x, forward.z);
	int8 nAngle = (int8)(fAngle * (127.0f / MATH_PI));

//	cPacket.Writeint8( nAngle );
	cPacket.WriteBits( nAngle >> nLossSize, nTotalSize - nLossSize );
}
...
float CLTMessage_Read::ReadCompPitch(CPacket_Read &cPacket)
{
	float fPitch = 0.0f;
	g_pCompressor->UncompressAngleFromByte(cPacket.Readuint8(), &fPitch); 
	return fPitch;
}
...
//	int8 nAngle = cPacket.Readint8();
	int8 nAngle = ((cPacket.ReadBits(nTotalSize - nLossSize)) << nLossSize);
