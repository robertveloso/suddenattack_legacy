...
private:
	friend CListQueue_Iterator_Base<T>;
	friend CListQueue_Iterator<T>;
	friend CListQueue_Const_Iterator<T>;
...
protected:
	...
	CListQueue_Iterator_Base(CListQueue<T>::CItem *pFinger) : m_pFinger(pFinger) {}
	CListQueue<T>::CItem *m_pFinger;
};
...
private:
	CListQueue_Iterator(CListQueue<T>::CItem *pItem) : CListQueue_Iterator_Base<T>(pItem) {}
};
...
struct CListQueue_Const_Iterator : public CListQueue_Iterator_Base<T>
{
	...
	CListQueue_Const_Iterator() : m_pFront(0) {}
...
private:
	CListQueue_Const_Iterator(const CListQueue<T>::CItem *pItem) : CListQueue_Iterator_Base<T>(const_cast<CListQueue::CItem*>(pItem)) {}
};
