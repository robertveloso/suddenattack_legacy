...
public:
	...
	// [NOVICE]
	virtual uint8 CompressAngleToByte( float fPitch );
	virtual void  UncompressAngleFromByte( uint8 cCompactedPitch, float *pfPitch );
...
// !!! WARNING !!!
// [NOVICE] 아래 두 함수는
// 게임단의 CommonUtilities.cpp / h 에 있는 내용을 그대로 옮긴 것이다.
// 게임 단에서는 CommonUtilities 를 사용하고 있기때문에,
// 엔진단에 같은 내용을 옯겨 놓았다.
// 만일 방식을 바꾼다면, 여기와, 게임단의 CommonUtilites 를 바꿔줘야 한다.
//////////////////////////////////////////////////////////////////////////
uint8 CCompress::CompressAngleToByte( float fPitch )
{
	//
	// pitch
	//
	...
	int8 cPitch =
		static_cast< int8 >( 
		fPitch *
		static_cast< float >( 127/*SCHAR_MAX*/ ) /
		1.570796326795f/*MATH_HALFPI*/
		);
	
	return static_cast< uint8 >( cPitch );
}

void CCompress::UncompressAngleFromByte( uint8 cCompactedPitch, float *pfPitch )
{
	int8 cPitch;
