...
//0..N
static void InstallTexture(LPDIRECT3DTEXTURE8 pTexture, uint32 nNumChannels)
{
...
static bool SetRenderTarget(LPDIRECT3DTEXTURE8 pTarget, LPDIRECT3DSURFACE8 pDepthBuffer)
{
	//get the first surface from the texture
	LPDIRECT3DSURFACE8 pRenderSurface = NULL;
...
bool CScreenGlowMgr::UpdateShader()
{
	...
	if (NULL == pPixelShader)
	{
		...
		ref.m_pFilename = "ps\\screenglow.ps";
		...
		if (NULL != pStream)
		{
			if (LTPixelShaderMgr::GetSingleton().AddPixelShader(pStream, ref.m_pFilename,
																LTPixelShader::PIXELSHADER_SCREENGLOW))
			{
...
bool CScreenGlowMgr::UpdateGlowTexture(uint32 nWidth, uint32 nHeight)
{
	...
	if(!m_pGlowTexture || !m_pDepthBuffer || (nWidth != m_nTextureWidth) || (nHeight != m_nTextureHeight))
	{
		...
		if(FAILED(PD3DDEVICE->CreateDepthStencilSurface(nWidth, nHeight, iDepthFormat, D3DMULTISAMPLE_NONE, &m_pDepthBuffer)))
		{
...
bool CScreenGlowMgr::RenderSceneToTexture(const SceneDesc& Scene)
{
	...
	d3d_InitFrustum(&NewViewParams, Scene.m_xFov, Scene.m_yFov, NEARZ, g_CV_FarZ,
...
bool CScreenGlowMgr::RenderBlendedTexture(bool bUsePixelShader)
{
	...
	StageStateSet tss00(0, D3DTSS_ADDRESSU, D3DTADDRESS_CLAMP);
	StageStateSet tss01(0, D3DTSS_ADDRESSV, D3DTADDRESS_CLAMP);
	StageStateSet tss10(1, D3DTSS_ADDRESSU, D3DTADDRESS_CLAMP);
	StageStateSet tss11(1, D3DTSS_ADDRESSV, D3DTADDRESS_CLAMP);
	...
	if(bUsePixelShader)
	{
		...
		StageStateSet tss20(2, D3DTSS_ADDRESSU, D3DTADDRESS_CLAMP);
		StageStateSet tss21(2, D3DTSS_ADDRESSV, D3DTADDRESS_CLAMP);
		StageStateSet tss30(3, D3DTSS_ADDRESSU, D3DTADDRESS_CLAMP);
		StageStateSet tss31(3, D3DTSS_ADDRESSV, D3DTADDRESS_CLAMP);
		...
		//setup our device
		PD3DDEVICE->SetVertexShader(GLOW4TEXTUREVERTEX_FORMAT);
	...
	else
	{
		...
		//setup our device
		PD3DDEVICE->SetVertexShader(GLOW2TEXTUREVERTEX_FORMAT);
...
bool CScreenGlowMgr::DisplayBlendedTexture()
{
	...
	float fWidth	= g_ViewParams.m_fScreenWidth;
	float fHeight	= g_ViewParams.m_fScreenHeight;
	...
	// Set up the vertices
	SetupRectVerts(0, 0, fWidth, fHeight, fU0, fV0, fU1, fV1, 0xFFFFFFFF, Verts);
	...
	D3D_CALL(PD3DDEVICE->SetVertexShader(TLVERTEX_FORMAT));
...
bool CScreenGlowMgr::DrawDebugFilter()
{
	...
	PD3DDEVICE->SetVertexShader(TLVERTEX_FORMAT);
