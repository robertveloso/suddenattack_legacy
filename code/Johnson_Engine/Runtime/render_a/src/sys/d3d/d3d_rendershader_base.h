...
template <class SVertex, class CShaderSection, uint32 nFVF>
...
protected:
	...
	virtual void DebugDrawTextureQuad(
...
protected:
	...
	typedef std::vector<IDirect3DVertexBuffer8*> TVBList;
	TVBList m_aVBs;
...
//////////////////////////////////////////////////////////////////////////////
// CRenderShader_Base implementation  (Must be in header due to templatization)

#include <algorithm>

template <class SVertex, class CShaderSection, uint32 nFVF>
CRenderShader_Base<SVertex, CShaderSection, nFVF>::~CRenderShader_Base()
{
	// Forget the VB & IB
	if (!m_aVBs.empty())
	{
...
template <class SVertex, class CShaderSection, uint32 nFVF>
uint32 CRenderShader_Base<SVertex, CShaderSection, nFVF>::PreviewRenderBlock(const CD3D_RenderBlock &cRenderBlock)
{
...
template <class SVertex, class CShaderSection, uint32 nFVF>
void CRenderShader_Base<SVertex, CShaderSection, nFVF>::PreviewSection(const CRBSection &cSection)
{
...
template <class SVertex, class CShaderSection, uint32 nFVF>
void CRenderShader_Base<SVertex, CShaderSection, nFVF>::Lock(uint32 nRenderBlock)
{
	if (m_bLocked)
	{
	...
	if (!m_pLockedIndices)
	{
		...
		for (; iCurVB != m_aVBs.end(); ++iCurVB)
		{
			...
			if (!SUCCEEDED((*iCurVB)->Lock(0, 0, (BYTE**)&m_aLockedVertices.back(), 0)))
			{
		...
		// Lock the IB
		if (!SUCCEEDED(m_pIB->Lock(0, 0, (BYTE**)&m_pLockedIndices, 0)))
		{
...
template <class SVertex, class CShaderSection, uint32 nFVF>
void CRenderShader_Base<SVertex, CShaderSection, nFVF>::AddSection(const CRBSection &cSection, const uint16 *aIndices, const SRBVertex *aVertices, const SRBVertex *aSrcVertices)
{
...
template <class SVertex, class CShaderSection, uint32 nFVF>
void CRenderShader_Base<SVertex, CShaderSection, nFVF>::Unlock()
{
...
template <class SVertex, class CShaderSection, uint32 nFVF>
void CRenderShader_Base<SVertex, CShaderSection, nFVF>::FinalizeUnlock()
{
	if (!m_pLockedIndices)
	{
...
template <class SVertex, class CShaderSection, uint32 nFVF>
void CRenderShader_Base<SVertex, CShaderSection, nFVF>::Draw(const DrawState &cState, uint32 nRenderBlock)
{
	// Make sure we can
...
template <class SVertex, class CShaderSection, uint32 nFVF>
void CRenderShader_Base<SVertex, CShaderSection, nFVF>::DrawGlow(uint32 nRenderBlock)
{
	// Get ready to draw...
	FinalizeUnlock();
...
template <class SVertex, class CShaderSection, uint32 nFVF>
CRenderShader_Base<SVertex, CShaderSection, nFVF>::CInternalSection *
	CRenderShader_Base<SVertex, CShaderSection, nFVF>::FindSection(uint32 nRenderBlock, uint32 nIndex)
{
	...
	if (nRenderBlock >= m_aRenderBlocks.size())
	{
		...
		TSectionList::iterator iEndSection = (nRenderBlock < (m_aPreviewRenderBlocks.size() - 1)) ? (m_aSections.begin() + m_aPreviewRenderBlocks[nRenderBlock + 1]) : m_aSections.end();
...
template <class SVertex, class CShaderSection, uint32 nFVF>
bool CRenderShader_Base<SVertex, CShaderSection, nFVF>::DrawAggregateShader(IAggregateShader *pShader, uint32 nRenderBlock)
{
	// Get ready to draw...
	FinalizeUnlock();
...
template <class SVertex, class CShaderSection, uint32 nFVF>
void CRenderShader_Base<SVertex, CShaderSection, nFVF>::QueueRenderBlock(uint32 nRenderBlock)
{
	CRenderBlockData &cRB = m_aRenderBlocks[nRenderBlock];
	CRenderBlockData::TSectionIndexList::iterator iCurSection = cRB.m_aSections.begin();
	for (; iCurSection != cRB.m_aSections.end(); ++iCurSection)
...
// (Which probably should be provided globally in the renderer...  But that would be silly)
template <class SVertex, class CShaderSection, uint32 nFVF>
void CRenderShader_Base<SVertex, CShaderSection, nFVF>::DebugDrawTextureQuad(
	IDirect3DTexture8 *pTexture,
	float fMinU, float fMinV, float fMaxU, float fMaxV,
	float fX, float fY, float fSizeX, float fSizeY)
{
	...
	StageStateSet stateWrap0(0, D3DTSS_ADDRESSU, D3DTADDRESS_WRAP);
	StageStateSet stateWrap1(0, D3DTSS_ADDRESSV, D3DTADDRESS_WRAP);
...
template <class SVertex, class CShaderSection, uint32 nFVF>
void CRenderShader_Base<SVertex, CShaderSection, nFVF>::PrepareSections()
{
...
template <class SVertex, class CShaderSection, uint32 nFVF>
void CRenderShader_Base<SVertex, CShaderSection, nFVF>::InitRenderBlocks()
{
...
template <class SVertex, class CShaderSection, uint32 nFVF>
void CRenderShader_Base<SVertex, CShaderSection, nFVF>::SortSections()
{
...
template <class SVertex, class CShaderSection, uint32 nFVF>
void CRenderShader_Base<SVertex, CShaderSection, nFVF>::MarkSectionBreaks()
{
...
template <class SVertex, class CShaderSection, uint32 nFVF>
void CRenderShader_Base<SVertex, CShaderSection, nFVF>::ChangeSection(const CInternalSection &cSection)
{
	if (m_nCurVB != cSection.m_nVBIndex)
	{
		m_nCurVB = cSection.m_nVBIndex;
		PD3DDEVICE->SetStreamSource(0, m_aVBs[m_nCurVB], sizeof(SVertex));
	}
}

template <class SVertex, class CShaderSection, uint32 nFVF>
void CRenderShader_Base<SVertex, CShaderSection, nFVF>::Flush()
{
	...
	PD3DDEVICE->SetVertexShader(nFVF);
...
template <class SVertex, class CShaderSection, uint32 nFVF>
void CRenderShader_Base<SVertex, CShaderSection, nFVF>::DrawFogPass(uint32 nRenderBlock)
{
...
template <class SVertex, class CShaderSection, uint32 nFVF>
void CRenderShader_Base<SVertex, CShaderSection, nFVF>::DrawLights(const DrawState &cState, uint32 nRenderBlock)
{
	{
...
template <class SVertex, class CShaderSection, uint32 nFVF>
void CRenderShader_Base<SVertex, CShaderSection, nFVF>::DrawLightList(const DrawState &cState, uint32 nRenderBlock)
{
