...
HLTBUFFER d3d_CreateSurface(int width, int height)
{
	...
	hResult = PD3DDEVICE->CreateImageSurface(width,height,g_Device.GetModeInfo()->Format,&pSurface);
...
void d3d_BlitFromScreen(BlitRequest* pRequest)
{
	...
	// Copy away!
	HRESULT hResult = PD3DDEVICE->CopyRects(pBackBuffer,&srcRect,1,pRSurface->m_pSurface,&destPt);
...
{
	LPDIRECT3DSURFACE8 pBackBuffer = NULL; D3DSURFACE_DESC SurfDesc;
	if (FAILED(PD3DDEVICE->GetBackBuffer(0,D3DBACKBUFFER_TYPE_MONO,&pBackBuffer)))	{ return; }
...
//this will perform a little trick that prevents cards from buffering up additional frames.
//this will request a single pixel from the back buffer, which forces the card to flush
//anything queued.
static void d3d_PreventFrameBuffering()
{
	//first we need to get the back buffer
	IDirect3DSurface8* pBackBuffer;

	if(FAILED(PD3DDEVICE->GetBackBuffer(0, D3DBACKBUFFER_TYPE_MONO, &pBackBuffer)))
	{
		//failed to get the surface, no biggie, we just have frames buffered
		return;
	}

	//alright, we now have the surface, so we need to lock it and read one pixel
	...
	//lock the rect
	if(SUCCEEDED(pBackBuffer->LockRect(&LockedRect, &RectToLock, D3DLOCK_READONLY)))
	{
		pBackBuffer->UnlockRect();
	}

	//and that is it, just clean up
	pBackBuffer->Release();
}
