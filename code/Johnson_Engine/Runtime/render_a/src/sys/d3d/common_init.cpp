...
LPDIRECT3DDEVICE8 d3d_GetD3DDevice();
...
int d3d_Init(RenderStructInit *pInit)
{
	...
	if (g_bInStandby) 
	{
		...
		else 
		{
			...
			if (!g_Device.SetMode(pModeInfo))	//[ÆÃ]
			{
	...
	if (!g_bInStandby) 
	{
		...
		{	
			...
			d3d_Term(true); 
			dsi_ConsolePrint( "[DX_INIT ERROR] g_D3DShell.PickDefaultDev" );
			return RENDER_ERROR; 
		}
		...
		{	
			// Couldn't find any devices that make the cut - fail out
			d3d_Term(true); 
			dsi_ConsolePrint( "[DX_INIT ERROR] g_D3DShell.PickDefaultMode" );
			return RENDER_ERROR; 
		}
	...
	SetWindowPos(g_hWnd, HWND_TOP, 0, 0, g_ScreenWidth, g_ScreenHeight, SWP_NOMOVE);
	...
	{
		d3d_Term(true);
		dsi_ConsolePrint("[DX_INIT ERROR] VisibleSet::Init failed");
