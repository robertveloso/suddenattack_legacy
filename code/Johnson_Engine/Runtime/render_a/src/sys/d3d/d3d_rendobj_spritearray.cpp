...
void CD3DSpriteArray::Render_PointSprites(vector<LPDIRECT3DBASETEXTURE8>& TextureList)
{
...
// Render using quads...
void CD3DSpriteArray::Render_Quads(vector<LPDIRECT3DBASETEXTURE8>& TextureList) 
{
	if (m_pTexIndexData) {																	// Render with texture indicies...
		uint32 iSpriteIndex = 0;															// Walk m_pUsedBitArray and render as you go...
		uint8* pTexIndex	= m_pTexIndexData;
		...
		while (iSpriteIndex < m_iMaxSpriteCount) {
			...
			if (m_pUsedBitArray[iByte] & iBitMask) {
				...
				if (iRendCnt) { 
					...
					m_VBController.SetStreamSources(iStartSprite*4);
					m_VBController.Render(0,0,iRendCnt*4,iRendCnt*2); 
			} }
			++iSpriteIndex; ++pTexIndex; } }
	else {
		uint32 iSpriteIndex = 0;															// Walk m_pUsedBitArray and render as you go...
...
void CD3DSpriteArray::Render(LTMatrix& WorldTransform, CRenderStyle* pRenderStyle, vector<LPDIRECT3DBASETEXTURE8>& TextureList)
{
	...
	g_RenderStateMgr.SetRenderState(D3DRS_SOFTWAREVERTEXPROCESSING, m_bSWVertProcessing);
	...
	for (uint32 iRenderPass = 0; iRenderPass < pD3DRenderStyle->GetRenderPassCount(); ++iRenderPass) 
	{
		...
		{																		// Using a custom vertex shader...
			...
			if (!m_bSWVertProcessing && pD3DRenderStyle->CreatedForSWVertProcessing()) {
				...
				g_RenderStateMgr.SetRenderState(D3DRS_SOFTWAREVERTEXPROCESSING,true); }
