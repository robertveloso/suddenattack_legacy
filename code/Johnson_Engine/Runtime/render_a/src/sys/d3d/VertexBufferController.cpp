...
void VertexBufferController::SetStreamSources(uint32 iVertOffset)
{
	// Setup the Streams...
	for (uint32 i = 0; i < VERTEXBUFFER_STREAMCOUNT; ++i) {
		if (m_pVB_VertStream[i]) {
			if (FAILED(PD3DDEVICE->SetStreamSource(i,m_pVB_VertStream[i],m_VertexSize[i])))	{ assert(0); return; } } }
	if (m_pVB_Index) { 
		if (FAILED(PD3DDEVICE->SetIndices(m_pVB_Index,iVertOffset)))							{ assert(0); return; } }
}

// One stop shop - transform/light/render all in one call...
void VertexBufferController::Render(uint32 MinVertIndexed,uint32 StartIndex,uint32 VertCount,uint32 TriCount)
{
	// Render the sucker...
	if (FAILED(PD3DDEVICE->DrawIndexedPrimitive(D3DPT_TRIANGLELIST,MinVertIndexed,VertCount,StartIndex,TriCount)))	{ return; } //assert(0); return; }
}
...
bool VertexBufferController::Lock(VB_TYPE Type,bool bDiscardContents,bool bNoOverWrite,uint32 iOffsetToLock_Bytes,uint32 iSizeToLock_Bytes)
{
	...
	case eVERTSTREAM0 :
		...
		if (FAILED(m_pVB_VertStream[0]->Lock(iOffsetToLock_Bytes,iSizeToLock_Bytes,(uint8**)&m_pVert_Data[0],uFlags)))	{ assert(0); return false; } break;
	case eVERTSTREAM1 :
		if (m_pVert_Data[1]) return true;
		if (FAILED(m_pVB_VertStream[1]->Lock(iOffsetToLock_Bytes,iSizeToLock_Bytes,(uint8**)&m_pVert_Data[1],uFlags)))	{ assert(0); return false; } break;
	case eVERTSTREAM2 :
		if (m_pVert_Data[2]) return true;
		if (FAILED(m_pVB_VertStream[2]->Lock(iOffsetToLock_Bytes,iSizeToLock_Bytes,(uint8**)&m_pVert_Data[2],uFlags)))	{ assert(0); return false; } break;
	case eVERTSTREAM3 :
		if (m_pVert_Data[3]) return true;
		if (FAILED(m_pVB_VertStream[3]->Lock(iOffsetToLock_Bytes,iSizeToLock_Bytes,(uint8**)&m_pVert_Data[3],uFlags)))	{ assert(0); return false; } break;
	case eINDEX :
		if (m_pIndex_Data) return true;
		if (FAILED(m_pVB_Index->Lock(iOffsetToLock_Bytes,iSizeToLock_Bytes,(uint8**)&m_pIndex_Data,uFlags)))			{ assert(0); return false; } break;
