...
bool CTextureManager::UploadRTexture(TextureData* pSrcTexture, uint32 iSrcLvl, RTexture* pDstTexture, uint32 iDstLvl)
{
	...
	{
		// Cube map texture...
		LPDIRECT3DCUBETEXTURE8 pD3DDstTexture = pDstTexture->m_pD3DCubeTexture;
		...
		{
			LPDIRECT3DSURFACE8 pDstSurface = NULL; 
	...
	{
		// Normal texture..
		LPDIRECT3DTEXTURE8 pD3DDstTexture = pDstTexture->m_pD3DTexture;
...
bool CTextureManager::SelectTextureFormats()
{
	...
	{ 
		g_pStruct->ConsolePrint("[DX_INIT ERROR] FORMAT_32BIT texture format missing.");
		SetTextureFormatFromD3DFormat(CTextureManager::FORMAT_32BIT,D3DFMT_X1R5G5B5); 
		//return false; 
	}	// It's a sad thing, but not a failable offense...
	...
	{ 
		g_pStruct->ConsolePrint("[DX_INIT ERROR] FORMAT_FULLBRITE texture format missing.");
		SetTextureFormatFromD3DFormat(CTextureManager::FORMAT_FULLBRITE,D3DFMT_R5G6B5);
		//return false;
	}
	...
	{ 
		g_pStruct->ConsolePrint("[DX_INIT ERROR] FORMAT_4444 texture format missing.");
		SetTextureFormatFromD3DFormat(CTextureManager::FORMAT_4444,D3DFMT_R5G6B5); 
		//return false;
	}
	...
	{ 
		g_pStruct->ConsolePrint("[DX_INIT ERROR] FORMAT_NORMAL texture format missing.");
		//return false; 
	}
	...
	{ 
		g_pStruct->ConsolePrint("[DX_INIT ERROR] FORMAT_INTERFACE texture format missing.");
		SetTextureFormatFromD3DFormat(CTextureManager::FORMAT_INTERFACE,D3DFMT_R5G6B5);
		//return false;
	}
	...
	{ 
		g_pStruct->ConsolePrint("[DX_INIT ERROR] FORMAT_LIGHTMAP texture format missing.");
		SetTextureFormatFromD3DFormat(CTextureManager::FORMAT_LIGHTMAP,D3DFMT_R5G6B5);
		//return false;
	}
	...
	if (PDIRECT3D->CheckDeviceFormat(pAdapterInfo->iAdapterNum,pDeviceInfo->DeviceType,pDisplayMode->Format,NULL,D3DRTYPE_TEXTURE,D3DFMT_DXT1) == D3D_OK)
	{
		m_bSupportsDXT1 = true; 
	}
	if (PDIRECT3D->CheckDeviceFormat(pAdapterInfo->iAdapterNum,pDeviceInfo->DeviceType,pDisplayMode->Format,NULL,D3DRTYPE_TEXTURE,D3DFMT_DXT3) == D3D_OK)
	{
		m_bSupportsDXT3 = true;
	}
	if (PDIRECT3D->CheckDeviceFormat(pAdapterInfo->iAdapterNum,pDeviceInfo->DeviceType,pDisplayMode->Format,NULL,D3DRTYPE_TEXTURE,D3DFMT_DXT5) == D3D_OK)
	{
		m_bSupportsDXT5 = true; 
	}
...
static void d3d_InternalSetCurrentTextureDirect(LPDIRECT3DBASETEXTURE8 pTexture, uint32 nStage, float fMipMapBias, uint32 nAlphaRef)
{
	...
	if(pTexture)
	{
		...
		{
			PD3DDEVICE->SetTextureStageState(nStage, D3DTSS_MIPMAPLODBIAS, *((DWORD*)&fMipMapBias));
...
bool d3d_SetTextureDirect(LPDIRECT3DBASETEXTURE8 pTexture, uint32 nStage)
{
...
bool d3d_SetTextureDirect(LPDIRECT3DBASETEXTURE8 pTexture, uint32 nStage, CTrackedTextureMem& TextureMem, ERendererFrameStats eMemType)
{
