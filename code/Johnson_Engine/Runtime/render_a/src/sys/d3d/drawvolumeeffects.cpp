...
static IDirect3DVertexBuffer8* g_pDynamicParticleVB = NULL;

// the index buffer for dynamic particle quads
static IDirect3DIndexBuffer8* g_pDynamicParticleQuadIB = NULL;
...
static void DrawDynamicParticles(const ViewParams& Params, LTVolumeEffect* pEffect )
{
	...
	if( renderQuads && !g_pDynamicParticleQuadIB )
	{
		...
		if( FAILED( g_pDynamicParticleQuadIB->Lock( 0, 0, &ibData, 0 ) ) )
		{
	...
	StageStateSet tssWrapModeU0( 0, D3DTSS_ADDRESSU, D3DTADDRESS_CLAMP );
	StageStateSet tssWrapModeV0( 0, D3DTSS_ADDRESSV, D3DTADDRESS_CLAMP );
	...
	while( !done )
	{
		...
		if( (vbRemainingVerts < 100) || !g_nDynamicParticlesDrawn )
		{
			...
			if( FAILED( g_pDynamicParticleVB->Lock( 0, 0, &vbData, D3DLOCK_DISCARD ) ) )
				return;
		...
		else
		{
			...
			if( FAILED( g_pDynamicParticleVB->Lock( g_nDynamicParticleVBIndex*DPVertSize, vbRemainingVerts*DPVertSize, &vbData, D3DLOCK_NOOVERWRITE ) ) )
				return;
		}
		...
		{
			first = false;
			if( FAILED( PD3DDEVICE->SetStreamSource( 0, g_pDynamicParticleVB, DPVertSize ) ) )
				return;
			if( FAILED( PD3DDEVICE->SetVertexShader( DYNAMICPARTICLE_FVF ) ) )
				return;
			if( renderQuads )
				if( FAILED( PD3DDEVICE->SetIndices( g_pDynamicParticleQuadIB, 0 ) ) )
					return;
		...
		if( !renderQuads )
		{
			...
			// draw the triangles
			if( numTris && FAILED( PD3DDEVICE->DrawPrimitive( D3DPT_TRIANGLELIST, g_nDynamicParticleVBIndex, numTris ) ) )
				return;
		...
		else
		{
			...
			if( numTris && FAILED( PD3DDEVICE->DrawIndexedPrimitive( D3DPT_TRIANGLELIST, g_nDynamicParticleVBIndex, curFilled, (g_nDynamicParticleVBIndex/4)*6, numTris ) ) )
				return;
