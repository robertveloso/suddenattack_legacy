...
void WaveFile::Close()
{
	...
	if ( m_hAcmStream )
	{
		if( m_acmStreamHeader.pbDst )	delete [] m_acmStreamHeader.pbDst;
...
bool C3DListener::Init( HRESULT& hResult, LPDIRECTSOUNDBUFFER pDSPrimaryBuffer )
{
	...
	m_pDS3DListener->SetDopplerFactor( m_fDopplerSetting, DS3D_IMMEDIATE );
...
void C3DListener::SetPosition( LTVector& pos )
{
	...
	m_pDS3DListener->SetPosition( pos.x, pos.y, pos.z, DS3D_IMMEDIATE );
}

void C3DListener::SetVelocity( LTVector& vel )
{
	I3DObject::SetVelocity( vel );
	m_pDS3DListener->SetVelocity( vel.x, vel.y, vel.z, DS3D_IMMEDIATE );
}

void C3DListener::SetOrientation( LTVector& up, LTVector& face )
{
	I3DObject::SetOrientation( up, face );
	m_pDS3DListener->SetOrientation( face.x, face.y, face.z, up.x, up.y, up.z, DS3D_IMMEDIATE );
}
				   ...
				   bool b3DBuffer, WAVEFORMATEX* pWaveFormat, LTSOUNDFILTERDATA* pFilterData )
{
	...
	if ( b3DBuffer )
		m_dsbDesc.guid3DAlgorithm = DS3DALG_HRTF_LIGHT;//DS3DALG_HRTF_FULL
	else
...
*/
	HRESULT hr = m_pDS3DBuffer->SetPosition( pos.x, pos.y, pos.z, DS3D_IMMEDIATE );
}
...
{
	I3DObject::SetOrientation( up, face );
	m_pDS3DBuffer->SetConeOrientation( face.x, face.y, face.z, DS3D_IMMEDIATE );
}
...
uint32 CDx8SoundSys::Thread_Func()
{
	...
	// Note : Wait 10 ms between updates
	const uint32 k_nUpdateDelayMS = 10;
	...
	while (WaitForMultipleObjects(3, aEvents, FALSE, INFINITE) != WAIT_OBJECT_0)
	{
		...
		// Add up the tick counts.
		{
			...
			m_nThreadedTickCounts += nTheadedTickCounts;
		}

		// Wait for a period of time and see if we are told to exit.
		if( m_cEvent_Shutdown.Block(k_nUpdateDelayMS))
			break;
	}
...
bool CDx8SoundSys::SupportsDS3DHardware()
{
	...
	{
		if( m_dscaps.dwMaxHw3DAllBuffers > 0 )
		{
			m_dwMinHardwareBuffers = m_dscaps.dwMaxHw3DAllBuffers;
			return true;
		}
		return false;
	}
...
bool CDx8SoundSys::Init( )
{
	...
	HWND hWnd = FindWindow("__GH_Sudden_Attack__", NULL);
	if(hWnd)
...
void CDx8SoundSys::Term( )
{
	...
	if (m_cThread_Handle)
	{
		if( WaitForSingleObject(m_cThread_Handle, 1000) == WAIT_TIMEOUT )
		{
	...
	if( m_hAcmMP3DriverId ) acmDriverRemove( m_hAcmMP3DriverId, 0 );
	if( m_hAcmADPCMDriverId ) acmDriverRemove( m_hAcmADPCMDriverId, 0 );
	if( m_hAcmPCMDriverId ) acmDriverRemove( m_hAcmPCMDriverId, 0 );	
...
void SelectDrivers( const SDriverEnumInfo &sEnumResults, CDx8SoundSys *pSoundSys )
{
	...
	//[MURSUM]======================================================
	//Free Memory
	if( false == aADPCMList.empty() ) aADPCMList.clear();
	if( false == aMP3List.empty() ) aMP3List.clear();
	if( false == aPCMList.empty() ) aPCMList.clear();
	//==============================================================
}
...
S32	CDx8SoundSys::Startup( void )
{
		...
		m_hAcmPCMDriverId == NULL )
	{
		...
		//[MURSUM]======================================================
		//Free Memory
		while( false != sEnumResults.m_aDrivers.empty() )
		{
			sEnumResults.m_aDrivers.pop_back();
		}
		sEnumResults.m_aDrivers.clear();

		while( false != sEnumResults.m_aSupportFlags.empty() )
		{
			sEnumResults.m_aSupportFlags.pop_back();
			
		}
		sEnumResults.m_aSupportFlags.clear();
		//==============================================================
...
void CDx8SoundSys::SetListenerDoppler( LH3DPOBJECT hListener, float fDoppler )
{
	...
	p3DListener->m_pDS3DListener->SetDopplerFactor( fDoppler, DS3D_IMMEDIATE );
}
...
/*
	...
	m_hResult = p3DSample->m_pDS3DBuffer->SetMinDistance( fMin_dist, DS3D_IMMEDIATE );
	m_hResult = p3DSample->m_pDS3DBuffer->SetMaxDistance( fRelDist, DS3D_IMMEDIATE );
*/
	...
	// our linear curve.
  	m_hResult = p3DSample->m_pDS3DBuffer->SetMinDistance( fMax_dist, DS3D_IMMEDIATE );
	m_hResult = p3DSample->m_pDS3DBuffer->SetMaxDistance( fMax_dist, DS3D_IMMEDIATE );
}
...
void CDx8SoundSys::TermSample( LHSAMPLE hS )
{
	if( hS == NULL )
		return;
...
#define CENTER_PAN		((MAX_MASTER_PAN - MIN_MASTER_PAN)/2 + MIN_MASTER_PAN)
#define DEL_SIDE_PAN	(MAX_MASTER_PAN - CENTER_PAN)

#define DSBPAN_MIN		DSBPAN_LEFT
#define DSBPAN_MAX		DSBPAN_RIGHT
...
#define LOG_PAN_ATTENUATION		( ATTENUATION_FACTOR * ( float )sqrt( CENTER_PAN ))
#define LOG_PAN_INVATTENUATION	( 1.0f / LOG_PAN_ATTENUATION )
...
S32 ConvertLogPanToLinPan( long logPan )
{
	...
	if ( logPan < 0 )
		linPan = CENTER_PAN - (S32) fDistFromCenter;
	else
		linPan = CENTER_PAN + (S32) fDistFromCenter;
