...
CSoundMgr::CSoundMgr()
{
	...
	// NOVICE
	memset(m_DeleteSoundList, 0, SOUNDMGR_MAXSOUNDINSTANCES * sizeof(CSoundInstance*));
	m_dwNumDeleteSounds = 0;
...
LTRESULT CSoundMgr::SetFrequency(HLTSOUND& hSound, unsigned long dwFrequency)
{
	LPDIRECTSOUND8 lpDirectSound;
	LPDIRECTSOUNDBUFFER lpDirectSoundBuffer;
    ...
    g_pSoundSys->GetDirectSoundInfo(pSoundInstance->GetSample(), (void**)&lpDirectSound, (void**)&lpDirectSoundBuffer);
	
	if( !lpDirectSoundBuffer )
		return LT_ERROR;
	lpDirectSoundBuffer->SetFrequency((DWORD)dwFrequency);
...
LTRESULT CSoundMgr::Update()
{
    ...
    if (!m_dwNumSoundInstances)
	{
		//OutputDebugString( "@@ Update was returned by (!m_dwNumSoundInstances) Condition\n" );
        return LT_OK;
	}
	...
	wsprintf( szTemp, "@@@ SoundMgr::Update Called %d times ( %d )!!!\n", nCnt++, timeGetTime() - nLastTime );
	nLastTime = timeGetTime();
	OutputDebugString( szTemp );
	*/

	if ( m_dwNumDeleteSounds > 0 )
	{
		for ( uint32 nCnt = 0; nCnt < m_dwNumDeleteSounds; nCnt++ )
		{
			if ( m_dwNumDeleteSounds > SOUNDMGR_MAXSOUNDINSTANCES )
			{
				dsi_ConsolePrint( "m_dwNumDeleteSounds was Over SOUNDMGR_MAXSOUNDINSTANCES !!!" );
				m_dwNumDeleteSounds = 0;
				break;
			}
			
			if ( NULL != m_DeleteSoundList[nCnt] )
			{
				KillSound( (HLTSOUND&)m_DeleteSoundList[nCnt] );
			}
		}
    ...
    for (dwIndex = 0; dwIndex < m_dwNumSoundInstances; dwIndex++)
    {
		...
		{
			if (pSoundInstance->AcquireSample() == LT_OK) continue;
		}
...
LTRESULT CSoundMgr::PlaySound(PlaySoundInfo &playSoundInfo, FileIdentifier &fileIdent, uint32 dwOffsetTime)
{
    ...
    if (nError == LT_OK)
    {
        ...
        if (m_dwNumSoundInstances < SOUNDMGR_MAXSOUNDINSTANCES)
        {
			...
			//OutputDebugString( "@@ Increase m_dwNumSoundInstances\n" );
        }
        else
		{
			nError = LT_ERROR;
...
LTRESULT CSoundMgr::PauseSounds()
{
    ...
    for (dwIndex = 0; dwIndex < m_dwNumSoundInstances; dwIndex++)
    {
		...
		if( pSoundInstance->GetSoundClass() == 5 /*BGM_SOUND_CLASSES*/ )
			continue;
...
LTRESULT CSoundMgr::PlaySound(PlaySoundInfo *pPlaySoundInfo, HLTSOUND &hResult)
{
	...
	//[SPIKE]
	(pPlaySoundInfo->m_nSoundVolumeClass == 5) ? SetBGMEnable(LTTRUE) : SetBGMEnable(LTFALSE);
	if(IsBGMEnabled())
	{
		SetEnable(LTTRUE);
	}
    ...
    if (playResult == LT_OK)
	{
		...
		//////////////////////////////////////////////////////////////////////////
		// NOVICE
		if ( m_nMaxSWSamples > 0 && m_dwNumSoundInstances > m_nMaxSWSamples )
		{
			CSoundInstance* pSoundInstance = m_SoundInstanceList[m_dwNumSoundInstances - 1];
			
			if ( !(pSoundInstance->GetPlaySoundFlags() & PLAYSOUND_LOOP)
				&& !(pSoundInstance->GetPlaySoundFlags() & (PLAYSOUND_GETHANDLE | PLAYSOUND_CTRL_VOL | PLAYSOUND_CTRL_PITCH)) )
			{
				// 루프 사운드도 아니고, PlaySoundFX 도 아니어야 함!!
				m_DeleteSoundList[m_dwNumDeleteSounds] = pSoundInstance;
				m_dwNumDeleteSounds++;
			}
		}
		
		if ( m_nMax3DSamples > 0 && m_dwNumSoundInstances > m_nMax3DSamples )
		{
			CSoundInstance* pSoundInstance = m_SoundInstanceList[m_dwNumSoundInstances - 1];
...
// 
//----------------------------------------------------------------------------------------------
LTRESULT CSoundMgr::KillSound(HLTSOUND& hSound,bool bTerm)
{
    CSoundInstance *pSoundInstance;
	...
	{
#ifndef _FINAL
		dsi_ConsolePrint( "CSoundMgr::KillSound Failed !!! (%X)", hSound );
#endif //#ifndef _FINAL
        return LT_INVALIDPARAMS;
	}

	CSoundBuffer* pBuffer	= pSoundInstance->GetSoundBuffer();
	LHSAMPLE hSample		= pSoundInstance->GetSample();

	pSoundInstance->SetVolume(0);
	RemoveInstance(*pSoundInstance);
	...
	{		
		...
		g_pSoundSys->TermSample( hSample );
	}

	//[SPIKE]
	if(hSound) hSound = LTNULL;
    return LT_OK;
}
...
LTRESULT CSoundMgr::KillSoundLoop(HLTSOUND hSound)
{
	...
	{
#ifndef _FINAL
		dsi_ConsolePrint( "CSoundMgr::KillSoundLoop Failed !!! (%X)", hSound );
#endif //#ifndef _FINAL
        return LT_INVALIDPARAMS;
	}
...
void CSoundMgr::SetReverbFilter( bool bUse )
{
	m_bUseReverbFilter = bUse;
}
