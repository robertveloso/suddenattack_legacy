...
static void _TagWorldBspTextures(const WorldBsp *bsp)
{
    uint32 i;
    ...
    for (i=0; i < bsp->m_nSurfaces; i++)
    {
        ...
        TagTexture(pSurface->GetSharedTexture());
    }
}
...
void CClientMgr::TagUsedTextures() {
    ...
    {
        pTexture = (SharedTexture*)pCur->m_pData;
        TagTexture(pTexture->GetLinkedTexture(eLinkedTex_Detail));
		TagTexture(pTexture->GetLinkedTexture(eLinkedTex_EnvMap));
		TagTexture(pTexture->GetLinkedTexture(eLinkedTex_BumpMap));
    }
    ...
    if (m_pCurShell) {
        for (i=0; i < world_bsp_client->NumWorldModels(); i++)
        {
            ...
            _TagWorldBspTextures(pWorldData->OriginalBSP());
