...
void CConHistory::Remove( int iIndex )
{
	...
	for ( int iCurEntry = iIndex; iCurEntry != m_iListEnd; iCurEntry = (iCurEntry + 1) % m_iListSize )
	{
		...
		// Clean cup, move down..
		LTStrCpy( m_pLines[iCurEntry].m_Text, m_pLines[iNextEntry].m_Text, MAX_CONSOLE_TEXTLEN );
	}
	// Adjust the end
...
void CConsole::DrawTextLine(const char *pText, const LTRect *pRect, COLORREF textColor)
{
	...
	if (!m_Font) {
		if (g_IFontManager && g_ITexInterface) {
			assert(!m_Tex); m_Tex = NULL; 
			if (g_CV_Console_FontTexFile) g_ITexInterface->CreateTextureFromName(m_Tex,g_CV_Console_FontTexFile);
			else g_ITexInterface->CreateTextureFromName(m_Tex,"console\\console_font.dtx");
			if (m_Tex) {
	...
	if (m_Font) {
		m_Font->SetDefColor(textColor);
 		char* pCh = const_cast<char*>(pText); char szBuff[256]; uint32 iCh = 0; char* pB = szBuff; uint32 iW = 0;
 		while (*pCh && (iW + iCharWidth <= (uint32)(pRect->right - pRect->left)) && iCh < 255) { *pB = *pCh; ++pCh; ++pB; ++iCh; iW += iCharWidth; } *pB = NULL;
...
void CConsole::PrintString(CONCOLOR theColor, int filterLevel, const char *pMsg)
{
	...
	if ( m_TextLines.GetSize() > 0 )
	{
		...
		do
		{
			...
			LTStrCpy( pLine->m_Text, pMsg, iLength );
...
void CConsole::FlushErrorLog()
{
	if( m_FlushErrorLogFn ) m_FlushErrorLogFn();
}
