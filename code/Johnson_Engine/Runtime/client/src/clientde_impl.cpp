...
#ifndef _FINAL
//[MURSUM]
extern int32 g_CV_OutputC2S;
void ci_OutputC2S( ILTMessage_Read *pMsg );
#endif
...
class CLTClient : public ILTClient {
  public:
	...
	// ±¤½Ä
	virtual LTRESULT GetMapExtentsMinMax(LTVector& vMin, LTVector& vMax);
...
{
	CBaseDriver *pDriver = g_pClientMgr->m_ClientNetMgr.GetMainDriver();
...
LTRESULT CLTClient::SendToServer(ILTMessage_Read *pMsg, uint32 flags)
{
	...
	// Send it
	bool bResult = g_pClientMgr->m_ClientNetMgr.SendPacket(CPacket_Read(cNewPacket), g_pClientMgr->m_pCurShell->m_HostID, flags);
...
LTRESULT CLTClient::SendTo(ILTMessage_Read *pMsg, const char *sAddr, uint16 port)
{
	...
	if (isdigit(sAddr[0]) && g_pClientMgr->m_pCurShell && g_pClientMgr->m_pCurShell->m_HostID)
	{
		...
		if ((nHostAddr[0] == nDestAddr[0]) &&
			...
			(nHostPort == port))
		{
			if (g_pClientMgr->m_ClientNetMgr.SendPacket(pPacketMsg->GetPacket(), g_pClientMgr->m_pCurShell->m_HostID))
				return LT_OK;
		}
	}

	for (uint32 i=0; i < g_pClientMgr->m_ClientNetMgr.m_Drivers; i++)
	{
		CBaseDriver *pDriver = g_pClientMgr->m_ClientNetMgr.m_Drivers[i];
...
LTRESULT CLTClient::StartPing(const char *pAddr, uint16 nPort, uint32 *pPingID)
{
	...
	if (g_pClientMgr->m_ClientNetMgr.m_Drivers.GetSize() == 0)
...
bool CLTClient::AddVertexShader(const char *pFileName, int VertexShaderID, const uint32 *pDeclaration, uint32 DeclarationSize)
{
	bool bSuccess = false;
...
bool CLTClient::AddPixelShader(const char *pFileName, int PixelShaderID)
{
	bool bSuccess = false;
	...
	if (OpenFile(pFileName, &pStream) == LT_OK)
	{
		bSuccess = LTPixelShaderMgr::GetSingleton().AddPixelShader(pStream, pFileName, PixelShaderID);
	}
...
{
	return g_pClientMgr->GetFrameTimeClient();
}
...
// ±¤½Ä
LTRESULT CLTClient::GetMapExtentsMinMax(LTVector& vMin, LTVector& vMax)
{
	vMin	= world_bsp_shared->ExtentsMin();
	vMax	= world_bsp_shared->ExtentsMax();
	return LT_OK;
}
...
//[MURSUM] Cone
LTRESULT ci_StartCheckConeType(const char* lpszIP1, uint16 nPort1,
							   const char* lpszIP2, uint16 nPort2,
							   const char* lpszID, uint8 nIDLen)
{
	return g_pClientMgr->StartCheckConeType(lpszIP1, nPort1, lpszIP2, nPort2, lpszID, nIDLen);
}

uint16 ci_GetHostPort()
{
	return g_pClientMgr->GetHostPort();
}

LTRESULT ci_PlayAmbientSound()
{
	return g_pClientMgr->PlayAmbientSound();
}

void ci_MakeScreenShot( const char* lpszFileName )
{
	g_pClientMgr->MakeScreenShot( lpszFileName );
}

void ci_SetDumpDir( const char* lpszDir )
{
	g_pClientMgr->SetDumpDir( lpszDir );
}

IDirect3DDevice8* ci_GetD3DDevice()
{
	return g_pClientMgr->GetD3DDevice();
}

void ci_CheckFireGuard()
{
	g_pClientMgr->CheckFireGuard();
}

long ci_InitFireGuard( const char* lpszGameName, const char* lpszUserID )
{
	return g_pClientMgr->InitFireGuard( lpszGameName, lpszUserID );
}

long ci_StartFireGuard()
{
	return g_pClientMgr->StartFireGuard();
}

void ci_ShowFireGuardErr( long lResult, bool bStartErr )
{
	g_pClientMgr->ShowFireGuardErr( lResult, bStartErr );
}

LTBOOL ci_CheckValidObject( HOBJECT hObject, int32 nType )
{
	LTObject *pObj = (LTObject*)hObject;

	if (!pObj || nType >= NUM_OBJECTTYPES ) return 0;
	
	LTLink *pListHead = &g_pClientMgr->m_ObjectMgr.m_ObjectLists[nType].m_Head;
	for (LTLink *pCur = pListHead->m_pNext; pCur != pListHead; pCur = pCur->m_pNext)
	{
		if( pCur->m_pData == pObj ) return 1;
	}

	return 0;
}

LTRESULT ci_StartDummyHost( const char* lpszWorldName )
{
	return g_pClientMgr->StartDummyHost( lpszWorldName );
}

uint8 ci_GetNetState()
{
	return g_pClientMgr->GetNetState();
}

bool ci_PreLoadFileList( const char* lpszName, bool bDir )
{
	return g_pClientMgr->PreLoadFileList( lpszName, bDir );
}

void ci_StopCheckConeType()
{
	g_pClientMgr->StopCheckConeType();
}

void ci_ToggleFullScreen()
{
	if( g_pClientMgr ) g_pClientMgr->ToggleFullScreen();
}

int32 ci_GetLoadingState()
{
	return g_pClientMgr->GetLoadingState();
}

const char* ci_GetCpCookie()
{
	return g_pClientMgr->GetCpCookie();
}

void ci_SendDummyPacket( const char* lpszIP, uint16 nPort )
{
	g_pClientMgr->SendDummyPacket( lpszIP, nPort );
}

void ci_SetUserNickName( const char* lpszNickName )
{
	g_pClientMgr->SetUserNickName( lpszNickName );
}

void ci_SendAddedFiles()
{
	g_pClientMgr->SendAddedFiles();
}
...
void ci_Disconnect()
{
	...
	g_pClientMgr->m_bConnected = true;
}
...
LTRESULT ci_InitNetworking(const char *sDriver, uint32 dwFlags, bool bTermShell)
{
	...
	if( g_pClientMgr->m_pCurShell )
	{
		...
		{
			g_pClientMgr->m_pCurShell->TermNetDriver();
		}
	}

	// Make sure no drivers are loaded.
	g_pClientMgr->m_ClientNetMgr.InitDrivers();
	return(LT_OK);
}
...
{
	return g_pClientMgr->m_ClientNetMgr.GetServiceList(pListHead);
}
...
LTRESULT ci_FreeServiceList(NetService *pListHead)
{
	...
	return g_pClientMgr->m_ClientNetMgr.FreeServiceList(pListHead);
}
...
LTRESULT ci_SelectService(HNETSERVICE hNetService)
{
	...
	return g_pClientMgr->m_ClientNetMgr.SelectService(hNetService);
}


LTRESULT ci_GetSessionList(NetSession *&pListHead, const char *pInfo)
{
	return g_pClientMgr->m_ClientNetMgr.GetSessionList(pListHead, pInfo);
}


LTRESULT ci_FreeSessionList(NetSession *pListHead)
{
	return g_pClientMgr->m_ClientNetMgr.FreeSessionList(pListHead);
}

LTRESULT ci_AddInternetDriver()
{
	// Add the internet driver
	if (g_pClientMgr->m_ClientNetMgr.GetDriver("internet"))
		return LT_ALREADYEXISTS;

	if (g_pClientMgr->m_ClientNetMgr.AddDriver("internet"))
		return LT_OK;
...
LTRESULT ci_RemoveInternetDriver()
{
	...
	CBaseDriver* pDriver = g_pClientMgr->m_ClientNetMgr.GetDriver("internet");
	if (pDriver)
	{
		g_pClientMgr->m_ClientNetMgr.RemoveDriver(pDriver);
		return LT_OK;
	}
...
LTRESULT ci_GetTcpIpAddress(char* sAddress, uint32 dwBufferSize, uint16 &hostPort)
{
	...
	return g_pClientMgr->m_ClientNetMgr.GetLocalIpAddress(sAddress, dwBufferSize, hostPort);
}
...
LTRESULT ci_IsLobbyLaunched(const char* sDriver)
{
	...
	CBaseDriver* pDriver = g_pClientMgr->m_ClientNetMgr.GetDriver(sDriver);
	if (!pDriver)
	{
		pDriver = g_pClientMgr->m_ClientNetMgr.AddDriver(sDriver);
		if (!pDriver)
...
// ±¤½Ä
bool ci_IntersectSegmentClient(IntersectQuery *pQuery, ClientIntersectInfo *pInfo)
{
	return world_bsp_client->IntersectSegmentClient(pQuery, pInfo);
}
...
void CLTClient::InitFunctionPointers() 
{
	...
	// ±¤½Ä
	IntersectSegmentClient = ci_IntersectSegmentClient;
	...
	GetVersionInfoExt = GetVersionInfoExt;
	GetPerformanceInfo = GetPerformanceInfo;

	SetConsoleEnable = ci_SetConsoleEnable;

	//[MURSUM] Cone
	StartCheckConeType	= ci_StartCheckConeType;
	StopCheckConeType	= ci_StopCheckConeType;
	GetHostPort			= ci_GetHostPort;
	PlayAmbientSound	= ci_PlayAmbientSound;
	SetDumpDir			= ci_SetDumpDir;
	MakeScreenShot		= ci_MakeScreenShot;
	GetD3DDevice		= ci_GetD3DDevice;
	CheckFireGuard		= ci_CheckFireGuard;
	InitFireGuard		= ci_InitFireGuard;
	StartFireGuard		= ci_StartFireGuard;
	ShowFireGuardErr	= ci_ShowFireGuardErr;
	CheckValidObject	= ci_CheckValidObject;
	GetNetState			= ci_GetNetState;
	StartDummyHost		= ci_StartDummyHost;
	PreLoadFileList		= ci_PreLoadFileList;
	GetLoadingState		= ci_GetLoadingState;
	SendDummyPacket		= ci_SendDummyPacket;
	GetCpCookie			= ci_GetCpCookie;
	SetUserNickName		= ci_SetUserNickName;
	SendAddedFiles		= ci_SendAddedFiles;
	ToggleFullScreen	= ci_ToggleFullScreen;
...
void ci_OutputC2S( ILTMessage_Read *pMsg )
{
	static uint8	nIgnoreID[] 
		= { 0 };
	static char		aMsgBuff[256]	= {0,};
	static int32	nBufferSize = sizeof(aMsgBuff);
	uint8			nCommand = pMsg->Readuint8();

	for( int32 i=0 ; i<sizeof(nIgnoreID)/sizeof(uint8) ; ++i )
	{
		if( nIgnoreID[i] == nCommand ) return;
	}
	...
	{
		LTSNPrintF(aMsgBuff, nBufferSize, "P_UPDATE" );

		uint16 nChangeID = pMsg->Readuint16();
		
		if( nChangeID & 0x0001 )
			LTStrCat(aMsgBuff, " CAMERAOFFSET", nBufferSize);

		if( nChangeID & 0x0020 )
			LTStrCat(aMsgBuff, " PLAYERROT", nBufferSize);
		
		if( nChangeID & 0x0040 ) 
			LTStrCat(aMsgBuff, " ACCURATEPLAYERROT", nBufferSize);

		if( nChangeID & 0x0080 )
			LTStrCat(aMsgBuff, " FULLPLAYERROT", nBufferSize);

		break;
	}

	case 103 : // MID_PLAYER_INFOCHANGE
	{
		LTSNPrintF(aMsgBuff, nBufferSize, "P_INFOCHANGE" );
		
		uint8 nChangeID = pMsg->Readuint8();

		switch (nChangeID)
		{
			case 1 :
				LTStrCat(aMsgBuff, " AMMO", nBufferSize);
				break;
			case 2 :
				LTStrCat(aMsgBuff, " HEALTH", nBufferSize);
				break;
			case 3 :
				LTStrCat(aMsgBuff, " ARMOR", nBufferSize);
				break;
			case 4 :
				LTStrCat(aMsgBuff, " DEFAULTWEAPON", nBufferSize);
				break;
			case 5 :
				LTStrCat(aMsgBuff, " WEAPON_PICKUP", nBufferSize);
				break;
			case 6 :
				LTStrCat(aMsgBuff, " AIRLEVEL", nBufferSize);
				break;
			case 7 :
				LTStrCat(aMsgBuff, " ROCKETLOCK", nBufferSize);
				break;
			case 8 :
				LTStrCat(aMsgBuff, " OUTOFAMMO", nBufferSize);
				break;
			case 9 :
				LTStrCat(aMsgBuff, " OBJECTIVE", nBufferSize);
				break;
			case 10 :
				LTStrCat(aMsgBuff, " MOD_PICKUP", nBufferSize);
				break;
			case 11 :
				LTStrCat(aMsgBuff, " FADE_SCREEN", nBufferSize);
				break;
			case 12 :
				LTStrCat(aMsgBuff, " RESET_INVENTORY", nBufferSize);
				break;
			case 13 :
				LTStrCat(aMsgBuff, " MISSION_TEXT", nBufferSize);
				break;
			case 14	:
				LTStrCat(aMsgBuff, " MISSION_FAILED", nBufferSize);
				break;
			case 15 :
				LTStrCat(aMsgBuff, " MAX_HEALTH", nBufferSize);
				break;
			case 16 :
				LTStrCat(aMsgBuff, " MAX_ARMOR", nBufferSize);
				break;
			case 17 :
				LTStrCat(aMsgBuff, " WEAPON_OBTAIN", nBufferSize);
				break;
			case 18 :
				LTStrCat(aMsgBuff, " OPTION", nBufferSize);
				break;
			case 19 :
				LTStrCat(aMsgBuff, " PARAMETER", nBufferSize);
				break;
			case 20:
				LTStrCat(aMsgBuff, " KEY", nBufferSize);
				break;
			case 21:
				LTStrCat(aMsgBuff, " SKILLS", nBufferSize);
				break;	
			case 22:
				LTStrCat(aMsgBuff, " REGION_CHANGE", nBufferSize);
				break;	
			case 23:
				LTStrCat(aMsgBuff, " MAX_ENERGY", nBufferSize);
				break;	
			case 24:
				LTStrCat(aMsgBuff, " ENERGY", nBufferSize);
				break;	
			case 25:
				LTStrCat(aMsgBuff, " PSETS", nBufferSize);
				break;	
			case 26:
				LTStrCat(aMsgBuff, " PERFORMANCE_RATING", nBufferSize);
				break;	
			case 27:
				LTStrCat(aMsgBuff, " BUILD_POINTS", nBufferSize);
				break;	
			default :
				LTStrCat(aMsgBuff, " UNKNOWN", nBufferSize);
				break;
		}
		break;
	}

	case 108 : // MID_PLAYER_ORIENTATION
		LTSNPrintF(aMsgBuff, nBufferSize, "P_ORIENTATION" );
		break;

	case 117 : // MID_PLAYER_DAMAGE
		LTSNPrintF(aMsgBuff, nBufferSize, "P_DAMAGE" );
		break;

	case 119 : // MID_PLAYER_CLIENTMSG
	{
		LTSNPrintF(aMsgBuff, nBufferSize, "P_CLIENTMSG" );

		uint16 nCP = pMsg->Readuint8();

		switch(nCP)
		{
		case 1:
			LTStrCat(aMsgBuff, " MOTION_STATUS", nBufferSize);break; // CP_MOTION_STATUS
		case 2:
			LTStrCat(aMsgBuff, " DAMAGE", nBufferSize);break; // CP_DAMAGE
		case 3:
			LTStrCat(aMsgBuff, " WEAPON_STATUS", nBufferSize);break; // CP_WEAPON_STATUS
		case 4:
			LTStrCat(aMsgBuff, " FLASHLIGHT", nBufferSize);break; // CP_FLASHLIGHT
		case 5:
			LTStrCat(aMsgBuff, " PLAYER_LEAN", nBufferSize);break; // CP_PLAYER_LEAN
		case 6:
			LTStrCat(aMsgBuff, " PHYSICSMODEL", nBufferSize);break; // CP_PHYSICSMODEL
		case 7:
			LTStrCat(aMsgBuff, " DAMAGE_VEHICLE_IMPACT", nBufferSize);break; // CP_DAMAGE_VEHICLE_IMPACT
		case 8:
			LTStrCat(aMsgBuff, " DEFLECT", nBufferSize);break; // CP_DEFLECT
		}

		break;
	}

	case 112 : // MID_PLAYER_RESPAWN
		LTSNPrintF(aMsgBuff, nBufferSize, "P_RESPAWN" );
		break;
	case 114: // MID_PLAYER_MULTIPLAYER_INIT
		LTSNPrintF(aMsgBuff, nBufferSize, "MULTIPLAYER_INIT" );
		break;
	case 130 : // MID_WEAPON_CHANGE
		LTSNPrintF(aMsgBuff, nBufferSize, "WPN_CHANGE" );
		break;
	case 131 : // MID_WEAPON_SOUND
		LTSNPrintF(aMsgBuff, nBufferSize, "WPN_SOUND" );
		break;
	case 132 : // MID_WEAPON_SOUND_LOOP
		LTSNPrintF(aMsgBuff, nBufferSize, "WPN_SOUND_LOOP" );
		break;
	case 133 : // MID_WEAPON_FIRE
		LTSNPrintF(aMsgBuff, nBufferSize, "WPN_FIRE" );
		break;
	case 134 : // MID_WEAPON_RELOAD
		LTSNPrintF(aMsgBuff, nBufferSize, "WPN_RELOAD" );
		break;
	case 138 : // MID_PROJECTILE
		LTSNPrintF(aMsgBuff, nBufferSize, "PROJECTILE" );
		break;
	case 139: // MID_CLIENT_LOADED
		LTSNPrintF(aMsgBuff, nBufferSize, "CLIENT_LOADED" );
		break;
	case 146 : // MID_PLAYER_TAUNT
		LTSNPrintF(aMsgBuff, nBufferSize, "P_TAUNT" );
		break;
	case 183: // MID_START_GAME
		LTSNPrintF(aMsgBuff, nBufferSize, "START_GAME" );
		break;
	case 185: // MID_START_LEVEL
		LTSNPrintF(aMsgBuff, nBufferSize, "START_LEVEL" );
		break;
	case 214 : //MID_PINGTIMES
		LTSNPrintF(aMsgBuff, nBufferSize, "PING" );
		break;
	case 231 : //MID_SFX_MESSAGE
		LTSNPrintF(aMsgBuff, nBufferSize, "SFX_MESSAGE" );
		break;
	case 240: // MID_DIFFICULTY
		LTSNPrintF(aMsgBuff, nBufferSize, "DIFFICULTY" );
		break;		
	case 245: // MID_HANDSHAKE
		LTSNPrintF(aMsgBuff, nBufferSize, "HANDSHAKE" );
		break;
	default:
		LTSNPrintF(aMsgBuff, nBufferSize, "UNKNOWN ID(%d)", nCommand );
		break;
	}

	dsi_ConsolePrint( "[ C2S] (%10d) %s |%d|", timeGetTime(), aMsgBuff, pMsg->Size() );
}
#endif