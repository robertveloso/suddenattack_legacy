...
void pd_OnObjectMove(CClientShell *pShell, LTObject *pObject, LTVector *pNewPos, LTVector *pNewVel, bool bNew, bool bTeleport)
{
	ClientData *pData;
	pData = &pObject->cd;
	if( pShell->GetClientObject() == pObject ) return;
	...
	//[MURSUM] 
	//캐릭터는 일정 거리 이상으로 멀어지면 텔레포트
	//벽에 찡기는 현상 줄이기 위해서..
	if( pObject->m_UserFlags&(1<<12) && //USRFLG_CHARACTER
		pNewPos->DistSqr( pData->m_LastUpdatePosServer ) > (80.0f*80.0f) )
	{
		bTeleport = true;
	}
...
void pd_Update(CClientShell *pShell)
{
	...
	if(g_bPrediction)
	{
		...
		while(pCur != &pShell->m_MovingObjects)
		{
			...
			//////////////////////////////
			//pClientData->m_fLastUpdatePosTime += fTimeOffset;
			//pClientData->m_LastUpdatePosServer = pObj->GetPos();
			//////////////////////////////
