...
#include ".././kernel/net/src/sys/win/udpdriver.h"
//const uint16 DEFAULT_LISTENPORT = 27888;		//modify by zhoujb 0814 

#ifdef USE_FIRE_GUARD
#define FG_SUCCESS	0L
CFireGuard* g_pFireGuard = LTNULL;
#endif
...
#pragma message ( "################## Network Monitoring Enabled ##################" )
#endif
//////////////////////////////////////////////////////////////////////////
...
SOCKET cm_BindToPort( sockaddr_in *pAddr, sockaddr_in *pFinalAddr )
{
	SOCKET			Sock;
	int				nStatus;
	int				nNameBufSize;
	unsigned long	lVal	= 1;	//Non-Blocking
	
	Sock = socket( AF_INET, SOCK_DGRAM, IPPROTO_UDP );
	if( Sock == SOCKET_ERROR )
	{
		dsi_ConsolePrint( "ERROR cm_BindToPort - socket()" );
		return INVALID_SOCKET;
	}

	nStatus = ioctlsocket( Sock, FIONBIO, &lVal );

	if( nStatus != 0 ) 
	{
		dsi_ConsolePrint( "ERROR cm_BindToPort - ioctlsocket() %d", nStatus );
		closesocket( Sock );
		return INVALID_SOCKET;
	}

	int bReuseAddr = 1;
	nStatus = setsockopt( Sock, SOL_SOCKET, SO_REUSEADDR, (char*)&bReuseAddr, sizeof(bReuseAddr) );
	if( nStatus != 0 ) 
	{
		dsi_ConsolePrint( "ERROR cm_BindToPort - setsockopt() %d", nStatus );
		closesocket( Sock );
		return INVALID_SOCKET;
	}

	nStatus = bind( Sock, (sockaddr*)pAddr, sizeof(*pAddr) );
	if( nStatus == 0 )
	{
		if(pFinalAddr)
		{
			nNameBufSize = sizeof(*pFinalAddr);
			getsockname( Sock, (sockaddr*)pFinalAddr, &nNameBufSize );
		}
	...
	{
		dsi_ConsolePrint( "ERROR cm_BindToPort - bind() %d", nStatus );
		closesocket( Sock );
		return INVALID_SOCKET;
	}
...
#ifdef USE_FIRE_GUARD
BOOL CALLBACK FireGuard_CallBack( DWORD dwFailCode, DWORD dwErrorCode, LPCTSTR message )
{
	char szBuffer[512];
	...
	case EINTERNAL:	
		...
		{	
		case HERR_OS_WIN95:
			//mesg = "Windows 95 는 지원하지 않습니다.";
			return LTTRUE;

		case HERR_OS_SAFE_MODE_BOOT:
			sprintf( szBuffer, "안전 모드에서는 게임을 실행할 수 없습니다.\r\n\r\n"
					 "정상적으로 재부팅 후 시도 해주시기 바랍니다.\r\n\r\n"
					 "에러코드 : 0x%08x", dwErrorCode );
			break;
		case HERR_OS_RESOURCE_NOT_RELEASED:
			sprintf( szBuffer, "실행에 필요한 자원이 제대로 반환되지 못했습니다.\r\n\r\n"
					"잠시 후 다시 실행시켜 주시기 바랍니다.\r\n\r\n"
					"", dwErrorCode );
			break;	
		case HERR_OS_REBOOT_NEEDED:
			sprintf( szBuffer, "윈도우 업데이트 때문에 재부팅이 필요합니다.\r\n\r\n"
					"재부팅 후 다시 실행 시켜주시기 바랍니다.\r\n\r\n"
					"", dwErrorCode );
			break;	
		default:
			sprintf( szBuffer, "해킹방지 시스템에서 알수없는 오류가 발생하였습니다.\r\n\r\n"
					"잠시 후 재실행 해주시고 같은 현상이 발생시 재부팅 후 게임을 실행시켜 주시기 바랍니다." );	
			break;
		}
		break;

	case ESPEEDHACK:
		sprintf( szBuffer, "스피드핵 프로그램이 탐지되었습니다.\r\n\r\n"
				"계속해서 해킹을 시도할 경우 운영정책에 따라\r\n\r\n"
				"계정이 삭제될 수 있으니 유의하시기 바랍니다." );
		break;	

	case EMEMORYHACK:
		sprintf( szBuffer, "게임 메모리 해킹이 탐지되었습니다.\r\n\r\n"
				"계속해서 해킹을 시도할 경우 운영정책에 따라\r\n\r\n"
				"계정이 삭제될 수 있으니 유의하시기 바랍니다." );
		break;

	case EGENERALHACK:
		sprintf( szBuffer, "디버거 프로그램이 탐지되었습니다.\r\n\r\n"
				"계속해서 해킹을 시도할 경우 운영정책에 따라\r\n\r\n"
				"계정이 삭제될 수 있으니 유의하시기 바랍니다." );
		break;

	case EMACRO:
		sprintf( szBuffer, "매크로 시도가 탐지되었습니다.\r\n\r\n"
				"계속해서 해킹을 시도할 경우 운영정책에 따라\r\n\r\n"
				"계정이 삭제될 수 있으니 유의하시기 바랍니다." );
		break;
		
	case ENETWORK:	
		sprintf( szBuffer, "네트워크 해킹 시도가 탐지되었습니다.\r\n\r\n"
				"계속해서 해킹을 시도할 경우 운영정책에 따라\r\n\r\n"
				"계정이 삭제될 수 있으니 유의하시기 바랍니다." );
		break;

	case EFIREGUARDHACK:
		sprintf( szBuffer, "FireGuard 자체 해킹 시도가 탐지되었습니다.\r\n\r\n"
				"계속해서 해킹을 시도할 경우 운영정책에 따라\r\n\r\n"
				"계정이 삭제될 수 있으니 유의하시기 바랍니다." );
		break;
	...
	if (i_client_shell != NULL) i_client_shell->OnHackDetected();
	
	dsi_ConsolePrint( "[FIREGUARD] FireGuard_Callback  ErrCode(0x%08x)  FailCode(0x%08x)",
						dwErrorCode, dwFailCode );
	ShowCursor(LTTRUE);
	MessageBox( (HWND)dsi_GetMainWindow(), szBuffer, "SuddenAttack", MB_OK|MB_ICONERROR );
	PostMessage( (HWND)dsi_GetMainWindow(), WM_CLOSE, LTNULL, LTNULL );
...
LTRESULT CClientMgr::Init(const char *resTrees[MAX_RESTREES], uint32 nResTrees, uint32 nNumConfigs, const char **ppConfigFileName)
{
	...
	m_ClientNetMgr.SetNetHandler(&m_cDefaultNetHandler);

#ifdef _FINAL
	MursumUT_MinimizeAllWindows(LTTRUE);
#endif
...
#ifdef _DEBUG
	srand(GetTickCount());	//modify by zhoujb 0814 
	if( false == InitNetworking( LTNULL, DEFAULT_LISTENPORT +rand() % 1000 ) ) return LT_ERROR;
#else
	if( false == InitNetworking( LTNULL, DEFAULT_LISTENPORT ) ) return LT_ERROR;
#endif
    ...
    InitErrorLog();		//modify by zhoujb 0814 
	MursumUT_InitErrorLog();
	dsi_ConsolePrint( "[ENGINE] Start Client..." );
...
//  VEC_INIT(m_vLastListenerPos);
    ...
    dResult = i_client_shell->OnEngineInitialized(&defaultMode, &m_ClientNetMgr.m_guidApp);

	dsi_ConsolePrint( "[ENGINE] OnEngineInitialized(%d)", dResult );

    // VideoMgr MUST be initialized AFTER the ClientShell, so that the SoundMgr is
	/*
    m_pVideoMgr = CreateVideoMgr("BINK");
   if (m_pVideoMgr) m_pVideoMgr->OnRenderInit();
   */
...
//[MURSUM] Cone
bool CClientMgr::InitNetworking( const char* lpszBindIP, uint16 uPort, bool bTermShell )
{
	uint16		wVersion;
	WSADATA		wsaData;
	int			nStatus;
	
	dsi_ConsolePrint( "[ENGINE] CClientMgr::InitNetworking( %s, %d, %d )", lpszBindIP, uPort, bTermShell );
	wVersion = MAKEWORD(1,1);
	nStatus = WSAStartup( wVersion, &wsaData );

	if( nStatus != 0 )
	{
		dsi_ConsolePrint("ERROR CClientMgr::InitSocket  WSAStartup() returned %d\n", nStatus);
		return false;
	}
	m_bWSAInitted = true;
	...
	m_SockAddr.sin_port = htons( uPort==0 ? DEFAULT_LISTENPORT : uPort );

	if( lpszBindIP )
	{
		m_SockAddr.sin_addr.s_addr = inet_addr(lpszBindIP);
	}
	else
	{
	...
	m_Socket = cm_BindToPort( &m_SockAddr, &m_SockAddr );
	if( INVALID_SOCKET == m_Socket )
	{
		return false;
	}
	
	dsi_ConsolePrint( "UDP: Create Socket  Port( %d )", ntohs( m_SockAddr.sin_port ) );
	return true;
}

void CClientMgr::TermNetworking()
{
	if( INVALID_SOCKET != m_Socket )
	{
...
{
	dsi_ConsolePrint( "[ENGINE] CClientMgr::OnEnterWorld( 0x%08x )", pShell );
...
{
	dsi_ConsolePrint( "[ENGINE] CClientMgr::OnExitWorld( 0x%08x )", pShell );
...
{
	dsi_ConsolePrint( "[ENGINE] CClientMgr::StartShell( %d )", pRequest->m_Type );
#ifdef SA_NETMONITORING
	g_CV_ParseSend = 32;
	g_CV_ParseRecv = 32;
#endif
	...
	m_nLoadingState = L_CONNECT_HOST;
	
	if( pRequest->m_Type != MANDATE_HOST &&
		pRequest->m_Type != REJOIN_HOST )
	{
		// Kill the old server connection if there is one.
		EndShell();		
	}
	
	if( LTNULL == m_pCurShell )
	{
    ...
    //  Start a shell.
	if( pRequest->m_Type == MANDATE_HOST || pRequest->m_Type == REJOIN_HOST )
	{
		pShell->m_bLoadWorld = LTFALSE;
	}
	else
	{
		pShell->m_bLoadWorld = LTTRUE;
	}
    ...
    switch(pRequest->m_Type)
    {
        case STARTGAME_CLIENT:
        {
            ...
            pDriver = m_ClientNetMgr.GetMainDriver();
            if (!pDriver)
            {
                RETURN_ERROR(1, StartGame, LT_NOTINITIALIZED);
            }
            ...
            // Setup the shell.
            dResult = pShell->StartupClient(pRequest,pDriver);
            if (dResult != LT_OK)
                return dResult;
        }
        break;
        ...
        {
            // Use the first net driver..
            pDriver = m_ClientNetMgr.GetDriver("internet");
            if (!pDriver)
            {
                RETURN_ERROR(1, StartGame, LT_NOTINITIALIZED);
            }
...
//			dResult = pDriver->ConnectTCP(pRequest->m_TCPAddress,m_Socket,ntohs( m_SockAddr.sin_port ));
			dResult = pDriver->ConnectTCP(pRequest->m_TCPAddress,m_Socket,ntohs( pRequest->m_HostInfo.m_Port ));	//modify by zhoujb 0814 
			if (dResult != LT_OK)
                return dResult;
		...
		case REJOIN_HOST:
        {
            ...
            // Setup the shell.
            dResult = pShell->StartupClient(LTNULL,pDriver);
            if (dResult != LT_OK)
                return dResult;      
        }
        break;
		...
		case MANDATE_HOST:
		{
            ...
            {
				pRequest->m_HostInfo.m_Port = ntohs( m_SockAddr.sin_port );
                dResult = pDriver->HostSession(&pRequest->m_HostInfo, m_Socket);
                if (dResult != LT_OK)
                {
                    return dResult;
                }
            ...
            // Setup the shell.
            dResult = pShell->MandateHost(pRequest, pRequest->m_Type==MANDATE_HOST, pDriver);
            if (dResult != LT_OK)
            {
                delete pShell;
                return dResult;
    ...
    if (m_pCurShell)
    {
        ...
        m_ClientNetMgr.SendPacket(CPacket_Read(cHello), m_pCurShell->m_HostID);	
    }
...
void CClientMgr::EndShell()
{
	if (m_pCurShell)
    {
		dsi_ConsolePrint( "[ENGINE] CClientMgr::EndShell" );
        delete m_pCurShell;
        m_pCurShell = NULL;
    }
...
//[MURSUM]
LTRESULT CClientMgr::StartCheckConeType(const char* lpszIP1, uint16 nPort1,
										...
										const char* lpszID, uint8 nIDLen)
{
	...
	if( LT_OK !=m_ClientNetMgr.InitDrivers() ) return LT_ERROR;	
	if( LT_OK !=m_ClientNetMgr.GetServiceList(pListHead) ) return LT_ERROR;	
	pCur = pListHead;
	while( pCur )
	{
	...
	if( LT_OK !=m_ClientNetMgr.FreeServiceList( pListHead ) ) return LT_ERROR;
	if( !hNetService ) return LT_ERROR;
	if( LT_OK != m_ClientNetMgr.SelectService( hNetService ) ) return LT_ERROR;
	
	pDriver = m_ClientNetMgr.GetMainDriver();
	if( !pDriver || INVALID_SOCKET == m_Socket ) return LT_ERROR;

	return pDriver->StartCheckConeType( m_Socket, lpszIP1, nPort1, lpszIP2, nPort2, lpszID, nIDLen );
}

void CClientMgr::StopCheckConeType()
{
	CBaseDriver* pDriver = m_ClientNetMgr.GetMainDriver();
	if( pDriver )
	{
		pDriver->StopCheckConeType();
		i_client_shell->HandleConeType( pDriver->GetConeType() );
	}
	m_ClientNetMgr.TermDrivers();
}

void CClientMgr::SendDummyPacket( const char* lpszIP, uint16 nPort )
{
	CBaseDriver* pDriver = m_ClientNetMgr.GetMainDriver();
	if( pDriver )
	{
		pDriver->SendDummyPacket( m_Socket, lpszIP, nPort );
	}	
}
...
LTRESULT CClientMgr::Update()
{
    ...
    if (m_pVideoMgr) m_pVideoMgr->UpdateVideos();
    ...
    if (GetMusicMgr()->m_bValid)
	{
        ...
        if ((g_bMusicEnable != 0) != GetMusicMgr()->m_bEnabled)
		{
			...
			GetClientILTSoundMgrImpl()->SetBGMEnable(g_bMusicEnable != 0);  //[SPIKE]
            if (!g_bMusicEnable)
			{
	...
	if( m_bConnected )
	{
		...
		/*
		RMode CurMode;
		dsi_GetRenderMode(&CurMode);
		if( CurMode.m_Width != 800 || CurMode.m_Height != 600 )
		{
			CurMode.m_Width = 800;
			CurMode.m_Height = 600;
			dsi_SetRenderMode(&CurMode);
		}
		*/
	}
	//-----------------------------------------
    ...
    else if (i_client_shell != NULL)
	{
        ...
        m_ClientNetMgr.Update("Client: ", m_CurTime);

        i_client_shell->PreUpdate();

        i_client_shell->Update();

        UpdateObjects( m_nFrameTimeMS );
...
void CClientMgr::ProcessAllInput(bool bForceClear) {
	...
	{
		SendUpdate(&m_ClientNetMgr, m_pCurShell->m_HostID);
	}
}
...
void CClientMgr::UpdateAllSounds()
{
	...
	//[SPIKE] BGM은 예외!
	if(GetClientILTSoundMgrImpl()->IsBGMEnabled())
	{
		if(!GetClientILTSoundMgrImpl()->IsValid())
			return;
	}
	...
	uint32 nPlayingNum	= GetClientILTSoundMgrImpl()->GetNumSoundsPlaying();
	uint32 nHeardNum	= GetClientILTSoundMgrImpl()->GetNumSoundsHeard();

    if (g_bSoundShowCounts)
    {
		con_Printf(CONRGB(255,0,0), 0, "Sounds: playing:%d, heard:%d", nPlayingNum, nHeardNum );
    }
}
...
void cm_Init()
{
	...
	g_pClientMgr->m_ClientNetMgr.Init("Player");
...
void CClientMgr::Term()
{
	...
	dsi_ConsolePrint( "[ENGINE] CClientMgr::Term" );
	...
	*/
...
#ifdef _FINAL
	MursumUT_MinimizeAllWindows(LTFALSE);
#endif
    ...
    TermErrorLog();	//modify by zhoujb 0814 
	MursumUT_TermErrorLog();
...
void CClientMgr::OnEnterServer()
{
	...
	// 광식
	if( m_ServerConsoleMirror.m_VarHash != LTNULL ) return;
...
void CClientMgr::OnExitServer(CClientShell *pShell) 
{
	...
	dsi_ConsolePrint( "[ENGINE] CClientMgr::OnExitServer( 0x%08x )", pShell );
...
bool CClientMgr::Render(CameraInstance *pCamera, int drawMode, LTObject **pObjects, int nObjects, float fFrameTime)
{
	...
	pDesc->m_fActualFrameTime = GetFrameTimeClient();
...
void CClientMgr::UpdateParticleSystems( uint32 nFrameTimeMS )
{
    ...
    if (nFrameTimeMS > 0)
    {
        ...
        for (pCur=pListHead->m_pNext; pCur != pListHead; pCur=pCur->m_pNext)
        {
            ...
            {
                spr_UpdateTracker(&pSystem->m_SpriteTracker, nFrameTimeMS);
            ...
            ps_UpdateParticles(pSystem, GetFrameTimeClient() );
...
void CClientMgr::UpdateAnimations( uint32 nFrameTimeMS )
{
    LTLink *pCur, *pListHead;
    ...
    if (nFrameTimeMS > 0)
    {
        ...
        for (pCur=pListHead->m_pNext; pCur != pListHead; pCur=pCur->m_pNext)
        {
			...
			if(!pSprite->IsPaused())
			{
				spr_UpdateTracker(&pSprite->m_SpriteTracker, nFrameTimeMS);
			}
        }
    }
}

void CClientMgr::UpdatePolyGrids( uint32 nFrameTimeMS )
{
    LTLink *pListHead, *pCur;
    LTPolyGrid *pGrid;
    uint32 flags, msFrameTime;

    msFrameTime = nFrameTimeMS;
    if (msFrameTime)
    {
...
void CClientMgr::UpdateLineSystems( uint32 nFrameTimeMS )
 {
    LTLink *pListHead, *pCur;
    LineSystem *pSystem;
...
void CClientMgr::UpdateModels( uint32 nFrameTimeMS )
{
    ...
    if (nFrameTimeMS)
    {
		...
		for (LTLink *pCur = pListHead->m_pNext; pCur != pListHead; pCur = pCur->m_pNext)
        {
			...
			if(!pModel->IsPaused())
			{
				...
				pModel->ClientUpdate(nFrameTimeMS);
				...
				for (i = 0; nSpritesLeft && (i < MAX_MODEL_TEXTURES); i++)
				{
					...
					{
						spr_UpdateTracker(&pModel->m_SpriteTrackers[i], nFrameTimeMS);
...
void CClientMgr::UpdateObjects( uint32 nFrameTimeMS )
{
	g_Ticks_UpdateObjects = 0;
	CountAdder cntUpdate(&g_Ticks_UpdateObjects);

    UpdateAnimations( nFrameTimeMS );
    UpdateParticleSystems( nFrameTimeMS );
    UpdatePolyGrids( nFrameTimeMS );
    UpdateLineSystems( nFrameTimeMS );
    UpdateModels( nFrameTimeMS );
...
//[MURSUM]
LTFLOAT CClientMgr::GetFrameRateServer()
{
#ifndef _FINAL
	if( m_pCurShell )
	{
		return m_pCurShell->m_FramerateTrackerServer.GetRate();
	}
#endif
...
LTFLOAT CClientMgr::GetFrameRateClient()
{
#ifndef _FINAL
	if( m_pCurShell )
	{
		return m_pCurShell->m_FramerateTrackerClient.GetRate();
	}
#endif
...
LTFLOAT CClientMgr::GetFrameTimeServer()
{
	if( m_pCurShell )
	{
		return m_pCurShell->m_fFrameTimeServer;
	}

	return m_FrameTime;
}

LTFLOAT CClientMgr::GetFrameTimeClient()
{
	if( m_pCurShell )
	{
		return m_pCurShell->m_fFrameTimeClient;
	}

	return m_FrameTime;
}

int32 CClientMgr::GetLoadingState()
{
	return m_nLoadingState;
}

void CClientMgr::SetLoadingState( int32 nState )
{
	m_nLoadingState = nState;
}

LTBOOL CClientMgr::GetCookieFromClipboard( HWND hWnd )
{
	HGLOBAL	hMem;
	char*	ptr;

	if( LTFALSE == IsClipboardFormatAvailable( CF_TEXT ) ) return LTFALSE;
	
	OpenClipboard( hWnd );
	hMem = GetClipboardData( CF_TEXT );
	if( LTNULL == hMem ) return LTFALSE;
	...
	memset( m_szCpCookie, 0, 1024*2 );
	strcpy( m_szCpCookie, ptr );

	OutputDebugString( m_szCpCookie );
	GlobalUnlock( hMem );
	CloseClipboard();
...
void CClientMgr::SetUserNickName( const char* lpszNickName )
{
	if( lpszNickName ) MursumCrashReport_SetUserID( lpszNickName );
}

//[MURSUM] 꼭 필요한 파일들은 미리 리스트에 추가해서 패킷으로 안 보낸다.
bool CClientMgr::PreLoadFileList( const char* lpszName, bool bDir )
{
	if( LTNULL == lpszName || strlen(lpszName) <=0 || LTNULL == m_pCurShell ) return false;

	if( bDir )
	{
		return PreLoadDir( lpszName );
	}
	else
	{
		return PreLoadFile( lpszName );
	}

	return true;	
}

bool CClientMgr::PreLoadFile( const char* lpszFileName )
{
	UsedFile* pFile;
	if( 0 == server_filemgr->AddUsedFile( lpszFileName,
										  (1<<3), //FFLAG_DONTSEND
										  &pFile ) ) return false;

	if( !pFile ) return false;

	client_file_mgr->OnNewFile( client_file_mgr->GetFTClient(),
								lpszFileName, 
								pFile->m_FileSize, pFile->m_FileID );

	return true;
}

bool CClientMgr::PreLoadDir( const char* lpszDirName )
{
	FileEntry* pEntry = server_filemgr->GetFileList( lpszDirName );
	if( !pEntry ) return false;
	
	while( pEntry )
	{
		if( pEntry->m_Type & TYPE_DIRECTORY )
		{
			if( false == PreLoadDir( pEntry->m_pFullFilename ) ) return false;
		}
		else
		{
			if( false == PreLoadFile( pEntry->m_pFullFilename ) ) return false;
		}
...
LTRESULT CClientMgr::StartDummyHost( const char* lpszWorldName )
{
	if( !m_pCurShell || !lpszWorldName ) return LT_ERROR;

	return m_pCurShell->StartupClientTCP( lpszWorldName );
}

uint8 CClientMgr::GetNetState()
{
	// 0 : Best
	// 4 : Worst
	...
	if( m_pCurShell )
	{
		if( m_pCurShell->m_bLocal )
		{
			float fPing = g_pServerMgr->GetNetMgr()->GetHostPing();
			...
			else if( fPing < 150.0f )
			{
				return 2;
			}
			else if( fPing < 300.0f )
			{
				return 3;
			}			
		}
		...
		{
			if( LTNULL == m_pCurShell->m_HostID ) return 0;

			float fPing = m_ClientNetMgr.GetConnPing(m_pCurShell->m_HostID);

			if( fPing < 150.0f )
			{
				return 0;
			}
			else if( fPing < 400.0f )
			{
				return 1;
			}
			else if( fPing < 750.0f )
			{
				return 2;
			}
			else if( fPing < 1500.0f )
			{
				return 3;
			}
		}
...
void CClientMgr::SendAddedFiles()
{
	MursumCrashReport_SendAddedFile();
}

void CClientMgr::ToggleFullScreen()
{
#ifndef _FINAL
	r_ToggleFullScreen();
#endif
...
long CClientMgr::InitFireGuard( const char* lpszGameName, 
								const char* lpszUserID )
{
#ifdef USE_FIRE_GUARD
	if( LTNULL == lpszUserID || LTNULL == lpszGameName ) return 0L;

	StopFireGuard();

	g_pFireGuard = new CFireGuard();
	dsi_ConsolePrint( "[FIREGUARD] Create Instance" );

	g_pFireGuard->LogClientInfo( lpszUserID );
	dsi_ConsolePrint( "[FIREGUARD] Set UserID" );

	return g_pFireGuard->Initialize( (HWND)dsi_GetMainWindow(),
								     lpszGameName,
									 (GAMEGUARD_CALLBACKFUNC)FireGuard_CallBack, 
									 (DWORD)eProtect_All );
#else
	return 0L;
#endif
...
long CClientMgr::StartFireGuard()
{
#ifdef USE_FIRE_GUARD
	if( g_pFireGuard )
	{
		return g_pFireGuard->Start();
	}
...
void CClientMgr::CheckFireGuard()
{
#ifdef USE_FIRE_GUARD
	if( g_pFireGuard && LTFALSE == g_pFireGuard->IsAlive() )
	{
		dsi_ConsolePrint( "[FIREGUARD] FireGuard is dead" );
		MessageBox( (HWND)dsi_GetMainWindow(), 
					"해킹방지 시스템이 동작하지 않습니다.\r\n\r\n"
					"프로그램을 종료합니다.",
					"SuddenAttack",
					MB_OK|MB_ICONERROR );
...
void CClientMgr::ShowFireGuardErr( long lResult, bool bStartErr )
{
#ifdef USE_FIRE_GUARD
	...
	if( bStartErr )
	{
		wsprintf( szBuffer, "해킹방지 시스템을 시작할 수 없습니다.\r\n\r\n"
					"프로그램을 종료합니다.\r\n\r\n"
					"에러코드 : 0x%08x", lResult );
		MessageBox( (HWND)dsi_GetMainWindow(), szBuffer, "SuddenAttack", MB_OK|MB_ICONERROR );
	}
	...
	{		
		switch( lResult )
		{
		case HERR_ALREADY_RUNNING:
			wsprintf( szBuffer, "해킹방지 시스템이 이미 실행 중 입니다.\r\n\r\n"
							  "잠시 후 재실행 해주시고 같은 현상 발생시 재부팅 후 게임을 실행해 주십시오.\r\n\r\n"
							  "에러코드 : 0x%08x", lResult );
			break;
		case HERR_LOG_SYSTEM_INIT_FAILED:
			wsprintf( szBuffer, "해킹방지 시스템 초기화에 실패하였습니다.\r\n\r\n"
							  "잠시 후 재실행 해주시고 같은 현상 발생시 재부팅 후 게임을 실행해 주십시오.\r\n\r\n"
							  "에러코드 : 0x%08x", lResult );
			break;
		case HERR_ENGINE_LOAD_FAILED:
			wsprintf( szBuffer, "해킹방지 시스템 초기화에 실패하였습니다.\r\n\r\n"
							  "잠시 후 재실행 해주시고 같은 현상 발생시 재부팅 후 게임을 실행해 주십시오.\r\n\r\n"
							  "에러코드 : 0x%08x", lResult );
			break;
		case HERR_UPDATESERVER_DEAD :
			wsprintf( szBuffer, "네트워크 문제로 인하여 업데이트를 수행하지 못했습니다.\r\n\r\n"
							  "네트워크 확인 후 다시 시도해 주십시오.\r\n\r\n"
							  "에러코드 : 0x%08x", lResult );
			break;
		case HERR_UPDATE_TIMEOUT :
			wsprintf( szBuffer, "인터넷 연결에 문제가 있는 듯 합니다.\r\n\r\n"
							  "네트워크 확인 후 다시 시도해 주십시오.\r\n\r\n"
							  "에러코드 : 0x%08x", lResult );
			break;	
		case HERR_FILESYSTEM_FULL:
			wsprintf( szBuffer, "해킹방지 시스템 구동에 필요한 디스크 용량이 부족합니다.\r\n\r\n"	
							  "불필요한 파일 삭제 후 다시 시도해 주십시오.\r\n\r\n"
							  "에러코드 : 0x%08x", lResult );
			break;	
		case HERR_FILE_LOCKED:
			wsprintf( szBuffer, "다른 프로그램이 필요한 파일을 사용 중인 듯 합니다.\r\n\r\n"
							  "실행중인 게임을 완전히 종료해 주시고 문제가 반복될 경우 재부팅 후 다시 시도해 주십시오.\r\n\r\n"
							  "에러코드 : 0x%08x", lResult );	
			break;	
		case HERR_ACCESS_DENIED:
			wsprintf( szBuffer, "게임 파일 접근 권한이 없습니다.\r\n\r\n"
							  "계정의 권한을 확인 한 후 다시 시도해 주십시오.\r\n\r\n"
							  "에러코드 : 0x%08x", lResult );
			break;	
		case HERR_UPDATE_GENERAL_ERROR:	
			wsprintf (szBuffer, "네트워크 문제로 인하여 업데이트를 수행하지 못했습니다.\r\n\r\n"
							  "네트워크 확인 후 다시 시도해 주십시오.\r\n\r\n"
							  "에러코드 : 0x%08x", lResult );
			break;
...
IDirect3DDevice8* CClientMgr::GetD3DDevice()
{
	return r_GetRenderStruct()->GetD3DDevice();
}

void CClientMgr::SetDumpDir( const char* lpszDir )
{
	MursumUT_SetDumpDir( lpszDir );
}

void CClientMgr::MakeScreenShot( const char* lpszFileName )
{
	if (r_GetRenderStruct()) 
	{
		r_GetRenderStruct()->MakeScreenShot(lpszFileName);
    }
}

LTRESULT CClientMgr::PlayAmbientSound()
{
	if( m_pCurShell ) return m_pCurShell->DoLoadSoundClient();
