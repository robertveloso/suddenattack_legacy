...
void ctik_ShowTickStatus()
{
	...
	static STickStat sTicksObjectShell;
	...
	int iTotalEngine;
	int iTotalSystem;
	int iTotalSoundThread;

	// Calculate the total times

	iTotalRender = (int)g_Ticks_Render + (int)g_Ticks_Render_Console;	
	sTicksRender.Update(nUpdateCount, iTotalRender);
	iTotalGame = ((int)g_Ticks_ClientShell + (int)g_Ticks_ClassUpdate) - ((int)sTicksRender.m_nCurTicks + (int)g_Ticks_MoveObject + (int)g_Ticks_Intersect);
	sTicksGame.Update(nUpdateCount, iTotalGame);	
	sTicksObjectShell.Update(nUpdateCount, (int)g_Ticks_ServerUpdate);
	iTotalEngine = (int)g_Ticks_Total - ((int)sTicksRender.m_nCurTicks + (int)sTicksGame.m_nCurTicks + (int)g_Ticks_ServerUpdate);	
	sTicksEngine.Update(nUpdateCount, iTotalEngine);
	iTotalSystem = (int)g_TotalGlobalTime - ((int)sTicksRender.m_nCurTicks + (int)sTicksGame.m_nCurTicks + (int)g_Ticks_ServerUpdate + (int)sTicksEngine.m_nCurTicks);	
	...
	iTotalSoundThread = (int)g_Ticks_SoundThread;	
	...
	if(g_iGraphMgr->Mgr()->CheckGraph(&g_ShowTickCounts, (g_ShowTickCounts & CLIENT_TICKS_GRAPH) != 0))
	{
		...
		fTotal = (float)sTicksRender.m_nCurTicks + (float)sTicksGame.m_nCurTicks + (float)sTicksObjectShell.m_nCurTicks + (float)sTicksEngine.m_nCurTicks + (float)sTicksSystem.m_nCurTicks;
		...
		fValue = ((fTotal - (float)(sTicksRender.m_nCurTicks + sTicksGame.m_nCurTicks + sTicksObjectShell.m_nCurTicks)) / fTotal) * 100.0f;
		g_iGraphMgr->Mgr()->UpdateGraph(&g_ShowTickCounts, fValue, tickParams);
		
		// System.
		tickParams.m_pColorTable = GetDGColorTable(PValue_Set(0, 255, 255, 255), PValue_Set(0, 255, 255, 255));
		fValue = ((fTotal - (float)(sTicksRender.m_nCurTicks + sTicksGame.m_nCurTicks + sTicksObjectShell.m_nCurTicks + sTicksEngine.m_nCurTicks)) / fTotal) * 100.0f;
		g_iGraphMgr->Mgr()->UpdateGraph(&g_ShowTickCounts, fValue, tickParams);
	...
	if ((g_ShowTickCounts & CLIENT_TICKS_SUMMARY) != 0)
	{
		...
		con_Printf(CONRGB(192,192,255), 0, "---- Summary ( S: %6.2ffps   C: %6.2ffps /%7u /%7u /%7u) ----",
			g_pClientMgr->GetFrameRateServer(),
			g_pClientMgr->GetFrameRateClient(), 
			sTicksGlobal.GetAveTicks(nUpdateCount), sTicksGlobal.m_nMaxTicks, sTicksGlobal.m_nCurTicks);
		con_WhitePrintf("     Render: (%3d%% /%6u /%6u /%6u)", TICK_PARAMS(sTicksRender, g_TotalGlobalTime));
		con_WhitePrintf("ClientShell: (%3d%% /%6u /%6u /%6u)", TICK_PARAMS(sTicksGame, g_TotalGlobalTime));
		con_WhitePrintf("ObjectShell: (%3d%% /%6u /%6u /%6u)", TICK_PARAMS(sTicksObjectShell, g_TotalGlobalTime));
