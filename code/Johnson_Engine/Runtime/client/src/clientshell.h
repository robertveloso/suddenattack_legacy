...
class CClientShell : public CNetHandler
{
	...
	public:
		...
		LTRESULT				StartupClientTCP( const char* pWorldName );
		LTRESULT				StartupClient(StartGameRequest *pRequest, CBaseDriver *pDriver);
		LTRESULT				StartupLocal(StartGameRequest *pRequest, bool bHost, CBaseDriver *pServerDriver);
		LRESULT					MandateHost(StartGameRequest *pRequest, bool bHost, CBaseDriver *pServerDriver);
		...
		//[MURSUM]
		LTRESULT				DoLoadWorldClient( uint8 nObjectType, uint16 nObjectID );
		LTRESULT				PostLoadClient( LTObject* pServerObj, LTObject* pClientObj );

		LTRESULT				DoLoadSoundClient();
	...
	public:
		...
		LTFLOAT					m_ServerPeriodTrack;
		LTFLOAT					m_ServerPeriod;
...
//		CServerMgr				*m_pServerMgr;
	...
	public:
		...
		//[MURSUM]
		LTBOOL					m_bLoadWorld;
		RateTracker             m_FramerateTrackerServer;
		RateTracker             m_FramerateTrackerClient;

		LTFLOAT					m_fLastUpdateServer;
		LTFLOAT					m_fLastUpdateClient;

		LTFLOAT					m_fFrameTimeServer;
		LTFLOAT					m_fFrameTimeClient;
};
