...
// ±¤½Ä
inline bool i_BoundingBoxTestClient(const LTVector& Point1, const LTVector& Point2, const LTObject *pServerObj, 
    LTVector *pIntersectPt, LTPlane *pIntersectPlane)
{
	...
	*pIntersectPt = LTVector(0.0f, 0.0f, 0.0f);
...
// ±¤½Ä
inline bool i_HandlePossibleIntersectionClient(const LTVector& Point1, const LTVector& Point2, LTObject *pServerObj)
{
	// Quick sphere test.
    if (i_QuickSphereTest(pServerObj)) 
	{
        ...
        else 
		{
            ...
            }
			{
				...
				i_BoundingBoxTestClient(Point1, Point2, pServerObj, &testPt, &testPlane);
				distToIntersectionSqr = testPt.DistSqr(g_pCurQuery->m_From);
				USE_THIS_OBJECT(pServerObj, distToIntersectionSqr, testPlane, testPt, INVALID_HPOLY);
                return true;
			}
...
// ±¤½Ä
static bool i_ISCallbackClient(WorldTreeObj *pObj, void *pCBUser) 
{    
    ASSERT(pObj->GetObjType() == WTObj_DObject);
    LTObject *pObject = (LTObject*)pObj;
    ...
    if (pObject->m_Flags & (FLAG_RAYHIT|FLAG_SOLID) || g_bProcessNonSolid) 
	{
        ...
        // This is basically everything except for objects with only touch-notify...
        // Do further tests..
        return i_HandlePossibleIntersectionClient(g_pCurQuery->m_From, g_pCurQuery->m_To, pObject);
    }

    return false;
}
// ±¤½Ä
bool i_IntersectSegmentClient(IntersectQuery *pQuery, IntersectInfo *pInfo, WorldTree *pWorldTree)
{
    ...
    pWorldTree->IntersectSegmentClient((LTVector*)&pQuery->m_From, (LTVector*)&pQuery->m_To, i_ISCallbackClient, LTNULL);
