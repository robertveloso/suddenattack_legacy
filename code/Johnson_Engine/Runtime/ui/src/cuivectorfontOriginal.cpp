...
bool InstalledFontFace::Init( char const* pszFontFile, char const* pszFontFace, int nHeight )
{
	...
	if( pszFontFile && pszFontFile[0] )
	{
		...
		bool bOutIsTemporary = false;
		if( GetOrCopyClientFile( szTempPath, pszFontFile, szOutName, sizeof( szOutName ), &bOutIsTemporary ) != LT_OK )
		{
		...
		uint8  asciiEnd)
{
	...
	bool bOk = Init( pszFontFile, pszFontFace, pointSize, szChars );
							 ...
							 char const* pszCharacters)
{
	...
	if( !installedFontFace.Init( pszFontFile, pszFontFace, pointSize ))
		return false;
										...
										int nLen, SIZE& sizeTexture )
{
	...
	int nRawWidth = ( int )( sqrtf( nTotalPixelArea ) + 0.5f );
						  ...
						  GLYPHMETRICS* pGlyphMetrics, SIZE& sizeMaxGlyphSize )
{
	...
	for( int nGlyph = 0; nGlyph < nLen; nGlyph++ )
	{
		...
		if( GDI_ERROR == GetGlyphOutline( hDC, ( uint8 )nChar, GGO_METRICS, &glyphMetrics, 
			0, NULL, &mat ))
		{
			return false;
		}
											  ...
											  char const* pszChars, bool bMakeMap )
{
	...
	if( bOk )
	{
		...
		m_DefaultCharScreenWidth 	= sizeTextExtent.cx;
		m_DefaultCharScreenHeight 	= sizeTextExtent.cy;
		...
		m_CharTexHeight = textMetric.tmHeight;
