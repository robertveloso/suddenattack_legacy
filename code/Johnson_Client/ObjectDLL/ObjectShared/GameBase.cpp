...
void GameBase::CreateBoundingBox()
{
    ...
    g_pLTServer->SetObjectColor(m_hDimsBox, vColor.x, vColor.y, vColor.z, GB_DIMS_ALPHA);
}
...
bool GameBase::ReadProp( ObjectCreateStruct* pStruct )
{
	...
	switch( g_pGameServerShell->GetGameType( ))
	{
		...
		{
			if( !pStruct->m_cProperties.GetPropBool( "Survival", true ))
				return false;
		}break;
	}
