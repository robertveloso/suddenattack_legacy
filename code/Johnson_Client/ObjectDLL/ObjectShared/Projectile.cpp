...
#define PRJT_INVISIBLE_MAX_TICKNESS 33.0f
#define PRJT_DEFLECTANGLE_RANGE		30.0f
	...
	, m_nLastRicochetUpdateNum( -1 )
{
	...
	// 광식
	//--------------------------------------
	m_eClientHitNode = eModelNodeInvalid;
	//m_hHitModel = LTNULL;
	//--------------------------------------
}
...
uint32 CProjectile::EngineMessageFn(uint32 messageID, void *pData, LTFLOAT fData)
{
	switch(messageID)
	{
		...
		case MID_PRECREATE:
		{
			...
			// 광식
			//------------------------------------------------------
			GameBase::EngineMessageFn(MID_PRECREATE, pData, fData);
			...
			*/
			...
			if( fData != PRECREATE_SAVEGAME )
			{
				...
				if (pStruct)
				{
					...
					// Edited By Bestline ==>
					// Time : 2004-07-16 오후 2:13:15
					// 경로 수정
					SAFE_STRCPY(pStruct->m_Filename, "GLOBAL\\Models\\Default.ltb");
					// Edited By Bestline <==					
...
LTBOOL CProjectile::Setup(CWeapon const* pWeapon, WeaponFireInfo const & info, uint8 nVertorsPerRoundIndex)
{
	...
	else
	{
		...
		if(m_fVelocity)
		{
			//[SPIKE] 상황에 따른 수류탄 속도증감
			LTVector vVel;
			LTFLOAT fMoveVelocity;
			CPlayerObj* pPlayer = (CPlayerObj*) g_pLTServer->HandleToObject(m_hFiredFrom);
			g_pPhysicsLT->GetVelocity(pPlayer->m_hObject, &vVel);
			fMoveVelocity = vVel.Length();
			
			if(pPlayer->IsDead())//Grenade DeathFire 
			{
				m_fVelocity = 100.0f;
			}
			else if(pPlayer->GetControlFlags() & BC_CFLG_FORWARD)
			{
				m_fVelocity += fMoveVelocity;
			}
			else if(pPlayer->GetControlFlags() & BC_CFLG_REVERSE)
			{
				m_fVelocity -= fMoveVelocity;
			}
			/////////////////////////////////////////////////
	...
	{
		DoVector(nVertorsPerRoundIndex);
	}
...
// ----------------------------------------------------------------------- //
//
//	ROUTINE:	CProjectile::SetupHitPlayer
//
//	PURPOSE:	Set up a projectile with the information needed
//
// ----------------------------------------------------------------------- //

LTBOOL CProjectile::SetupHitPlayer( CWeapon const* pWeapon, WeaponFireInfo const& info )
{
	if (!pWeapon || !info.hFiredFrom || !g_pWeaponMgr) return LTFALSE;

	//uint8	nShooterTeamID	= GetPlayerTeamId(info.hFiredFrom);
	//uint8	nDamagerTeamID	= GetPlayerTeamId(info.hClientObjImpact);
	//if( nShooterTeamID == nDamagerTeamID ) return LTTRUE;	//같은팀 무시

	LTVector	vImpactPos;
	
	g_pLTServer->GetObjectPos( info.hFiredFrom, &m_vFirePos );
	g_pLTServer->GetObjectPos( info.hClientObjImpact, &vImpactPos );
	m_hFiredFrom        = info.hFiredFrom;
	m_vFlashPos         = m_vFirePos;
	m_vDir				= vImpactPos-m_vFirePos;
	m_vDir.Normalize();

	if( IsTeamGameType() ) SetTeamId( GetPlayerTeamId(m_hFiredFrom) );
	...
	if( !m_pWeaponData || !m_pAmmoData ) return LTFALSE;
	...
	CPlayerObj* pPlayer		= (CPlayerObj*) g_pLTServer->HandleToObject(m_hFiredFrom);
	CPlayerObj* pDamager	= (CPlayerObj*) g_pLTServer->HandleToObject(m_hClientObjImpact);
	if( !pPlayer || pPlayer->IsDead() ) return LTTRUE;
	if( !pDamager || pDamager->IsDead() || pDamager->IsGodMode() ) return LTTRUE;

	pDamager->SetModelNodeLastHit(m_eClientHitNode);
	AdjustDamage(pDamager->ComputeDamageModifier(m_eClientHitNode));

	ImpactDamageObject( m_hFiredFrom, m_hClientObjImpact );
	
	CAutoMessage cMsg;
	cMsg.Writeuint8(MID_HIT_SFX);
	cMsg.Writeuint8((uint8)g_pLTServer->GetClientID(pPlayer->GetClient()));
	cMsg.Writeuint8((uint8)g_pLTServer->GetClientID(pDamager->GetClient()));
    cMsg.Writeuint8((uint8)m_eClientHitNode);  
	cMsg.Writeuint8((uint8)m_nWeaponId);
	...
	while( iter != CPlayerObj::GetPlayerObjList( ).end( ))
	{
		...
		if( pPlayerObj && pPlayerObj != pPlayer )
		{
			g_pLTServer->SendToClient( cMsg.Read(), pPlayerObj->GetClient(), 0 ); //UNGUARANTEED
		}
		++iter;
	}
	CHECK_PLAYEROBJ_END();

	CharFireInfo charfireinfo;
	charfireinfo.hObject    = m_hClientObjImpact;
	charfireinfo.vFiredPos  = m_vFlashPos;  // Use initial flash pos
	charfireinfo.vImpactPos = vImpactPos;
	charfireinfo.nWeaponId  = m_nWeaponId;
	charfireinfo.nAmmoId    = m_nAmmoId;
	charfireinfo.fTime      = g_pLTServer->GetTime();
	charfireinfo.bSilenced  = m_bSilenced;
	charfireinfo.eSurface   = ST_ARMOR;
	pPlayer->SetLastFireInfo(&charfireinfo);
							 ...
							 bool bDeflected)
{
	...
	if (m_hFiredFrom && IsCharacter(m_hFiredFrom) && CanSetLastFireInfo())
	{
		...
		if (pChar)
		{
			...
			{
				CCharacter* pVictim = (CCharacter*)g_pLTServer->HandleToObject(hObj);
				if( pVictim )
				{
					pVictim->m_LastDamageWeapon.hObject	   = LTNULL; // 내가 맞은거니까 NULL 처리 함
					pVictim->m_LastDamageWeapon.vFiredPos  = m_vFlashPos;
					pVictim->m_LastDamageWeapon.vImpactPos = vImpactPos;
					pVictim->m_LastDamageWeapon.nWeaponId  = m_pWeaponData->nId;
					pVictim->m_LastDamageWeapon.nAmmoId    = m_pAmmoData->nId;
					pVictim->m_LastDamageWeapon.fTime      = g_pLTServer->GetTime();
					pVictim->m_LastDamageWeapon.bSilenced  = m_bSilenced;
					pVictim->m_LastDamageWeapon.eSurface   = eType;
				}
			}
			//--------------------------------------------------------
			*/
...
void CProjectile::DoVector(uint8 nVertorsPerRoundIndex)
{
	IntersectQuery qInfo;
	...
	while (!bDone)
	{
		...
		if (g_pLTServer->IntersectSegment(&qInfo, &iInfo))
		{
			if (HandleVectorImpact(iInfo, vFrom, vTo) || m_pAmmoData->eInstDamageType == DT_MELEE )
			{
				return;
			}
                                     ...
                                     LTVector & vTo)
{
	...
	if( IsCharacterHitBox(iInfo.m_hObject) )
	{
		...
		if (pCharHitBox)
		{
			HOBJECT hObj = pCharHitBox->GetModelObject();
			if( IsBody(hObj) )
			{
				LTVector	vDir;
				vDir = vTo-vFrom;
				...
				uint8 nHitNode = pCharHitBox->FindHitNodeFromBody( vDir, vFrom );
				
				//[MURSUM] 시체에 피뿌리기
				Body* pBody			= (Body*)g_pLTServer->HandleToObject(hObj);
				CPlayerObj* pPlayer	= (CPlayerObj*)g_pLTServer->HandleToObject(m_hFiredFrom);
				
				if( pBody && nHitNode != 255 )
				{			
					CAutoMessage cMsg;
					cMsg.Writeuint8(MID_BODY_HIT_SFX);
					cMsg.Writeuint8((uint8)g_pLTServer->GetClientID(pPlayer->GetClient()));
					cMsg.Writeuint8((uint8)g_pLTServer->GetClientID(pBody->GetParentObj()->GetClient()));
					cMsg.Writeuint8(nHitNode);  
					cMsg.Writeuint8((uint8)m_nWeaponId);

					g_pLTServer->SendToClient( cMsg.Read(), LTNULL, 0 ); //UNGUARANTEED

					m_bCheckShootThrough	= LTFALSE;
					eSurfType				= ST_ARMOR;
				}
			}
		}
...
//	AddImpact(iInfo.m_hObject, vFirePos, iInfo.m_Point, iInfo.m_Plane.m_Normal, eSurfType);
	...
	if (pSurf->bCanShootThrough)
	{
        ...
        if (g_pLTServer->IntersectSegment(&qTestInfo, &iTestInfo))
		{
			...
			{
				AddImpact(iTestInfo.m_hObject, vFirePos, iTestInfo.m_Point, iTestInfo.m_Plane.m_Normal, eTestSurfType);
			}
			return UpdateDoVectorValues(*pSurf, fThickness, iTestInfo.m_Point, vFrom, vTo);
			*/
			//--------------------------------------------------------------------------------------
...
LTBOOL CProjectile::CalcInvisibleImpact(IntersectInfo & iInfo, SurfaceType & eSurfType)
{
	...
	qTestInfo.m_From = iInfo.m_Point + (m_vDir * PRJT_INVISIBLE_MAX_TICKNESS);
	qTestInfo.m_To   = iInfo.m_Point - m_vDir;
                                               ...
                                               LTVector & vFrom)
{
	...
	// 광식
	//----------------------------
	//g_hHitModel = iInfo.m_hObject;
	//m_hHitModel = iInfo.m_hObject;
	//----------------------------
...
bool CProjectile::Deflect( HOBJECT hTarget, const LTVector &vFirePos, const LTVector &vImpactPos, const LTVector &vNormal )
{
	...
	float fAngleInRadians = PRJT_DEFLECTANGLE_RANGE * MATH_PI / 180.0f;
...
void CProjectile::ImpactDamageObject(HOBJECT hDamager, HOBJECT hObj)
{
	...
	//------------------------------------------------------------------------------------------
	// 광식 : Weapon ID 를 이곳에서 첨가함.. NOLF2 source는 damage type으로 구별하도록 되어 있음
	//------------------------------------------------------------------------------------------
	damage.nWeaponId = m_nWeaponId;
	//------------------------------------------------------------------------------------------
	...
	if ( m_eInstDamageType != DT_UNSPECIFIED )
	{
		...
		CCharacterHitBox* pHitBox = LTNULL;	
		//pHitBox = (CCharacterHitBox*)g_pLTServer->HandleToObject(g_hHitModel);
		pHitBox = (CCharacterHitBox*)g_pLTServer->HandleToObject(m_hHitModel);
		if(pHitBox != LTNULL )
		{
			damage.nHitNode	= pHitBox->m_eModelNode;
		}
		*/
		//-------------------------------------------------------------
