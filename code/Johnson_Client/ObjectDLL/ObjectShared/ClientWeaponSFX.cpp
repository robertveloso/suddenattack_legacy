...
void CreateClientWeaponFX(CLIENTWEAPONFX & theStruct)
{
	...
	//cMsg.WriteCompLTVector(theStruct.vSurfaceNormal);
	//cMsg.WriteCompPos(theStruct.vFirePos);
	cMsg.WriteCompPos(theStruct.vPos);
	//cMsg.WriteCompPos(theStruct.vSurfaceNormal);
//    cMsg.Writeuint8(theStruct.eImpactType);

	WEAPON const *pWeaponData = g_pWeaponMgr->GetWeapon(theStruct.nWeaponId);
	if( pWeaponData )
	{
		if(pWeaponData->nAniType == 3)	// ¼ö·ùÅº
		{
			g_pLTServer->SendWeaponSFXMessage(cMsg.Read(), theStruct.vPos, 0 );
		}
		else
		{
			g_pLTServer->SendWeaponSFXMessage(cMsg.Read(), theStruct.vPos, 0, theStruct.hFiredFrom );
		}
	}
	else
	{
	...
	if( pWeaponData )
	{
		...
		{
			//g_pLTServer->SendSFXMessage(cMsg.Read(), theStruct.vPos, 0, true);
			//g_pLTServer->SendSFXMessage(cMsg.Read(), theStruct.vPos, 0, theStruct.hFiredFrom);
			g_pLTServer->SendWeaponSFXMessage(cMsg.Read(), theStruct.vPos, 0, theStruct.hFiredFrom);
		}
	}
	else
