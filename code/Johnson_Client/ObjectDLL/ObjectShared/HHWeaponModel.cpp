...
void CHHWeaponModel::Setup(CWeapon* pParent, WEAPON const *pWeaponData)
{
	...
	{
		// [MURSUM]===================================================
		// 무기변경일 경우 ObjectShared에서 처리
		//SetObjectFilenames(m_hObject, pWeaponData->szHHModel, pWeaponData->blrHHSkins.GetItem(0));
		SetObjectFilenames( m_hObject,
							pWeaponData->szHHModel,
							pWeaponData->blrHHSkins.GetItem(0),
							0xF000F000 );
		//============================================================
		uint32 nNetFlags;
		g_pLTServer->GetNetFlags(m_hObject, nNetFlags);
		nNetFlags |= NETFLAG_ANIM_HHWEAPON;
		g_pLTServer->SetNetFlags(m_hObject, nNetFlags);
		//g_pLTServer->SetNetFlags(m_hObject, NETFLAG_ANIM_HHWEAPON);		
