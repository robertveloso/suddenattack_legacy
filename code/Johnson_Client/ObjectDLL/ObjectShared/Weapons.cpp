...
LTBOOL CWeapons::AddWeapon(LPBASECLASS pObject, HOBJECT hSender, ILTMessage_Read *pMsg)
{
	...
	{
		g_pLTServer->CPrint("========= AddWeapon Error : defaultAmmoId != nAmmoId [ %d ] IsValidAmmoId [ %d ] ===",
							(pWeapon->nDefaultAmmoId != nAmmoId), IsValidAmmoId(pWeapon->nDefaultAmmoId) );
	}
	
	return AddWeapon(pObject, hSender, nWeaponId, nAmmoId, nAmmo, nAmmoInClip, bForce);
}
...
LTBOOL CWeapons::AddWeapon(LPBASECLASS pObject, HOBJECT hSender, uint8 nWeaponId,
						   uint8 nAmmoId, int nAmmo, uint8 nAmmoInClip, bool bForcePickup)
{
	...
	if (m_pWeapons && m_pWeapons[nWeaponId] && m_pWeapons[nWeaponId]->Have())
	{
		...
		// ±¤½Ä : ¶¥¿¡ ¶³¾îÁ®ÀÖ´Â ÃÑÀ» ½ÀµæÇßÀ» ¶§ ÃÑ¾ËÀÌ ´©ÀûµÇ´Â°ÍÀ» ¸·±â À§ÇØ...
		SetAmmo( nAmmoId, nAmmo );
	}
	else
	{
        bHaveIt   = LTFALSE;        
	...
	m_pWeapons[nWeaponId]->SetAmmoInClip(nAmmoInClip);
	...
	{
        CPlayerObj* pPlayer = dynamic_cast<CPlayerObj*>( g_pLTServer->HandleToObject(m_hCharacter) );
		...
		{
			CAutoMessage cMsg;
			cMsg.Writeuint8(MID_PICKUP_WEAPON);
            cMsg.Writeuint8(nWeaponId);
            cMsg.Writeuint8(m_pWeapons[nWeaponId]->GetAmmoInClip());                
...
bool CWeapons::AddIsAmmoWeapon(uint8 nAmmoId)
{
	...
	if (pAmmo)
	{
		...
		if (pWeapon && pWeapon->bIsAmmo)
		{
			...
			AddWeapon(LTNULL, LTNULL, pWeapon->nId, nAmmoId, 0, 0, true);
			return true;
		}
	}
