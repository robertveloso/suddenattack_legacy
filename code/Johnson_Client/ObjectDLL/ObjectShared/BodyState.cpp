...
#define BS_REMOVE_POWER_UPS	1.0f
#define BS_BODY_EXPLODE_VEL	200.0f
...
void CBodyStateNormal::Update()
{
	...
	float a = m_pBody->GetLifetime();
	float b = m_pBody->GetStarttime();
	float c = m_pBody->GetLifetime();
...
void CBodyStateCrush::Init(Body* pBody)
{
	...
	*/
	HMODELANIM hAni = g_pLTServer->GetAnimIndex(m_pBody->m_hObject, "D_CROUCH");
...
void CBodyStateExplode::Init(Body* pBody)
{
	...
	// 나중에 Animation 을 만들어야 할 듯...
	m_hAniStart = g_pLTServer->GetAnimIndex(m_pBody->m_hObject, "D_BOMB");
    ...
    LTVector vTemp(vDir*BS_BODY_EXPLODE_VEL);
...
void CBodyStateExplode::HandleTouch(HOBJECT hObject)
{
	...
	if (vVelocity.Length() < 5.0f || vVelocity.y < 5.0f )
	{
		...
		{
			// 광식 : SA는 죽는 Animation 이 start 밖에 없어서 나머지는 삭제
			//g_pLTServer->SetModelAnimation(m_pBody->m_hObject, m_hAniStop);
			//g_pLTServer->SetModelLooping(m_pBody->m_hObject, LTFALSE);
...
void CBodyStateExplode::SetRestingState()
{
	...
	if (g_kfDefaultBodyExplosionVelocity < g_kfDefaultBodyExplosionVelocity )
	{
