...
//[SPIKE]
GadgetTarget* g_pGadgetTarget = LTNULL;
	...
	switch( messageID )
	{
		case MID_PRECREATE :
		{
			...
			if( fData == PRECREATE_WORLDFILE || fData == PRECREATE_STRINGPROP )
			{
				...
				//[SPIKE] TimeBomb 초기위치 저장  
				SetInitPosTimeBomb( pStruct->m_Pos );
		
				//Extern
				g_pGadgetTarget = this;
			}
...
void GadgetTarget::HandleStoppedMsg( const CParsedMsg &cMsg, HOBJECT hSender )
{
	if( cMsg.GetArgCount() < 2 ) return;
...
void GadgetTarget::SetupDisablingState( HOBJECT hSender )
{
	...
	if( !pPlayer || m_bRestoreFlagsOnLoad ) return;
...
void GadgetTarget::KillDisablingSound()
{
	if( m_hSound )
	{
