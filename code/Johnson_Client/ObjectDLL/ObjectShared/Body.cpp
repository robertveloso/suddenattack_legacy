...
#define BODY_CAP_RADIUS			500.0f
#define BODY_CAP_RADIUS_COUNT	4
#define BODY_CAP_TOTAL_COUNT	10
#define BODY_EXPLOSION_VEL_MUL	3.0f
...
float g_kfDefaultBodyExplosionVelocity = BODY_EXPLOSION_VEL_MUL;
	...
	if (MID_DAMAGE == messageID)
	{
		...
		if ( !m_damage.IsDead( ) && m_damage.GetLastDamageType() == DT_EXPLODE )
		{
			...
			if ( m_eBodyState == kState_BodyNormal && !m_bPermanentBody )
			{
				...
				m_vDeathDir *= (1.0f + BODY_EXPLOSION_VEL_MUL);
...
void Body::Init(const BODYINITSTRUCT& bi)
{
	...
	// ±¤½Ä
	//-------------------------------------------------------
	m_nHeadSkinId		= bi.pCharacter->GetHeadSkinId();
	m_nBodySkinId		= bi.pCharacter->GetBodySkinId();
	//-------------------------------------------------------
	...
	//[MURSUM]
	m_pParentObj		= (CPlayerObj*)bi.pCharacter;
...
// ±¤½Ä
void Body::FadeBodyInit(const BODYINITSTRUCT& bi)
{
	if (!bi.pCharacter || !bi.pCharacter->m_hObject) return;
...
CBodyState* Body::AI_FACTORY_NEW_State(EnumAIStateType eStateType)
{
	...
	//extern CAIClassAbstract* AIFactoryCreateCBodyStateNormal();
	//return (CBodyStateNormal*)AIFactoryCreateCBodyStateNormal();
	return ( new CBodyStateNormal() );
#endif	
}
...
void Body::SetState(EnumAIStateType eBodyState, LTBOOL bLoad /* = LTFALSE */)
{
	...
	// ±¤½Ä
	m_pState->m_bImmediateBody = m_bImmediateBody;
...
void CAnimatorBody::Twitch()
{
	...
	{
		SetAniTwitch(m_eAniTwitch, m_eAniTrackerTwitch, LTTRUE, LTTRUE);
	}
	else
	{
		SetAni(m_eAniTwitch, m_eAniTrackerTwitch);
...
bool  Body::CanBeSearched()
{
#ifdef USE_AI
	return m_pSearch->IsEnabled();
#else
	return false;
#endif
}
...
void Body::CapNumberOfBodies( )
{
	...
	float fBodyCapRadiusSqr = BODY_CAP_RADIUS;
	fBodyCapRadiusSqr *= fBodyCapRadiusSqr;
	int nBodyCapRadiusCount = Max( 0, BODY_CAP_RADIUS_COUNT );
	int nBodyCapTotalCount = Max( 0, BODY_CAP_TOTAL_COUNT );
...
void Body::CreateSpecialFX()
{
	...
	// ±¤½Ä
	//--------------------------------------------------------
	//cMsg.Writeuint8(m_nHeadSkinId);
	//cMsg.Writeuint8(m_nBodySkinId);
	//--------------------------------------------------------
