...
class CProjectile : public GameBase
{
	public :
		...
		virtual LTBOOL	Setup(CWeapon const* pWeapon, WeaponFireInfo const& info, /*[SPIKE]추가*/uint8 nVertorsPerRoundIndex = 0);
		virtual LTBOOL	SetupHitPlayer( CWeapon const* pWeapon, WeaponFireInfo const& info);
	...
	// should be added here (to keep them together)...
		...
		ModelNode		m_eClientHitNode;

		//------------------------------------------------
		// 광식 [ Hit Node 찾기 위해서 만들었음]
		//------------------------------------------------
		//HOBJECT		m_hHitModel;
		//------------------------------------------------
		LTBOOL			m_bCheckShootThrough;
	...
	private :
		...
		// 
		void DoProjectile();
		void DoVector(uint8 nVertorsPerRoundIndex = 0);
