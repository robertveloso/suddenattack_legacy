...
//	}
	TRACE( "%s",pStruct->m_Name );
	if (m_damage.GetMass() < g_pModelButeMgr->GetModelMass(g_pModelButeMgr->GetModelId(pStruct->m_Name)))
	{
		m_dwFlags |= FLAG_CLIENTNONSOLID;
	}
// Edited By Bestline <==
...
void Prop::HandleAttachmentImpact( CAttachmentPosition *pAttachPos, const LTVector& vDir )
{
	...
	LTVector vVel = vDir;
	vVel.Normalize();
	vVel *= GetRandom( DEFAULT_ATTACH_MIN_VEL, DEFAULT_ATTACH_MAX_VEL );
	vVel.y += GetRandom( DEFAULT_ATTACH_MIN_YADD, DEFAULT_ATTACH_MAX_YADD );
	...
	if( pInfo->m_hObject )
	{
		...
		if( pInfo->m_Plane.m_Normal.y > 0.75f )
		{
			...
			if( m_bRotating )
			{
				...
				m_bRotating = false;
				StartFade( DEFAULT_ATTACH_FADE_DURATION, DEFAULT_ATTACH_FADE_DELAY );
			}
		}
	}
