...
class CCharacter : public GameBase, public IHitBoxUser, public IPlayerTrackerReceiver
{
	public :
		...
		virtual LTBOOL	IsDead()			const { return m_damage.IsDead(); }
		...
		//------------------------------------------------
		uint8		GetHeadSkinId()		{ return m_nHeadSkinId; }
		uint8		GetBodySkinId()		{ return m_nBodySkinId; }
		//------------------------------------------------
		...
		virtual	LTBOOL	SetPitch( float fPitch );
		...
		// 광식
		LTBOOL		WeaponThrow(int nWeaponId = -1, LTVector vPath = LTVector(0.0f,0.0f,0.0f), LTVector vPos = LTVector(0.0f,0.0f,0.0f), int nClass = 0, bool bDeath = true );	// 현재 들고 있는 무기 던지기..
		// Character Hit Box 버그 수정
		virtual void		EnablePitch( bool bEnable );
		virtual void		HandlePitch( float fPitch );
		virtual void		ResetPitchTracking();
	...
	protected :
		...
		// 광식
		//----------------------------------------------------------
		virtual void	CreateFadeBody();
		virtual void	SetupFadeBody(Body* pProp);
		virtual void	RemoveFadeBody();
		virtual void	SetDeathAniAndPos();
		//----------------------------------------------------------
	...
	protected :
		...
		ModelLoadType		m_eModelLoadType;
	...
	// should be added here (to keep them together)...
		...
		// [NOVICE] 피치가 일정량 이상 변했을때만 브로드케스팅 하기 위해
		LTBOOL				m_bLastUpdated;
		...
		// 광식 : Character Hit Box 어긋나는 버그 때문에
		HTRACKEDNODE				m_hLowerTorsoNode;
		CServerTrackedNodeContext	m_TrackedNodeContext;
		// 광식 [05.03.18] : Head skin 과 Body Skin 추가
		//----------------------------------------------
		uint8		m_nHeadSkinId;
		uint8		m_nBodySkinId;
		//----------------------------------------------
		LTObjRef	m_hFadeBody;
		HOBJECT		m_hObjBody;

		LTBOOL		m_bJumped;
		LTBOOL		m_bJumpRequested;
		LTBOOL		m_bJumpReady;
