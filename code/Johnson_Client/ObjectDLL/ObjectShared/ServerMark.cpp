...
#define SM_MARK_WM_SHOW			1.0f
#define SM_MARK_WM_FADE_TIME	3.0f
#define SM_MARK_WM_SOLID_TIME	3.0f
...
uint32 CServerMark::EngineMessageFn(uint32 messageID, void *pData, LTFLOAT fData)
{
	switch(messageID)
	{
		...
		case MID_UPDATE:
		{
			...
			float fSolidTime = SM_MARK_WM_SOLID_TIME;
			float fFadeTime = SM_MARK_WM_FADE_TIME;

			//however, if we are disabled, set our time to 0
			if(SM_MARK_WM_SHOW < 0.99f)
			{
