...
#define MAX_ATTACH_PLAYER	8
#define USED_WEAPON_CLASS	5

//[MURSUM]
#ifndef _FINAL
#define CHECK_PLAYEROBJ_BEGIN()	CPlayerObj::CheckPlayerObjListBegin()
#define CHECK_PLAYEROBJ_END()	CPlayerObj::CheckPlayerObjListEnd()
#else
#define CHECK_PLAYEROBJ_BEGIN()	(void)0
#define CHECK_PLAYEROBJ_END()	(void)0
#endif


struct AttachPlayerInfo
{
	AttachPlayerInfo()
		: hAttachPlayer(0)
	{
...
class CPlayerObj : public CCharacter
{
	public :
		...
		void SetClient(HCLIENT hClient, bool bCreate);
        ...
        void Respawn(bool bSpectator, bool bGetStartPt=true);
		void Revive();
        void CancelRevive( );
		...
		LTBOOL	GetMandateHost()		{ return m_bMandateHost; }
		void InitializeModel(ModelId eModelId, ModelLoadType eModelType );

		//[MURSUM]======================================
		void StartGodMode();
		void StopGodMode();
		LTBOOL	IsGodMode() { return m_bGodMode; }
		LTBOOL	IsDeathAfterRespawn();
		void	SetHostPlayer( bool bHost ) { m_bHostPlayer = bHost; }
		bool	IsSameRound( int nCurrentRound );
		bool	IsHostPlayer() { return m_bHostPlayer; }
		bool	IsMultiplayerInit() { return m_bMultiplayerInit; }
		HOBJECT	GetWeaponModel();
		void	ChangeTeam( uint8 nTeam, uint8 nModelID );
		//==============================================
		...
		void HandleHitMessage(ILTMessage_Read* pMsg);
		...
		void    DoWeaponChange(uint8 nWeaponId, uint8 nAmmoId, uint8 nAmmoCount, uint8 nClipInAmmo);
		...
		// ±¤½Ä
		void TeleportClientToServerPosWithoutDims();
		LTVector GetLastClientPos() { return m_vLastClientPos; }
		...
		static PlayerObjList const& GetPlayerObjListBackup() { return m_lstPlayerObjsBackup; }
		static uint32 GetNumberPlayersWithClients( );

#ifndef _FINAL
		static void CheckPlayerObjListBegin();
		static void CheckPlayerObjListEnd();
#endif
		...
		// ±¤½Ä : Spectator Mode Á¦°Å
		//bool	RequestedTeamChange() {return (m_nRequestedTeam != INVALID_TEAM) && (m_nRequestedTeam != SA_TEAM_SPACTATOR) ; }
		bool	RequestedTeamChange() {return (m_nRequestedTeam != INVALID_TEAM); }

		// ±¤½Ä [04.09.23] : player ÀÇ ÇöÀç ¹«±â ¾Ë¾Æ ¿À±â À§ÇØ Ãß°¡
		CPlayerAttachments* GetPlayerAttachments() { return m_pPlayerAttachments; }
		int32	GetServerIndex() { return m_nServerIndex; }
		
		HCLIENT		AttachPlayer	(HCLIENT hPlayer);
		LTBOOL		IsAttachPlayer	(HCLIENT hPlayer);
		LTBOOL		DetachPlayer	(HCLIENT hPlayer);
		void		DetachAllPlayer	();
		void		AutoMoveAttachProcess();
		int		GetCurrentAttachedWeaponId();
		int		GetCurrentAttachedAmmoId();
		int		GetCurrentAttachedAmmoCount( int nAmmoId );
		int		GetCurrentAttachedAmmoInClip();
		virtual LTBOOL	IsDead()			const { return ( m_damage.IsDead() == LTTRUE || m_eState == PS_GHOST ); }

		void	RestartRemoveBody();

		void	RestartWeaponClass();

		AttachPlayerInfo	m_AttachPlayer[MAX_ATTACH_PLAYER];
		HCLIENT				m_hAttachedPlayer;
		uint8				m_nAttachState;

		LTBOOL				m_bWeaponClass[USED_WEAPON_CLASS];
		uint8				m_nCurWeaponClass;
	...
	protected :
		...
		void UpdateClientPhysics( LTBOOL bRespawn=LTFALSE );		// Update client physics state.
		...
		uint8 DecryptFireValue( uint8 nVal );
	...
	// should be added here (to keep them together)...
		...
		static PlayerObjList	m_lstPlayerObjsBackup;
		...
		LTVector	m_vBackupPos;
		LTVector	m_vBackupPYR;
		LTFLOAT		m_fBackupHP;
		LTFLOAT		m_fBackupAP;
		
		T_BACKUP_WEAPON	m_BackupWeapons[USED_WEAPON_CLASS];

		LTBOOL		m_bDeathAfterRespawn;

		bool		m_bHostPlayer;
		int			m_nEnterRound;
		bool		m_bMultiplayerInit;
		//==============================================

		// ±¤½Ä : µî±Þ Ãß°¡
		//----------------------------------------------
		int32		m_nServerIndex;
		int			m_nManualFireCount;
		uint8		m_nNetAniFlagCount;
