...
class Body : public Prop, public IHitBoxUser
{
	...
	public : // Public methods
		...
		// ±¤½Ä
		void FadeBodyInit(const BODYINITSTRUCT& bi);
		...
		// ±¤½Ä
		//--------------------------------------------------------------------------
		void			SetBodySkinId(uint8 nSkinId) { m_nBodySkinId = nSkinId; }
		void			SetHeadSkinId(uint8 nSkinId) { m_nHeadSkinId = nSkinId; }
		uint8			GetBodySkinId()				 { return m_nBodySkinId; }
		uint8			GetHeadSkinId()				 { return m_nHeadSkinId; }
		void			SetBodyInLiquid(bool bLiquid)	{ m_bBodyInLiquid = bLiquid; }
		bool			IsBodyInLiquid()			 { return m_bBodyInLiquid; }
		//--------------------------------------------------------------------------
		...
		// ±¤½Ä
		void	SetImmediateBody( LTBOOL bImmediate )	{ m_bImmediateBody = bImmediate; }

		// ±¤½Ä : Protected ¿¡ ÀÖ´ø°Í..
		void CreateSpecialFX();

		//[MURSUM]
		CPlayerObj*	GetParentObj() { return m_pParentObj; }
	...
	protected :
		...
		// ±¤½Ä
		LTBOOL				m_bImmediateBody;
		bool				m_bBodyInLiquid;

		CPlayerObj*			m_pParentObj;
};
