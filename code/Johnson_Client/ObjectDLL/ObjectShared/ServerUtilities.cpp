...
{
	return;
    if (!g_pLTServer || FALSE /*g_ShowTimingTrack.GetFloat() < 1.0f*/ ) return;
...
void SendTriggerMsgToObject(LPBASECLASS pSender, LPBASECLASS pObj, LTBOOL, const char* szMessage)
{
	...
	if (FALSE)//g_ShowTriggersTrack.GetFloat() != 0.0f)
	{
...
// ±¤½Ä
LTBOOL NonTeleportMoveObjectToFloor(HOBJECT hObj, HOBJECT *pFilterList, ObjectFilterFn pFilterFn)
{
	if (!hObj) return LTFALSE;
    ...
    if (g_pLTServer->IntersectSegment(&IQuery, &IInfo))
	{
		...
		if (fDist > vDims.y)
		{
			vPos.y -= (fDist - (vDims.y + 5.0f));
			//g_pLTServer->MoveObject(hObj, &vPos);
			g_pLTServer->SetObjectPosNonTeleport(hObj, &vPos);
            return LTTRUE;
		}
	}
