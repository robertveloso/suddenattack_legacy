...
BEGIN_CLASS(TimeBombArea)
    ...
    ADD_COLORPROP_FLAG(LightAdd, 0.0f, 0.0f, 0.0f, PF_HIDDEN)
	PROP_DEFINEGROUP(FogStuff, PF_GROUP(2) | PF_HIDDEN)
END_CLASS_DEFAULT_FLAGS(TimeBombArea, VolumeBrush, NULL, NULL, CF_WORLDMODEL)

//
// Register the class with the command mgr plugin and add any messages to the class
//

CMDMGR_BEGIN_REGISTER_CLASS( TimeBombArea )
CMDMGR_END_REGISTER_CLASS( TimeBombArea, VolumeBrush )
//---------------------------------------------------------------------------------------------
...
// ±¤½Ä
//-------------------------------------------------------------------------------------
void TimeBombArea::ReadProp(ObjectCreateStruct *pStruct)
{
	VolumeBrush::ReadProp(pStruct);
	m_fFriction	= 5.0f;
	m_fGravity = -2000.0f;
