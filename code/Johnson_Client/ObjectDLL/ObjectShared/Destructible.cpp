...
#define DSTB_DAMAGE_SCALE	1.0f
#define DSTB_HEAL_SCALE		1.0f
...
LTBOOL DamageStruct::InitFromMessage(ILTMessage_Read *pMsg)
{
    ...
    fDamage    = pMsg->Readfloat() * DSTB_DAMAGE_SCALE;
	...
	// 광식
	//-------------------------------------------
	nWeaponId	= pMsg->Readuint8();
	//nHitNode	= (ModelNode)pMsg->Readuint32();
	//-------------------------------------------
...
CDestructible::CDestructible() : IAggregate()
{
	...
	m_fRandomVal	= float( GetTickCount()%1000 );
}
...
void CDestructible::AddProgressiveDamage(DamageStruct & damage)
{
	...
	if (IsScoreTagType(damage.eType) && IsPlayer(hDamager) && m_bIsPlayer && !IsTakingProgressiveDamage(damage.eType))
	{
		...
		if (hClientTarget)
		{
			...
			//[MURSUM]
			//Prog Damage를 사용하려면 WeaponID, NodeID를 패킷에 추가해야함!!!	
			/*
			CAutoMessage cMsg;
...
void CDestructible::DoDamage(DamageStruct & damage)
{
	...
	if (fDamage > 0.0f)
	{
		...
		if (GetArmorPoints() > 0.0 && ArmorCanAbsorb(eType))		
		{
			//[SPIKE] + 광식 
			//------------------------------------------------------------------------
			if(eType != DT_MELEE) // Melee Damage 유효하지 않음 
			{
				CPlayerObj* pPlayer = (CPlayerObj*) g_pLTServer->HandleToObject(m_hObject);
				...
				if( pPlayer )
				{
					...
					// 폭발형 데미지 이외에는  몸통만 유효 
					if(eType == DT_EXPLODE || HL_TORSO == g_pModelButeMgr->GetSkeletonNodeLocation(eModelSkeleton, mHitNode))
					{
						fAbsorb = fDamage * (0.6f + GetRandom(0.01f,0.1f));
						fDamage = fDamage - fAbsorb;
					}					
				}
				else
				{
	...
	{
		SetArmorPoints( GetArmorPoints()-fAbsorb );	//m_fArmorPoints -= fAbsorb;

		if (GetArmorPoints() < 0.0f)
		{
			SetArmorPoints(0.0f);
		}

		SetHitPoints( GetHitPoints()-fDamage );	//m_fHitPoints -= fDamage;		
	...
	if ( m_sDamagePercentCommand.length() && (GetHitPoints() < m_fMaxHitPoints * m_fDamagePercentCommand) )
	{
	...
	if (GetHitPoints() <= 1.0f && !m_bNeverDestroy && ( fDamage > 0.f ) )
	{
//		SetHitPoints(0.0f);
		...
		//광식
		//--------------------------------------
		//m_eModelHitNode		= damage.nHitNode;
		m_nWeaponId		= damage.nWeaponId;
		//--------------------------------------
...
void CDestructible::ProcessPlayerDeath(HOBJECT hDamager)
{
	...
	if( pPlayerKiller )
	{
		...
		//---------------------------------------------------------
		// 광식[04.09.22] : 죽은 회수를 표시 하기 위해서 변경 함 
		// 현재는 자살 한 회수를 표시하게 되어 있음
		//---------------------------------------------------------
		/*
		CWeapon* pKillerWeapon = pPlayerKiller->GetPlayerAttachments()->GetWeapon();
		if( !pKillerWeapon )
		{
			g_pLTServer->CPrint( "================= ERROR pKillerWeapon = LTNULL" );
			return;
		}
			
		nWeaponID = pKillerWeapon->GetId();
		*/

		
		if( pPlayerKiller->GetTeamID() != pVictim->GetTeamID() && pPlayerKiller != pVictim )
		{
			pPlayerKiller->GetPlayerScore()->AddFrag();
			pVictim->GetPlayerScore()->AddDeaths();
		}		
 	...
 	if (!pVictim->RequestedTeamChange())
	{
		...
		if (hClientVictim )
		{
			...
			//[MURSUM] REM 
			//cMsg.Writebool( true );
			cMsg.Writeuint8( (uint8)nVictimID);
			cMsg.Writeuint8( (uint8)nKillerID);
			...
			case DT_CHOKE:	//익사
				cMsg.Writeuint8(WMGR_INVALID_ID-1);
				break;
			...
			cMsg.Writeuint8((uint8)pVictim->GetModelNodeLastHit());			
			//cMsg.Writeuint32(nWeaponID);
			//cMsg.Writeuint32(m_eModelHitNode);
			//----------------------------

			g_pLTServer->SendToClient(cMsg.Read(), g_pGameServerShell->GetHostPlayerHandle(), MESSAGE_GUARANTEED);
			//g_pCharacterHitBox->m_eModelNode = eModelNodeInvalid;
		}
	}
}
...
void CDestructible::HandleHeal(LPBASECLASS pObject, HOBJECT hSender, ILTMessage_Read *pMsg)
{
    ...
    LTFLOAT fAmount = pMsg->Readfloat() * DSTB_HEAL_SCALE;
...
LTBOOL CDestructible::Heal(LTFLOAT fAmount)
{
    ...
    if (GetHitPoints() >= fMax) return LTFALSE;

	// only heal what we need to (cap hit points at maximum)

	if (GetHitPoints() + fAmount > fMax)
	{
		fAmount = fMax - GetHitPoints();
	}
...
void CDestructible::DoActualHealing(LTFLOAT fAmount)
{
	...
	SetHitPoints( GetHitPoints()+fAmount );	//m_fHitPoints += fAmount;
}
...
LTBOOL CDestructible::Repair(LTFLOAT fAmount)
{
    ...
    if (GetArmorPoints() >= fMax) return LTFALSE;

	SetArmorPoints( GetArmorPoints()+fAmount );	//m_fArmorPoints += fAmount;

	if (GetArmorPoints() > fMax)
	{
		SetArmorPoints(fMax);
	}
...
void CDestructible::Reset(LTFLOAT fHitPts, LTFLOAT fArmorPts, LTFLOAT fEnergy)
{
	...
	SetHitPoints( fHitPts <= fMaxHitPoints ? fHitPts : fMaxHitPoints );
	SetArmorPoints( fArmorPts <= fMaxArmorPoints ? fArmorPts : fMaxArmorPoints );
...
void CDestructible::SetArmorPoints(LTFLOAT fap)
{ 
#ifdef USE_RANDOM_VEL
	m_fArmorPoints = fap+m_fRandomVal;
#else
	m_fArmorPoints = fap;
#endif
}

LTFLOAT CDestructible::GetArmorPoints() const
{
#ifdef USE_RANDOM_VEL
	return m_fArmorPoints-m_fRandomVal;
#else
	return m_fArmorPoints;
#endif
}

void CDestructible::SetHitPoints(LTFLOAT fhp)
{
#ifdef USE_RANDOM_VEL
	m_fHitPoints = fhp+m_fRandomVal;
#else
	m_fHitPoints = fhp; 
#endif
}

LTFLOAT CDestructible::GetHitPoints() const
{
#ifdef USE_RANDOM_VEL
	return m_fHitPoints-m_fRandomVal;
#else
	return m_fHitPoints; 
#endif
}
