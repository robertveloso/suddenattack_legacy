...
#define CHAR_LOWER_TORSO_PITCH_MAX	45.0f
#define CHAR_PITCH_SCALE			0.5f
#define CHAR_PITCH_BIAS				-3.0f
...
#define CHAR_BODY_STICK_ANGLE	0.8f
#define CHAR_BODY_STICK_DIST	150.0f
...
CCharacter::~CCharacter()
{
	...
	{
		g_pServerTrackedNodeMgr->DestroyTrackingNode( m_hLowerTorsoNode );
		m_hLowerTorsoNode = INVALID_TRACKEDNODE;
	}
	//--------------------------------------------------------------------------------
}
...
uint32 CCharacter::ObjectMessageFn(HOBJECT hSender, ILTMessage_Read *pMsg)
{
	...
	//-------------------------------------------------
	pMsg->SeekTo(0);
	messageID = pMsg->Readuint8();
	switch( messageID )
	{
	case MID_PROJECTILE:
		{
			uint8 nType = pMsg->Readuint8();
			if( nType == MPROJ_WEAPON_THROW )
			{
				uint8 nWeaponId = pMsg->Readuint8();
				LTVector vPath	= pMsg->ReadLTVector();
				LTVector vPos	= pMsg->ReadLTVector();
				uint8 nClass	= pMsg->Readuint8();
				WeaponThrow( nWeaponId , vPath , vPos , nClass, false);
			}
		}break;
	}
	//-------------------------------------------------
...
uint32 CCharacter::GetUserFlagSurfaceType()
{
	...
	ModelNode mHitNode = pPlayer->GetModelNodeLastHit();
	if(mHitNode == eModelNodeInvalid) return LTNULL;
	ModelSkeleton eModelSkeleton = pPlayer->GetModelSkeleton();
	...
	if(HL_HEAD == g_pModelButeMgr->GetSkeletonNodeLocation(eModelSkeleton, mHitNode))	
	{
		eType =	g_pModelButeMgr->GetHeadSurfaceType(m_eModelId);
	}
...
LTBOOL CCharacter::ShouldWallStick()
{
	...
	IQuery.m_From	  = vPos;
	IQuery.m_To		  = (vPos - (fDir * vForward * CHAR_BODY_STICK_DIST));
	...
	if ( g_pLTServer->IntersectSegment(&IQuery, &IInfo) )
	{
		...
		if (pSurf && (pSurf->eType != ST_SKY))
		{
			...
			vForward *= fDir;
			if ( IInfo.m_Plane.Normal().Dot(vForward) > CHAR_BODY_STICK_ANGLE )
			{
				g_pLTServer->CPrint("plane dot charfwd > %.2f", CHAR_BODY_STICK_ANGLE);
...
//    g_pCommonLT->SetObjectFlags(m_hObject, OFT_Flags2, FLAG2_DYNAMICDIRLIGHT, FLAGMASK_ALL);
	...
	// 광식 : server 에서 Model의 tracking을 하려고 수정 함.. character 의 hit box가 client 와
	//        다르기 때문에 targeting 시 잘 맞지 않는 문제 발생
	//--------------------------------------------------------------------------------------------
	//ResetPitchTracking();	
	//--------------------------------------------------------------------------------------------
}
...
void CCharacter::CreateSpecialFX(LTBOOL bUpdateClients /* =LTFALSE */)
{
	...
	m_pcs->eModelLoadType			= m_eModelLoadType;
...
void CCharacter::SetInstantDamageFlags( const DamageFlags nDmgFlags )
{
	...
	//[MURSUM] CCharacter::UpdateDamageFX 사용안함!
	/*
	CAutoMessage cMsg;
	cMsg.Writeuint8( MID_SFX_MESSAGE );
	cMsg.Writeuint8( SFX_CHARACTER_ID );
...
LTBOOL CCharacter::SetPitch( float fPitch )
{
	// [NOVICE] //////////////////////////////////////////////////////////////
	if ( m_fPitch == fPitch )
		m_bLastUpdated = LTFALSE;

	float fIgnoreDelta = ( m_bLastUpdated ) ? 0.0f : 0.28f;
	//////////////////////////////////////////////////////////////////////////
	

	// We only care about player pitch if we are in a multiplayer game...
#ifndef GAME_SHOW
	if( fabs(fPitch - m_fPitch) > fIgnoreDelta )
#endif
	{
		m_fPitch = fPitch;

		// 광식 : Character Hit Box 어긋나는 bug 수정
		//----------------------------------------------------------
		HandlePitch(m_fPitch);
		//----------------------------------------------------------
...
*/
		...
		// [NOVICE]	//////////////////////////////////////////////////////////////
		m_bLastUpdated = LTTRUE;
		//////////////////////////////////////////////////////////////////////////
	}

	return m_bLastUpdated;
}
...
void CCharacter::InitAnimation()
{
	...
	if ( m_bShortRecoil )
	{
		...
		// 광식
		uint32 nAniLength;
		g_pModelLT->GetAnimLength(m_hObject, 0, nAniLength);
	}
	...
	if ( m_bShortRecoiling )
	{
        ...
        if ( LT_OK == g_pModelLT->GetPlaybackState(m_hObject, m_RecoilAnimTracker, dwFlags) )
		{
			if ( MS_PLAYDONE & dwFlags )
			{
				...
				// 광식
				//---------------------------------------------------------------------------
				g_pModelLT->SetCurAnim(m_hObject, m_RecoilAnimTracker, 0);
				g_pModelLT->SetWeightSet(m_hObject, m_RecoilAnimTracker, m_hTwitchWeightset);
...
HMODELANIM CCharacter::GetDeathAni(LTBOOL bFront)
{
	...
	// 물속에서 죽을 때
	if( m_bBodyWasInLiquid )
	{
		hAni = g_pLTServer->GetAnimIndex( m_hObject, "D_SwimDeath");
		if( hAni == INVALID_ANI )
		{
			g_pLTServer->CPrint( "Character Liquid is missing animation Death." );
	...
	if ( bFront )
	{
		...
		{
			hAni = g_pLTServer->GetAnimIndex( m_hObject, "D_BOMB");
		}
...
void CCharacter::PlayDialogSound(const char* pSound, CharacterSoundType eType)
{
	...
	if (bLipSync || DoDialogueSubtitles())
	{
...
void CCharacter::CreateBody()
{
	...
	if( m_bBodyWasInLiquid )
	{
		...
		if( pPlayerObj )
		{
			vPos = pPlayerObj->GetLastClientPos();
		}
	}
	...
	// 광식 : 머리 날리기 Test
	//------------------------------------------------------------------------------------------
	if( HL_HEAD == g_pModelButeMgr->GetSkeletonNodeLocation(GetModelSkeleton(), m_eModelNodeLastHit) )
	{		
		strcpy(theStruct.m_RenderStyleNames[0], theStruct.m_RenderStyleNames[2]);
	}
	//------------------------------------------------------------------------------------------
	*/
...
//	LTSNPrintF(theStruct.m_Name, sizeof(theStruct.m_Name), "%s_body", GetObjectName(m_hObject));
	LTSNPrintF(theStruct.m_Name, sizeof(theStruct.m_Name), "%s", GetObjectName(m_hObject));
...
LTBOOL CCharacter::SetDims(LTVector* pvDims, LTBOOL bSetLargest)
{
	...
	//[MURSUM] 사라지는 문제~~
	if( pvDims->x < 24.0f ) pvDims->x = 24.0f;
	if( pvDims->y < 53.0f ) pvDims->y = 53.0f;
	if( pvDims->z < 24.0f ) pvDims->z = 24.0f;
	//ASSERT( pvDims->x >= 24.0f );
	//ASSERT( pvDims->y >= 53.0f );
	//ASSERT( pvDims->z >= 24.0f );
...
LTBOOL CCharacter::HitFromFront(const LTVector& vDir)
{
	...
	if((vDir.Dot(rRot.Forward()) < 0.0f) && (fabs(vDir.Dot(rRot.Forward())) > 0.65f))
	{
		return LTTRUE;
	}
	else
			...
			if ( LT_OK == g_pModelLT->GetNodeTransform(m_hObject, hNode, transform, LTTRUE) )
			{
				...
				if ( LT_OK == g_pLTServer->CreateAttachment(m_hObject, hSpear, szNode, &vAttachment, &rAttachment, &hAttachment) )
				{
					...
					if ( bCanWallStick && m_damage.IsDead() && (NODEFLAG_WALLSTICK & g_pModelButeMgr->GetSkeletonNodeFlags(m_eModelSkeleton, eModelNode)) )
					{
						...
						if ( fForwardThreshhold > CHAR_BODY_STICK_ANGLE )
						{
							bTryWallStick = true;
							m_bStuckInFront = true;
						}
						else if ( fBackwardThreshhold > CHAR_BODY_STICK_ANGLE )
						{
							bTryWallStick = true;
...
// --------------------------------------------------------------------------- //
//
//	ROUTINE:	CCharacter::WeaponThrow
//
//	PURPOSE:	광식 : 현재 들고 있는 무기를 world 로 던질 때.
//
// --------------------------------------------------------------------------- //
LTBOOL CCharacter::WeaponThrow(int nWeaponId, LTVector vPath, LTVector vPos, int nClass, bool bDeath)
{
	ASSERT( nClass >= 0 && nClass < USED_WEAPON_CLASS );
	CPlayerObj* pPlayerObj = dynamic_cast< CPlayerObj* >( g_pLTServer->HandleToObject( m_hObject ));
	//ASSERT(pPlayerObj);
	if( !pPlayerObj ) return LTFALSE;
	CPlayerAttachments* PlayerAttach = pPlayerObj->GetPlayerAttachments();
	//ASSERT(PlayerAttach);
	if( !PlayerAttach ) return LTFALSE;
	CWeapon* PlayerWeapon = PlayerAttach->GetWeapon();
	//ASSERT(PlayerWeapon);
	if( !PlayerWeapon ) return LTFALSE;
	
	if( g_pWeaponMgr->GetWeaponFireType( PlayerWeapon->GetId() ) == 3 ) return LTFALSE;	// 수류탄

	WEAPON const *pW = g_pWeaponMgr->GetWeapon(PlayerWeapon->GetId());
	if( pW )
	{
		if( pW->nAniType == CAnimatorPlayer::eMelee || pW->nAniType == CAnimatorPlayer::eThrow )
			return LTFALSE;
	}
	
	//[MURSUM] int AmmoID = PlayerWeapon->GetAmmoId();
	// AmmoID가 255로 날라와서 총을 줍지 못하는 경우가 생김.
	int AmmoID = pW->nDefaultAmmoId;

	int AmmoCount = PlayerAttach->GetAmmoCount(AmmoID);
	//LTVector vPos;
    LTRotation rRot;
	...
	ocs.m_Flags = FLAG_FORCECLIENTUPDATE |
				  FLAG_POINTCOLLIDE |
				  FLAG_NOSLIDING |
				  FLAG_TOUCH_NOTIFY |
				  /*FLAG_BOXPHYSICS |*/
	...
	ocs.m_Rotation   = rRot;
    ocs.m_Pos        = vPos;
	//ocs.m_Pos        = vPos + ( vPath * 40.0f);
    
	WEAPON* tempWeapon = LTNULL;
	if( nWeaponId == -1 )
	{
		tempWeapon = (WEAPON*)g_pWeaponMgr->GetWeapon( PlayerWeapon->GetId() );
		g_pLTServer->GetObjectPos(m_hObject, &vPos);		
		ocs.m_Pos	= vPos;
		g_pLTServer->GetObjectRotation(m_hObject, &rRot);
		vPath = rRot.Forward();
		...
		if( bDeath )
		{
			...
			IQuery.m_From	= ocs.m_Pos;
			IQuery.m_To		= ocs.m_Pos + LTVector(0.0f, -10000.0f, 0.0f);
			...
			if (g_pLTServer->IntersectSegment(&IQuery, &IInfo))
			{
				//LTFLOAT fDist = ocs.m_Pos.y - IInfo.m_Point.y;
				//ocs.m_Pos.y -= (fDist - (5 + 0.1f));
				ocs.m_Pos = IInfo.m_Point;
			}
		}
		ASSERT( nClass >= 0 && nClass < USED_WEAPON_CLASS );
		nClass		= pPlayerObj->m_nCurWeaponClass;
	}
	else
	{
		tempWeapon = (WEAPON*)g_pWeaponMgr->GetWeapon( nWeaponId );
	}

	//tempWeapon->blrHHSkins.CopyList(0, ocs.m_SkinNames[0], MAX_CS_FILENAME_LEN+1);
	/*
	char szSkin[MAX_CS_FILENAME_LEN] = {0, };	

	if( tempWeapon == LTNULL )
	{
		return LTFALSE;
	}
	else
	{
		tempWeapon->blrHHSkins.CopyList(0, szSkin, MAX_CS_FILENAME_LEN);		
		ASSERT(strlen(tempWeapon->szHHModel) < 127);
		ASSERT(strlen(szSkin) < 127);
		
		strncpy(ocs.m_Filename, tempWeapon->szHHModel, MAX_CS_FILENAME_LEN -1);
		strncpy(ocs.m_SkinName, szSkin, MAX_CS_FILENAME_LEN -1);
	}
	*/
	if( tempWeapon == LTNULL )
	{
	...
	// Create Projectile
    HCLASS hClass = g_pLTServer->GetClass("CProjectileWeapon");
    CProjectileWeapon* pObject = (CProjectileWeapon*)g_pLTServer->CreateObject(hClass, &ocs);
	if( pObject )
	{
		pObject->SetWeaponId(PlayerWeapon->GetId());
		pObject->SetAmmoId(AmmoID);
		pObject->SetAmmo(AmmoCount);
		pObject->SetWeaponClass(nClass);
		pObject->SetAmmoInClip(PlayerWeapon->GetAmmoInClip());
		
		//MoveObjectToFloor(pObject->m_hObject);	

		//LTVector Dims (16.0f, 16.0f, 16.0f);
		//g_pPhysicsLT->SetObjectDims (pObject->GetHOBJECT (), &Dims, 0 );
		// 던지는 형태로 만들기 위해 수정
		PROJECTILEWEAPON_INFO weaponInfo;
		weaponInfo.m_vStartPos = ocs.m_Pos;
		if( nWeaponId == -1 )
		{
			weaponInfo.m_fVelocity = 400;
		}
		else
		{
			weaponInfo.m_fVelocity = 800;
		}
		//weaponInfo.m_fVelocity = 800;
		weaponInfo.m_vDir = vPath;
		pObject->WeaponInfoSetUp( weaponInfo );
		
		// Targeting 되었을 때 USE 문구 나오도록 하는 Flag.. 일단 삭제..
		//------------------------------------------------------------------------------------------------------
		//g_pCommonLT->SetObjectFlags( pObject->m_hObject, OFT_User, USRFLG_CAN_ACTIVATE, USRFLG_CAN_ACTIVATE );
		//------------------------------------------------------------------------------------------------------

		pPlayerObj->m_bWeaponClass[nClass] = LTFALSE;
	}	
...
// 광식
void CCharacter::EnablePitch( bool bEnable )
{
	if( m_hLowerTorsoNode != INVALID_TRACKEDNODE )
	{
		g_pServerTrackedNodeMgr->EnableTracking( m_hLowerTorsoNode, bEnable );
	}
}

void CCharacter::HandlePitch( float fPitch )
{
	LTRotation	rUpperTorsoRot = LTRotation(LTVector(1.0f, 0.0f, 0.0f), fPitch * CHAR_PITCH_SCALE + DEG2RAD(CHAR_PITCH_BIAS));
	LTVector vLookAt = rUpperTorsoRot.Forward() * 1000.0f;
	
	g_pServerTrackedNodeMgr->SetOrientOnAnim( m_hLowerTorsoNode, true );
	g_pServerTrackedNodeMgr->SetTargetObject( m_hLowerTorsoNode, vLookAt );
}

void CCharacter::ResetPitchTracking()
{
	/*
	if( m_eModelSkeleton != eModelSkeletonInvalid )
	{
		if( m_TrackedNodeContext.IsInited() ) m_TrackedNodeContext.Term();
		m_TrackedNodeContext.Init( m_hObject, m_eModelSkeleton, g_pServerTrackedNodeMgr );
	}
	*/
	...
	m_hLowerTorsoNode = g_pServerTrackedNodeMgr->CreateTrackingNode( m_hObject, "Bip01 Spine1" );
	if( m_hLowerTorsoNode != INVALID_TRACKEDNODE )
	{
		float fMax = CHAR_LOWER_TORSO_PITCH_MAX;
		
		g_pServerTrackedNodeMgr->SetNodeConstraints( m_hLowerTorsoNode, 0.0f, 0.0f, DEG2RAD(fMax), DEG2RAD(fMax), DEG2RAD(180.0f) );
		g_pServerTrackedNodeMgr->EnableTracking( m_hLowerTorsoNode, true );
		g_pServerTrackedNodeMgr->SetOrientOnAnim( m_hLowerTorsoNode, true );
	}
	//EnablePitch( true );
}

void CCharacter::CreateFadeBody()
{
	LTVector vPos;
	g_pLTServer->GetObjectPos(m_hObject, &vPos);
	...
	pProp->HideBody(true);
	//m_hFadeBody = pProp->m_hObject;	
	m_hObjBody = pProp->m_hObject;	
}

void CCharacter::SetupFadeBody(Body* pProp)
{
	if( !pProp )
		return;

	bool bRevivableBody = true;	
...
void CCharacter::RemoveFadeBody()
{
	if( m_hFadeBody )
		g_pLTServer->RemoveObject(m_hFadeBody);

	m_hFadeBody = LTNULL;
}

void CCharacter::SetDeathAniAndPos()
{
	/*
	HMODELANIM hAni = g_pLTServer->GetModelAnimation(this->m_hObject);
	g_pLTServer->SetModelAnimation(m_hFadeBody, hAni);
    g_pLTServer->SetModelLooping(m_hFadeBody, LTFALSE);
	g_pLTServer->SetModelPlaying( m_hFadeBody, LTTRUE );

	ANIMTRACKERID nTracker;
	if( LT_OK == g_pModelLT->GetMainTracker( this->m_hObject, nTracker ) )
	{
		uint32 nTime;
		g_pModelLT->GetCurAnimTime( this->m_hObject, nTracker, nTime );
		nTime += (uint32)( g_pLTServer->GetFrameTime() * 1000.f );

		if( LT_OK == g_pModelLT->GetMainTracker( m_hFadeBody, nTracker ) )
		{
			g_pModelLT->SetCurAnimTime( m_hFadeBody, nTracker, nTime );
		}
	}
	
	LTVector vPos;
	g_pLTServer->GetObjectPos(this->m_hObject, &vPos);
	g_pLTServer->SetObjectPos(m_hFadeBody, &vPos);

	Body *pBody = dynamic_cast<Body*>(g_pLTServer->HandleToObject( m_hFadeBody ));
	if( pBody )
	{
	...
	*/
	HMODELANIM hAni = g_pLTServer->GetModelAnimation(this->m_hObject);
	g_pLTServer->SetModelAnimation(m_hObjBody, hAni);
    g_pLTServer->SetModelLooping(m_hObjBody, LTFALSE);
	g_pLTServer->SetModelPlaying( m_hObjBody, LTTRUE );
	...
	if( LT_OK == g_pModelLT->GetMainTracker( this->m_hObject, nTracker ) )
	{
		...
		if( LT_OK == g_pModelLT->GetMainTracker( m_hObjBody, nTracker ) )
		{
			g_pModelLT->SetCurAnimTime( m_hObjBody, nTracker, nTime );
		}
	}
	
	LTVector vPos;
	g_pLTServer->GetObjectPos(this->m_hObject, &vPos);
	g_pLTServer->SetObjectPos(m_hObjBody, &vPos);

	Body *pBody = dynamic_cast<Body*>(g_pLTServer->HandleToObject( m_hObjBody ));
	if( pBody )
	{
		pBody->HideBody(false);
	}
...
uint8 CCharacter::GetClientID()
{
	if( m_pcs ) return m_pcs->nClientID;
	return 255;
}