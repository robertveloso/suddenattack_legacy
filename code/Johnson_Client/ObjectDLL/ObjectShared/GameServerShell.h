...
class CGameServerShell : public IServerShellStub
{
	public :
		...
		//[MURSUM]=======================================================
		bool		IsDedicated() const { return m_ServerGameOptions.m_bDedicated; }

		void		SetResultState( bool bEnable ) { m_bResultState	= bEnable; }
		bool		IsResultState() const { return m_bResultState; }
		
		CPlayerObj*	GetHostPlayer();
		HCLIENT		GetHostPlayerHandle();
		void		SetMissionInfo(ILTMessage_Read *pMsg);
		void		HandlePlayerAttachMessage		(HCLIENT, ILTMessage_Read*);
		//===============================================================
		// ±¤½Ä		
		LTBOOL		FirstAttachProcess( CPlayerObj* pPlayer );
		LTBOOL		ThirdAttachProcess( CPlayerObj* pPlayer );
		LTBOOL		AttachAfterDetach( CPlayerObj* pPlayer );
		LTBOOL		AttachProcess( CPlayerObj* pPlayer );
		LTBOOL		DetachProcess( HCLIENT hPlayer, uint8 Msg );
		LTBOOL		ObserverProcess( HCLIENT hPlayer, uint8 Msg );
		//===============================================================
		...
		bool		StartClientTCP( const char* lpszWorldName );
		...
		// Added By kwangsik ==>
		// Time : 2004-07-09 ¿ÀÈÄ 3:00:38
		int			 GetCurrentClientTotleNumber() { return m_ClientDataList.size(); }
		ClientData*  GetCurrentClientElement(int number) { return m_ClientDataList[number]; }
		// Added By kwangsik <==
	...
	protected :
		...
		void		HandleHitPlayer					(HCLIENT, ILTMessage_Read*);
...
#endif
		void		HandleServerConsole				(HCLIENT, ILTMessage_Read*);		
		...
		void		HandlePlayerKickBan				(HCLIENT, ILTMessage_Read*);
		void		HandleChangeTeam				(HCLIENT, ILTMessage_Read*);
		...
		virtual CPlayerObj* CreatePlayerExt(HCLIENT hClient, ModelId ePlayerModelId, ModelLoadType eModelType) = 0;
		void RespawnPlayer(CPlayerObj* pPlayer, HCLIENT hClient,bool bCreateFX);
		...
		//[MURSUM]
		bool	KickBanIfExist( int32 nServerIndex );
	...
	protected:
		...
		// CRC for cshell.dll
		uint32		m_nCShellCRC;
		uint32		m_nModelButesCRC;
		uint32		m_nSurfaceCRC;
		uint32		m_nDamageFxCRC;
