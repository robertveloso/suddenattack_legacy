...
{
//[SPIKE] 게임이 끝나면 TimeBomb Timer초기화 ======================
	if(g_pServerMissionMgr->GetTimeBomb_InitTimer()) // 누군가가 설치 했다면 들어오면 안됨!!!
	{
		SetupDefusedState( false );  //  TimeBomb 해제 
...
{
	//[SPIKE] TimeBomb이 해체된 상황이니까 모두 삭제...
...
*/	///////////////////////////////////////////////////////////////
    ...
    //////////////////////////cMsg.WriteCompPos(vImpactPos);		// 광식
...
//	g_pCommonLT->SetObjectFlags( m_hTimeBombModel, OFT_User, USRFLG_GLOW, USRFLG_GLOW );
	...
	g_pServerMissionMgr->SetTimeBomb( LTTRUE, m_fDetonateTime, &vPos ); // TimeBomb이 설치된 상태 
...
void Bombable::ForcePlantedTimeBomb( float fTime )
{
	...
	g_pServerMissionMgr->SetTimeBomb( LTTRUE, fTime, &vPos ); // TimeBomb이 설치된 상태 
}	
...
void Bombable::HandleStoppedMsg( const CParsedMsg &cMsg, HOBJECT hSender)
{
	...
	if( !m_SoundItemList.empty() )
	{
		...
		while (itr!=m_SoundItemList.end()) 
		{
			...
			if( pSoundInfo->m_hSISound && pSoundInfo->m_hSenderObject == hSender)
			{
				...
				m_SoundItemList.erase(itr); // erase()내부적으로 삭제한 다음값을 리턴! 
			}
			else 
			{
				itr++;
	...
	//[SPIKE 주석] Defuse 했다 취소시 폭탄이 안터지는 현상때문에
		m_bCanDeactivate = false;
	}
	else
	{
