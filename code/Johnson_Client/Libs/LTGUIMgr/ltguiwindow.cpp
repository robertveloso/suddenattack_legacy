...
// ±¤½Ä
LTBOOL CLTGUIWindow::Create (HTEXTURE hFrame, uint16 nWidth, uint16 nHeight, float xRate, float yRate, LTBOOL bSimpleStretch )
{
	m_nHeight=nHeight;
	m_nWidth=nWidth;

	m_Frame.Create(hFrame, nWidth, nHeight, xRate, yRate, bSimpleStretch);
...
LTBOOL CLTGUIWindow::OnLButtonDown(int x, int y)
{
	...
	if(pCtrl)
	{
		...
		PlayClickSound( (CLTGUICtrl*)m_controlArray[nControlIndex] );
        return LTTRUE;
	}
	else
	{
...
LTBOOL CLTGUIWindow::OnMouseMove(int x, int y)
{
	...
	if(pCtrl)
	{
		...
		//[MURSUM]============================================
		//if (pCtrl->OnMouseMove(x,y)) return LTTRUE;
	...
	else
	{
		if (m_nCurrentIndex < m_controlArray.size() )
		{
			...
			m_nCurrentIndex = m_controlArray.size()+1;
		}
	}
...
LTBOOL   
CLTGUIWindow::OnWheelUp(int x, int y)
{
	return LTFALSE;
}

LTBOOL   
CLTGUIWindow::OnWheelDown(int x, int y)
{
	return LTFALSE;
}
...
uint16 CLTGUIWindow::AddControl ( CLTGUICtrl *pControl)
{
	ASSERT(pControl);
	if (!pControl) return kNoSelection;
...
uint16 CLTGUIWindow::SetSelection( uint16 nIndex )
{
	...
	PlaySelectSound( (CLTGUICtrl*)m_controlArray[m_nCurrentIndex] );
...
CLTGUICtrl *CLTGUIWindow::GetControlUnderPoint(int xPos, int yPos, uint16 *pnIndex)
{
	...
	*/
	...
	for (uint16 i = 0; i < m_controlArray.size(); i++)
	{
		...
		// Check to see if the click is in the bounding box for the control
		if ( m_controlArray[ndx]->IsOnMe(xPos,yPos) && m_controlArray[ndx]->IsEnabled() && !pCtrl )
		{
			*pnIndex=ndx;

			pCtrl = (CLTGUICtrl*)m_controlArray[ndx];
		}
	}
...
void CLTGUIWindow::SetBasePos ( int nPosX, int nPosY )
{
	LTIntPt ptWndPos;
	ptWndPos.x = nPosX;
	ptWndPos.y = nPosY;
