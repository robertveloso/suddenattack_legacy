...
// ±¤½Ä
LTBOOL	CLTGUIFrame::Create (HTEXTURE hFrame, uint16 nWidth, uint16 nHeight, float xRate, float yRate, LTBOOL bSimpleStretch)
{
	if (!hFrame || !nHeight || !nWidth) return LTFALSE;
...
{
	// Sanity checks...
	if (!IsVisible() || LTNULL==m_hFrame) return;
...
// ±¤½Ä
void CLTGUIFrame::InitPolies(float xRate, float yRate)
{
	ScalePolies();

	if (m_bSimpleStretch)
	{
		SetupQuadUVs(m_Poly[0], m_hFrame, 0.0f, 0.0f, xRate, yRate);
...
void CLTGUIFrame::SetFrame(HTEXTURE hFrame, LTBOOL bRate)
{
	if( hFrame )
	{
		m_hFrame	= hFrame;
