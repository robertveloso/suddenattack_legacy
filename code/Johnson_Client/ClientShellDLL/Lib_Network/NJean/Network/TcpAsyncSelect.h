...
//					INT							nServicerIndex;
			...
			public:
				// 다른 서버와 연결을 한다.
				virtual BOOL		Connect( unsigned long lpOwnerKey, const sockaddr* addr, INT namelen );
			...
			public:
				...
				//virtual char*		GetSendBufferRef( unsigned long lpNetworkKey );
				...
				//		Ret : 전송 성공 여부
				virtual BOOL		Send( unsigned long lpNetworkKey, const char* sendBuff, INT nLen, INT& nOutError );

				// 해당 키의 소켓 객체를 닫는다
				//		In	: lpNetworkKey - IOCP에 등록한 Completion Key
				virtual void		CloseSocket( unsigned long lpNetworkKey );

				// 해당키의 소켓 객체를 Shutdown한다.
				//		In	: lpNetworkKey - IOCP에 등록한 Completion Key
				virtual void		ShutDown( unsigned long lpNetworkKey );

				virtual void	SetConnectRecvEvent			( unsigned long lpCompletionKey );
				virtual void	ResetConnectRecvEvent		( unsigned long lpCompletionKey );
				virtual void	WaitConnectRecvEvent		( unsigned long lpCompletionKey );
				virtual void	SetRecvDisconnectEvent		( unsigned long lpCompletionKey );
				virtual void	ResetRecvDisconnectEvent	( unsigned long lpCompletionKey );
				virtual void	WaitRecvDisconnectEvent		( unsigned long lpCompletionKey );

				virtual void	ResetRttTime				( unsigned long lpNetworkKey );
				virtual BOOL	CanStartRTT					( unsigned long lpNetworkKey );
				virtual void	SetRttStartTime				( unsigned long lpNetworkKey );
				virtual void	SetRttEndTime				( unsigned long lpNetworkKey );
				virtual void	GetRttTime					( unsigned long lpNetworkKey, DWORD& out_dwRttTime );

				// 해당 키의 Servicer 인덱스를 가져온다.
				virtual INT			GetServicerIndex( unsigned long lpNetworkKey )
				{
				...
				virtual void		SetServicerIndex( unsigned long lpNetworkKey, INT nServicerIndex )
				{
					CCommSocket* pCommSocket;
					pCommSocket = (CCommSocket*)lpNetworkKey;
				...
				virtual unsigned long GetOwnerKey( unsigned long lpNetworkKey )
				{
					CCommSocket* pSocket = (CCommSocket*)lpNetworkKey;
					return pSocket->GetOwnerKey();
				}

				virtual void SetOwnerKey( unsigned long lpNetworkKey, unsigned long lpOwnerKey )
				{
					CCommSocket* pSocket = (CCommSocket*)lpNetworkKey;
					pSocket->SetOwnerKey( lpOwnerKey );
				}

				virtual void GetAssemblerKey( unsigned long lpNetworkKey, unsigned long& lpAssemblerKey )
				{
					DWORD dwAssemblerHandle = 0;
					CCommSocket* pSocket = (CCommSocket*)lpNetworkKey;
					pSocket->GetAssemblerKey( lpAssemblerKey, dwAssemblerHandle );
				}

				virtual void SetAssemblerKey( unsigned long lpNetworkKey, unsigned long lpAssemblerKey )
				{
					DWORD dwAssemblerHandle = 0;
					CCommSocket* pSocket = (CCommSocket*)lpNetworkKey;
				...
				virtual void FreeNetworkKey( unsigned long lpNetworkKey )
				{
					CCommSocket*						pCommSocket;
				...
				virtual LPCTSTR GetLocalAddr( unsigned long lpNetworkKey )
				{
					CCommSocket* pSocket = (CCommSocket*)lpNetworkKey;
					return pSocket->GetLocalAddr();
				}

				virtual LPCTSTR GetPeerAddr( unsigned long lpNetworkKey )
				{
					CCommSocket* pSocket = (CCommSocket*)lpNetworkKey;
			...
			private:
				...
				/*
				// 등록된 IO핸들 처리중 Accept
				void	ProcessAcceptCompletion( DWORD dwNumberOfBytesTransferred, unsigned long lpNetworkKey, LPOVERLAPPED lpOverlapped );
				// 등록된 IO핸들 처리중 Accept and Recv
				void	ProcessAcceptRecvCompletion( DWORD dwNumberOfBytesTransferred, unsigned long lpNetworkKey, LPOVERLAPPED lpOverlapped );
				// 등록된 IO핸들 처리중 Recv
				void	ProcessRecvCompletion( DWORD dwNumberOfBytesTransferred, unsigned long lpNetworkKey, LPOVERLAPPED lpOverlapped );
				// 등록된 IO핸들 처리중 Send
				void	ProcessSendCompletion( DWORD dwNumberOfBytesTransferred, unsigned long lpNetworkKey, LPOVERLAPPED lpOverlapped );
				*/
			...
			private:
				...
				virtual void OnGetQueuedCompletionStatus( BOOL bRet, DWORD dwNumberOfBytesTransferred, unsigned long lpNetworkKey, LPOVERLAPPED lpOverlapped );
				*/
