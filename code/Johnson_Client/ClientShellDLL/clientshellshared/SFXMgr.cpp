...
static unsigned int s_nDynArrayMaxNums[DYN_ARRAY_SIZE] =
{
	...
	50,		// Sprites/Models (base scale) ÇÇ, ¹ßÀÚ±¹, ±é½º?
	100,	// Debris
	50,		// Death
	50,		// Gibs
	50,		// Projectile
	30,		// Marks - bullet holes
	100,	// Light
	50,		// Random sparks
	50,		// Pickup item
	16,		// Character
...
//[MURSUM]===================================================================
void CSFXMgr::RestartFX()
{
	int i, j, nNumSFX;

	for (j=0; j < DYN_ARRAY_SIZE; j++)
	{
		if( j == SFX_CHARACTER_ID )
		{
			nNumSFX  = m_dynSFXLists[j].GetSize();

			for (i=0; i < nNumSFX; i++)
			{
				CCharacterFX* pFX = (CCharacterFX*)m_dynSFXLists[j][i];
				if( pFX )
				{
					//[MURSUM] Del Nick 
					//pFX->SetUpdateNick(false);
#ifdef GAME_SHOW
					T_PLAYER_INFOptr pPI = g_pInterfaceMgr->GetPlayerInfoMgr()->GetPlayerByHandle( pFX->GetServerObj() );
					if( pPI )
					{
						if( LTTRUE == pPI->bCaptain )
						{
							g_pCommonLT->SetObjectFlags( pFX->GetServerObj(), OFT_Flags, 0, (FLAG_VISIBLE|FLAG_SOLID|FLAG_GRAVITY) );
						}
						else							
						{
							g_pCommonLT->SetObjectFlags( pFX->GetServerObj(), OFT_Flags, (FLAG_VISIBLE|FLAG_SOLID|FLAG_GRAVITY), (FLAG_VISIBLE|FLAG_SOLID|FLAG_GRAVITY) );
						}					
					}
#else
					g_pCommonLT->SetObjectFlags( pFX->GetServerObj(), OFT_Flags, (FLAG_VISIBLE|FLAG_SOLID|FLAG_GRAVITY), (FLAG_VISIBLE|FLAG_SOLID|FLAG_GRAVITY) );
#endif
					pFX->SetLastContainerCode( CC_NO_CONTAINER );
				}
			}
			continue;
		}
		else if( j == SFX_GADGETTARGET_ID ||
				 j == SFX_VOLUMEBRUSH_ID ||
				 j == SFX_POLYGRID_ID ||
				 j == SFX_RADAROBJECT_ID )
		{
			continue;
		}
		else
...
//===========================================================================
// ±¤½Ä
LTBOOL CSFXMgr::PlayerDeathAttach(HLOCALOBJ hServerObj )
{
	for( int i = 0; i < m_dynSFXLists[SFX_CHARACTER_ID].GetSize() ; i++ )
	{
		CCharacterFX* pFX = (CCharacterFX*)m_dynSFXLists[SFX_CHARACTER_ID][i];
		if( pFX && pFX->AttachAfterDeath( hServerObj ) == LTTRUE )
		{
			return LTTRUE;
		}
	}
...
LTRESULT CSFXMgr::OnObjectPitch(HOBJECT hObj, float fPitch)
{
	if ( !hObj )
		return LT_INVALIDPARAMS;

	CCharacterFX* pChar = GetCharacterFX(hObj);

	if ( pChar )
	{
		pChar->HandlePitchMsg( fPitch );
	}
	else
	{
		return LT_INVALIDPARAMS;
...
void CSFXMgr::OnSFXMessage(ILTMessage_Read *pMsg)
{
	...
	switch( nFXType )
	{
		...
		// ±¤½Ä : SFX Packet ÁÙÀÌ±â [ Weapon Fire ½Ã ]...
		//------------------------------------------------
		case SFX_WEAPON_ID :
		{
			CreateSFX( nFXType, LTNULL, pMsg, hObj );
			return;
		} break;
		/*
		case SFX_BEAM_ID :
		{
			//HOBJECT hModelObj	= pMsg->ReadObject();
...
CCharacterFX* CSFXMgr::GetCharacterFX( uint8 nID )
{
	CCharacterFX* pCharacterFX = NULL;
	int cCharacterFX  = m_dynSFXLists[SFX_CHARACTER_ID].GetSize();
	...
	{
		pCharacterFX = (CCharacterFX*)m_dynSFXLists[SFX_CHARACTER_ID][iCharacterFX];
		if (pCharacterFX && pCharacterFX->GetPlayerID() == nID)
		{
			return pCharacterFX;
		}
	}
...
HOBJECT CSFXMgr::GetWeaponFromCharacter( HOBJECT hObject )
{
	CCharacterFX* pCharacterFX = NULL;
	int cCharacterFX  = m_dynSFXLists[SFX_CHARACTER_ID].GetSize();
	...
	for ( int iCharacterFX = 0 ; iCharacterFX < cCharacterFX ; iCharacterFX++ )
	{
		...
		if (pCharacterFX && pCharacterFX->GetServerObj() == hObject)
		{
			return pCharacterFX->GetCurrentWeapon() ? pCharacterFX->GetCurrentWeapon() : hObject;
		}
	}
...
CCharacterFX* CSFXMgr::GetCharacterFromHitBox(HOBJECT hHitBox)
{
	...
	for ( int iCharacterFX = 0 ; iCharacterFX < cCharacterFX ; iCharacterFX++ )
	{
		...
		if (pCharacterFX)
		{
			if ( FALSE == pCharacterFX->IsPlayerDead() && pCharacterFX->GetHitBox() == hHitBox)
			{
				return pCharacterFX;
			}
		}
...
void CSFXMgr::SetCharFX_PlayerNick()
{
	CCharacterFX* pCharacterFX = NULL;
	int cCharacterFX  = m_dynSFXLists[SFX_CHARACTER_ID].GetSize();
	...
	for ( int iCharacterFX = 0 ; iCharacterFX < cCharacterFX ; iCharacterFX++ )
	{
		...
		if (pCharacterFX)
		{
			pInfo = pInfoMgr->GetPlayerByHandle(pCharacterFX->GetServerObj());
			if( pInfo && pInfo->bInGame )
			{
				g_pNickList->AddNick( pInfo->hObject, pInfo->szNickName, pInfo->nTeamID );
				pCharacterFX->SetUpdateNick(true);
			}
		}
	}
}
...
CBodyFX* CSFXMgr::GetBodyFromParentID( uint8 nID )
{
	CBodyFX* pBodyFX = NULL;
	int cBodyFX  = m_dynSFXLists[SFX_BODY_ID].GetSize();
...
void CSFXMgr::CreateHitFX( ILTMessage_Read *pMsg )
{
	CPlayerInfoMgr* pInfoMgr = g_pInterfaceMgr->GetPlayerInfoMgr();
	
	LTRotation	rRot;
	uint32		nLocalId	= 0;
	uint8		nShooterID	= pMsg->Readuint8();
	uint8		nDamagerID	= pMsg->Readuint8();
	uint8		nHitNode	= pMsg->Readuint8();
	uint8		nWeaponID	= pMsg->Readuint8();
	HOBJECT		hShooter	= pInfoMgr->GetHandleByID(nShooterID);
	HOBJECT		hDamager	= pInfoMgr->GetHandleByID(nDamagerID);
	CCharacterFX*	pCharFX	= GetCharacterFX(hDamager);
	if( LTNULL == hShooter || LTNULL == hDamager || LTNULL == pCharFX ) return;

	g_pLTClient->GetLocalClientID(&nLocalId);
	g_pLTClient->GetObjectRotation(hDamager, &rRot);

	WCREATESTRUCT w;
	w.hFiredFrom	= hShooter;
	w.hObjectHit	= hDamager;
	w.nWeaponId		= nWeaponID;
	w.wIgnoreFX 	= 24064;
	if( GetRandom(0,3) != 3 ) w.wIgnoreFX |= WFX_TRACER;
	w.nShooterId    = nShooterID;
	w.bLocal        = !!(nLocalId == nShooterID);
	g_pLTClient->GetObjectPos( hShooter, &w.vFirePos );
	pCharFX->GetNodePos(nHitNode, &w.vPos);	

	w.nSurfaceType	= (nHitNode==NODE_HEAD) ? ST_HEAD : ST_ARMOR;
	w.vSurfaceNormal = rRot.Forward();
	w.vSurfaceNormal.Normalize();
	
	CreateSFX( SFX_WEAPON_ID, &w );
}

void CSFXMgr::CreateBodyHitFX( ILTMessage_Read *pMsg )
{
	CPlayerInfoMgr* pInfoMgr = g_pInterfaceMgr->GetPlayerInfoMgr();
	...
	uint32		nLocalId	= 0;
	uint8		nShooterID	= pMsg->Readuint8();
	uint8		nBodyID		= pMsg->Readuint8();
	uint8		nHitNode	= pMsg->Readuint8();
	uint8		nWeaponID	= pMsg->Readuint8();

	CBodyFX*	pBodyFX		= GetBodyFromParentID(nBodyID);
	if( LTNULL == pBodyFX ) return;

	HOBJECT		hShooter	= pInfoMgr->GetHandleByID(nShooterID);
	HOBJECT		hBody		= pBodyFX->GetServerObj();
	
	
	if( LTNULL == hShooter || LTNULL == hBody ) return;

	g_pLTClient->GetLocalClientID(&nLocalId);
	g_pLTClient->GetObjectRotation(hBody, &rRot);
	...
	w.bLocal        = !!(nLocalId == nShooterID);
	g_pLTClient->GetObjectPos( hShooter, &w.vFirePos );
	pBodyFX->GetNodePos(nHitNode, &w.vPos);	
...
void CSFXMgr::CreateProjectileFX( HOBJECT hObj, const LTVector& vFirePos, uint8 nAmmoID )
{
	s_WeaponFX.CreateProjectileFX( hObj, vFirePos, nAmmoID );
}
