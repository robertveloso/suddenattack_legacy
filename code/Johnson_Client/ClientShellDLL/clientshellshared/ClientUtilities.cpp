...
HLTSOUND PlayWeaponSound(WEAPON const *pWeapon, const LTVector &vPos, PlayerSoundId eSoundId,
					 LTBOOL bLocal)
{
	if (!pWeapon) return LTNULL;
	...
	if (pSnd && pSnd[0])
	{
		uint32 dwFlags = 0;
 		...
 		if (bLocal)
		{
			return g_pClientSoundMgr->PlaySoundLocal(pSnd, SOUNDPRIORITY_PLAYER_HIGH,
				dwFlags, 
				g_pMoveMgr->IsHeadInLiquid() ? 80 : 70, 1.0f, WEAPONS_SOUND_CLASS );
		}
		else
		{
 			return g_pClientSoundMgr->PlaySoundFromPos(const_cast<LTVector&>(vPos), pSnd,
				fRadius, SOUNDPRIORITY_PLAYER_HIGH, dwFlags,
				g_pMoveMgr->IsHeadInLiquid() ? 80 : 60, 1.0f, -1.0f, WEAPONS_SOUND_CLASS );
		}		
	}
