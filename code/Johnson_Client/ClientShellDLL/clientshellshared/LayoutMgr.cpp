...
//}
LTIntPt CLayoutMgr::GetAmmoIconSize(int nLayout)
{
	sprintf(s_aTagName, "%s%d", LO_HUD_TAG, nLayout);
	if (!m_buteMgr.Exist(s_aTagName,LO_HUD_AMMO_ICONSZ))
		sprintf(s_aTagName, "%s0", LO_HUD_TAG);

	return GetPoint(s_aTagName, LO_HUD_AMMO_ICONSZ);
}
// Edited By Bestline <==
...
LTIntPt	CLayoutMgr::GetChatTexturePos(int nLayout)
{
	sprintf(s_aTagName, "%s%d", LO_HUD_TAG, nLayout);
	if( !m_buteMgr.Exist(s_aTagName, LO_HUD_CHAT_POS))
		sprintf(s_aTagName, "%s0", LO_HUD_TAG);

	return GetPoint(s_aTagName, LO_HUD_CHAT_POS);
}
...
////////////////////////////////////////////////////////////////////////
//  [12/19/2003]GameHi
LTIntPt	CLayoutMgr::GetArmorIconPos(int nLayout)
{
	sprintf(s_aTagName, "%s%d", LO_HUD_TAG, nLayout);
	if (!m_buteMgr.Exist(s_aTagName,LO_HUD_ARMOR_POS))
		sprintf(s_aTagName, "%s0", LO_HUD_TAG);

	return GetPoint(s_aTagName, LO_HUD_ARMOR_POS);
}

LTIntPt	CLayoutMgr::GetArmorIconSize(int nLayout)
{
	sprintf(s_aTagName, "%s%d", LO_HUD_TAG, nLayout);
	if (!m_buteMgr.Exist(s_aTagName,LO_HUD_ARMOR_SZ))
		sprintf(s_aTagName, "%s0", LO_HUD_TAG);

	return GetPoint(s_aTagName, LO_HUD_ARMOR_SZ);
}

LTIntPt	CLayoutMgr::GetGameHIHealthIconPos(int nLayout)
{
	sprintf(s_aTagName, "%s%d", LO_HUD_TAG, nLayout);
	if (!m_buteMgr.Exist(s_aTagName,LO_HUD_HEALTH_POS))
		sprintf(s_aTagName, "%s0", LO_HUD_TAG);

	return GetPoint(s_aTagName, LO_HUD_HEALTH_POS);
}

LTIntPt	CLayoutMgr::GetGameHIHealthIconSize(int nLayout)
{
	sprintf(s_aTagName, "%s%d", LO_HUD_TAG, nLayout);
	if (!m_buteMgr.Exist(s_aTagName,LO_HUD_HEALTH_SZ))
		sprintf(s_aTagName, "%s0", LO_HUD_TAG);

//	return (uint16)GetInt(s_aTagName, LO_HUD_HEALTH_SZ);
	return GetPoint(s_aTagName, LO_HUD_HEALTH_SZ);
}

LTIntPt CLayoutMgr::GetGameHIBulletBarIconPos(int nLayout)
{
	sprintf(s_aTagName, "%s%d", LO_HUD_TAG, nLayout);
	if (!m_buteMgr.Exist(s_aTagName,LO_HUD_BULLET_BAR_POS))
		sprintf(s_aTagName, "%s0", LO_HUD_TAG);

	return GetPoint(s_aTagName, LO_HUD_BULLET_BAR_POS);
}

LTIntPt	CLayoutMgr::GetGameHIBulletBarIconSize(int nLayout)
{
	sprintf(s_aTagName, "%s%d", LO_HUD_TAG, nLayout);
	if (!m_buteMgr.Exist(s_aTagName,LO_HUD_BULLET_BAR_SZ))
		sprintf(s_aTagName, "%s0", LO_HUD_TAG);

	return GetPoint(s_aTagName, LO_HUD_BULLET_BAR_SZ);
}

LTIntPt	CLayoutMgr::GetTimePos(int nLayout)
{
	sprintf(s_aTagName, "%s%d", LO_HUD_TAG, nLayout);
	if (!m_buteMgr.Exist(s_aTagName,LO_HUD_TIME_BAR_POS))
		sprintf(s_aTagName, "%s0", LO_HUD_TAG);

	return GetPoint(s_aTagName, LO_HUD_TIME_BAR_POS);
}

LTIntPt	CLayoutMgr::GetTimeSize(int nLayout)
{
	sprintf(s_aTagName, "%s%d", LO_HUD_TAG, nLayout);
	if (!m_buteMgr.Exist(s_aTagName,LO_HUD_TIME_BAR_SZ))
		sprintf(s_aTagName, "%s0", LO_HUD_TAG);

	return GetPoint(s_aTagName, LO_HUD_TIME_BAR_SZ);
}
