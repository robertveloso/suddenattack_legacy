...
void CGadgetDisabler::Update( )
{
	...
	{
		m_fTimer -= g_pLTClient->GetFrameTime();
		if (m_fTimer < 0.0f)
		{
			m_fTimer = 0.0f;
		}
...
//							pClientWeapon->IsFireButtonDown());
	LTBOOL bButtonDown =  g_pLTClient->IsCommandOn( COMMAND_ID_LEAN_RIGHT ) || pClientWeapon->IsFireButtonDown();

	//if we somehow switched weapons... STOP!
	if (g_pMissionMgr->IsRoundOver() || g_pPlayerMgr->IsPlayerDead() || m_pGTInfo->m_eTargetType != eINVALID && 
		(g_pPlayerMgr->GetTargetMgr()->RequiredGadgetDamageType() != pAmmoData->eInstDamageType || pClientWeapon->GetState() != W_FIRING))
	{
		bButtonDown = LTFALSE;
	}
	...
	if( m_pGTInfo->m_eTargetType == eCodedText )
	{
		...
		else if( m_bDisabling )
		{
			...
			//[MURSUM] g_pPopup->Show( 2, m_szCodedText );
		}
	}
	else if( m_pGTInfo->m_eTargetType == eInvisibleInk )
	{
		...
		else if( m_bDisabling )
		{
			...
			//[MURSUM] g_pPopup->SetTextColor( r, g, b, nA ) ;
			//[MURSUM] g_pPopup->Show( 4, szString );
