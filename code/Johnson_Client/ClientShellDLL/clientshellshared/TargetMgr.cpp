...
/*	if (m_hLockedTarget && m_hTarget == m_hLockedTarget)
	...
	//[SPIKE] 위치이동 이후위치!! 타켓팅을 잃지 않으면서 먼곳을 이동시 계속 해체되는 버그때문에!! 
	if (m_hLockedTarget && m_hTarget == m_hLockedTarget)
	{
		//are we disabling a GadgetTarget?
		if (g_pPlayerMgr->IsDisabling())
		...
		if (pBody || pCharacter)
		{
			...
			if (pBody)
			{
				...
				if (pBody->CanBeRevived() && fDistAway <= g_vtReviveDistance.GetFloat() && IsRevivePlayerGameType( ))
				{
					...
					CPlayerInfoMgr* pPIMgr = g_pInterfaceMgr->GetPlayerInfoMgr();
					T_PLAYER_INFOptr pPI = pPIMgr->GetPlayerByID(nId);
					...
					if( pPI )
					{
						if (pPI->nTeamID == g_GameDoc.GetTeam())
						{
							m_nString = 0;
							m_bCanActivate = true;
							m_bMoveTarget = true;
							LTStrCpy(m_szString, pPI->szNickName, ARRAY_LEN(m_szString));							
	...
	if (dwUserFlags & USRFLG_CHARACTER)
	{
		...
		if (IsMultiplayerGame() && pFX && pFX->m_cs.bIsPlayer )
		{
			...
			{
				m_nString = 0;
				SAFE_STRCPY(m_szString,pPI->szNickName);

				if (IsTeamGameType())
				{
					m_nTargetTeam = pPI->nTeamID;
				}
			}
			return;
		}
					...
					case 509:
						{
							...
							m_bCanActivate = true;
							m_nTargetTeam = INVALID_TEAM;
							sprintf(m_szString, "E 키를 누르시면 문을 열 수 있습니다.");
						}return;
					case 520:
						{
							m_nString = 520;
							m_bCanActivate = true;
							m_nTargetTeam = INVALID_TEAM;
							sprintf(m_szString, "E 키를 누르시면 문을 닫으실 수 있습니다.");							
						}return;
					}
		...
		if (pGTFX)
		{
			...
			if (eBombable == eGadgetType)
			{
				...
				if( IsTeamGameType() )
				{
					...
					{
						if( g_GameDoc.GetTeam() == pGTFX->GetTeamID() )
						{
							m_bCanActivate = false;
						}
					}
...
static bool ActivateFilterFn(HOBJECT hTest, void *pUserData)
{
	...
	if( pDDPiece )
	{
		...
		{
			if( g_GameDoc.GetTeam() == pDDPiece->GetTeam() )
			{
				return false;
			}
		}
...
void CTargetMgr::SetGadgetTarget( bool bDisabling )
{
	...
	if( IsTeamGameType() )
	{
		...
		if( (pFX->GetTeamID() != INVALID_TEAM) && (g_GameDoc.GetTeam() != pFX->GetTeamID()) )
		{
...
DamageType CTargetMgr::RequiredGadgetDamageType()
{
	...
	if( IsTeamGameType() )
	{
		...
		{
			if( g_GameDoc.GetTeam() != pFX->GetTeamID() )
			{
				eDamageType = DT_INVALID;
			}
		}
