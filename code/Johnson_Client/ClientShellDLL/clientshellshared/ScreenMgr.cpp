...
LTBOOL CScreenMgr::Render(HSURFACE hDestSurf)
{
	...
	if (m_pCurrentScreen)
	{			
		if( m_pCurrentScreen->Render(hDestSurf) )
		{
...
void CScreenMgr::HandleHotKey( unsigned char c )
{
	if(m_pCurrentScreen)
	{
		m_pCurrentScreen->HandleHotKey(c);
	}
}
...
//[MURSUM]
eScreenID CScreenMgr::GetPreviousScreenID()
{
	return m_ePreScreenID;
}

void CScreenMgr::SetPreviousScreenID( eScreenID eID )
{
	m_ePreScreenID	= eID;
}
...
void CScreenMgr::SwitchToScreen(CBaseScreen *pNewScreen)
{
	...
	if (m_pCurrentScreen && pNewScreen != m_pCurrentScreen)
	{
		...
		eScreenID currentScreenID = m_ePreScreenID = (eScreenID)m_pCurrentScreen->GetScreenID();
...
void	CScreenMgr::OnWheelUp(int x, int y)
{
	if (m_pCurrentScreen)
		m_pCurrentScreen->OnWheelUp(x,y);
}
void	CScreenMgr::OnWheelDown(int x, int y)
{
	if (m_pCurrentScreen)
		m_pCurrentScreen->OnWheelDown(x,y);
}
...
CBaseScreen* CScreenMgr::GetCurrentScreen()
{
	return GetScreenFromID(m_eCurrentScreenID);
}
