...
class CMoveMgr
{
  public:
	...
	uint32		GetLastControlFlags()		const { return m_dwLastControlFlags; }
	void		SetControlFlags( uint32 nFlags )	{ m_dwDeathControlFlags = nFlags; }
	uint32		GetDeathControlFlags()				{ return m_dwDeathControlFlags; }
	void		SetDeathVelocity(LTVector vVel)		{ m_vDeathVelocity = vVel; }
	LTVector	GetDeathVelocity()					{ return m_vDeathVelocity; }
	...
	// ±¤½Ä
	void		SetMoverPos( LTVector vPos )	{ g_pLTClient->SetObjectPos(m_hObject, &vPos); }

    void        SetSpectatorMode(LTBOOL bSet);
	void		OnServerForcePos(ILTMessage_Read *pMsg, bool bMandate = false);
	...
	//[SPIKE]
	LTBOOL		IsGrenadeDeathFireReady(){return m_bGrenadeDeathFireReady;}
	VOID		SetGrenadeDeathFireReady(LTBOOL ReadyOK){m_bGrenadeDeathFireReady = ReadyOK;}
  ...
  protected :
	...
	// ±¤½Ä
	// [05.02.11] : ControlFlags Test
	uint32		m_dwDeathControlFlags;
	LTVector	m_vDeathVelocity;
	...
	//[SPIKE]
	LTBOOL		m_bLastBodyOnLadder;
	LTBOOL		m_bGrenadeDeathFireReady;
	...
	//float		m_fFB_Friction;
	//float		m_fLR_Friction;

	LTVector	m_vBackupVel;
