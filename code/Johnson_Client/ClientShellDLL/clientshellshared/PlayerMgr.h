...
public:
	...
	void	SetDetachCamera(bool bType) { m_bTestCamera = bType; }
	void	SetControlFlags(uint32 controlFlags) { m_dwControlFlags |= controlFlags; }
	uint32	GetControlFlags()					 { return m_dwControlFlags; }
	void	ClearControlFlags()					 { m_dwControlFlags = 0; }

	void	SetPlayerInfoChangeFlags( uint16 nFlags )	{ m_nPlayerInfoChangeFlags = nFlags; }
	uint16	GetPlayerInfoChangeFlags()					{ return m_nPlayerInfoChangeFlags; }
	
	LTBOOL	WeaponThrow();

	// [05.02.16] Zoom RoutineÀ» ÇÔ¼ö·Î ¸¸µë.
	bool	ClientWeaponZoomIn();
	void	DetachProcess();
	uint8	GetAttachState()							{ return m_nAttachState; }
	void	SetAttachState(uint8 nState)				{ m_nAttachState = nState; }
	//--------------------------------------------------------------------------------

	//[SPIKE]//////////////////////////////////////////////////////////////////////////////////////////////////// 
	// TimeBomb °ü·Ã ======================================================================
	VOID	SetSelectTimeBomb(LTBOOL bSelBomb){m_bTimeBombSelected = bSelBomb;}
	LTBOOL  GetSelectTimeBomb(){return m_bTimeBombSelected;}
	VOID	SetTargetingTimeBomb(LTBOOL bTargetingBomb){m_bTargetingBomb = bTargetingBomb;}
	LTBOOL  GetTargetingTimeBomb(){return m_bTargetingBomb;}
	VOID	SetPlantedTimeBomb(LTBOOL bPlantedBomb){m_bPlantedBomb = bPlantedBomb;}
	LTBOOL  GetPlantedTimeBomb(){return m_bPlantedBomb;}
	VOID	SetTimeBombPlanting_Level1(LTBOOL bBombPlanting_Level1){m_bTimeBombPlanting_Level1 = bBombPlanting_Level1;}
	LTBOOL  GetTimeBombPlanting_Level1(){return m_bTimeBombPlanting_Level1;}
	VOID	SetTimeBombPlanting_Level2(LTBOOL bBombPlanting_Level2){m_bTimeBombPlanting_Level2 = bBombPlanting_Level2;}
	LTBOOL  GetTimeBombPlanting_Level2(){return m_bTimeBombPlanting_Level2;}
	VOID	SetTimeBombPos(LTVector vTimeBombSoundPos){m_vTimeBombSoundPos = vTimeBombSoundPos;}
	LTVector GetTimeBombPos(){return m_vTimeBombSoundPos;}
	
	VOID	  SetTimeBombNormal(LTVector vN) {m_vTimeBombNormal = vN;}
	LTVector  GetTimeBombNormal() const {return m_vTimeBombNormal;}
	LTBOOL  PreLoadingWeapon();
	// ÅõÃ´¹«±âµéÀÌ ÇÉÀ» »Ì¾ÒÀ» ¶§ÀÇ Fire ===================================================
	VOID	SetWeaponChanging(LTBOOL bOK){ m_bWeaponChanging = bOK; }
	LTBOOL	IsWeaponChanging(){ return m_bWeaponChanging; }
	//----- Á×ÀÎ »ç¶÷ À§Ä¡°ª 
	VOID	SetKillerObject(HOBJECT hKiller, LTBOOL bExistKiller = LTTRUE);

	//----  ÁÜ Release
	VOID	ReleaseZoom();
	//----  1ÀÎÄª Spectator
	LTBOOL	DoFirstViewMode();
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	// ±¤½Ä
	bool		InTimeBombArea() const				{ return m_bInTimeBombArea; }
	...
	// ±¤½Ä : PS_GHOST µµ Á×Àº °ÍÀ¸·Î °£ÁÖ
    //LTBOOL      IsPlayerDead()      const { return (m_ePlayerState == PS_DEAD || m_ePlayerState == PS_DYING); }
	LTBOOL      IsPlayerDead()      const { return (m_ePlayerState == PS_DEAD || m_ePlayerState == PS_DYING || m_ePlayerState == PS_GHOST || m_ePlayerState == PS_MANDATE_DEAD); }
	...
	void        ChangeWeapon(uint8 nWeaponId, uint8 nAmmoId = WMGR_INVALID_ID, int dwAmmo = -1, bool bForceSetting = LTFALSE, bool bForceLastSetting = LTFALSE);
	...
	// ±¤½Ä
	//-------------------------------------------------------------------------
	void	SetModelSleeve( LTBOOL bIsSleeve )	{ m_bLocalSleeve = bIsSleeve; }
	LTBOOL	GetModelSleeve()					{ return m_bLocalSleeve; }
	//-------------------------------------------------------------------------
	HOBJECT	GetThirdAttach()					{ return m_hThirdAttach; }
...
protected:
	...
	void	HandleMsgWeaponReload			(ILTMessage_Read*);	// ±¤½Ä Ãß°¡
	void	HandleMsgAttach					(ILTMessage_Read*); // ±¤½Ä Ãß°¡
	void	HandleProjectileFX				(ILTMessage_Read*);
	void	HandlePreloadWeapon				(ILTMessage_Read*);
	...
	//±¤½Ä
	void	HideShowAttachmentsAfterDeath(HOBJECT hObj);
	...
	void	PlayDamageSound(DamageType eType);

	//[SPIKE]================================================================================
	VOID	UpdateAirLevel();
	LTFLOAT						m_fAirLevel, m_fLastAirLossDamageTime;
	LTFLOAT						m_fZoomIn_Increase, m_fZoomIn2_Increase;
	uint8						m_nTempZoomLevel;
	LTBOOL						m_bTimeBombSelected, m_bTargetingBomb, m_bPlantedBomb; 
	LTBOOL						m_bTimeBombPlanting_Level1, m_bTimeBombPlanting_Level2;
	LTVector					m_vTimeBombSoundPos;
	LTVector					m_vTimeBombNormal;
	LTBOOL						m_bWeaponChanging;
	LTBOOL						m_bSpecialModeOn;
	//----- ÃÑ±â »óÅº°ü·Ã -------------------
	LTFLOAT						m_fFireRecoilAccel;
	LTFLOAT						m_fCurFireRecoilMaxPitch;
	//----- ½ºÆåÅ×ÀÌÅÍ ½ÃÁ¡ °ü·Ã ------------------
	uint8						m_nSpectatorViewIndex;
	LTBOOL						m_bFreeSpectatorMode;
	//=======================================================================================
...
protected:
	...
	// Added By Bestline ==>
	// Time : 2004-07-13 ¿ÀÀü 11:49:05
	bool		m_bTestCamera;
	// Added By Bestline <==

	//[MURSUM]===========================================
	float		m_fSendRespawnTime;
	float		m_fSendRate;
	
	float		m_fZoomInBreathTime;
	bool		m_bFirstRespawn;
	bool		m_bUpdateTargetMgr;
	//===================================================
	// ±¤½Ä
	uint32		m_dwControlFlags;
	bool		m_bAttachedCamera;
	uint8		m_nAttachState;
	LTBOOL		m_bLocalSleeve;
	
	HOBJECT		m_hThirdAttach;
