...
public:
	...
	//[MURSUM] Cone
	bool		InitNetworking( bool bTermShell=false );

	// Disconnect on the next update
	void		ForceDisconnect() {	m_bForceDisconnect = true;}	


	bool		SetupClient( char const* pszIpAddress, char const* pszHostName, char const* pszPassword, bool bSpectator );
	...
	bool		SetupMandateHost();

	bool		SetupRejoin( char const* pszIpAddress, char const* pszHostName, char const* pszPassword, bool bSpectator);
	
	bool		SetMultiplayerData( GameType eGameType, const char* lpszHostIP, int nHostPort );
	...
	void		SetupDefaultSetting( GameType eGameType, int nMaxPlayer, bool bFriendlyFire, uint8 nTimeLimit );
	
	void		SetWorldName( const char* lpszWorldName );
	
	void		SetMissionInfo( ILTMessage_Write *pMsg );

	void		CalcCRC();

#ifdef BIG_FAT_MODE_TEST
	void			SetPlayerModelType(ModelLoadType eModelType)	{ m_ePlayerModelType = eModelType; }
	ModelLoadType	GetPlayerModelType()							{ return m_ePlayerModelType; }
#endif 
...
#ifdef BIG_FAT_MODE_TEST
	ModelLoadType	m_ePlayerModelType;
#endif 
};

extern CClientMultiplayerMgr* g_pClientMultiplayerMgr;
