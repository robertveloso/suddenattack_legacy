...
IClientWeaponBase *CClientWeaponMgr::GetWeaponFromID( int32 nWeaponID ) const
{
	if( nWeaponID == 0xFFFFFFFF ) return LTNULL;

	for( int i=0 ; i<m_nMaxWeapons ; ++i )
	{
		if( m_apClientWeapon[i]->GetWeaponId() == nWeaponID ) return m_apClientWeapon[i];
	}
...
uint8 CClientWeaponMgr::GetSequentialWeaponId( uint8 nWeapon, uint8 nClass, bool bNext ) const
{
	...
	while ( iCur != iStart && ( CWM_NO_WEAPON == iFound ))
	{
		...
		/*
// Edited By Bestline ==>
// Time : 2004-06-18 오전 10:57:49
// 총알이 없어도 무기 선택이 가능 하도록 수정
...
*/
		...
		if ( g_pPlayerStats->HaveWeapon( nWeaponId ) )		// have the weapon			
		{
			if( g_pWeaponMgr->GetWeapon(nWeaponId)->m_nFireType > 1 )
			{
				if( m_apClientWeapon[ iCur ]->HasAmmo() )
				{
...
bool CClientWeaponMgr::ChangeWeapon( uint8 nWeaponId,
                                     ...
                                     int nAmmoAmount /*=-1*/,
									 bool bForceSetting, //[SPIKE] 게임시작전 소유무기 강제로딩
									 bool bForceLastSetting, //[SPIKE] 게임시작시 들고 시작하는 무기 때문에 
                                     bool bPlayDeselect /*=true*/)
{
	if( nWeaponId ==  WMGR_INVALID_ID ) return false;

	// check to see if weapon is a valid weapon to switch to
	if ( m_pCurrentWeapon && !CanChangeToWeapon( nWeaponId ) && !bForceLastSetting)
	{
		if(!bForceSetting) 
			return false;
	}
	...
	if ( m_pCurrentWeapon )
	{
		...
		if ( nWeaponId == nCurrentWeaponId && !bForceLastSetting)
		{
			bChangeWeapon = false;
	...
	if ( bChangeWeapon )
	{
		...
		if ( bPlayDeselect )
		{
			if ( CWM_NO_WEAPON != m_iCurrentWeapon )
			{
				...
				if ( bResult )
				{
					...
					//SPIKE
					if(bForceSetting)
						bSwitchInstantly = bForceSetting;
				}
			}
		}
		...
		if ( bSwitchInstantly )
		{
			...
			{
				m_pCurrentWeapon->Select(true);
			}
			else
			{
				m_pCurrentWeapon->Select(false);
			}
			
			//[SPIKE] /////////////////////////////////////////////////////////////////////////
			WriteConsoleFloat("PVModelFOV", m_pCurrentWeapon->GetWeapon()->m_fPVModelFOVY);
			WriteConsoleFloat("PVModelAspect", m_pCurrentWeapon->GetWeapon()->m_fPVModelAspect);

			WriteConsoleFloat("PVModelPitch", m_pCurrentWeapon->GetWeapon()->m_fPVModelPitch);
			WriteConsoleFloat("PVModelYaw", m_pCurrentWeapon->GetWeapon()->m_fPVModelYaw);
			WriteConsoleFloat("PVModelRoll", m_pCurrentWeapon->GetWeapon()->m_fPVModelRoll);			
			///////////////////////////////////////////////////////////////////////////////////
	...
	{
		g_pPlayerMgr->ReleaseZoom();
	}
	//===========================================================

	
//[SPIKE] TimeBomb 선택유무 판단 ======================================//
	LTBOOL bSelectTimeBomb = (nWeaponId == *(g_pPlayerStats->GetOrderedWeaponId()+4)) ? LTTRUE : LTFALSE;
	g_pPlayerMgr->SetSelectTimeBomb(bSelectTimeBomb);
//=====================================================================//
...
/*
// Edited By Bestline ==>
// Time : 2004-06-18 오전 10:24:01
// 총알이 없다 해도 모기를 보여주기로 하자..
...
void CClientWeaponMgr::EnableWeapons()
{
	...
	// 광식 : 죽었을 때도 다른 사람의 총으로 attach 시키려고 dead 뺐음
	if( g_pPlayerMgr->IsCarryingHeavyObject() || !g_pDamageFXMgr->AllowWeapons( ))
...
void CClientWeaponMgr::AutoSelectWeapon()
{
	...
	if (m_pCurrentWeapon)
	{
		...
		g_pWpnChooser->SelectFromID( m_pCurrentWeapon->GetWeaponId() );

		this->ClearLastWeapon();
...
// 광식
// [05.02.11]
void CClientWeaponMgr::SetDeathAmmoInClip( uint8 nWeaponId, int nAmmoCount )
{
	int iNewWeapon;
	iNewWeapon = WeaponIdToIndex( nWeaponId );

	if( CWM_NO_WEAPON != iNewWeapon )
	{
		CClientWeapon* TempWeapon = dynamic_cast<CClientWeapon*>(m_apClientWeapon[iNewWeapon]);
		TempWeapon->SetAmmoInClip(nAmmoCount);
	}
}