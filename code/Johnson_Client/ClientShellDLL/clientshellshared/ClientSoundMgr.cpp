...
//[SPIKE] BGM Player
HLTSOUND	
CClientSoundMgr::PlayBGMSound(const char *pName, LTBOOL bForcePlay,  SoundPriority ePriority,
	uint32 dwFlags, uint8 nVolume, float fPitchShift, uint8 nSoundClass )
{
	if (!pName) return LTNULL;
	if (!GetConsoleInt("MusicEnable",1) && !bForcePlay) return LTNULL;
...
//	if (!GetConsoleInt("SoundEnable",1)) return LTNULL;
	...
	{
#ifndef _FINAL
		g_pLTClient->CPrint("ERROR in CClientSoundMgr::PlaySound() - Couldn't play sound '%s'",
							psi.m_szSoundName);
#endif
