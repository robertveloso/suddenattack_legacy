...
LTBOOL CShellCasingFX::Init(SFXCREATESTRUCT* psfxCreateStruct)
{
	...
	if (!g_vtShellMaxBounceCountTrack.IsInitted())
	{
		...
		g_vtShellMaxBounceCountTrack.Init(g_pLTClient, "ShellMaxBounceCount", NULL, 2.0);
	}
	...
	{
		//g_vtShellMinLifetime.Init(g_pLTClient, "ShellMinLifetime", NULL, 1.0f);
		g_vtShellMinLifetime.Init(g_pLTClient, "ShellMinLifetime", NULL, 0.7f);
	}
	if (!g_vtShellMaxLifetime.IsInitted())
	{
		//g_vtShellMaxLifetime.Init(g_pLTClient, "ShellMaxLifetime", NULL, 3.0f);
		g_vtShellMaxLifetime.Init(g_pLTClient, "ShellMaxLifetime", NULL, 1.5f);
	}
	...
	if (!g_vtShellMaxSpinsPerSecond.IsInitted())
	{
		...
		g_vtShellMaxSpinsPerSecond.Init(g_pLTClient, "ShellMaxSpinsPerSecond", NULL, 3.0f);
	}
			...
			if (m_nBounceCount >= MAX_BOUNCE_COUNT)
			{
				if (!(m_movingObj.m_dwPhysicsFlags & MO_LIQUID))
				{
					...
					if (pSurf)
					{
						...
						if (pSurf->szShellImpactSnds[nIndex])
						{
							g_pClientSoundMgr->PlaySoundFromPos(vNewPos, pSurf->szShellImpactSnds[nIndex], pSurf->fShellSndRadius,
