...
class CClientSoundMgr : public CGameSoundMgr
{
	public :
        ...
        HLTSOUND	PlaySoundLocal(const char *pName, SoundPriority ePriority=SOUNDPRIORITY_MISC_LOW,
			...
			uint32 dwFlags=0, uint8 nVolume=SMGR_LOCAL_VOLUME, float fPitchShift=1.0f, 
			uint8 nSoundClass = 0 );
 
		HLTSOUND	PlayInterfaceSound(const char *pName, uint32 dwFlags=0);
		//[SPIKE]
		HLTSOUND	PlayBGMSound(	const char *pName, LTBOOL bForcePlay = LTFALSE, SoundPriority ePriority=SOUNDPRIORITY_MISC_HIGH,
									uint32 dwFlags=PLAYSOUND_LOOP, uint8 nVolume=100, float fPitchShift=1.0f, 
									uint8 nSoundClass = BGM_SOUND_CLASSES);
