...
LTBOOL CPolyLineFX::AddLine(PLFXLINESTRUCT ls)
{
	...
	if (SetupLine(pLine, ls.vStartPos, ls.vEndPos))
	{
		if( m_hTexture )
		{
			g_pLTClient->GetTexInterface()->ReleaseTextureHandle( m_hTexture );
			m_hTexture	= LTNULL;
		}

		if( m_cs.pTexture && strlen(m_cs.pTexture)>0  )
		{
			g_pLTClient->GetTexInterface()->CreateTextureFromName( m_hTexture, m_cs.pTexture );
		}
...
//[MURSUM]=========================================================================
// Tracer FX를 위해서 만듬
LTBOOL CPolyLineFX::MoveLine(PolyLine* pLine, LTVector vStartPos, LTVector vEndPos)
{
    if (!pLine) return LTFALSE;
    ...
    PolyVertStruct** ppCurVert;

	ppCurVert = pLine->list.GetItem(TLIT_FIRST);
	if( ppCurVert && *ppCurVert )
	{
		(*ppCurVert)->fPosOffset = fTotalDist - fDist;
		(*ppCurVert)->vPos		 = vPos - m_vPos;
		(*ppCurVert)->fOffset	 = CalcCurOffset(0);
	}

	ppCurVert = pLine->list.GetItem(TLIT_LAST);
	if( ppCurVert && *ppCurVert )
	{
		(*ppCurVert)->fPosOffset = fTotalDist;
		(*ppCurVert)->vPos		= vEndPos - m_vPos;
		(*ppCurVert)->fOffset	 = CalcCurOffset(0);
	}
...
LTBOOL CPolyLineFX::Draw(ILTDrawPrim   *pDraw)
{
	...
	if( m_cs.pTexture )
	{
		...
		//hTex = g_pInterfaceResMgr->GetTexture( m_cs.pTexture );
		//========================================================================
		pDraw->SetTexture( hTex );
	}
	*/
	pDraw->SetTexture( m_hTexture );
