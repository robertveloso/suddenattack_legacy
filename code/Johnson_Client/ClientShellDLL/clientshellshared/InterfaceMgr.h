...
#define AO_DEFAULT_ENABLED		(AO_MUSIC | AO_SOUND | AO_MOVIES | AO_CURSOR | AO_FOG | AO_LINES | AO_TJUNCTIONS | AO_HARDWARESOUND | AO_SOUNDFILTERS | AO_TRIPLEBUFFER )
...
class CInterfaceMgr
{
	public :
		...
		//[MURSUM]=====================================================
		void	EnableInput( LTBOOL bEnable ) { m_bEnableInput = bEnable; }
		bool	ExitGame();
		static void		ExitGameCallBack( LTBOOL bReturn, void* pData ); 

		void	SetExitMessage( char* lpszMsg );
		char*	GetExitMessage() { return m_szExitMessage; }
		bool	IsExitMessage() { return m_bExitMessage; }
		
		int		ChangeSensitive( int nDelta );

		void	HandlePlayerScored( int32 nScorer, int32 nVictim, bool bHeadshot, uint8 nWeaponID );

		void	DoPlayerRadio( uint16 nPlayerID, uint16 nTauntID );
		void	HandlePlayerRadio( uint16 nPlayerID, uint16 nTauntID, uint16 nTeamID );
		void	HandlePlayerTeamChange( int32 nServerIndex, uint8 nTeamID );
		//=============================================================

		//[NOVICE]------------------------------------
		LTBOOL	IsServerPatchExist()	{ return m_bServerPatch; }
		char*	GetServerPatchNotice()	{ return m_szServerPatchNotice; }
		void	SetServerPatchNotice( char* szNotice );
		//--------------------------------------------
...
*/
		...
		void OnChar(unsigned char c, uint32 lParam);
		...
		CPlayerInfoMgr*	GetPlayerInfoMgr()	{ return &m_PlayerInfoMgr; }
...
//	    void	ShowMessageBox(int nStringID, MBCreate* pCreate, uint8 nFontSize = 0, LTBOOL bDefaultReturn = LTTRUE);
		void	ShowMessageBox(DWORD dwIdx, MBCreate* pCreate, uint8 nFontSize = 0, LTBOOL bDefaultReturn = LTTRUE, LTBOOL bIgnoreAfter = LTFALSE);
	    void	ShowMessageBox(const char *pString, MBCreate* pCreate, uint8 nFontSize = 0, LTBOOL bDefaultReturn = LTTRUE, LTBOOL bIgnoreAfter = LTFALSE);
		void	ShowMessageBox(const char* pString, eMBType eType, MBCallBackFn pfnCallBack, void* pData, LTBOOL bIgnoreAfter = LTFALSE );
		void	ShowMessageBox(DWORD dwIdx, eMBType eType, MBCallBackFn pfnCallBack, void* pData, LTBOOL bIgnoreAfter = LTFALSE );
		...
		//-----------------------------------------------------------------------------------
		// 광식 [04.09.22] : 누가 누구를 죽였는지 표시하기 위해 이 곳에 Text를 넣었음
		// MessageQueue를 수정하려면 작업이 커 보임
		//-----------------------------------------------------------------------------------
	//public :
		//char			m_szKillText[256];
		//-----------------------------------------------------------------------------------
		//------------------------------------------------------------------------
		// 광식
		//------------------------------------------------------------------------
		...
		//[SPIKE] Scope관련 
		VOID	SetScopeEnable(LTBOOL bScopeOn){ m_bScopeOn = bScopeOn;}
		LTBOOL  GetScopeOn(){return m_bScopeOn;}
		
		void	PlayBGM( const char* lpszSndName, LTBOOL bForcePlay = LTFALSE );
		void	KillBGM();
		LTBOOL	IsPlayBGM();
...
//		CLayoutMgr			m_LayoutMgr;			// bute mgr for layout info
		...
		bool				m_bExitMessage;
		char				m_szExitMessage[512];
		//==========================================
		
		//[NOVICE]------------------------------------
		LTBOOL				m_bServerPatch;
		char				m_szServerPatchNotice[256];
		//--------------------------------------------
        ...
        CPlayerInfoMgr	m_PlayerInfoMgr;
...
*/
		...
		//[MURSUM]=============================================================
		LTBOOL		m_bEnableInput;
		LTBOOL		m_bChangeModeReserved;
		int			m_nBackupSoundVar;
		//=====================================================================
...
private:
		...
		LTBOOL  PreMandateHostState(GameState eCurState);
		void	UpdateMandateHostState();
        LTBOOL  PostMandateHostState(GameState eNewState);

		LTBOOL  PreRejoinHostState(GameState eCurState);
		void	UpdateRejoinHostState();
        LTBOOL  PostRejoinHostState(GameState eNewState);
