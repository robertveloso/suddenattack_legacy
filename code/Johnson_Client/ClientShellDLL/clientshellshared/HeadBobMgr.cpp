...
void CHeadBobMgr::UpdateHeadBob()
{
	...
	// [05.02.11] : ControlFlags Test
	//-------------------------------------------------------------------
	uint32 dwPlayerFlags = 0;
	if( g_pPlayerMgr->IsPlayerDead() )
	{
		dwPlayerFlags = g_pMoveMgr->GetDeathControlFlags();
	}
	else
	{
		dwPlayerFlags = g_pPlayerMgr->GetPlayerFlags();
	}
	//-------------------------------------------------------------------
	...
	if( g_pPlayerMgr->IsPlayerDead() )
	{		
		fMoveDist  = pMoveMgr->GetDeathVelocity().Mag() * fFrameTime;
		//LTVector newVel;
		//g_pPhysicsLT->GetVelocity(g_pPlayerMgr->GetPlayerCamera()->GetAttachedObject(), &newVel);
		//fMoveDist = newVel.Mag() * fFrameTime;
	}
	else
	{
		fMoveDist  = pMoveMgr->GetVelocity().Mag() * fFrameTime;
	}
	//-------------------------------------------------------------------
	...
	if (bMoving && bFootstep)
	{
		...
		if (pCharFX)
		{
			...
			//[SPIKE] RunÀÏ¶§¸¸ ¹ß¼Ò¸® µé¸².
			if((pMoveMgr->GetControlFlags() & BC_CFLG_RUN) && (pMoveMgr->GetControlFlags() & BC_CFLG_MOVING) 
				&& !(pMoveMgr->GetControlFlags() & BC_CFLG_DUCK))
			{
