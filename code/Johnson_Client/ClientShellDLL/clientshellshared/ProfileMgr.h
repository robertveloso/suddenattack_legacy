...
public:
	...
	// SPIKE
	VOID		SetVideoMem(uint32 dwVideoMem){m_dwVideoMem = dwVideoMem;}
	VOID		AutoSetting();		  // 등급별 세팅 
	int8		GetHardWareGrade();   // 하드웨어 등급 추출
	int8		GetGrade(uint32 iVideoMem, LTFLOAT fCpuSpeed, char* szVGA_CompanyName); // 등급 계산 
	VOID		GetVGACompanyName(const char* szVGAName, char* szVGA_CompanyName);		// 그래픽카드 회사 추출 
	VOID		SetChangeOptionSetting();   // 게임에서 변경한 옵션 Profile에 세팅
	...
	//[SPIKE] BGM 볼륨ReSet
	VOID		ReSetBGMVolumeRatio(LTFLOAT fVolumeRatio);
...
// Added By IN'T ==>
// Time : 2004-05-06 오후 6:21:25
	uint8			m_nDMPlayerSkin;
// Added By IN'T <==
	...
	int		m_nSoundVolume;
	int		m_nMusicVolume;
	LTBOOL	m_bSoundEnable;
	LTBOOL	m_bMusicEnable;
...
// SPIKE Add Option Value /////////////////////////////////////////////
	uint32 m_dwVideoMem;	// 비디오 메모리 
	LTBOOL m_bVSyncOnFlip;	//SoundFilters 로 사용  
	LTBOOL m_bModelShadow;
	LTBOOL m_bPolyGrid;
	LTBOOL m_bSprite;
	LTBOOL m_bTracer;
	LTBOOL m_bParticle;
	LTBOOL m_bCacheTexture;
	LTBOOL m_bWeather;
	LTBOOL m_bShellCasing;
    LTBOOL m_bGore;
	LTBOOL m_bNameRadar;
	LTBOOL m_bDynaLight;
	LTBOOL m_bRejectWhisper;
	LTBOOL m_bRejectInvite;
	char   m_szVGAName[1024];
