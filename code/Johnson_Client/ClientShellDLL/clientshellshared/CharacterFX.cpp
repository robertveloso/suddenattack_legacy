...
#define FOOTSTEP_SOUND_RADIUS	1400.0f
#define DEATH_SOUND_RADIUS		800.0f
...
// 광식
#define WEAPON_SOUND_KEY		"sound_key"
#define JUMP_SOUND_KEY			"JUMP_KEY"

#define POWER_SWORD				"power_sword"
...
LTBOOL CCharacterFX::Init(HLOCALOBJ hServObj, ILTMessage_Read *pMsg)
{
	...
	m_hCurrentWeapon	= LTNULL;
	m_bRecvModelInfo	= false;
	
	m_cs.eModelLoadType = ch.eModelLoadType;

	ChangeModel( ch.eModelId, 0, 0 );
...
LTBOOL CCharacterFX::Init(SFXCREATESTRUCT* psfxCreateStruct)
{
...
#ifdef GAME_SHOW
	m_fPacketUpdateRate = 1.0f / GetConsoleFloat( "UpdateRate", DEFAULT_CSENDRATE );
#else
	m_fPacketUpdateRate	= 1.0f / GetConsoleFloat( "UpdateRate", DEFAULT_CSENDRATE );
#endif
	...
	g_pPhysicsLT->SetFrictionCoefficient(m_cs.hServerObj, 5.0f);
...
LTBOOL CCharacterFX::CreateObject(ILTClient* pClientDE)
{
	...
	if( !g_vtShowNode.IsInitted() )
	{
		g_vtShowNode.Init( g_pLTClient, "ShowNode", LTNULL, 0.0f );
	}
	...
	if( false == IsRecvModelInfo() )
	{
		T_PLAYER_INFOptr pPlayerInfo = g_pInterfaceMgr->GetPlayerInfoMgr()->GetPlayerByID(m_cs.nClientID);
		if( pPlayerInfo && pPlayerInfo->bRecvInfo )
		{
			pPlayerInfo->hObject = m_hServerObject;
			pPlayerInfo->nPlayerID = m_cs.nClientID;
...
LTBOOL CCharacterFX::Update()
{
	...
	*/
	...
	if( g_pPlayerMgr->IsPlayerDead() && 
		g_pPlayerMgr->GetPlayerCamera()->GetAttachedObject() == m_hServerObject )
	{
		LTVector	vPos;
		LTRotation	rRot;
		...
		g_pLTClient->SetObjectPos( g_pMoveMgr->GetObject(), &vPos, true );
		...
		g_pPlayerMgr->SetPitch( m_fPitch );
	}

	// 광식
	//UpdateAttachmentsWeapon();
	...
	int nShowNode = (int)g_vtShowNode.GetFloat(0.0f);
	switch( nShowNode )
	{
	case 1:
		...
		break;
	case 2:
		if( !m_bLocalPlayer ) UpdateNodeRadiusModels();
		break;
	default:
		RemoveNodeRadiusModels();
	}
	...
	*/
	...
	UpdateServerObjectCheckLiquid(); //[SPIKE] 물속을 들어가거나 나올때 체크 

	if (m_bUpdateAttachments)
		UpdateAttachments();

	if( m_bUpdateNick && g_pNickList->IsVisible() && !IsPlayerDead() ) UpdateNickName();

	return LTTRUE;
}

VOID
CCharacterFX::UpdateServerObjectCheckLiquid()
{
	if(g_pPlayerMgr->IsSpectatorMode()) return;
	LTVector vPos;
	g_pLTClient->GetObjectPos( m_hServerObject, &vPos );
	uint32 nIgnoreFlags = (CC_DYNAMIC_OCCLUDER_VOLUME_FLAG | CC_PLAYER_IGNORE_FLAGS);
    ContainerCode eContainerCode = ::GetContainerCode(vPos, nIgnoreFlags);

	if(m_eLastContainerCode != eContainerCode)
	{
		switch( eContainerCode )
		{
		case CC_WATER:
		case CC_CORROSIVE_FLUID:
		case CC_FREEZING_WATER:
		case CC_LADDER:
			g_pCommonLT->SetObjectFlags( m_hServerObject, OFT_Flags, 0, FLAG_GRAVITY );
			break;
		default:
			g_pCommonLT->SetObjectFlags( m_hServerObject, OFT_Flags, FLAG_GRAVITY, FLAG_GRAVITY );
			break;
		}

		if( LTTRUE == IsLiquid(eContainerCode) && LTFALSE ==  IsLiquid(m_eLastContainerCode) )
		{
			//들어갈때
			LTFLOAT fOffSet = m_vLandPos.y - vPos.y;
			m_vLandPos = vPos;
			
			CLIENTFX_CREATESTRUCT	fxCS("SA_Impact_Water", LTNULL, vPos );
			fxCS.m_rRot	 = LTRotation( LTVector(0.0f, 0.0f, 1.0f), LTVector(0.0f, 1.0f, 0.0f) );	

			if( fOffSet > 150.0f) 
			{
				strcpy(fxCS.m_sName, "SA_HardImpact_Water");
				g_pClientFXMgr->CreateClientFX(LTNULL, fxCS, LTTRUE );	
			}
			else if( fOffSet > 50.0f )
			{
				g_pClientFXMgr->CreateClientFX(LTNULL, fxCS, LTTRUE );	
			}
		}
		else if( LTFALSE == IsLiquid(eContainerCode) && LTTRUE ==  IsLiquid(m_eLastContainerCode) )
		{
			//나올때
			LTFLOAT fOffSet = m_vLandPos.y - vPos.y;
			m_vLandPos = vPos;
		...
		m_eLastContainerCode = eContainerCode;
	}
}
void CCharacterFX::UpdateNickName()
{
	LTVector vPos(0,0,0);
	...
	LTVector pos = g_pInterfaceMgr->GetScreenFromWorldPos( vPos, g_pPlayerMgr->GetCamera() );

	g_pNickList->Update( m_hServerObject, &pos );
}
...
// ----------------------------------------------------------------------- //
//
//	ROUTINE:	CCharacterFX::TurnOnFlashLight
//
//	PURPOSE:	Update the flash light fx
//
// ----------------------------------------------------------------------- //

void CCharacterFX::TurnOnFlashLight( LTVector* pvFlashPos, LTRotation* prRot )
{
	m_3rdPersonFlashlight.TurnOn(pvFlashPos, prRot);
}

// ----------------------------------------------------------------------- //
//
//	ROUTINE:	CCharacterFX::TurnOffFlashLight
//
//	PURPOSE:	Remove the flash light fx
//
// ----------------------------------------------------------------------- //

void CCharacterFX::TurnOffFlashLight()
{
...
{
	static CParsedMsg::CToken s_cTok_KeyPutStepSound(KEY_FOOTSTEP_SOUND);
	static CParsedMsg::CToken s_cTok_KeyMovementLoud(KEY_MOVEMENT_LOUD);
	static CParsedMsg::CToken s_cTok_KeyMovementQuiet(KEY_MOVEMENT_QUIET);
	static CParsedMsg::CToken s_cTok_KeyShowAttachFX(KEY_SHOW_ATTACHFX);
	static CParsedMsg::CToken s_cTok_KeyHideAttachFX(KEY_HIDE_ATTACHFX);
	static CParsedMsg::CToken s_cTok_KeyFX(KEY_FX);
	static CParsedMsg::CToken s_cTok_KeyRenderStyleModel(RENDERSTYLE_MODEL_KEY);
	static CParsedMsg::CToken s_cTok_KeyFlashLight("FLASHLIGHT");
	// 광식
	static CParsedMsg::CToken s_cTok_KeyWeaponSound(WEAPON_SOUND_KEY);
	static CParsedMsg::CToken s_cTok_KeyJumpSound(JUMP_SOUND_KEY);	

	static CParsedMsg::CToken s_cTok_KeyPowerSword(POWER_SWORD);
	...
	else if( tok == s_cTok_KeyJumpSound )	// 광식 추가
	{
		LTVector vSoundPos;
		/*
		HOBJECT hLocalObj = g_pLTClient->GetClientObject();
		if( hLocalObj == m_hServerObject ) return;		
		if( g_pPlayerMgr->IsPlayerDead() &&
			IsRevivePlayerGameType() &&
			g_pPlayerMgr->GetAttachState() == MID_ATTACH_THIRD )
		{
			g_pLTClient->GetObjectPos(hLocalObj, &vSoundPos);
		}
		else
		*/
		{
			g_pLTClient->GetObjectPos(m_hServerObject, &vSoundPos);
		}
		//g_pLTClient->GetObjectPos(m_hServerObject, &vSoundPos);
		if( m_cs.eModelLoadType == eModelNormal )
		{
			char* pSounds[] = { "SA_CHARACTERS\\SND\\jump1.wav", "SA_CHARACTERS\\SND\\jump2.wav" };
			g_pClientSoundMgr->PlaySoundFromPos(vSoundPos, pSounds[GetRandom(0,1)], 1000.0f, SOUNDPRIORITY_MISC_HIGH);
		}
		else
		{
		...
		//char* pSounds[] = { "SA_CHARACTERS\\SND\\jump1.wav", "SA_CHARACTERS\\SND\\jump2.wav" };
		//		g_pClientSoundMgr->PlaySoundFromPos(vSoundPos, pSounds[GetRandom(0,1)], 1000.0f,
		//			SOUNDPRIORITY_MISC_HIGH);
	}
	else if( tok == s_cTok_KeyPowerSword )	// 광식 : 내려찍기 칼질..
	{
		if (pArgs->argc > 1)
		{
			if( !m_hCurrentWeapon ) return;
			
			if (stricmp(pArgs->argv[1], "1") == 0)	// 칼 뒤집기
			{
				uint32 dwAni = g_pLTClient->GetAnimIndex(m_hCurrentWeapon, "Knife1");
				g_pLTClient->SetModelAnimation(m_hCurrentWeapon, dwAni);
			}
			else									// 칼 원위치
			{
				uint32 dwAni = g_pLTClient->GetAnimIndex(m_hCurrentWeapon, "Knife");
				g_pLTClient->SetModelAnimation(m_hCurrentWeapon, dwAni);
			}
		}
	}
	else if( tok == s_cTok_KeyFlashLight )
	{		
	}
	else if( tok == s_cTok_KeyRenderStyleModel )
	{
	...
	else if( tok == s_cTok_KeyMovementLoud )
	{
	...
	else if( tok == s_cTok_KeyMovementQuiet )
	{
	...
	else if( tok == s_cTok_KeyShowAttachFX )
	{
		if( pArgs->argc > 1 )
		{
	...
	else if( tok == s_cTok_KeyHideAttachFX )
	{
		if( pArgs->argc > 1 )
		{
			uint32 i = atoi( pArgs->argv[1] );
	...
	else if( tok == s_cTok_KeyWeaponSound )		// 광식 : 모든 Weapon Sound는 Local 에서 처리
	{
		WEAPON const* pWeapon = g_pWeaponMgr->GetWeapon(m_nCurrentWeaponID);
		if( !pWeapon ) return;

		if( g_pPlayerMgr->IsPlayerDead() &&
			IsRevivePlayerGameType() &&
			g_pPlayerMgr->GetAttachState() == MID_ATTACH_FIRST )
		{
			return;
		}
		...
		LTVector vSoundPos;
		HLOCALOBJ hPlayerObj = g_pLTClient->GetClientObject();
		if( hPlayerObj == m_hServerObject ) return;		
		/*
		if( g_pPlayerMgr->IsPlayerDead() &&
			IsRevivePlayerGameType() &&
			g_pPlayerMgr->GetAttachState() == MID_ATTACH_THIRD )
		{
			g_pLTClient->GetObjectPos(hPlayerObj, &vSoundPos);
		}
		else
		*/
		{
		...
		//g_pLTClient->GetObjectPos(m_hServerObject, &vSoundPos);
		PlayerSoundId nId = static_cast< PlayerSoundId >( atoi( pArgs->argv[ 1 ] ) );
		PlayWeaponSound(pWeapon, vSoundPos, nId);
	}
}
...
void CCharacterFX::DoFootStepKey(HLOCALOBJ hObj, LTBOOL bForceSound)
{
	...
	if (bIsLocalClient)
	{
		...
		if (g_pPlayerMgr->IsSpectatorMode() ||
			g_pPlayerMgr->GetAttachState() == MID_DETACH_OBSERVER ||
			!pMoveMgr->CanDoFootstep())
		{
			return;
		}
	...
	//[SPIKE] 벽을 타고 올라가면 사다리 취급 
 	if(m_eLastSurface == ST_UNKNOWN)
	{
		LTRotation rRot;
		g_pLTClient->GetObjectRotation(hObj, &rRot);
		iQuery.m_To	= iQuery.m_From;
		iQuery.m_To += (rRot.Forward() * INTERSECT_Y_OFFSET * 2.0f);
		if (m_pClientDE->IntersectSegment(&iQuery, &iInfo))
		{
			if (IsMainWorld(iInfo.m_hObject) ||
				GetObjectType(iInfo.m_hObject) == OT_WORLDMODEL)
			{
				m_eLastSurface = ST_LADDER;
			}
		}	
	}
	//========================================================
	...
	/*
	// 광식 : 공중에 떠 다니는 녀석들 때문에...
	//------------------------------------------------------
	float fDelta = vPos.y - iInfo.m_Point.y;
	if( fDelta > 65 )
	{
		vPos.y = iInfo.m_Point.y + 64.11f;
		g_pLTClient->SetObjectPos(hObj, &vPos);
		g_pPhysicsLT->SetFrictionCoefficient(hObj, 5.0f);
	}
	//------------------------------------------------------
	*/
	...
	/*
	// 내가 죽고 다른 사람에 3 인칭으로 붙었다면 Sound를 내 위치에서 Play
	if( g_pPlayerMgr->IsPlayerDead() &&
		IsRevivePlayerGameType() &&
		g_pPlayerMgr->GetAttachState() == MID_ATTACH_THIRD )
	{
                                     ...
                                     LTBOOL bLeftFoot, PlayerPhysicsModel eModel)
{
	...
	if (pSound && *pSound)
	{
		...
		//[SPIKE]  발소리 사운드필터링		
	/*	char szSound[256] = {LTNULL};	
		CONST FLOAT SOUNDHIGH_Y = 150.0f;
		memcpy(szSound, pSound, strlen(pSound));
		strtok(szSound, ".");
		...
		if(!bIsLocalClient)
		{
			LTVector vLocalPos;
			LTFLOAT fOffsetY;
			g_pLTClient->GetObjectPos(pMoveMgr->GetObject(), &vLocalPos);
			fOffsetY = vPos.y - vLocalPos.y;

			if(fOffsetY > SOUNDHIGH_Y) // 위에서 
			{
				strcat(szSound, "_Up.wav");
			}
			else if(fOffsetY < -SOUNDHIGH_Y) // 아래에서 들리는 발소리 
			{
				strcat(szSound, "_Down.wav");
			}
			else
			{
				strcat(szSound, ".wav");
...
LTBOOL CCharacterFX::OnServerMessage(ILTMessage_Read *pMsg)
{
	...
	switch(nMsgId)
	{
		...
		case CFX_HITBOX_MSG:
		{
			...
			//--------------------------------------------------------------------
			// 광식 : 죽었을 때 다른 Player 의 움직임에따라 Move flag를 주려고..
			//--------------------------------------------------------------------
			if( g_pPlayerMgr->IsPlayerDead() && 
				m_hServerObject == g_pPlayerMgr->GetPlayerCamera()->GetAttachedObject() )
			{
				g_pPlayerMgr->SetControlFlags(BC_CFLG_MOVING);
				g_pPlayerMgr->SetControlFlags(BC_CFLG_RUN);
			}
			//--------------------------------------------------------------------
		...
		{
			if( m_cs.bIsPlayer && g_pGameClientShell->ShouldUseRadar() )
			{
				m_bPlayerDead = true;

				//광식 : Mini Map
				//-----------------------------------------------------
				g_pHUDMiniMap->SetPlayerState( m_hServerObject, true );
				//-----------------------------------------------------
				...
				g_pInterfaceMgr->GetPlayerInfoMgr()->SetPlayerState( m_hServerObject, PS_DEAD );
				g_pScores->SetUpdate(true);
				
				g_pCommonLT->SetObjectFlags( m_hServerObject, OFT_Flags, 0, FLAG_SOLID );
			}
			
			if( !g_GameDoc.IsLowViolence() ) PlayDeathSound();
			ShutdownDamageFX();
		}
		break;
		...
		case CFX_PLAYER_REVIVED:
		{
			if( m_cs.bIsPlayer && g_pGameClientShell->ShouldUseRadar() )
			{
				...
				g_pInterfaceMgr->GetPlayerInfoMgr()->SetPlayerState( m_hServerObject, PS_ALIVE );
				g_pScores->SetUpdate(true);	
		...
		case CFX_GHOST_RESPAWN:
		{
			if( m_cs.bIsPlayer && g_pGameClientShell->ShouldUseRadar())
			{
				...
				g_pInterfaceMgr->GetPlayerInfoMgr()->SetPlayerState( m_hServerObject, PS_GHOST );
				g_pScores->SetUpdate(true);

				g_pCommonLT->SetObjectFlags( m_hObject, OFT_Flags, FLAG_SOLID|FLAG_GRAVITY, FLAG_SOLID|FLAG_GRAVITY );

				//[SPIKE] 카메라 안움직이는 버그 때문에.
				g_pPlayerMgr->SetDetachCamera(false);
...
void CCharacterFX::CreateAttachClientFX()
{
	...
	// 광식 : ClientFX 를 모델마다 따로 가져야 해서 지웠음.
	// 이 부분을 따로 처리 할 수 있도록 만들어야 함.
	//RemoveAttachClientFX();
...
LTRESULT CCharacterFX::HandlePitchMsg( float fPitch/*ILTMessage_Read *pMsg*/ )
{
...
//	UncompressAngleFromByte( ucPitch, &m_fNewPitch );
...
//[MURSUM]
void CCharacterFX::OnWeaponChanged( uint8 nWeaponId, int nAmmoId, int nAmmoCount, int nAmmoInClip )
{
	m_nCurrentWeaponID	= nWeaponId;
	m_nCurrentAmmoID	= nAmmoId;
	m_nCurrentAmmoCount	= nAmmoCount;
		
	if( g_pPlayerMgr->IsPlayerDead() )
	{
		g_pPlayerStats->UpdateAmmo(nWeaponId, nAmmoId, nAmmoCount);
		g_pPlayerStats->UpdatePlayerWeapon(nWeaponId, nAmmoId);
		g_pPlayerMgr->GetClientWeaponMgr()->SetDeathAmmoInClip(nWeaponId, nAmmoInClip);

		if( g_pPlayerMgr->GetPlayerCamera()->GetAttachedObject() == m_hServerObject &&
			LTTRUE == g_pPlayerMgr->IsFirstPerson() )
		{
			g_pPlayerMgr->ChangeWeapon( nWeaponId, nAmmoId, nAmmoCount );
		}		
	}
}

// minjin with MURSUM.
void CCharacterFX::pd_UpdatePitch()
{
	if( !m_bPrediction || m_fPitch == m_fNewPitch ) return;	

	LTFLOAT fFrameTime	= g_pLTClient->GetFrameTime();

	if ( 0.0f == fFrameTime )		fFrameTime = 0.014f;
	
	if ( m_fLastPitch != m_fNewPitch )
	{
		m_fLastPitch		= m_fNewPitch;
		m_fPredictPitch		= m_fPitch;
	}

	if ( ( m_fLastPitch - m_fPredictPitch < 0 && m_fPitch < m_fNewPitch ) ||
		 ( m_fLastPitch - m_fPredictPitch > 0 && m_fPitch > m_fNewPitch )  )
	{
		m_fPitch			= m_fNewPitch;
		m_fLastPitch		= m_fNewPitch;
		m_fPredictPitch		= m_fPitch;
	}
	
	LTFLOAT fDelta = ( m_fLastPitch - m_fPredictPitch ) / ( m_fPacketUpdateRate / fFrameTime );

	m_fPitch += fDelta;				// PREDICTION!!!


	//노드 꺾는 부분
	LTRotation	rUpperTorsoRot	= LTRotation(LTVector(1.0f, 0.0f, 0.0f), m_fPitch * 0.5f + DEG2RAD(g_vtPitchBias.GetFloat()));
...
void CCharacterFX::ResetPitchTracking()
{
	...
	if( m_cs.bIsPlayer && IsMultiplayerGame() )
	{
		...
		//m_hUpperTorsoNode = g_pClientTrackedNodeMgr->CreateTrackingNode( m_hServerObject, "Bip01 Neck" );
		if( m_hUpperTorsoNode != INVALID_TRACKEDNODE )
		{
			float fMax = g_vtUpperTorsoPitchMax.GetFloat();

			g_pClientTrackedNodeMgr->SetNodeConstraints( m_hUpperTorsoNode, 0.0f, 0.0f, DEG2RAD(fMax), DEG2RAD(fMax), DEG2RAD(360.0f) );
			//g_pClientTrackedNodeMgr->SetNodeConstraints( m_hUpperTorsoNode, 0.0f, 0.0f, DEG2RAD(360.0f), DEG2RAD(360.0f), DEG2RAD(360.0f) );
		...
		//광식
		m_hLowerTorsoNode = g_pClientTrackedNodeMgr->CreateTrackingNode( m_hServerObject, "Bip01 Spine1" );
		if( m_hLowerTorsoNode != INVALID_TRACKEDNODE )
		{
	...
	{
		if( m_TrackedNodeContext.IsInited() ) m_TrackedNodeContext.Term();
		m_TrackedNodeContext.Init( m_hServerObject, m_cs.eModelSkeleton, g_pClientTrackedNodeMgr );
	}

	//[MURSUM]
	if( GetConsoleFloat( "prediction", 0.0f ) > 0.0f )
	{
		m_bPrediction	= true;
	}
	else
...
// 광식
void CCharacterFX::ChangeModel(ModelId eModelId, uint8 nHeadSkin, uint8 nBodySkin)
{
	...
	// 광식 : 소매가 있는 옷 인지 check
	//--------------------------------------------------------------------------------
	HLOCALOBJ hPlayerObj = g_pLTClient->GetClientObject();
	if( hPlayerObj == m_hServerObject)
	{
		g_pPlayerMgr->SetModelSleeve(g_pModelButeMgr->GetModelSleeve( eModelId ));
	}
	//--------------------------------------------------------------------------------
	...
	switch(m_cs.eModelLoadType)
	{
	case eModelNormal :
		pFilename = g_pModelButeMgr->GetModelFilename(eModelId);
		break;
	case eModelBig :
		pFilename = g_pModelButeMgr->GetMultiBigModelFilename(eModelId);
		break;
	case eModelFat :
		pFilename = g_pModelButeMgr->GetMultiFatModelFilename(eModelId);
		break;
	}
	if( !pFilename ) return;
	...
	*/
	...
	//--------------------------------------------------------------------
	//memset( theStruct.m_SkinNames[0], 0, MAX_CS_FILENAME_LEN+1);
	//memset( theStruct.m_SkinNames[1], 0, MAX_CS_FILENAME_LEN+1);

	char* szTemp;
	g_pModelButeMgr->GetModelHeadSkinFilename(eModelId, nHeadSkin, szTemp);	
	if( szTemp ) strcpy( theStruct.m_SkinNames[0], szTemp );
	g_pModelButeMgr->GetModelBodySkinFilename(eModelId, nBodySkin, szTemp);
	if( szTemp ) strcpy( theStruct.m_SkinNames[1], szTemp );
	//--------------------------------------------------------------------
...
void CCharacterFX::WantRemove( LTBOOL bRemove )
{
	if (bRemove)
	{
		...
		g_pPlayerMgr->GetPlayerCamera()->GetAttachedObject() == m_hServerObject )
		{
			g_pPlayerMgr->SetDetachCamera(true);
			g_pPlayerMgr->GetPlayerCamera()->AttachToObject(LTNULL);
		}		
	}
...
void CCharacterFX::UpdateAttachmentsWeapon()
{
	if( !g_pPlayerMgr->IsPlayerDead() ) return;
	...
	{
		HMODELSOCKET hSocket;
		if( LT_OK == g_pModelLT->GetSocket(m_hServerObject, "RightHand", hSocket))
		{
			LTransform transform;
			if( LT_OK == g_pModelLT->GetSocketTransform(m_hServerObject, hSocket, transform, LTTRUE) )
			{
				g_pLTClient->SetObjectPosAndRotation(attachList[i], &transform.m_Pos, &transform.m_Rot);
			}
		}
	}
}
...
// 광식 : 상대방에게 Attach 되었을 때 SOLID 속성을 뺀다. 계단 오를 때 떨림.
//---------------------------------------------------------------------------------
LTBOOL CCharacterFX::AttachAfterDeath( HLOCALOBJ hServerObj )
{
	if( this->GetServerObj() == hServerObj )
	{
		// 화면이 덜덜 거려서 다시 뺌
		/*
		CharFXList::iterator iter = m_lstPlayersInGame.begin();
		while( iter != m_lstPlayersInGame.end() )
		{
			if( (*iter)->GetServerObj() != g_pLTClient->GetClientObject() )
			{
				g_pCommonLT->SetObjectFlags((*iter)->GetServerObj(), OFT_Flags, FLAG_SOLID, FLAG_SOLID);
			}
...
bool CCharacterFX::IsPlayerDead()
{
	uint32 nFlags;
	g_pCommonLT->GetObjectFlags( m_hServerObject, OFT_Flags, nFlags );
	if( !(nFlags&FLAG_VISIBLE) ) return LTTRUE;
	
	return m_cs.bIsPlayer && m_bPlayerDead;
}

bool CCharacterFX::IsRadarPlayerDead()
{
	return m_cs.bIsPlayer && m_bPlayerDead;
}
//---------------------------------------------------------------------------------

void CCharacterFX::PlayDeathSound()
{
	...
	if( IsLiquid(m_eLastContainerCode) )
	{
		strcat(szSound, "last_breath.wav");
	}
	else
	{
		char* DeathSounds[] =  { "death01.wav", "death02.wav", "death03.wav", "death04.wav" };
		int nSize = (sizeof(DeathSounds)/sizeof(DeathSounds[0])) - 1;
		strcat(szSound, DeathSounds[GetRandom(0, nSize)]);
	}

	g_pClientSoundMgr->PlaySoundFromPos(vPos, szSound,
				DEATH_SOUND_RADIUS, SOUNDPRIORITY_PLAYER_HIGH, 0,
				SMGR_LOCAL_VOLUME, 1.0f, -1.0f, WEAPONS_SOUND_CLASS );
}

void CCharacterFX::GetNodePos( uint8 nNode, LTVector* pvPos )
{
	ModelSkeleton	eModelSkeleton = m_cs.eModelSkeleton;
	const char*		szNodeName = g_pModelButeMgr->GetSkeletonNodeName(eModelSkeleton, (ModelNode)nNode);
	...
	if( GetNodeTransform( szNodeName, resultTransform ) != LTFALSE )
	{
		if( nNode == NODE_HEAD )
		{
			...
			tempVector += resultTransform.m_Rot.Forward() * 1.0f;
			resultTransform.m_Pos += tempVector;
		}
		else if( nNode == NODE_NECK )
		{
			LTVector tempVector;
			tempVector.Init();
			tempVector += resultTransform.m_Rot.Right() * 5.5f;			
			resultTransform.m_Pos += tempVector;
		}
		
		pvPos->x = resultTransform.m_Pos.x;		
		pvPos->y = resultTransform.m_Pos.y;		
		pvPos->z = resultTransform.m_Pos.z;		
	}
}

LTBOOL CCharacterFX::GetNodeTransform( const char* const pszNodeName, LTransform& NodeTransform )
{
	LTRESULT ltResult;
	HMODELNODE hNode;
	ltResult = g_pModelLT->GetNode(m_cs.hServerObj, const_cast<char*>(pszNodeName), hNode);
    ...
    LTransform transform;
    ltResult = g_pModelLT->GetNodeTransform(m_cs.hServerObj, hNode, NodeTransform, LTTRUE);
	if ( ltResult != LT_OK )
	{
		return LTFALSE;
	}
...
#ifdef _DEBUG
// 광식 : Model Test
void CCharacterFX::CreateTestModelFX()
{
	if( m_TestModelCount >= 100 ) return;

	HOBJECT hObj = m_TestModel[m_TestModelCount].GetObject();
	if( !hObj )
	{
		LTVector vPos;
		LTRotation rRot;
		...
		g_pLTClient->GetObjectRotation(hLocalObj, &rRot);
		m_TestModelScalecs.dwFlags = FLAG_VISIBLE | FLAG_NOLIGHT;
		m_TestModelScalecs.pFilename	= g_pModelButeMgr->GetModelFilename(ModelId(0));//"SA_CHARACTERS\\MODELS\\Korea_defense.ltb";
		
		CButeListReader blrSkinReader;
		blrSkinReader.SetItem(0, "SA_CHARACTERS\\SKINS\\K_defense_face.dtx", MAX_CS_FILENAME_LEN+1);
		m_TestModelScalecs.pSkinReader = &blrSkinReader;

		CButeListReader blrRenderStyles;
		blrRenderStyles.SetItem(0, "RenderStyles\\default.ltb", MAX_CS_FILENAME_LEN+1);
		m_TestModelScalecs.pRenderStyleReader = &blrRenderStyles;

		m_TestModelScalecs.vPos = vPos;
		m_TestModelScalecs.rRot = rRot;
		m_TestModelScalecs.vVel.Init();
		m_TestModelScalecs.vInitialScale.Init(1, 1, 1);
		m_TestModelScalecs.vFinalScale.Init(1, 1, 1);
		m_TestModelScalecs.vInitialColor.Init(1, 1, 1);
		m_TestModelScalecs.vFinalColor.Init(1, 1, 1);
        m_TestModelScalecs.bUseUserColors    = LTFALSE;
		m_TestModelScalecs.fLifeTime		= 1000000.0f;
		m_TestModelScalecs.fInitialAlpha	= 1.0f;
		m_TestModelScalecs.fFinalAlpha		= 1.0f;
        m_TestModelScalecs.bLoop            = LTTRUE;
		m_TestModelScalecs.fDelayTime		= 0.0f;
        m_TestModelScalecs.bAdditive        = LTFALSE;
		m_TestModelScalecs.nType			= OT_MODEL;

		if( m_TestModel[m_TestModelCount].Init(&m_TestModelScalecs) )
		{
			m_TestModel[m_TestModelCount].CreateObject(g_pLTClient);
			if( m_TestModel[m_TestModelCount].GetObject() )
			{
				uint32 dwAni = g_pLTClient->GetAnimIndex(m_TestModel[m_TestModelCount].GetObject(), "IF_SELECT");
				if( dwAni != (HMODELANIM)-1 )
				{
					g_pLTClient->SetModelAnimation(m_TestModel[m_TestModelCount].GetObject(), dwAni);
				}
			}
		}
		...
		if( m_TestModel[m_TestModelCount].GetObject() )
		{
			...
			SAFE_STRCPY(createStruct.m_Filename, g_pModelButeMgr->GetModelFilename(ModelId(0)));
			//SAFE_STRCPY(createStruct.m_Filename, g_pModelButeMgr->GetModelFilename(ModelId(3)));

			//char tempFilename [MAX_CS_FILENAME_LEN+1] = {0, };
			//sprintf(tempFilename, "SA_CHARACTERS\\MODELS\\Test.ltb");
			//SAFE_STRCPY(createStruct.m_Filename, tempFilename);

			if( g_pModelButeMgr->GetSkinReader(ModelId(0)) )
			//if( g_pModelButeMgr->GetSkinReader(ModelId(3)) )
			{
				g_pModelButeMgr->GetSkinReader(ModelId(0))->CopyList(0, createStruct.m_SkinNames[0], MAX_CS_FILENAME_LEN+1);
				//g_pModelButeMgr->GetSkinReader(ModelId(3))->CopyList(0, createStruct.m_SkinNames[0], MAX_CS_FILENAME_LEN+1);
			}
			//---------------------------------------------------------------------------
			char* szTemp;
			g_pModelButeMgr->GetModelHeadSkinFilename(ModelId(0), 0, szTemp);	
			//g_pModelButeMgr->GetModelHeadSkinFilename(ModelId(3), 0, szTemp);	
			if( szTemp ) strcpy( createStruct.m_SkinNames[0], szTemp );
			g_pModelButeMgr->GetModelBodySkinFilename(ModelId(0), 0, szTemp);
			//g_pModelButeMgr->GetModelBodySkinFilename(ModelId(3), 0, szTemp);
			if( szTemp ) strcpy( createStruct.m_SkinNames[1], szTemp );
			//---------------------------------------------------------------------------
			g_pModelButeMgr->CopyRenderStyleFilenames( ModelId(0), &createStruct );
			//g_pModelButeMgr->CopyRenderStyleFilenames( ModelId(3), &createStruct );
			g_pCommonLT->SetObjectFilenames(m_TestModel[m_TestModelCount].GetObject(), &createStruct);
...
void CCharacterFX::UpdateTestModelFX()
{
	/*
	// 광식 : Model test 함 해보자.
	//-----------------------------------------------------------------
	uint32 dwUpperFlags;
	uint32 dwLowerFlags;
	g_pModelLT->GetPlaybackState(m_hServerObject, 1, dwUpperFlags);
	g_pModelLT->GetPlaybackState(m_hServerObject, 2, dwLowerFlags);
	static bool btest = true;
	static bool blowertest = true;
	if( dwUpperFlags & MS_PLAYDONE )
	{
		// reset
		g_pModelLT->ResetAnim(m_hServerObject, 1);
		btest = true;
	}
	else
	{		
		if(btest)
		{
			g_pModelLT->SetCurAnim(m_hServerObject, 1, 41);	
			btest = false;
		}
		else
		{			
	...
	if( dwLowerFlags & MS_PLAYDONE )
	{
		g_pModelLT->ResetAnim(m_hServerObject, 2);
		blowertest = true;
	}
	else
	{
		if( blowertest )
		{
			g_pModelLT->SetCurAnim(m_hServerObject, 2, 12);
			blowertest = false;
		}
		else
		{
	...
	*/	
	...
	for( int i = 0; i < 100; i++ )
	{
		if( m_TestModel[i].GetObject() )
		{
		}
	}
}

void CCharacterFX::RemoveTestModelFX()
{
	for(int i = 0; i <100; i++ )
	{
		HOBJECT hObj = m_TestModel[i].GetObject();
		if (hObj)
		{
			g_pCommonLT->SetObjectFlags(hObj, OFT_Flags, 0, FLAG_VISIBLE);
		}
...
void CCharacterFX::CreateNodeRadiusModels()
{
	if( !m_cs.hServerObj )
		return;
	...
	for( int iNode = 0; iNode < cNodes; iNode++ )
	{
		...
		theStruct.m_Scale.Init(fNodeRadius, fNodeRadius, fNodeRadius);

        HOBJECT hModel = g_pLTClient->CreateObject(&theStruct);
		if (!hModel)
		{
			return;
		}
		...
		g_pLTClient->SetObjectColor(hModel, vColor.x, vColor.y, vColor.z, 0.9f);		

		m_NodeRadiusList.AddTail(pNRS);
	}
}

void CCharacterFX::RemoveNodeRadiusModels()
{
	if( m_NodeRadiusList.GetLength() )
	{
		...
		{
			g_pLTClient->RemoveObject((*pNRS)->hModel);
...
void CCharacterFX::UpdateNodeRadiusModels()
{	
	if (!m_NodeRadiusList.GetLength())
	{
	...
	while (pNRS && *pNRS && (*pNRS)->hModel)
	{
		...
		if ( GetNodeTransform( szNodeName, resultTransform ) != LTFALSE )
		{
			...
			g_pLTClient->SetObjectPos((*pNRS)->hModel, &(resultTransform.m_Pos));			
		}
...
LTVector CCharacterFX::GetNodeModelColor(ModelSkeleton eModelSkeleton, ModelNode eModelNode)
{
