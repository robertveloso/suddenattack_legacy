...
class CSFXMgr
{
	public :
		...
		//[MURSUM]=========================================
		void	RestartFX();
		void	SetCharFX_PlayerNick();
		void	CreateProjectileFX( HOBJECT hObj, const LTVector& vFirePos, uint8 nAmmoID );
		void	CreateHitFX( ILTMessage_Read *pMsg );
		void	CreateBodyHitFX( ILTMessage_Read* pMsg );
		//=================================================
		// ±¤½Ä
		LTBOOL	PlayerDeathAttach(HLOCALOBJ hServerObj );
		...
		HOBJECT			GetWeaponFromCharacter( HOBJECT hObject );
		CCharacterFX* GetCharacterFX(HOBJECT hObject);
		CCharacterFX* GetCharacterFX(uint8 nID);
		...
		CBodyFX* GetBodyFromParentID( uint8 nID );
