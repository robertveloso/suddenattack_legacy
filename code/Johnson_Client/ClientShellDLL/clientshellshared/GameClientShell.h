...
//[SPIKE] 시작로딩 화면 텍스쳐분할 갯수 
CONST UINT8 STARTLOADING_PARTITIONCOUNT = 9;
...
public:
	...
	static void OnMouseWheel(HWND hwnd, int x, int y, UINT Partition);
	static void OnWheelUp(HWND hwnd, int x, int y, UINT UpVelue);
	static void OnWheelDown(HWND hwnd, int x, int y, UINT DownVelue);
	...
	// 광식 : Logo
	void		LogoInitialize(uint32 nWidth, uint32 nHeight);
	void		LogoRender();
	void		ScreenRender(const char* lpszText);

	//[MURSUM]========================================================
	static void	OnConnectFail( LTBOOL bReturn, void *pData );
	void	SetPauseGame( bool bPause );
	void	SetGammaRamp( float fGamma );
	void	SetGammaRamp( float fGammaR, float fGammaG, float fGammaB );
	void	SetDrawLogo( bool bDraw ) { m_bDrawLogo = bDraw; }

	void	SetSwitchingWorldState( SwitchingWorldsState eState );

	void	CloneCharacter();

#ifndef _FINAL
	void	CreateSphere( LTVector vPos, float fR, float fG, float fB );
#endif

	void	DrawLogText( const char* lpszText );
	CSAD3DText*	m_pLogText;
	CSAD3DText*	m_p18Notice;
	//================================================================
...
protected :
	...
	// [NOVICE]
	LTRESULT	OnObjectPitch(HOBJECT hObj, float fPitch);
	
	void		OnHackDetected();
	
	LTRESULT	OnObjectAnimSwitch( HLOCALOBJ hObj, uint32 nAnimIndex, bool bLooping );
	void		HandleConeType( uint32 nConeType );
	...
	void		SendPingToGameServer();
	void		OnScreenShotNotify( const char* lpszFileName );
private:
...
public:
    ...
    void        HandleWeaponPickup(uint8 nWeaponId, uint32 nAmmoCount, uint8 nAmmoInClip);
	...
	void		HandleWeaponChange( uint16 nPlayerID, uint16 nWeaponID, uint16 nAmmoCount, uint16 nAmmoInClip );
	...
	// SPIKE
	CGlobalClientMgr*	GetGlobalClientMgr()		{ return &m_GlobalMgr; }
	LTBOOL				IsMainWindowFocused()		{ return m_bMainWindowFocus; }

	bool				IsSoundInitted()			{ return m_bSoundInitted; }
...
// should be added here (to keep them together)...
	...
	void	HandleMsgHHWeapon				(ILTMessage_Read*);
	void	HandleMsgHitSFX					(ILTMessage_Read*);
	void	HandleMsgBodyHitSFX				(ILTMessage_Read*);
	
	void	HandleMsgWeaponFire				(ILTMessage_Read*);
	void	HandleMsgDeathUpdate			(ILTMessage_Read*);
	...
	CClientMultiplayerMgr*	m_pClientMultiplayerMgr;
	...
	CSAD3DTextMgr	m_SAD3DTextMgr;

	// 광식 : Initialize 화면의 Logo
	//------------------------------------
	HTEXTURE		m_hLogoFrame[STARTLOADING_PARTITIONCOUNT];
	LTPoly_GT4		m_pLogoPoly[STARTLOADING_PARTITIONCOUNT];
	LTRect			m_rLogoRect[STARTLOADING_PARTITIONCOUNT];
	HTEXTURE		m_hAgeMark;
	LTPoly_GT4		m_PolyAge;
	bool			m_bDrawLogo;
	//------------------------------------
