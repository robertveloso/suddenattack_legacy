...
LTBOOL CBodyFX::Init(SFXCREATESTRUCT* psfxCreateStruct)
{
	...
	const char* pFilename = g_pModelButeMgr->GetModelFilename(m_bs.eModelId);
	SAFE_STRCPY(ocs.m_Filename, pFilename);
	g_pModelButeMgr->CopySkinFilenames(m_bs.eModelId, 0, ocs.m_SkinNames[0], MAX_CS_FILENAME_LEN+1);
	g_pModelButeMgr->CopyRenderStyleFilenames(m_bs.eModelId, &ocs);
	char* szTemp;
	g_pModelButeMgr->GetModelHeadSkinFilename(m_bs.eModelId, m_nHeadSkinId, szTemp);	
	if( szTemp ) strcpy( ocs.m_SkinNames[0], szTemp );
	g_pModelButeMgr->GetModelBodySkinFilename(m_bs.eModelId, m_nBodySkinId, szTemp);
	if( szTemp ) strcpy( ocs.m_SkinNames[1], szTemp );
	g_pCommonLT->SetObjectFilenames(m_hServerObject, &ocs);
	//----------------------------------------------------------	
	*/
...
LTBOOL CBodyFX::Update()
{
	...
	*/
	...
	if(IsLiquid(eContainerCode))
	{
		...
		g_pLTClient->SetObjectPos(m_hServerObject, &vPos);
		m_bDeadInWater = LTTRUE;
	}
	else if(m_bDeadInWater)
	{
		vPos.x += GetRandom(-1.0f, 1.0f);
		vPos.z += GetRandom(-1.0f, 1.0f);
		g_pLTClient->SetObjectPos(m_hServerObject, &vPos);
...
void CBodyFX::CreateBackpack()
{
	...
	//[MURSUM]====
	//경로수정
	SAFE_STRCPY(createStruct.m_Filename, "SA_PROPS\\models\\PuBackpack.ltb");
	SAFE_STRCPY(createStruct.m_SkinNames[0], "SA_PROPS\\skins\\PuBackpack.dtx");
	SAFE_STRCPY(createStruct.m_RenderStyleNames[0], "RENDERSTYLE\\glass.ltb");
	//===
...
void CBodyFX::GetNodePos( uint8 nNode, LTVector* pvPos )
{
	const char*		szNodeName = g_pModelButeMgr->GetSkeletonNodeName(GetModelSkeleton(), (ModelNode)nNode);
...
LTBOOL CBodyFX::GetNodeTransform( const char* const pszNodeName, LTransform& NodeTransform )
{
	LTRESULT ltResult;
	HMODELNODE hNode;
	ltResult = g_pModelLT->GetNode(m_bs.hServerObj, const_cast<char*>(pszNodeName), hNode);
    ...
    ltResult = g_pModelLT->GetNodeTransform(m_bs.hServerObj, hNode, NodeTransform, LTTRUE);
	if ( ltResult != LT_OK )
	{
		return LTFALSE;
	}
