...
LTBOOL CBulletTrailFX::Init(SFXCREATESTRUCT* psfxCreateStruct)
{
	...
	m_pTextureName	= (GetRandom(0,1) != 1) ? DEFAULT_BUBBLE_TEXTURE : DEFAULT_BUBBLE_TEXTURE1;
...
LTBOOL CBulletTrailFX::Update()
{
	...
	if (m_bFirstUpdate)
	{
		...
		ContainerCode eFirePosCode = CC_NO_CONTAINER;
		...
		if (dwNum > 0 && objList[0])
		{
			...
			if (m_pClientDE->GetContainerCode(objList[0], &dwCode))
			{
				...
				eFirePosCode = (ContainerCode)dwCode;
			}
		}
		...
		//[SPIKE] 물속뿐만 아니라 물밖에서도 물속 궤적 그리기 위해 =============
		if(!IsLiquid(eFirePosCode) && iInfo.m_SurfaceFlags == 201)
		{
			m_vStartPos = iInfo.m_Point  + (m_vDir * 10.0f);
			VEC_MULSCALAR(vTemp, m_vDir, MAX_TRAIL_LENGTH);
			VEC_ADD(vEndPoint, m_vStartPos, vTemp);
		...
		else if(!IsLiquid(eFirePosCode) && iInfo.m_SurfaceFlags != 201)
		{
			m_fDistance = 0.0f;
			m_fFadeTime = 0.0f;
		}
		m_fNumParticles = m_fDistance / 20.0f;
		//=======================================================================		
	...
	for (int i=0; i < nNumParticles; i++)
	{
		...
		for (int j=0; j < 1; j++)
		{
			...
			VEC_SET(vDriftVel, GetRandom(-1.0f, 1.0f), GetRandom(-1.0f, 4.0f), GetRandom(-1.0f, 1.0f));
