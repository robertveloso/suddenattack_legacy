...
public:
    ...
    virtual void        UpdateAir(uint8 nAirMode) = 0;
	...
	virtual inline uint8    GetHalf_Scope(const WEAPON* pW=LTNULL) = 0;
	...
	virtual void		 SetAmmoCount( uint8 nAmmoId, uint32 nAmmoCount ) = 0;
	...
	// ±¤½Ä
	virtual void		RestartWeapon() = 0;
	virtual LTBOOL		ThrowWeapon( int nWeaponId, int nClass ) = 0;
	
	// SPIKE
	virtual int*		GetOrderedWeaponId() = 0;
	
	//[MURSUM]
	virtual void		RestartWpnChooser() = 0;
};
