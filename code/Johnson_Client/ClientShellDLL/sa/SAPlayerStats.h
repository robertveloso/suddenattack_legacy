...
#ifndef __SAPLAYERSTATS_H
#define __SAPLAYERSTATS_H
...
#define MAX_WEAPON_CLASS		5

class CSAPlayerStats : public CPlayerStats
{
public:
	...
	//void		SetCanUseWeapon( int nWeapon );
	void		SetCanUseWeapon( int nWeapon, int nClass );
	void		SetResetWeapon( );
	void		RestartWeapon();
	void		RestartWpnChooser();
	...
	inline uint8    GetHalf_Scope(const WEAPON* pW=LTNULL)      { return GetMod(HALF_SCOPE, pW); }
	...
	void		 SetAmmoCount( uint8 nAmmoId, uint32 nAmmoCount );
    ...
    uint8		GetAirPoint()	{ return m_nAir;}
	...
	// 광식
	LTBOOL		ThrowWeapon( int nWeaponId, int nClass );

	//SPIKE
	//int*        GetOrderedWeaponId(){return m_nOrderedWeaponId;}
	int*        GetOrderedWeaponId(){return m_nTempOrderWeaponId;}
...
protected:
	...
	LTBOOL*		m_pbSelectAmmo;		// 채널에서 무기를 가지로 들어올때 세팅된다.
	int			m_nOrderedWeaponId[MAX_WEAPON_CLASS];
	int			m_nTempOrderWeaponId[MAX_WEAPON_CLASS];
