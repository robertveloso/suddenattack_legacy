...
{	
	// Sensitivity를 위한 Bute객체 
	m_buteMgr.Term();
	m_buteMgr.Parse("Profiles\\Player.txt");

	// 마우스 민감도 Text ====================================================================
	m_pSensitivityText = new CSAD3DTextEx();		
	m_pSensitivityText->CreateD3DTextWithFontSize( 256, 20, 12, true );
...
CHUDSensiCtrl::Term()
{
	if(m_pSensitivityText)
	{
		delete m_pSensitivityText;
		m_pSensitivityText = LTNULL;
	}
}
...
CHUDSensiCtrl::CalculateSensitivity(LTBOOL bLow_Or_High)
{
	...
	m_buteMgr.SetInt("Controls","Sensitivity",iCurrentSensi);	// 세팅 
	m_buteMgr.Save("Profiles\\Player.txt");						// 저장 
