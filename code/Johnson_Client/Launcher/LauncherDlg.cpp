...
BOOL CLauncherDlg::LaunchGame(DWORD dwAction)
{
	...
	{
		AfxMessageBox("ID를 입력하세요!");
		CEdit* pEdit = (CEdit*) GetDlgItem(IDC_EDIT_USERID);
		if (pEdit)
			pEdit->SetFocus();
	}
// Edited By IN'T ==>
// Time : 2004-04-28 오전 11:38:19
...
*/
	...
	CString strNewCmdLine;
	char szCmdLine[MAX_PATH];

	fscanf(fp, " %[^\n]", szCmdLine);	// 한줄을 전부 읽음
	strNewCmdLine.Format("%s", szCmdLine);
	fclose(fp);

	strNewCmdLine += " +UserID " + m_strUserID;
//	fprintf(fp, strNewCmdLine);
//	fclose(fp);

	CString strCommand = "lithtech.exe ";
	strCommand += strNewCmdLine;;
//	strCommand.Format("lithtech.exe -cmdfile launchcmds.txt";
//	strCommand += " +UserID " + m_strUserID;

	if (!CreateProcess(NULL, (LPSTR)(LPCTSTR)strCommand, NULL, NULL, FALSE, CREATE_DEFAULT_ERROR_MODE | CREATE_NEW_PROCESS_GROUP,	NULL, NULL, &si, &pi))
	{
