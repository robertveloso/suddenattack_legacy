...
// Edited By IN'T ==>
// Time : 2004-04-29 오전 10:07:20
// void CTeam::Init(uint8 nID, const char *szName, uint32 nModel)
void CTeam::Init(uint8 nID, const char *szName)
// Edited By IN'T <==
{
	m_nID = nID;
	m_nScore = 0;
...
//
//}
uint8 CTeamMgr::AddTeam(const char *szName, uint32 nTeamID)
{
	CTeam* pTeam = debug_new(CTeam);
	if( !pTeam )
		return INVALID_TEAM;

	pTeam->Init(nTeamID, szName);
...
CTeam*	CTeamMgr::GetTeamWithLeastPlayers()
{
	...
	{
		// 광식 :Spectator Mode 삭제
// Added By IN'T ==>
// Time : 2004-04-29 오전 10:37:41
// Spectator 가 적다고 자동으로 Spectator 에 넣으면 안되니까
		//if (!(*iter)->GetID())		// TeamID == 0 => Spectator
		//	continue;
// Added By IN'T <==
