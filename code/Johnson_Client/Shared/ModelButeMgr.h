...
class CModelButeMgr : public CGameButeMgr
{
	...
	public : // Public member variables
		...
		ModelSkeleton	GetModelBigSkeleton(ModelId eModelId);
		ModelSkeleton	GetModelFatSkeleton(ModelId eModelId);
		...
		// ±¤½Ä
		LTBOOL			GetModelSleeve( ModelId eModelId );
		...
		const char*		GetMultiBigModelFilename(ModelId eModelId);
		const char*		GetMultiFatModelFilename(ModelId eModelId);
		...
		// ±¤½Ä
		//-----------------------------------------------------------------
		const char*		GetSleeveSkinFilename(ModelId eModelId);
		const char*		GetArmsSkinFilename(ModelId eModelId);
		// shop °ü·Ã
		std::string		GetShopNormalIcon(ModelId eModelId) const;
		std::string		GetShopDetailIcon(ModelId eModelId) const;
		int				GetShopIconNameIndex(ModelId eModelId);
		//-----------------------------------------------------------------

		int				GetModelTeamID(ModelId eModelId);
		int				GetModelDescIndex(ModelId eModelId);
		...
		// ±¤½Ä
		//--------------------------------------------------------------------------
		const char*		GetModelHeadSkinTitlename(ModelId eModelId);
		void			GetModelHeadSkinFilename(ModelId eModelId, int nSkinNumber, const char*& szFileName);
		uint8			GetNumModelHeadSkins(ModelId eModelId);
		const char*		GetModelBodySkinTitlename(ModelId eModelId);
		void			GetModelBodySkinFilename(ModelId eModelId, int nSkinNumber, const char*& szFileName);
		uint8			GetNumModelBodySkins(ModelId eModelId);
		//--------------------------------------------------------------------------
		...
		SurfaceType		GetHeadSurfaceType(ModelId eModelId);
		...
		//-----------------------------------------------------------
		int				GetNumTeamRedModels()	{ return m_cTeamRedModels; }
		ModelId			GetTeamRedModel(int num);
		int				GetNumTeamBlueModels()	{ return m_cTeamBlueModels; }
		ModelId			GetTeamBlueModel(int num);
		//-----------------------------------------------------------
	...
	protected : // Protected member variables
		...
		// ±¤½Ä [05.03.18] : ModelÀ» ÆÀ¿¡ µû¶ó ±¸ºÐÇÏ±â.
		//-----------------------------------------------
		int				m_cTeamRedModels;
		ModelId*		m_aTeamRedModels;

		int				m_cTeamBlueModels;
		ModelId*		m_aTeamBlueModels;
		//-----------------------------------------------
...
typedef std::vector<sSkinPath> SKIN_FILE_LIST;
//-----------------------------------------------------------------------
...
class CModelButeMgr::CModel
{
	public :
		...
		CModel()
		{
			...
			m_eModelBigSkeleton	= eModelSkeletonInvalid;
			m_eModelFatSkeleton	= eModelSkeletonInvalid;
			...
			m_szModelFile[0] = 0;
			m_szBigModelFile[0]	= 0;
			m_szFatModelFile[0] = 0;
			m_szHandsSkin[0] = 0;
			m_szSleeveSkin[0] = 0;
			m_szArmsSkin[0] = 0;
			m_szAIName[0] = 0;
			...
			m_eHeadSurfaceType	= ST_HEAD;	
			// ±¤½Ä
			//--------------------------------------
			m_szHeadSkinName[0] = 0;
			m_szBodySkinName[0] = 0;
			m_cHeadSkinFiles	= 0;
			m_cBodySkinFiles	= 0;
			m_IsSleeve			= LTTRUE;
			m_szIcon[0]			= 0;
		...
		~CModel()
		{
			...
			// ±¤½Ä
			//--------------------------------------------------------------------------------------
			SKIN_FILE_LIST::iterator headit;
			for( headit = m_lstHeadSkinFile.begin(); headit != m_lstHeadSkinFile.end(); ++headit )
			{
				debug_deletea( headit->szFilePath );
			}

			SKIN_FILE_LIST::iterator bodyit;
			for( bodyit = m_lstBodySkinFile.begin(); bodyit != m_lstBodySkinFile.end(); ++bodyit )
			{
				debug_deletea( bodyit->szFilePath );
			}
			//--------------------------------------------------------------------------------------			
		}
	...
	public :
		...
		ModelSkeleton	m_eModelBigSkeleton;
		ModelSkeleton	m_eModelFatSkeleton;
		...
		char			m_szBigModelFile[MAX_MODELBMGR_MAX_PATH];
		char			m_szFatModelFile[MAX_MODELBMGR_MAX_PATH];
		char			m_szHandsSkin[MAX_MODELBMGR_MAX_PATH];
		// ±¤½Ä
		//-------------------------------------------------------
		char			m_szSleeveSkin[MAX_MODELBMGR_MAX_PATH];
		char			m_szArmsSkin[MAX_MODELBMGR_MAX_PATH];
		//-------------------------------------------------------
		...
		SurfaceType		m_eHeadSurfaceType;
		...
		// ±¤½Ä
		//------------------------------------------------
		int				m_cHeadSkinFiles;
		SKIN_FILE_LIST	m_lstHeadSkinFile;								// Skin File Path List
		char			m_szHeadSkinName[MAX_MODELBMGR_NAME_LEN];		// Skin Name
		int				m_cBodySkinFiles;
		SKIN_FILE_LIST	m_lstBodySkinFile;								// Skin File Path List
		char			m_szBodySkinName[MAX_MODELBMGR_NAME_LEN];		// Skin Name
		BOOL			m_IsSleeve;
		char			m_szIcon[64];									// Shop ¿¡¼­ º¸¿©Áö´Â ³ð
		char			m_szIconName[64];								// Shop ¿¡¼­ º¸¿©Áö´Â ÀÌ¸§
		int				m_nIconNameIndex;
		//------------------------------------------------
