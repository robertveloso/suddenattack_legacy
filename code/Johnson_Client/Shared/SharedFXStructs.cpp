...
void CHARCREATESTRUCT::Write(ILTMessage_Write *pMsg)
{
	...
	pMsg->Writeuint8(eModelLoadType);
}
...
void CHARCREATESTRUCT::Read(ILTMessage_Read *pMsg)
{
    ...
    nTrackers       = 2;	//"Upper" N' "Lower"
    nDimsTracker    = 0xFF; //eAniTrackerInvalid
	fStealthPercent	= 1.0f;
    nClientID       = pMsg->Readuint8();
   	eCrosshairCharacterClass	= UNKNOWN;
	nDamageFlags	= 0;
	fPitch			= 0.5f;
	eModelLoadType	= (ModelLoadType)pMsg->Readuint8();
	...
	{
		switch( eModelLoadType )
		{
			case eModelNormal :
				eModelSkeleton = g_pModelButeMgr->GetModelSkeleton(eModelId);
				break;
			case eModelBig :
				eModelSkeleton = g_pModelButeMgr->GetModelBigSkeleton(eModelId);
				break;
			case eModelFat :
				eModelSkeleton = g_pModelButeMgr->GetModelFatSkeleton(eModelId);
				break;
		}
		//eModelSkeleton	= g_pModelButeMgr->GetModelSkeleton(eModelId);
...
void BODYCREATESTRUCT::Read(ILTMessage_Read *pMsg)
{
	...
	// ±¤½Ä
	//-------------------------------------------------
	//vHitBoxDims.x = 240.0f;
	//vHitBoxDims.y = 15.0f;
	//vHitBoxDims.z = 960.0f;
