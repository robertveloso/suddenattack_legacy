...
// 광식 : 무기의 무게에 따른 속도 변화를 주기 위해 추가 함
#define WMGR_WEAPON_WEIGHT					"Weight"
#define WMGR_WEAPON_FIRETYPE				"FireType"

#define	WMGR_WEAPON_PERTURBINCSPEEDFIRE     "PerturbIncreaseSpeedFire"
#define	WMGR_WEAPON_PERTURBDECSPEEDFIRE		"PerturbDecreaseSpeedFire"
#define	WMGR_WEAPON_SPECIALMODEDECSPEEDFIRE	"SpecialModeDecrease"
#define	WMGR_WEAPON_FIRERECOILPITCHMAX		"FireRecoilPitchMax"
#define	WMGR_WEAPON_FIREMAXGABOFFSET		"FireMaxGapOffset"
#define	WMGR_WEAPON_FIRECAMRECOILYAW		"FireCamRecoilYaw"
#define WMGR_WEAPON_FIRECAMRECOILYAWMAX		"FireCamRecoilYawMax"

#define WMGR_WEAPON_PERTURBDUCK				"PerturbDuck"
#define WMGR_WEAPON_PERTURBSTANDARD			"PerturbStandard"
#define WMGR_WEAPON_PERTURBMOVE				"PerturbMove"
#define WMGR_WEAPON_PVMODELFOVY				"PVModelFOVY"
#define WMGR_WEAPON_PVMODELASPECT			"PVModelAspect"

#define WMGR_WEAPON_PVMODELPITCH			"PVModelPitch"
#define WMGR_WEAPON_PVMODELYAW				"PVModelYaw"
#define WMGR_WEAPON_PVMODELROLL				"PVModelRoll"

// 광식
//-------------------------------------------------------------------------------
#define WMGR_WEAPON_PERFORMANCE				"Performance"
#define WMGR_WEAPON_DESCRIPTION				"Description"
#define WMGR_WEAPON_ITEM_NAME				"IconNameIndex"
//-------------------------------------------------------------------------------
...
#define WMGR_AMMO_SPECIALINSTDAMAGE "SpecialInstDamage"
...
#define WMGR_MOD_MAXZOOMLEVEL			"MaxZoomLevel"
...
CWeaponMgr::CWeaponMgr()
{
	...
	m_nFileCRC = 0;
	m_bSpecialMode = LTFALSE;
	m_bPlantedTimeBomb = LTFALSE;
...
/*	if(nMinPerturb >= 0 )
	{
		int nPerturb = nMinPerturb + (int)(wp.fPerturbR * 40);
		LTFLOAT fRPerturb = ((LTFLOAT)GetRandom(-nPerturb, nPerturb))/1000.0f;
		
		nPerturb = nMinPerturb + (int)(wp.fPerturbU * 40);
		LTFLOAT fUPerturb = ((LTFLOAT)GetRandom(-nPerturb, nPerturb))/1000.0f;

		wp.vPath += (wp.vR * fRPerturb); 
	//	wp.vPath += (wp.vU * fUPerturb); // 위아래는 적용하지 말자!!
	...
	*/
	if(nMinPerturb >= 0 )
	{
		...
		LTFLOAT fUPerturb, fRPerturb = ((LTFLOAT)GetRandom(-nPerturb, nPerturb))/1000.0f;
		wp.vPath += (wp.vR * fRPerturb); 

		if(nPerturb > 25)
		{		
			if(fabs(g_fUPerturbTotal) > pWeapon->fFireMaxGapOffset)
			{
				g_fUp *= -1;
				if(g_fUPerturbTotal > 0)
				{
					g_fUPerturbTotal = pWeapon->fFireMaxGapOffset;
				}
				else
				{
					g_fUPerturbTotal = -(pWeapon->fFireMaxGapOffset);
			...
			g_fUPerturbTotal += g_fUp * ((float)nPerturb/GetRandom(1.5f, 3.0f));
			LTFLOAT fRandomOffset = g_fUPerturbTotal / 10.0f;
			fUPerturb = (GetRandom(g_fUPerturbTotal-fRandomOffset, g_fUPerturbTotal+fRandomOffset) /1000.0f);
		}
		else
		{
			g_fUp = 1.0;
			g_fUPerturbTotal = 0.0f;

			fUPerturb = ((LTFLOAT)GetRandom(-nPerturb, nPerturb))/1000.0f;
		}
		wp.vPath += (wp.vU * fUPerturb); 
...
WEAPON::WEAPON()
{
	...
	for(i=0; i<WMGR_MAX_MUZZLEFLASH; i++)
		szHHMuzzleFxName[i]	= LTNULL;
	...
	fFireRecoilPitchMax	= 0.0f;
	fFireMaxGapOffset  = 0.0f;
	fFireCamRecoilYaw   = 3.0f;
	fFireCamRecoilYawMax= 35.0f;
...
WEAPON::~WEAPON()
{
	...
	for(i=0; i<WMGR_MAX_MUZZLEFLASH; i++)
		debug_deletea( szHHMuzzleFxName[i] );
...
void WEAPON::InitMembers( CButeMgr &buteMgr, char *aTagName )
{
	...
	// 광식 : 총의 무게 얻어오기
	m_fWeight = (float) buteMgr.GetDouble(aTagName, WMGR_WEAPON_WEIGHT, 0.0f);
	m_nFireType = buteMgr.GetInt(aTagName, WMGR_WEAPON_FIRETYPE, 0);

	m_Description = buteMgr.GetInt(aTagName, WMGR_WEAPON_DESCRIPTION, 113);
	m_Performance = buteMgr.GetRect(aTagName, WMGR_WEAPON_PERFORMANCE);
	m_nIconNameIndex  = buteMgr.GetInt( aTagName, WMGR_WEAPON_ITEM_NAME, 0 );

	fPerturbIncreaseSpeedFire = (LTFLOAT) buteMgr.GetDouble (aTagName, WMGR_WEAPON_PERTURBINCSPEEDFIRE, 4.0f) ;
	fPerturbDecreaseSpeedFire = (LTFLOAT) buteMgr.GetDouble (aTagName, WMGR_WEAPON_PERTURBDECSPEEDFIRE, 1.0f) ;
	
	//[SPIKE] //////////////////////////////////////
	fSpecialModeDecrease	  = (LTFLOAT) buteMgr.GetDouble (aTagName, WMGR_WEAPON_SPECIALMODEDECSPEEDFIRE, 1.0f) ;
	m_fPerturbDuck			  = (LTFLOAT) buteMgr.GetDouble (aTagName, WMGR_WEAPON_PERTURBDUCK, 1.5f) ;
	m_fPerturbStandard		  = (LTFLOAT) buteMgr.GetDouble (aTagName, WMGR_WEAPON_PERTURBSTANDARD, 1.5f) ;
	m_fPerturbMove			  = (LTFLOAT) buteMgr.GetDouble (aTagName, WMGR_WEAPON_PERTURBMOVE, 1.5f) ;
	m_fPVModelFOVY			  = (LTFLOAT) buteMgr.GetDouble (aTagName, WMGR_WEAPON_PVMODELFOVY, 68.0f) ;
	m_fPVModelAspect		  = (LTFLOAT) buteMgr.GetDouble (aTagName, WMGR_WEAPON_PVMODELASPECT, 1.333333f) ;
	
	m_fPVModelPitch		  = (LTFLOAT) buteMgr.GetDouble (aTagName, WMGR_WEAPON_PVMODELPITCH, 0.0f) ;
	m_fPVModelYaw		  = (LTFLOAT) buteMgr.GetDouble (aTagName, WMGR_WEAPON_PVMODELYAW, 0.0f) ;
	m_fPVModelRoll		  = (LTFLOAT) buteMgr.GetDouble (aTagName, WMGR_WEAPON_PVMODELROLL, 0.0f) ;
	
	for( int i = 0; i < WMGR_MAX_MUZZLEFLASH; ++i )
	{
		sprintf(s_aAttName, "%s%d", WMGR_WEAPON_HHMUZZLEFXNAME, i );
		GetStringIfExist( buteMgr, aTagName, s_aAttName, &szHHMuzzleFxName[i], WMGR_MAX_NAME_LENGTH );
	}
	////////////////////////////////////////////////
	for( i = 0; i < WMGR_MAX_RELOAD_SNDS; ++i )
	{
	...
	fFireRecoilPitchMax = (LTFLOAT)buteMgr.GetDouble(aTagName, WMGR_WEAPON_FIRERECOILPITCHMAX, fFireRecoilPitchMax) ;
	fFireMaxGapOffset  = (LTFLOAT)buteMgr.GetDouble(aTagName, WMGR_WEAPON_FIREMAXGABOFFSET, 40.0f) ;
	fFireCamRecoilYaw   = (LTFLOAT)buteMgr.GetDouble(aTagName, WMGR_WEAPON_FIRECAMRECOILYAW, 3.0);
	fFireCamRecoilYawMax= (LTFLOAT)buteMgr.GetDouble(aTagName, WMGR_WEAPON_FIRECAMRECOILYAWMAX, 35.0);
...
std::string WEAPON::GetSelectedIcon() const
{
	std::string str(szIcon);
	str += "S.dtx";
	return str;
...
std::string WEAPON::GetInvenTitleIcon()	const
{
	std::string str(szIcon);
	str += "_IT.dtx";	// Inventory Title
	return str;
}

std::string WEAPON::GetInvenNormalIcon() const
{
	std::string str(szIcon);
	str += "_IN.dtx";	// Inventory Normal
	return str;
}

std::string	WEAPON::GetInvenSelectIcon() const
{
	std::string str(szIcon);
	str += "_IS.dtx";	// Inventory Select
	return str;
}
...
void AMMO::InitMembers( CButeMgr &buteMgr, char *aTagName )
{
	...
	nSpecialInstDamage    = buteMgr.GetInt( aTagName, WMGR_AMMO_SPECIALINSTDAMAGE, nSpecialInstDamage );

	//[MURSUM]
	//pszValue = buteMgr.GetString( aTagName, WMGR_AMMO_INSTDAMAGETYPE, "" );
	//if( pszValue[0] ) eInstDamageType       = StringToDamageType( pszValue );
	...
	//if( pszValue[0] )	eAreaDamageType       = StringToDamageType( pszValue );
	...
	//if( pszValue[0] ) eProgDamageType       = StringToDamageType( pszValue );
...
void MOD::InitMembers( CButeMgr &buteMgr, char *aTagName )
{
	...
	nMaxZoomLevel		= buteMgr.GetInt( aTagName, WMGR_MOD_MAXZOOMLEVEL, nMaxZoomLevel );
