...
#define	MODELBMGR_MODEL					"Model"
	...
	#define MODELBMGR_MODEL_BIG_SKELETON						"BigSkeleton"
	#define MODELBMGR_MODEL_FAT_SKELETON						"FatSkeleton"
	...
	#define	MODELBMGR_MODEL_BIG_MODEL							"BigModelFile"
	#define MODELBMGR_MODEL_FAT_MODEL							"FatModelFile"
	#define	MODELBMGR_MODEL_SKIN								"Skin"
	#define	MODELBMGR_MODEL_HANDSSKIN							"HandsSkin"
	// ±¤½Ä
	//-----------------------------------------------------------------------------
	#define MODELBMGR_MODEL_SLEEVESKIN							"SleeveSkin"
	#define MODELBMGR_MODEL_ARMSSKIN							"ArmsSkin"
	#define MODELBMGR_MODEL_SHOP_ICON							"ShopIcon"
	#define MODELBMGR_MODEL_SHOP_ICON_INDEX						"IconNameIndex"
	//-----------------------------------------------------------------------------
	...
	#define MODELBMGR_MODEL_HEADSURFACETYPE						"HeadSurfaceType"
	#define MODELBMGR_MODEL_NAMEID								"NameId"
	#define MODELBMGR_MODEL_PLAYERPAINSNDDIR					"PlayerPainSoundDir"
// ±¤½Ä
//-----------------------------------------------------------------------------
	#define MODELBMGR_MODEL_SLEEVE								"ModelSleeve"
//-----------------------------------------------------------------------------

	#define MODELBMGR_MODEL_TEAM_ID								"TeamID"
	#define MODELBMGR_MODEL_DESCRIPTION							"DescIndex"
...
// Edited By IN'T <===
	...
	#define MODELBMGR_MODEL_DEFAULT_HEADSURFACETYPE				"Head"
	// ±¤½Ä
	#define MODELBMGR_MODEL_DEFAULT_SLEEVE						LTTRUE
...
// ±¤½Ä [05.03.18] : Model À» ÆÀ¿¡ µû¶ó ±¸ºÐ
//--------------------------------------------------------------------------
#define MODELBMGR_TEAM_RED_MODEL		"MultiRedTeamModels"
#define MODELBMGR_TEAM_BLUE_MODEL		"MultiBlueTeamModels"
//--------------------------------------------------------------------------

	#define	MODELBMGR_MP_MODEL_NAME								"Name"
	#define	MODELBMGR_MP_MODEL_TEAM								"Team"

// ±¤½Ä [05.03.18] : Model ÀÇ Skin À» User°¡ ¼±ÅÃ ÇÒ ¼ö ÀÖµµ·Ï ¿©·¯ ÇüÅÂ °®µµ·Ï ¼öÁ¤
//-----------------------------------------------------------------------------------
#define MODELBMGR_MODEL_HEAD_SKIN_FILE	"HeadSkinFile"
#define MODELBMGR_MODEL_BODY_SKIN_FILE	"BodySkinFile"

#define MODELBMGR_HEAD_SKIN				"HeadSkin"
#define MODELBMGR_BODY_SKIN				"BodySkin"
//#define MODELBMGR_SKIN_FILE_NAME							"SkinName"
#define MODELBMGR_SKIN_FILE_PATH							"SkinFile"
//-----------------------------------------------------------------------------------
...
CModelButeMgr::CModelButeMgr()
{
	...
	// ±¤½Ä [05.03.18] : ModelÀ» ÆÀ¿¡µû¶ó ±¸ºÐÇÏ±â.
	//-------------------------------------------------
	m_cTeamRedModels	= 0;
	m_aTeamRedModels	= LTNULL;
	m_cTeamBlueModels	= 0;
	m_aTeamBlueModels	= LTNULL;
	//-------------------------------------------------
...
LTBOOL CModelButeMgr::Init(const char* szAttributeFile)
{
	...
	for ( int iModel = 0 ; iModel < m_cModels ; iModel++ )
	{
		...
		m_aModels[iModel].m_eModelBigSkeleton = (ModelSkeleton)(uint8)m_buteMgr.GetInt(s_aTagName, MODELBMGR_MODEL_BIG_SKELETON);
		m_aModels[iModel].m_eModelFatSkeleton = (ModelSkeleton)(uint8)m_buteMgr.GetInt(s_aTagName, MODELBMGR_MODEL_FAT_SKELETON);
		...
		m_buteMgr.GetString(s_aTagName, MODELBMGR_MODEL_BIG_MODEL, MODELBMGR_MODEL_DEFAULT_MODELFILE,m_aModels[iModel].m_szBigModelFile, MAX_MODELBMGR_MAX_PATH);
		m_buteMgr.GetString(s_aTagName, MODELBMGR_MODEL_FAT_MODEL, MODELBMGR_MODEL_DEFAULT_MODELFILE,m_aModels[iModel].m_szFatModelFile, MAX_MODELBMGR_MAX_PATH);
		...
		// ±¤½Ä
		//------------------------------------------------------------------------------------------------------------------------------------------------------
		m_buteMgr.GetString(s_aTagName, MODELBMGR_MODEL_SLEEVESKIN, MODELBMGR_MODEL_DEFAULT_SLEEVESKIN,m_aModels[iModel].m_szSleeveSkin, MAX_MODELBMGR_MAX_PATH);
		m_buteMgr.GetString(s_aTagName, MODELBMGR_MODEL_ARMSSKIN, MODELBMGR_MODEL_DEFAULT_ARMSSKIN, m_aModels[iModel].m_szArmsSkin, MAX_MODELBMGR_MAX_PATH);
		// shop °ü·Ã
		m_buteMgr.GetString(s_aTagName, MODELBMGR_MODEL_SHOP_ICON, MODELBMGR_MODEL_DEFAULT_SHOP_ICON, m_aModels[iModel].m_szIcon, MAX_MODELBMGR_MAX_PATH);
		m_aModels[iModel].m_nIconNameIndex = m_buteMgr.GetInt(s_aTagName, MODELBMGR_MODEL_SHOP_ICON_INDEX);
		//------------------------------------------------------------------------------------------------------------------------------------------------------

		m_aModels[iModel].m_nTeamID	= m_buteMgr.GetInt(s_aTagName, MODELBMGR_MODEL_TEAM_ID);
		m_aModels[iModel].m_nDescIndex = m_buteMgr.GetInt(s_aTagName,MODELBMGR_MODEL_DESCRIPTION);
		...
		//[SPIKE] Head
		m_buteMgr.GetString( s_aTagName, MODELBMGR_MODEL_HEADSURFACETYPE, MODELBMGR_MODEL_DEFAULT_HEADSURFACETYPE, pszSurfaceType, MAX_MODELBMGR_NAME_LEN );
		pSurface = g_pSurfaceMgr->GetSurface(pszSurfaceType);
		m_aModels[iModel].m_eHeadSurfaceType = (pSurface) ? pSurface->eType : ST_HEAD;
		...
		// ±¤½Ä		
		m_aModels[iModel].m_IsSleeve = (LTBOOL)m_buteMgr.GetBool(s_aTagName, MODELBMGR_MODEL_SLEEVE);
		...
		// ±¤½Ä : ModelÀÌ °¡Áú ¼ö ÀÖ´Â Head Skin °ú Body Skin
		//-------------------------------------------------------------
		// Head Skin Read
		uint8 skinNumber = (uint8)m_buteMgr.GetInt(s_aTagName, MODELBMGR_MODEL_HEAD_SKIN_FILE);

		sprintf(s_aAttName, "%s%d", MODELBMGR_HEAD_SKIN, skinNumber);
		if( m_buteMgr.Exist(s_aAttName) )
		{
			memcpy(m_aModels[iModel].m_szHeadSkinName, s_aAttName, MAX_MODELBMGR_NAME_LEN);

			uint8 iHeadSkin = 0;
			sprintf(s_aAttName, "%s%d", MODELBMGR_SKIN_FILE_PATH, iHeadSkin);
			while (m_buteMgr.Exist(m_aModels[iModel].m_szHeadSkinName, s_aAttName))
			{
				sSkinPath element;
				element.szFilePath = debug_newa(char, MAX_MODELBMGR_MAX_PATH);
				m_buteMgr.GetString(m_aModels[iModel].m_szHeadSkinName, s_aAttName, element.szFilePath, MAX_MODELBMGR_MAX_PATH);
				m_aModels[iModel].m_lstHeadSkinFile.push_back( element );
		...
		{
			m_aModels[iModel].m_cHeadSkinFiles = 0;
		}
		// Body Skin Read
		skinNumber = (uint8)m_buteMgr.GetInt(s_aTagName, MODELBMGR_MODEL_BODY_SKIN_FILE);
		sprintf(s_aAttName, "%s%d", MODELBMGR_BODY_SKIN, skinNumber);
		if( m_buteMgr.Exist(s_aAttName) )
		{
			memcpy(m_aModels[iModel].m_szBodySkinName, s_aAttName, MAX_MODELBMGR_NAME_LEN);

			uint8 iBodySkin = 0;
			sprintf(s_aAttName, "%s%d", MODELBMGR_SKIN_FILE_PATH, iBodySkin );
			while (m_buteMgr.Exist(m_aModels[iModel].m_szBodySkinName, s_aAttName))
			{
				sSkinPath element;
				element.szFilePath = debug_newa(char, MAX_MODELBMGR_MAX_PATH);
				m_buteMgr.GetString(m_aModels[iModel].m_szBodySkinName, s_aAttName, element.szFilePath, MAX_MODELBMGR_MAX_PATH);
				m_aModels[iModel].m_lstBodySkinFile.push_back( element );
		...
		{
			m_aModels[iModel].m_cBodySkinFiles = 0;
		}
		//-------------------------------------------------------------
	}

	// ±¤½Ä [05.03.18] : Team ¿¡ µû¶ó ¸ðµ¨À» ±¸º°ÇÏ¿© Loading ÇÑ´Ù.
	//--------------------------------------------------------------------------------------
	m_cTeamRedModels	= 0;
	sprintf(s_aAttName, "%s%d", MODELBMGR_MP_MODEL_NAME, m_cTeamRedModels);

	while(m_buteMgr.Exist(MODELBMGR_TEAM_RED_MODEL, s_aAttName))
	{
		m_cTeamRedModels++;
		sprintf(s_aAttName, "%s%d", MODELBMGR_MP_MODEL_NAME, m_cTeamRedModels);
	}

	m_aTeamRedModels	= debug_newa(ModelId, m_cTeamRedModels);
	for( int iRedModel = 0; iRedModel < m_cTeamRedModels; iRedModel++ )
	{
		sprintf(s_aAttName, "%s%d", MODELBMGR_MP_MODEL_NAME, iRedModel);
		m_buteMgr.GetString(MODELBMGR_TEAM_RED_MODEL, s_aAttName, "", s_szBuffer, ARRAY_LEN(s_szBuffer) );
		m_aTeamRedModels[iRedModel] = GetModelId(s_szBuffer);
	}

	m_cTeamBlueModels	= 0;
	sprintf(s_aAttName, "%s%d", MODELBMGR_MP_MODEL_NAME, m_cTeamBlueModels);

	while(m_buteMgr.Exist(MODELBMGR_TEAM_BLUE_MODEL, s_aAttName))
	{
		m_cTeamBlueModels++;
		sprintf(s_aAttName, "%s%d", MODELBMGR_MP_MODEL_NAME, m_cTeamBlueModels);
	}

	m_aTeamBlueModels	= debug_newa(ModelId, m_cTeamBlueModels);
	for( int iBlueModel = 0; iBlueModel < m_cTeamBlueModels; iBlueModel++ )
	{
		sprintf(s_aAttName, "%s%d", MODELBMGR_MP_MODEL_NAME, iBlueModel);
		m_buteMgr.GetString(MODELBMGR_TEAM_BLUE_MODEL, s_aAttName, "", s_szBuffer, ARRAY_LEN(s_szBuffer) );
		m_aTeamBlueModels[iBlueModel] = GetModelId(s_szBuffer);
	}
	//--------------------------------------------------------------------------------------
...
void CModelButeMgr::Term()
{
	...
	if( m_aTeamRedModels )
	{
		debug_deletea(m_aTeamRedModels);
		m_aTeamRedModels = LTNULL;
	}
	if( m_aTeamBlueModels )
	{
		debug_deletea(m_aTeamBlueModels);
		m_aTeamBlueModels = LTNULL;
	}
	//-----------------------------------------------------
...
ModelSkeleton CModelButeMgr::GetModelBigSkeleton(ModelId eModelId)
{
	if(eModelId >= 0 && eModelId < m_cModels)
	{
		return m_aModels[eModelId].m_eModelBigSkeleton;
	}

	_ASSERT( !"CModelButeMgr::GetModelBigSkeleton: Invalid ModelId" );
	return eModelSkeletonInvalid;
}

ModelSkeleton CModelButeMgr::GetModelFatSkeleton(ModelId eModelId)
{
	if(eModelId >= 0 && eModelId < m_cModels)
	{
		return m_aModels[eModelId].m_eModelFatSkeleton;
	}

	_ASSERT( !"CModelButeMgr::GetModelFatSkeleton: Invalid ModelId" );
	return eModelSkeletonInvalid;
}
...
// ±¤½Ä
LTBOOL CModelButeMgr::GetModelSleeve( ModelId eModelId )
{
	if( eModelId >= 0 && eModelId < m_cModels )
	{
		return m_aModels[eModelId].m_IsSleeve;
	}
...
// ±¤½Ä
const char* CModelButeMgr::GetSleeveSkinFilename(ModelId eModelId)
{
	_ASSERT(eModelId >= 0 && eModelId < m_cModels);
	if(eModelId >= 0 && eModelId < m_cModels)
	{
		return m_aModels[eModelId].m_szSleeveSkin;
	}

	return LTNULL;
}

const char* CModelButeMgr::GetArmsSkinFilename(ModelId eModelId)
{
	_ASSERT(eModelId >= 0 && eModelId < m_cModels);
	if(eModelId >= 0 && eModelId < m_cModels)
	{
		return m_aModels[eModelId].m_szArmsSkin;
	}

	return LTNULL;
}

int CModelButeMgr::GetShopIconNameIndex(ModelId eModelId)
{
	_ASSERT(eModelId >= 0 && eModelId < m_cModels);
	if( eModelId >= 0 && eModelId < m_cModels )
	{
		return m_aModels[eModelId].m_nIconNameIndex;
	}

	return 0;
}

int CModelButeMgr::GetModelTeamID(ModelId eModelId)
{
	_ASSERT(eModelId >= 0 && eModelId < m_cModels);
	if( eModelId >= 0 && eModelId < m_cModels )
	{
		return m_aModels[eModelId].m_nTeamID;
	}

	return 0;
}

int CModelButeMgr::GetModelDescIndex(ModelId eModelId)
{
	_ASSERT(eModelId >= 0 && eModelId < m_cModels);
	if( eModelId >= 0 && eModelId < m_cModels )
	{
		return m_aModels[eModelId].m_nDescIndex;
	}

	return 0;
}

std::string CModelButeMgr::GetShopNormalIcon(ModelId eModelId) const
{
	_ASSERT(eModelId >= 0 && eModelId < m_cModels);
	if(eModelId >= 0 && eModelId < m_cModels)
	{
		std::string str(m_aModels[eModelId].m_szIcon); // Shop Normal Icon
		str += "_S.dtx";
		return str;
	}
...
std::string	CModelButeMgr::GetShopDetailIcon(ModelId eModelId) const
{
	_ASSERT(eModelId >= 0 && eModelId < m_cModels);
	if(eModelId >= 0 && eModelId < m_cModels)
	{
		std::string str(m_aModels[eModelId].m_szIcon); // Shop Detail Icon
		str += "_M.dtx";
		return str;
	}
...
const char*	CModelButeMgr::GetMultiBigModelFilename(ModelId eModelId)
{
	_ASSERT(eModelId >= 0 && eModelId < m_cModels);
	if(eModelId >= 0 && eModelId < m_cModels)
	{
		return m_aModels[eModelId].m_szBigModelFile;
	}

	return LTNULL;
}

const char*	CModelButeMgr::GetMultiFatModelFilename(ModelId eModelId)
{
	_ASSERT(eModelId >= 0 && eModelId < m_cModels);
	if(eModelId >= 0 && eModelId < m_cModels)
	{
		return m_aModels[eModelId].m_szFatModelFile;
	}
...
// ±¤½Ä
//---------------------------------------------------------------------------------------------------
const char* CModelButeMgr::GetModelHeadSkinTitlename(ModelId eModelId)
{
	_ASSERT(eModelId >= 0 && eModelId < m_cModels);
	if(eModelId >= 0 && eModelId < m_cModels)
	{
		return m_aModels[eModelId].m_szHeadSkinName;
	}
	return LTNULL;
}
		
void CModelButeMgr::GetModelHeadSkinFilename(ModelId eModelId, int nSkinNumber, const char*& szFileName)
{
	_ASSERT(eModelId >= 0 && eModelId < m_cModels);
	if(eModelId >= 0 && eModelId < m_cModels)
	{
		szFileName = m_aModels[eModelId].m_lstHeadSkinFile[nSkinNumber].szFilePath;
	}
}

uint8 CModelButeMgr::GetNumModelHeadSkins(ModelId eModelId)
{
	_ASSERT(eModelId >= 0 && eModelId < m_cModels);
	if(eModelId >= 0 && eModelId < m_cModels)
	{
		return m_aModels[eModelId].m_lstHeadSkinFile.size();
	}

	return 0;
}

const char* CModelButeMgr::GetModelBodySkinTitlename(ModelId eModelId)
{
	_ASSERT(eModelId >= 0 && eModelId < m_cModels);
	if(eModelId >= 0 && eModelId < m_cModels)
	{
		return m_aModels[eModelId].m_szBodySkinName;
	}
	return LTNULL;
}

void CModelButeMgr::GetModelBodySkinFilename(ModelId eModelId, int nSkinNumber, const char*& szFileName)
{
	_ASSERT(eModelId >= 0 && eModelId < m_cModels);
	if(eModelId >= 0 && eModelId < m_cModels)
	{
		szFileName = m_aModels[eModelId].m_lstBodySkinFile[nSkinNumber].szFilePath;
	}
}

uint8 CModelButeMgr::GetNumModelBodySkins(ModelId eModelId)
{
	_ASSERT(eModelId >= 0 && eModelId < m_cModels);
	if(eModelId >= 0 && eModelId < m_cModels)
	{
		return m_aModels[eModelId].m_lstBodySkinFile.size();
	}
...
//[SPIKE]
SurfaceType CModelButeMgr::GetHeadSurfaceType(ModelId eModelId)
{
	_ASSERT(eModelId >= 0 && eModelId < m_cModels);
	if(eModelId >= 0 && eModelId < m_cModels)
	{
		return m_aModels[eModelId].m_eHeadSurfaceType;
	}
...
// ±¤½Ä [05.03.18] : ModelsÀ» ÆÀ¿¡µû¶ó ±¸ºÐÇÏ±â.
ModelId CModelButeMgr::GetTeamRedModel(int num)
{
	ASSERT(m_aTeamRedModels);
	if( !m_aTeamRedModels ) return eModelIdInvalid;

	if( num >= m_cTeamRedModels )
	{
		ASSERT("Error in CModelButeMgr::GetTeamRedModel() : Invalid TeamRedModel index");
		num = 0;
	}

	return m_aTeamRedModels[num];
}

ModelId CModelButeMgr::GetTeamBlueModel(int num)
{
	ASSERT(m_aTeamBlueModels);
	if( !m_aTeamBlueModels ) return eModelIdInvalid;

	if( num >= m_cTeamBlueModels )
	{
		ASSERT("Error in CModelButeMgr::GetTeamBlueModel() : Invalid TeamBlueModel index");
		num = 0;
	}

	return m_aTeamBlueModels[num];
