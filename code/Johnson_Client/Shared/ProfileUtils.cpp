...
bool ServerGameOptions::LoadFromBute( CButeMgr& bute )
{
	...
	m_eGameType = ( GameType )bute.GetInt( szTagName,"GameType", eGameTypeTeamDeathmatch );
...
const char*	ServerGameOptions::GetCampaignName() const
{
	...
	case eGameTypeSurvival :
		return m_SG.m_sCampaignName.c_str();
		break;
...
const char*	ServerGameOptions::GetSessionName() const
{
	...
	case eGameTypeSurvival :
		return m_SG.m_sSessionName.c_str();
		break;
...
bool TeamDMGameOptions::LoadFromBute( CButeMgr& bute )
{
	...
	m_bFriendlyFire = bute.GetBool(szTagName,"FriendlyFire",false);	
...
/************************************************************************************
**** Survival game options
************************************************************************************/
SurvivalGameOptions::SurvivalGameOptions( )
{
}

void SurvivalGameOptions::Clear( )
{
	m_sSessionName = "";
	m_sCampaignName = DEFAULT_CAMPAIGN;
...
bool SurvivalGameOptions::LoadFromBute( CButeMgr& bute )
{
	char const szTagName[] = "Survival";
	std::string sAttName = "";
...
bool SurvivalGameOptions::SaveToBute( CButeMgr& bute )
{
	char const szTagName[] = "Survival";
...
void SurvivalGameOptions::Write( ILTMessage_Write *pMsg )
{
	pMsg->Writeuint16(m_nTimeLimit);
	pMsg->Writeuint8(m_nScoreLimit);
	pMsg->Writeuint8(m_nRounds );
...
void SurvivalGameOptions::Read( ILTMessage_Read *pMsg )
{
	m_nTimeLimit = pMsg->Readuint8();
	m_nScoreLimit = pMsg->Readuint8();
	m_nRounds = pMsg->Readuint8();
...
SurvivalGameOptions& SurvivalGameOptions::Copy( SurvivalGameOptions const& other )
{
	if( &other == this )
		return *this;
