...
#define SRFMGR_SURFACE_MELEEHOLESPR				"MeleeHoleSpr"
#define SRFMGR_SURFACE_SPECIALMELEEHOLESPR		"SpecialMeleeHoleSpr"
...
#define	SRFMGR_SURFACE_EXPLODEIMPACTFXNAME		"ExplodeImpactFXName"
...
//[SPIKE]
#define SRFMGR_SURFACE_ENABLEBULLETDIR			"EnableBulletDir"
#define SRFMGR_SURFACE_IMPACT_ARRIVECOUNT		"ImpactArriveCount"
...
SURFACE::SURFACE()
{
	...
	//[SPIKE] 
	for (i=0; i < SRF_MAX_BULLETHOLE_SPR; i++)
	{
		szBulletHoleSpr[i][0]	= '\0';
	}
	for (i=0; i < SRF_MAX_BULLETHOLE_SPR; i++)
	{
		szMeleeHoleSpr[i][0]	= '\0';
		szSpecialMeleeHoleSpr[i][0] = '\0';
	}
	...
	for (i=0; i < SRF_MAX_IMPACTFX_COUNT; i++)
	{
		szImpactFXName[i][0]		= '\0';
		szExplodeImpactFXName[i][0]	= '\0';
	}
	...
	//[SPIKE]
	m_bEnableBulletDir		= LTFALSE;
	m_iImpactArriveCount	= 0;
}
...
LTBOOL SURFACE::Init(CButeMgr & buteMgr, char* aTagName)
{
	...
	//[SPIKE]
	m_bEnableBulletDir		= (LTBOOL) buteMgr.GetInt(aTagName, SRFMGR_SURFACE_ENABLEBULLETDIR, LTFALSE);
	m_iImpactArriveCount	= buteMgr.GetInt(aTagName, SRFMGR_SURFACE_IMPACT_ARRIVECOUNT, 0);
	...
	for (i=0; i < SRF_MAX_BULLETHOLE_SPR; i++)
	{
		if(i == 0) 
		{
			buteMgr.GetString( aTagName, SRFMGR_SURFACE_BULLETHOLESPR, szBulletHoleSpr[i], ARRAY_LEN( szBulletHoleSpr[i] ));
		}
		else
		{
			sprintf(s_aAttName, "%s%d", SRFMGR_SURFACE_BULLETHOLESPR, i);
			buteMgr.GetString(aTagName, s_aAttName, szBulletHoleSpr[i], ARRAY_LEN(szBulletHoleSpr[i]));
		}		
	}
	for (i=0; i < SRF_MAX_BULLETHOLE_SPR; i++)
	{
		if(i == 0) 
		{
			buteMgr.GetString( aTagName, SRFMGR_SURFACE_MELEEHOLESPR, szMeleeHoleSpr[i], ARRAY_LEN( szMeleeHoleSpr[i] ));

			buteMgr.GetString( aTagName, SRFMGR_SURFACE_SPECIALMELEEHOLESPR, szSpecialMeleeHoleSpr[i], ARRAY_LEN( szSpecialMeleeHoleSpr[i] ));
		}
		else
		{
			sprintf(s_aAttName, "%s%d", SRFMGR_SURFACE_MELEEHOLESPR, i);
			buteMgr.GetString(aTagName, s_aAttName, szMeleeHoleSpr[i], ARRAY_LEN(szMeleeHoleSpr[i]));

			sprintf(s_aAttName, "%s%d", SRFMGR_SURFACE_SPECIALMELEEHOLESPR, i);
			buteMgr.GetString(aTagName, s_aAttName, szSpecialMeleeHoleSpr[i], ARRAY_LEN(szSpecialMeleeHoleSpr[i]));
		}		
	}
	...
	for (i=0; i < SRF_MAX_IMPACTFX_COUNT; i++)
	{
		if(i == 0) 
		{
			buteMgr.GetString( aTagName, SRFMGR_SURFACE_IMPACTFXNAME, szImpactFXName[i], ARRAY_LEN( szImpactFXName[i] ));

			buteMgr.GetString( aTagName, SRFMGR_SURFACE_EXPLODEIMPACTFXNAME, szExplodeImpactFXName[i], ARRAY_LEN( szExplodeImpactFXName[i] ));
		}
		else
		{
			sprintf(s_aAttName, "%s%d", SRFMGR_SURFACE_IMPACTFXNAME, i);
			buteMgr.GetString(aTagName, s_aAttName, szImpactFXName[i], ARRAY_LEN(szImpactFXName[i]));

			sprintf(s_aAttName, "%s%d", SRFMGR_SURFACE_EXPLODEIMPACTFXNAME, i);
			buteMgr.GetString(aTagName, s_aAttName, szExplodeImpactFXName[i], ARRAY_LEN(szExplodeImpactFXName[i]));
		}		
	}
...
void SURFACE::Cache(CSurfaceMgr* pSurfaceMgr)
{
	...
	for (i=0; i < SRF_MAX_BULLETHOLE_SPR; i++)
	{
		if (szBulletHoleSpr[i][0])
		{
            g_pLTServer->CacheFile(FT_SOUND, szBulletHoleSpr[i]);
		}
	}
	for (i=0; i < SRF_MAX_BULLETHOLE_SPR; i++)
	{
		if (szMeleeHoleSpr[i][0])
		{
            g_pLTServer->CacheFile(FT_SOUND, szMeleeHoleSpr[i]);
            g_pLTServer->CacheFile(FT_SOUND, szSpecialMeleeHoleSpr[i]);
		}
	}
