					  ...
					  char* pBuf, int nBufLen)
{
	...
	if (stricmp(pBuf, "SURFACE") == 0)
	{
		...
		if (pSurf)
		{
			if(pAmmo->eInstDamageType == DT_BULLET && pSurf->szBulletHoleSpr)
			{
				if(*(pSurf->szBulletHoleSpr[1]))
				{
					uint8 nIndex = GetRandom(0, 2);
					strcpy(pBuf, pSurf->szBulletHoleSpr[nIndex]);
				}
				else
				{
					strcpy(pBuf, pSurf->szBulletHoleSpr[0]);	
				}			
			}
			...
			{
				// 특수 무기사용 
				if(g_pSurfaceMgr->IsSpecialDamageMode() && pSurf->szSpecialMeleeHoleSpr)
				{
					if(*(pSurf->szSpecialMeleeHoleSpr[1]))
					{
						uint8 nIndex = GetRandom(0, 2);
						strcpy(pBuf, pSurf->szSpecialMeleeHoleSpr[nIndex]);
					}
					else
					{
						strcpy(pBuf, pSurf->szSpecialMeleeHoleSpr[0]);	
					}	
				}
				else //일반 무기 
				{
					if(pSurf->szMeleeHoleSpr)
					{
						if(*(pSurf->szMeleeHoleSpr[1]))
						{
							uint8 nIndex = GetRandom(0, 2);
							strcpy(pBuf, pSurf->szMeleeHoleSpr[nIndex]);
						}
						else
						{
							strcpy(pBuf, pSurf->szMeleeHoleSpr[0]);	
						}	
					}
				}
			}
