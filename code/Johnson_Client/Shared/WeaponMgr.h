...
#define WEAPON_KEY_RELOAD					"RELOAD_KEY"

// 광식 추가
#define WEAPON_KEY_ZOOMIN					"ZoomIn"
#define WEAPON_KEY_ZOOMOUT					"ZoomOut"
...
struct WEAPON
{
	...
	std::string	GetNormalIcon() const;
	std::string	GetDisabledIcon() const;
	std::string GetSelectedIcon() const;
	
	std::string GetInvenTitleIcon()		const;
	std::string	GetInvenNormalIcon()	const;
	std::string	GetInvenSelectIcon()	const;
	...
	float	fFireRecoilPitchMax;
	float	fFireMaxGapOffset;
	float	fFireCamRecoilYaw;
	float   fFireCamRecoilYawMax ;

	CMuzzleFX	*pPVMuzzleFX;	// Player-view muzzle fx

	char	*szPVMuzzleFxName;
	//[SPIKE]
	char	*szHHMuzzleFxName[WMGR_MAX_MUZZLEFLASH];
	...
	// 광식 : 무기의 종류에 따라 이동속도를 다르게 적용하기위해 추가 함
	float m_fWeight;
	int	  m_nFireType;	
	// Inventory 에서 사용
	//---------------------------------------
	...
	float	fPerturbIncreaseSpeedFire ;
	float	fPerturbDecreaseSpeedFire ;
	
	//[SPIKE] 
	float	fSpecialModeDecrease ;
	// 상황에 따른 Perturb 
	float	m_fPerturbDuck;
	float	m_fPerturbStandard;
	float	m_fPerturbMove;
	// 
	float	m_fPVModelFOVY;
	float	m_fPVModelAspect;
	
	float	m_fPVModelPitch;
	float	m_fPVModelYaw;
	float	m_fPVModelRoll;
	///////////////////////////////
private:
...
class CWeaponMgr : public CGameButeMgr
{
	...
	public :
		...
		// 광식 : 총의 무게 얻는 함수
		float		GetWeaponWeight(int nWeaponId) const;
		int			GetWeaponFireType(int nWeaponId) const;
		// Inventory에서 사용
		int			GetWeaponDescription(int nWeaponId) const;
		int			GetWeaponIconNameIndex(int nWeaponId) const;
		CRect		GetWeaponPerformance(int nWeaponId) const;
		uint8		GetLastWeaponOfClass(int nClass)	const;
		const char* GetWeaponIconName( int nWeaponId );
		...
		//[SPIKE]=====================
		VOID		SetSpecialMode(LTBOOL bSpecialMode){m_bSpecialMode = bSpecialMode;}
		LTBOOL		IsSpecialMode(){ return m_bSpecialMode; }
		VOID		SetPlantedTimeBomb(LTBOOL bPlantedTimeBomb){m_bPlantedTimeBomb = bPlantedTimeBomb;}
		LTBOOL		IsPlantedTimeBomb(){ return m_bPlantedTimeBomb;}
		//============================
	protected :
		...
		//[SPIKE]
		LTBOOL				m_bSpecialMode;
		LTBOOL				m_bPlantedTimeBomb;		
};
...
// 광식
inline float CWeaponMgr::GetWeaponWeight(int nWeaponId) const
{
	if ( IsValidWeaponId(nWeaponId) ) 
		return m_pWeaponList[nWeaponId]->m_fWeight;

	return 0.0f;
}

inline int CWeaponMgr::GetWeaponFireType( int nWeaponId ) const
{
	if( IsValidWeaponId(nWeaponId) )
		return m_pWeaponList[nWeaponId]->m_nFireType;

	return 0;
}

inline int CWeaponMgr::GetWeaponDescription(int nWeaponId) const
{
	if ( IsValidWeaponId(nWeaponId) ) 
		return m_pWeaponList[nWeaponId]->m_Description;
	
	return 133;
}

inline int CWeaponMgr::GetWeaponIconNameIndex(int nWeaponId) const
{
	if ( IsValidWeaponId(nWeaponId) ) 
		return m_pWeaponList[nWeaponId]->m_nIconNameIndex;
	
	return 0;
}

inline CRect CWeaponMgr::GetWeaponPerformance(int nWeaponId) const
{
	CRect rect(0,0,0,0);
	if ( IsValidWeaponId(nWeaponId) ) 
		return m_pWeaponList[nWeaponId]->m_Performance;
...
// 광식
inline uint8 CWeaponMgr::GetLastWeaponOfClass(int nClass) const
{
	if (!m_pClasses || !m_pWeaponOrder)
	{
		return 0;
	}

	if( nClass < 0 || nClass >= m_nNumClasses )
	{
		return 0;
	}

	if( !IsValidWeaponId(m_pWeaponOrder[m_pClasses[nClass]]) )
	{
		return 0;
	}
	
	return m_pWeaponOrder[m_pClasses[nClass]];
}
