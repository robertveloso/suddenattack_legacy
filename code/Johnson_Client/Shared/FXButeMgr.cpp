...
#define FXBMGR_TRACERFX_LENGTH			"Length"
...
static char s_aTagName[MAX_DESCRIPTION_LENGTH];
static char s_aAttName[MAX_DESCRIPTION_LENGTH];
...
LTBOOL CFXButeMgr::Init(const char* szAttributeFile)
{
	...
	int nNum = 0;
	memset( s_aTagName, 0, sizeof(s_aTagName) );
...
LTBOOL TRACERFX::Init(CButeMgr & buteMgr, char* aTagName)
{
	...
	fLength			= (LTFLOAT) buteMgr.GetDouble(aTagName, FXBMGR_TRACERFX_LENGTH);
...
void CFXButeMgr::CreateImpactFX(IMPACTFX* pImpactFX, IFXCS & cs)
{
	...
	//[SPIKE] 연막탄 디테일체크 
	char szTempFXName[256]={LTNULL};
	strcpy(szTempFXName, pImpactFX->szFXName);
	if(pImpactFX->nId == 7) // 연막효과는 '7' 
	{
		if(!GetConsoleInt("SmokeGrenadeDetail", 1))  // LowDetail 연막모드인가?
		{
			strcat(szTempFXName, "_Low");
		}
	}
	CLIENTFX_CREATESTRUCT	fxCS( szTempFXName, 0, cs.vPos );
	//=================================================================

//	fxCS.m_vTargetNorm = cs.vSurfNormal; //[SPIKE]
