...
*/
void CAnimator::SetAni(Ani eAni, AniTracker eAniTracker, LTBOOL bRepeat )
{
	if ( eAni == eAniInvalid || eAniTracker == eAniTrackerInvalid )
		return;
	...
	if ( hCurAni != hNewAni )
	{
		...
		{
			LTRESULT dwResult = g_pModelLT->SetCurAnim(m_hObject, m_aAniTrackers[eAniTracker].m_AnimTracker, hNewAni);
			// Twitch 할 때 처음 Ani 안나와서 수정 함.
			//------------------------------------------------------------------------------------------------
			dwResult = g_pModelLT->SetCurAnimTime(m_hObject, m_aAniTrackers[eAniTracker].m_AnimTracker, 0);
			//------------------------------------------------------------------------------------------------
			_ASSERT(LT_OK == dwResult);
		}
	}
	...
	{		
		...
		if ( IsAniTrackerDone(eAniTracker) && bRepeat == LTTRUE )
		{
			ResetAniTracker(eAniTracker);
		}
	}
	/*
	// 광식
	g_pLTServer->CPrint("m_AnimTracker [%d], hNewAni [%d]",m_aAniTrackers[eAniTracker].m_AnimTracker, hNewAni);
	*/
}

void CAnimator::SetAniTwitch(Ani eAni, AniTracker eAniTracker, LTBOOL bRepeat, LTBOOL bTwitch)
{
	if ( eAni == eAniInvalid || eAniTracker == eAniTrackerInvalid )
		return;
