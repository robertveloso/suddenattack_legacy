...
enum PlayerState { PS_UNKNOWN=0, PS_ALIVE, PS_DEAD, PS_DYING, PS_GHOST, PS_MANDATE_DEAD };
...
#define USRFLG_SERVER_HITBOX			(1<<13)
...
// ±¤½Ä : Client-Sid Hit Detection (Model Check)
#define USRFLG_HITBOX_PLAYER			(1<<25)
...
inline LTRESULT SetObjectFilenames(HOBJECT hObj, const char *pFilename, const char *pSkinName, uint32 dwUserData=0)
{
	ObjectCreateStruct cOCS;
	cOCS.Clear();
	SAFE_STRCPY(cOCS.m_Filename, pFilename);
	SAFE_STRCPY(cOCS.m_SkinName, pSkinName);
	cOCS.m_UserData	= dwUserData;
